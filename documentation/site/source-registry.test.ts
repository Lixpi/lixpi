...
afterEach(() => {
    for (const root of roots.splice(0)) rmSync(root, {
        recursive: true,
        force: true,
    })
})
...
describe('documentation source registry', () => {
    ...
    it('renders package-local links and images with the same resolver used by validation', () => {
        ...
        const html = Markdoc.renderers.html(Markdoc.transform(ast, createConfig({
            sources: registry,
            source: engine,
        })))
    ...
    it('discovers explicit documentation roots without ingesting package source, dependencies or examples', () => {
        ...
        write('services/nats/README.md')
        write('services/nats/documentation/ARCHITECTURE.md')
        write('services/nats/internal/auth/ignored.md')
        write('services/nats/policy/ignored.md')
        write('services/caddy/README.md')
        write('services/caddy/documentation/ARCHITECTURE.md')
        write('services/caddy/internal/ignored.md')
        ...
        expect([...registry.pages.values()].filter(page => page.route.startsWith('services/nats/')).map(page => page.route).sort()).toEqual([
            'services/nats/documentation/ARCHITECTURE.html',
            'services/nats/index.html',
        ])
        expect([...registry.pages.values()].filter(page => page.route.startsWith('services/caddy/')).map(page => page.route).sort()).toEqual([
            'services/caddy/documentation/ARCHITECTURE.html',
            'services/caddy/index.html',
        ])
    })
...
describe('repository documentation', () => {
    it('publishes the NATS manuals and resolves links to and from the central documentation', () => {
        const registry = new DocumentationSources('/repository').discover()
        const source = '/repository/services/nats/README.md'
        const architecture = '/repository/services/nats/documentation/ARCHITECTURE.md'
        registry.setHeadings(architecture, collectHeadingIds(Markdoc.parse(readFileSync(architecture, 'utf8'))))
        expect(registry.resolve('/repository/documentation/README.md', '../services/nats/README.md')).toBe('services/nats/index.html')
        expect(registry.resolve(source, 'documentation/ARCHITECTURE.md#one-connection-attempt')).toBe('documentation/ARCHITECTURE.html#one-connection-attempt')
        expect(registry.resolve(source, '../../documentation/README.md')).toBe('../../README.html')
        expect(registry.pages.has('/repository/services/nats/internal/README.md')).toBe(false)
    })
    it('resolves authored documentation links without building the site', () => {
        const registry = new DocumentationSources('/repository').discover()

        // Check source-directory READMEs without publishing implementation folders.
        for (const service of ['nats', 'caddy']) {
            const serviceRoot = `/repository/services/${service}`

            for (const relative of readdirSync(serviceRoot, { recursive: true })) {
                if (!relative.endsWith('README.md'))
                    continue

                const source = path.join(serviceRoot, relative)

                if (!registry.pages.has(source))
                    registry.register(source, `source-docs/${service}/${relative.replaceAll(path.sep, '/').replace(/\.md$/, '.html')}`)
            }
        }
        ...
        for (const {
            ...
            if (
                page.route.startsWith('packages/')
                || page.route.startsWith('services/nats/')
                || page.route.startsWith('services/caddy/')
                || page.route.startsWith('source-docs/nats/')
                || page.route.startsWith('source-docs/caddy/')
            ) {
                ...
                for (const item of Markdoc.validate(ast, config)) {
                    if (['error', 'critical'].includes(item.error.level))
                        failures.push(`${page.route}: ${item.error.message}`)
                }
