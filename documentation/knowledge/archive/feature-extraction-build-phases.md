---
title: Feature Extraction — Build Phases (Archived)
description: Historical implementation record of the feature-extraction build — the file-structure map and the eleven-phase work sequence. Preserved for provenance, not as live reference.
---

# Feature Extraction — Build Phases (Archived)
...
feature-extraction build. It is *not* live reference documentation and should not
be used as instructions for new contributors. For current behaviour see the live
library docs:
[Feature Extraction Overview](../../library/FEATURE-EXTRACTION-OVERVIEW.md),
[Extraction Pipeline](../../library/EXTRACTION-PIPELINE.md), and
[Feature Storage](../../library/FEATURE-STORAGE.md).
{% /callout %}
...
- Modify `services/api/src/llm/providers/base-provider.ts` — register `resolveFeatures` between `START` and `validateRequest`. **Remove** the `validateFeatureSpec` and `executeFeatureExtraction` nodes (subsumed by Phase 4a–4c's dedicated extraction graph). **Remove** the conditional edge from `streamTokens` to `validateFeatureSpec`.
