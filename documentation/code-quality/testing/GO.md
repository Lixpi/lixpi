...
`fix` runs the AST-based source convention fixes, `gofumpt`, `goimports`, `golines`, and `golangci-lint run --fix`; it is required during implementation. The linter checks context propagation and argument order, package-level runtime state and `init` exceptions, interface size and use, error wrapping and naming, error message wording, wrap-aware error matching, third-party error packages, `os.Exit` outside `main`, unexplained `//nolint` directives, structured logging, SQL construction, resource cleanup, modern APIs, explicit command output, control-flow spacing, and `//` comments. An assignment stays attached to a following validation `if` when its condition reads an assigned value. Go syntax that has no TypeScript equivalent stays under the Go formatter and compiler. The quality image build applies the same formatter and linter configuration to its own `source-conventions.go` checker before compiling it. `validate` checks formatting and lint without editing source. A formatting diff exits nonzero before lint runs. [The version registry](../../../dev-tools/versions-registry/README.md) supplies the tools and base image versions generated into the runner Dockerfiles.
