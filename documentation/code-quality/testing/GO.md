...
Read the [`code-quality` skill](${LIXPI_REPOSITORY_PATH}/skills/code-quality/SKILL.md) before writing or running tests. Tests require an explicit request in the active task. Use colocated `*_test.go` files and the standard `testing` package. Use table-driven tests when several inputs or cases exercise the same behavior and setup; use named subtests so a failure identifies the case. Keep a direct test when a table would hide the behavior or require case-specific control flow.

When a service uses `stretchr/testify`, use `require` for preconditions and error checks that must stop the test, and `assert` when the test can continue and report more failures. Use `testify/mock` or `suite` only when they simplify the test. The service's local guide decides whether it uses that library.
...
`fix` runs the AST-based source convention fixes, `gofumpt`, `goimports`, `golines`, and `golangci-lint run --fix`; it is required during implementation. The specialized linters check contexts, runtime globals, interfaces, error wrapping, structured logging, SQL, cleanup, command exits, and control-flow spacing. A typed `go/analysis` checker also runs through `go vet`: it resolves prohibited builtin, package, and method symbols, checks constant error messages, and recognizes comparisons against actual error methods. Its checks distinguish aliases and shadowed names without regex source matching. An assignment stays attached to a following validation `if` when its condition reads an assigned value. `validate` checks without editing source, and both actions report failure if any stage fails while still running the remaining checks.

The `go-quality-tools` target runs the checkers' own regression tests and quality checks. The quality image build validates, tests, and compiles both checkers; rebuild it after editing their source. [Go Quality Runner](../../../dev-tools/code-quality/go-quality-runner/README.md) describes the rules, panic exceptions, and tool verification commands. [The version registry](../../../dev-tools/versions-registry/README.md) supplies the selected tools, module dependencies, and base image versions.
