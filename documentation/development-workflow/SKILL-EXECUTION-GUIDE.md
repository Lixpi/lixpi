...
Every skill under `skills/` that has to execute code executes it inside the shared `lixpi-utils` container. Nothing a skill ships is ever run on the host.
...
There is no host toolchain to fall back on. A script run on the host picks up whatever Node the developer's machine happens to have, which is exactly the difference that makes a skill work for one person and fail for the next. It is also why [`AGENT-SKILLS.md`](AGENT-SKILLS.md) bans host `pnpm` outright.

Prohibited, without exception:

- Running `node`, `pnpm`, `npm`, `npx`, `pnpx`, or `python` against a file in `skills/` on the host.
...
docker compose -f docker-compose.lixpi-utils.yml run --rm -T lixpi-utils \
    <skill-name> <script-path> [args...]
```

`<skill-name>` is a directory under `skills/`. `<script-path>` is relative to that skill's directory. For example:

```bash
docker compose -f docker-compose.lixpi-utils.yml run --rm -T lixpi-utils \
    fetch-byteplus-documentation scripts/fetch-byteplus-doc.ts ModelArk/2377608
```
...
```bash
docker compose -f docker-compose.lixpi-utils.yml build lixpi-utils
```

Editing a skill script or `run-skill.sh` needs no rebuild. Both are bind-mounted, so the next run picks up the change.
...
| `skills/` | Bind-mounted at `/skills`, writable. A script may write output next to itself and the file lands on the host. |
...
1. Put it under `skills/<skill-name>/scripts/`.
...
4. Document the exact `docker compose ... run --rm -T lixpi-utils ...` invocation in the skill's `SKILL.md`, with the flags spelled out. An agent copies that line verbatim.
...
|---|---|
| `No skill directory: skills/<name>` | Typo in the skill name. The entrypoint lists what exists. |
| `No script: skills/<name>/<path>` | Path is relative to the skill directory, not the repository root. The entrypoint lists the scripts it found. |
| `Don't know how to run <script>` | Extension isn't `.ts`, `.mjs`, `.js`, or `.sh`. Rename it or add a dispatch case in `run-skill.sh`. |
| A Dockerfile change appears to have no effect | Rebuild. Only `skills/` and `run-skill.sh` are bind-mounted. |
...
Another repository can install the skills in `skills/` as symlinks into that project through the interactive [`setup-skills.sh`](../../setup-skills.sh) installer. A skill script still executes here: run the `docker compose -f docker-compose.lixpi-utils.yml ...` command from the Lixpi repository root. [`AGENT-SKILLS.md`](AGENT-SKILLS.md) covers the installer and the direction of the dependency.
