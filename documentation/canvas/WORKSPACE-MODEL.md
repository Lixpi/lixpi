...
| Document content | Live batched step submit + settled 3s server snapshot | `DOCUMENT_STEP.DOC_SUBMIT_STEPS` / `DOCUMENT_STEP.DOC_RESUME`; API writes `Document.content` with `proseMirrorVersion` after the edit burst settles |
