...
This page is the **image-specific delta**. It does not re-explain the shared LangGraph workflow, the dual-model architecture, the `generate_image` tool schema, the post-stream router, `ProviderState`, the `ImageRouter`, or the stream lifecycle — those are owned by [AI Generation Pipeline](../platform/AI-GENERATION-PIPELINE.md). What lives here is everything that is genuinely image-specific: the three image-model provider execution paths, how reference images are extracted from each provider's message format, the image system-prompt instructions, image sizes and hardcoded options, content-hash dedup, multi-turn editing, and the image-specific stream-event nuances.

{% callout type="note" %}
**What this page does *not* own.** The shared workflow, dual-model routing, tool injection/extraction mechanism, and `ImageRouter` live in [AI Generation Pipeline](../platform/AI-GENERATION-PIPELINE.md). Canvas placement, branch lineage, branch-origin circles, and VLM reference selection live in [Branch Lineage](./BRANCH-LINEAGE.md). The full stream-event catalog lives in [Streaming and Events](../platform/STREAMING-AND-EVENTS.md).
{% /callout %}
...
|-------|-----------------------|
| `IMAGE_PARTIAL` (empty) | An empty `imageBase64`/`imageUrl` is a *signal*, not pixels: it tells the canvas to create the placeholder `ImageCanvasNode` (a transparent 1×1 PNG) and start the PIXI traveling progress border before any real pixel data exists. |
| `IMAGE_PARTIAL` (non-empty) | Up to three progressive partials (`partial_images=3`) each replace the **same** preview sprite in place — the canvas updates one node, it does not create new ones. Gemini delivers thought images through this same event. |
| `IMAGE_COMPLETE` | Carries the final base64 plus `revisedPrompt`; this is the event that finalizes the node, clears the traveling outline, and persists `generatedBy` metadata. The transient image provider never emits `START_STREAM` / `END_STREAM` — the text model owns the stream lifecycle. |
