...
3. **Placement geometry:**
   ...
   - **Fresh / reference-only** generations place their API-planned root marker from the **combined bounds of all reference media**. The marker preserves the configured first generated-media slot when it fits, but clamps to the right of the reference group so long marker labels cannot overlap source/reference media. If no source/thread node or reference group exists, the planned root marker and first generated-media slot are centered as one group so the marker does not start outside the visible viewport. A parentless `branchFork` is the root marker for reasoning-fanout runs without a lineage source.
