...
The bottom composer never becomes a receiving/stop control. Each active branch marker owns the control for its submitted user-message run: a persistent pause/stop button sits at the marker's right-center while the matching generation request still has any planned or active media run, and disappears only after every spanned branch finishes. The control is a direct marker child above the drag/details overlay; it consumes its own pointer and click events, while clicking the rest of the marker still opens thread details. Activation immediately removes every tracker-backed, waiting, or API-projected pending media node for the request from the browser, locally treats the entire reasoning response as settled, ignores late nonterminal events for that `generationRequestId`, and sends request/reply `CHAT_STOP_MESSAGE` with the same request id. Cancellation strips the marker's pending state without recalculating its content-derived dimensions, and the API uses the same dimension-preserving rule during durable pending-media removal; any remaining tree rebalance therefore cannot squash the stopped pill. The API aborts and awaits every grouped reasoning/media provider, persists all matching ProseMirror response/reasoning sections with receiving animations disabled and streaming prompt blocks closed, then removes the request's persisted pending/partial canvas media nodes. When no live instance, request group, or retained publisher exists, cancellation performs the same transcript and canvas mutations directly against DynamoDB by `generationRequestId`; the API acknowledges stop only after durable cleanup, and the browser then refreshes the stored thread. Both transcript and canvas cancellation therefore survive API restarts and terminal races, so reload cannot resurrect either spinner. Concurrent user-message runs stop independently instead of being coupled to one global composer state.
