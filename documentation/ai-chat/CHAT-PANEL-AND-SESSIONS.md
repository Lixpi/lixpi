...
Every standalone conversation is an Asset with a `conversation` document role. There is no separate chat-thread persistence table. The workspace right-side panel is a view over conversation Assets and Extraction Runs; its tabs and layout remain workspace-local state.
...
type CanvasAiChatPanelState = {
  ...
  topLevelMode: 'features' | 'media' | 'aiThreads'
  tabs: Array<{
    tabId: string
...
Feature extraction sessions remain `ExtractionRun` records and appear alongside conversations. Deleting an Extraction Run does not delete a saved Feature.
...
Pipeline replay is separate from document authority. The durable pipeline log supports reconnect and generated-output provenance materialization. The conversation streams once; output Assets do not receive duplicate live transcript steps.
...
On terminal state, the provenance materializer reads the pipeline log and writes one sealed provenance document per output. Shared reasoning events are included for siblings, while media-run events are filtered to the output’s own `mediaRunId`.
...
On reconnect, the browser resumes the conversation snapshot and step stream, then replays pipeline events from its last sequence. Duplicate messages are suppressed by JetStream message IDs and expected subject sequences.
