...
Every partial image received from a shot provider immediately replaces that panel's live bytes and publishes a newly composed full-sheet preview on the preassigned output Asset. Terminal shot bytes replace the panel's last provider partial; unrendered cells remain blank. Capture-only provider calls never expose an isolated panel as the top-level result. Partial portrait, front-body, and back-body images are presentation-only: only successful terminal results satisfy `generated-identity-anchor`, `generated-outfit-anchor`, or `generated-back-outfit-anchor` and release consumers. When optional shots run concurrently, each preview contains every completed panel plus the latest partial from every in-flight panel. Presentation failures are logged and cannot invalidate provider work or already-rendered results.
...
Observed facts require an authorized source and in-bounds coordinates. Evidence analysis extracts explicit prompt directives and the observed feature names those directives change. The per-shot prompt excludes overridden source facts, places the complete request and sibling Capability instructions ahead of baseline evidence, and never limits changes to camera, crop, or pose. Otherwise observed evidence overrides descriptive prompt text, and evidence from the closest target angle wins. Unresolved conflicts remain in the trace.

Source medium is a separate baseline constraint. Photographic references produce photographic characters unless the raw request or a sibling Capability explicitly requests a different depiction medium or named visual style. Character archetypes and state transformations such as zombie, undead, alien, robot, fantasy creature, or superhero change the depicted character without implicitly authorizing cartoon, chibi, anime, painting, toy-like, illustration, or 3D-rendered styling. Evidence analysis may conservatively normalize an obvious misspelled archetype from context, but it cannot turn an unknown word into an unrequested style.
...
The common graph uses provider-neutral roles such as `canonical-anchor`, `adjacent-angle`, `opposite-angle`, `original-source`, `capability-reference`, `pose-reference`, `face-crop`, `body-outfit-crop`, and `prop-crop`. Sibling Capability reference images are attached to every shot and are scoped by their shared Capability instructions rather than treated as character identity. For the required neutral-front portrait, provider adapters order original evidence before its spatial control. The front full-body request prioritizes the generated portrait, its pose control, and then original evidence. The back full-body request prioritizes the generated front outfit anchor, generated portrait, pose control, and original evidence. Optional requests prioritize the generated front outfit anchor, generated portrait, generated rear-outfit anchor, shot-specific pose control, and original evidence in that order. Adapters trim remaining optional inputs to declared model limits and record included and omitted roles. Other head and outfit-detail shots omit `pose-reference` entirely. OpenAI's multi-image edit conditioning accepts spatial controls as normal image inputs; Google can combine character evidence with a spatial control where the selected model exposes that support. Character Creator and provider logs record whether a shot used a pose control, plus the exact control filename, byte length, digest, final reference order, and included/omitted roles.
