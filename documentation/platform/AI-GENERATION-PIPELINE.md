...
Every durable media run can publish an extensible nested `items` tree with its progress payload. The API creates the run's pending media node when it accepts the request and streams the accumulated tree through that run's JetStream progress subject. The media node exposes a compact progress-ripple control; clicking it mounts the shared `@lixpi/ui-kit` timeline in the existing right sidebar. The control pulses only while the run is active. Switching the sidebar to another media node uses the accumulated node state restored by replay, then continues applying the same live stream without losing earlier steps. Runs that do not publish domain-specific items receive a generic media timeline, so image, video, Capability, Skill, and Tool execution share one component without sharing hardcoded step names. Heartbeats do not update DynamoDB. A terminal transition copies the accumulated progress into the request once, seals it into output provenance, and purges every JetStream subject for the request; accepted-node history mounts the same timeline from provenance.
