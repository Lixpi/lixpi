...
Every durable media run can publish an extensible nested `items` tree with its progress payload. The API creates the run's pending media node when it accepts the request and mirrors progress directly onto that node. The browser renders the tree through `@lixpi/ui-kit` as background-free chrome to the right of that media node, independent of its traveling outline. Runs that do not publish domain-specific items receive a generic media timeline, so image, video, Capability, Skill, and Tool execution share one component without sharing hardcoded step names. Terminal progress is embedded in the output Asset's sealed provenance document and the accepted-node history mounts that same component from provenance.
