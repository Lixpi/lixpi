...
`MediaGenerationRequestEventLog` creates one file-backed stream per Workspace. Each request owns a control-event subject plus one accumulated progress subject per concrete run; request replay reads their shared wildcard in stream order. Every event uses its unique event ID as the JetStream dedupe identity. Progress publication uses `lastSubjectSequence` against the run subject, so concurrent reporters retry from the latest accumulated tree instead of overwriting each other. Character Creator long-running stages emit elapsed-time heartbeats every five seconds, but those heartbeats never rewrite the DynamoDB request. On a terminal run transition, the service reads the latest run subject, settles that complete progress tree into the request's single DynamoDB transition, publishes the terminal event, and purges the request wildcard once all runs are terminal. Payloads may contain run identity, recursive progress, execution traces, current/result summaries, candidate Asset IDs, or normalized `MediaGenerationProblem`; media bytes and secrets remain excluded.

Media-operation nodes and generated-media `generationProgress` are server-managed canvas fields. Each operation stores its stable `outputNodeId`; no branch-marker ownership edge or `branch*.mediaGeneration` state is used. The request-submission acknowledgement carries the initial authoritative canvas geometry, so every stable pending output appears before branch resolution or Capability execution. Browser canvas saves merge in-flight operation nodes and progress-bearing image/video nodes from the current consistent Workspace record, so a stale snapshot cannot delete progress or resurrect a terminal operation. Event recovery resolves the same output node by request/run identity when the temporary operation node is missing.
...
| `CAPABILITY_RUN_EVENT` | `{ runId, event }` with safe `CapabilityRunEvent` data | Shared Tool progress projection in the transcript and live canvas preflight marker; detached clients consume the tokenized run-status family directly | Mirrors a durable chat-originated Tool event after it has been appended to the run log. Step states include pending, running, completed, skipped, failed, and cancelled, with safe current-work and result summaries. |
