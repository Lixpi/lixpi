...
`MediaGenerationRequestEventLog` creates one file-backed stream per Workspace and one subject per request. Events are appended with the request revision as the JetStream dedupe identity. Payloads contain status, run identity, bounded recursive progress, current/result summaries, candidate Asset IDs, or normalized `MediaGenerationProblem`; prompt text, provider response bodies, media bytes, and secrets are excluded. Character Creator long-running stages emit elapsed-time heartbeats every five seconds. Request persistence is the source of truth; request events and the canvas projection are dispatched together after each durable transition, so a temporary projection failure cannot suppress the replayable live update.
