...
Media-operation nodes, their ownership edges, and `branch*.mediaGeneration` are server-managed canvas fields. Browser canvas saves merge those fields from the current consistent Workspace record: they cannot delete an in-flight operation with a stale snapshot or resurrect an operation after terminal removal. Event recovery also projects progress directly onto a matching request marker when the temporary operation node or edge is missing.
