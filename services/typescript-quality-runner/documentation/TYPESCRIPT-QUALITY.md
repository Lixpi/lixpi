...
- A function, method, constructor, arrow function, call, or `new` expression with more than two parameters or arguments is multiline. The shared delimited-list formatter puts one item on each line and adds the trailing comma. One or two items stay on the signature or call line. A call in a member chain is indented against the line its own callee ends on, not against the start of the chain, so the arguments and the closing parenthesis line up under that call.
...
- A conditional expression whose test has more than one logical evaluation splits into a parenthesized multiline test followed by an indented `?` line and an indented `:` line. Every parenthesized subgroup inside the test nests one level further. The rewrite is bounded by the conditional's own `?` and `:` punctuators and by the test's range, so it cannot swallow half of a parenthesis pair belonging to a surrounding call, a grouping around the whole conditional, or a parenthesized branch.
