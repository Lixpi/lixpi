...
- The formatter never introduces a parenthesis. The only pairs it writes are the ones an `if`, `while`, `for` or `do...while` requires and the ones the source already wrote, and it never removes a pair either, because a pair that looks redundant can carry the meaning of the expression. A group the source left unparenthesized stays on one line instead of gaining a bracket to sit inside.
