...
- A function, method, constructor or arrow function with two or more parameters is multiline, and a call or `new` expression with more than two arguments is multiline. The shared delimited-list formatter puts one item on each line and adds the trailing comma. A single parameter stays on the signature line, and one or two arguments stay on the call line.
