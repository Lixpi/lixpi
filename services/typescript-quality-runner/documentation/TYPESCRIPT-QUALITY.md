...
- An assigned logical expression splits the same way once it has more than two operands, or as soon as one of its operands is a parenthesized group: the first operand stays on the assignment line and the rest are indented one level under it. A group breaks the line even with only two operands, so the group and everything the rules put inside it line up one level in from the assignment rather than trailing off the end of it.
