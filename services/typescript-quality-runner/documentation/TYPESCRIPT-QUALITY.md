...
- A call around a single object or array argument keeps that argument's bracket on the call's own line, so it reads `f({` and closes on `})` rather than opening two brackets on two lines for one argument. This applies whenever that argument spans lines, whether a rule above asked for the split or the source was already written that way. A call carrying a second argument alongside it puts each argument on its own line as usual.
