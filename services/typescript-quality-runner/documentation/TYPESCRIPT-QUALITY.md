...
- A function, method, constructor, arrow function, call, or `new` expression with two or more parameters or arguments is multiline. The shared delimited-list formatter puts one item on each line and adds the trailing comma. A call in a member chain is indented against the line its own callee ends on, not against the start of the chain, so the arguments and the closing parenthesis line up under that call.
