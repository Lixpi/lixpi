...
- A callback whose body is an expression rather than a block splits the call as soon as that callback spans lines, so it starts on a line of its own and reads as its own block. A block-bodied callback keeps hugging the call, since its braces already do that job.
