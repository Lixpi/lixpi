...
- A conditional expression whose test has more than one logical evaluation splits into a parenthesized multiline test followed by an indented `?` line and an indented `:` line. The formatter and Oxlint's fixer both address the test, consequent, and alternate through their own AST ranges, so neither can swallow a call's or a grouping's parenthesis that belongs to a surrounding node.
