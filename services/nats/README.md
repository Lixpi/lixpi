...
Builds, dependency installation, formatting and tests run inside Docker. The [Go Testing and Tooling guide](../../documentation/code-quality/testing/GO.md) has the commands. Test runners mount source read-only and keep module/build caches in Docker volumes; the quality runner mounts the directories it needs to edit. [The version registry](../../dev-tools/versions-registry/README.md) generates Go requirements, image tags, and Go tool versions. The [Dockerfile](Dockerfile) compiles Go and ships the executable, NATS configuration and CA roots in its production target. Application registrations come from initialization, and Go tooling stays in build/development images.
