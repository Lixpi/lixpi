...
export const transcodeAudioVideo = async (
    buffer: Buffer,
...
const extractFrameFromVideo = async (
    ...
    try {
        return await withTempDir('av-frame-', async dir => {
            ...
            const seekArgs = typeof atSeconds === 'number'
                && atSeconds > 0
...
export const extractRepresentativeFrame = async (
    videoBuffer: Buffer,
    atSeconds?: number,
): Promise<Buffer | null> => extractFrameFromVideo(videoBuffer, atSeconds)
