...
const errorCodeFor = (
    ...
    const raw = candidate.code
        ?? candidate.message
        ?? `${name.toUpperCase()}_GENERATION_FAILED`
...
const renderTextDocumentPoster = async (buffer: Buffer): Promise<Buffer> => {
    ...
    const text = lines.map(
        (currentLine, index) =>
            `<text x="72" y="${112 + index * 34}" font-family="sans-serif" font-size="24" fill="#182033">${escapeXml(currentLine)}</text>`,
    )
        .join('')
    const svg = Buffer.from(
        `<svg xmlns="http://www.w3.org/2000/svg" width="1024" height="1448"><rect width="1024" height="1448" fill="#fffdf9"/>${text}</svg>`,
    )
...
const makeCanonical = async (
    ...
    if (request.mediaKind === 'image') {
        return await transcodeImage(
            original,
            request.canonicalMimeType,
            {
...
export const generateAssetRenditions = async (
    ...
    if (request.bucketName !== expectedBucketName)
        throw new Error('BLOB_BUCKET_TENANT_MISMATCH')

    if (request.sourceObjectKey !== getObjectKey(request.sourceBlobHash))
        throw new Error('SOURCE_OBJECT_KEY_MISMATCH')

    if (!/^[a-f0-9]{64}$/.test(request.sourceBlobHash))
        throw new Error('INVALID_SOURCE_BLOB_HASH')
    ...
    if (hashBytes(original) !== request.sourceBlobHash)
        throw new Error('SOURCE_BLOB_HASH_MISMATCH')
    ...
    if (requested.has('original'))
        renditions.push(originalResult)
        ...
        try {
            ...
            for (const name of requested) {
                if (
                    ...
                    renditions.push(
                        failed(name, 'CANONICAL_UNAVAILABLE'),
                    )
            }
    ...
    if (request.mediaKind === 'image') {
        ...
        if (
            ...
            response.aspectRatio = metadata.width / metadata.height
        ...
        for (const config of [
            ...
            if (!requested.has(config.name))
                continue

            try {
                const generated = await createImageRendition({
                    buffer: modelBuffer,
                    ...config,
                })
                renditions.push(
                ...
                renditions.push(
                    failed(
                        config.name,
                        errorCodeFor(config.name, error),
                    ),
                )
            }
        }
    ...
    if (request.mediaKind === 'video') {
        ...
        if (probe.aspectRatio !== null)
            response.aspectRatio = probe.aspectRatio
        ...
        if (
            requested.has('poster')
            || requested.has('thumbnail')
        ) {
            try {
                ...
                if (poster) {
                    try {
                        ...
                        renditions.push(
                            failed(
                                'poster',
                                errorCodeFor('poster', error),
                            ),
                        )
                    }
                } else
                    renditions.push(
                        failed('poster', posterErrorCode),
                    )
            }
        }
        ...
        if (requested.has('thumbnail')) {
            if (!poster)
                renditions.push(
                    failed('thumbnail', 'POSTER_UNAVAILABLE'),
                )
            else {
                try {
        ...
        if (requested.has('representativeFrame')) {
            try {
                ...
                if (frame) {
                    ...
                    renditions.push(
                        failed('representativeFrame', 'REPRESENTATIVE_FRAME_EXTRACTION_FAILED'),
                    )
            } catch (error) {
                renditions.push(
                    failed(
                        'representativeFrame',
                        errorCodeFor('representativeFrame', error),
                    ),
                )
            }
        }
    ...
    if (modelMimeType === 'application/pdf') {
        ...
        modelMimeType === 'text/plain'
        || modelMimeType === 'text/markdown'
    ) {
        try {
