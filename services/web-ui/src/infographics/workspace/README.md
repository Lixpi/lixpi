...
- Multi-reasoning media requests create one temporary `branchFork` marker per reasoning run under the current lineage source; generated media from that reasoning run persist `generatedBy.branchForkNodeId` and edge from that fork, so the canvas shows separate reasoning-model lineages before the image/video variants fan out. Clicking a `branchFork` opens the same generated branch provenance panel as a branch origin, scoped to media under that fork so the chat reconstruction uses only the relevant reasoning model response. Fresh standalone runs with no source still render the `branchOrigin` first, then the `branchFork` split marker
