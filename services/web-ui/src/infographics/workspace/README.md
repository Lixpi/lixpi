...
- Render the editable Asset title through a title-only ProseMirror document above the media node; render editable title and description metadata together at the top of its details panel. The background-free info-letter button sits before the background-free model glyph and opens only Asset metadata/details. Sealed provenance is resumed for candidate outputs as soon as it exists so replay and history read the same immutable per-Asset source as the API; accepting an output changes topology and history visibility, not the provenance source. All media chrome icon controls use equal boxes, normalized visual glyph bounds, the provider-icon color at rest, and the branch-review controls' 30px visual baseline; both surfaces follow the same adaptive bounded zoom curve. The info/model pair and accept/retry pair each use the same vertical branch-lineage fading gradient divider. Candidate generated media exposes background-free checkmark and refresh actions; prompt regeneration belongs exclusively to the branch-lineage controls. The larger right-aligned, branch-marker-styled user-message pill is hidden until acceptance, uses a compact whole-atom preview that never slices through Capability badges, is prefixed by the same reasoning-model provider icon used on branch-lineage messages, and is the only media-node action that opens the generating chat-turn history in a separate panel.
...
- Completed branch markers expose right-aligned, background-free checkmark and refresh icons separated by the same fading divider and optical spacing used by media review controls. The complete control pair uses the generated-media chrome adaptive bounded zoom curve, while hover colors use the shared settings-backed hover duration and `hoverTransition(...)`. The refresh icon opens a two-option menu for Regenerate variants and Regenerate prompt. Asset completion, rather than delayed request-group cleanup, restores these controls; provenance sealing then enables them. Accept all detaches every candidate child and prunes an empty marker. Regenerate variants preserves the marker and replays each child's sealed media prompt without rerunning reasoning. The API selects the exact trace by immutable reasoning/media run identity and supplies the canonical stored prompt; client prompt text is not authoritative. The API lineage plan explicitly identifies the preserved regeneration parent, and the browser never synthesizes, promotes, repositions, or deletes that marker while replaying variants; every client applies the same persisted API canvas geometry. Replay planning cannot inherit a resolver-selected media parent because the preserved marker is its only topology parent. Regenerate prompt removes the candidate set and old empty marker before submitting the full model set through normal reasoning as a new lineage.
...
1. **Candidate snapshot** - On submit with an image model selected, the browser builds an `ImageBranchCandidateSnapshot` from composer context preview media, generated image nodes, lineage metadata, and thread transcript labels. Every candidate has a stable `candidateId` and `assetId`; canvas-backed candidates also carry `nodeId`. Standalone panel submits include generated media from the active chat thread plus explicit panel references so deictic follow-ups stay scoped to that chat before workspace-wide relevance runs. If exactly one image node in the active thread context is selected, the snapshot marks it as `active-target` and records `activeTargetCandidateId` so purely deictic prompts can continue the selected lineage. The selected image is not sorted ahead of other candidates; the API VLM must still inspect all candidate pixels and let explicit subject text override a conflicting selection. This snapshot is non-authoritative. The API augments it with authorized Asset-only prompt references, which can influence context and provenance but never canvas placement.
