...
- Fresh multi-model generations with no source/thread node render an API-declared temporary `branchOrigin` root marker, using the configured branch icon and referenced by every child output through API-assigned `generatedBy.branchOriginNodeId`; the marker is draggable lineage chrome rather than chat context, so it is not selectable or context-chip eligible. Branch origin, fork, and continuation markers render as labeled ovals with the marker icon, the marker action label, one-pixel grayscale fading dividers, provider-icon model rows for reasoning and invoked image/video generation, and a two-line truncated prompt preview. Clicking a marker opens the provenance panel for the user's prompt, explicitly provided references, and branch-fork decision.
...
- The bottom-center canvas AI input creates pending branch markers immediately for media-enabled submits, right-aligned above the input with the submitted prompt as the primary row, a reasoning-model icon, media model rows only, and a spinner while the marker is still pending. Multi-reasoning submits create one stacked pending marker per reasoning model, each duplicating the prompt and later promoting independently to its API-declared fork/line marker. Marker height is derived from prompt/media-model content instead of a fixed oval size. While preflight is waiting on the API lineage plan, each marker uses inverse viewport scaling inside the transformed canvas layer so it stays a comfortable screen size at any zoom. When the API publishes the media lineage plan, each marker animates both position and scale into its own planned canvas position before generated media placeholders or completions attach.
...
Toolbar image insertion uses `settings.mediaNode.image.defaultInsertionWidth` as the canvas-unit width for new image nodes. The inserted height is derived from the natural image aspect ratio; if the client cannot probe dimensions, the fallback node is a square using that same configured width.
