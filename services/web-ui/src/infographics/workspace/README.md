...
- Generated media chrome renders in a screen-space layer above the viewport DOM but below the PIXI media layer. Before the first frame, its centered control is the active progress ripple; after the frame resolves, the below-node strip contains that trace control, the info button, provider/model badge, and candidate review actions. The trace control remains visible after the run finishes and after acceptance: active nodes read their live accumulated progress, while terminal nodes recover the same timeline from sealed provenance. Only active states animate. The strip uses the shared bounded zoom curve and participates in collision geometry, while the right-sidebar timeline does not. The trace control opens the selected node's accumulated progress without changing node bounds or connector anchors. The info button still opens the decoupled metadata/provenance panel in `.workspace-generated-media-info-panel-layer`, and accepted history remains sourced from the producing thread turn plus sealed Asset provenance.
...
| `mediaGenerationTraceButton.ts` | Media-node progress-ripple control; pulses for active runs, stays static for terminal runs, and opens that node's accumulated trace in the right sidebar |
...
| `.media-generation-trace-button` | Compact media-node control using the shared progress ripple; opens the node-scoped timeline in the right sidebar |
| `.workspace-media-generation-trace-panel` | Single white right-sidebar surface, aligned to the mode tabs with a compact 8px content inset, that mounts and live-updates the selected media node's complete timeline; inner trace sections remain box-free, share the sidebar's 14px content baseline with chat messages, use the shared media-model badge, render categorical roles and outcomes with intrinsic-width inline-flex tag pills whose 12px regular-weight typography comes from `settings.rightSidePanel.typography`, translate raw trace keys and scores into readable labels and percentages, group duplicate facts, and suppress internal UUID metadata |
...
| `.workspace-generated-media-pending-icon` | Centered progress-ripple trace control used while a generated-media placeholder has not received its first frame |
