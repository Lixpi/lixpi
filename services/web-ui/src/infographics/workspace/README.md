...
- `WorkspaceCanvas.ts` is presentational for generated-media lineage. It applies `IMAGE_BRANCH_RESOLVED` and `MEDIA_LINEAGE_PLANNED`, computes marker/media geometry, and re-tidies the visible tree. It must not decide branch IDs, branch-root creation, fork creation, lineage parentage, reasoning-model fanout, resolver outcomes, or marker provenance. Those decisions belong in `services/api`.
- Missing API lineage is a hard failure for generated-media topology. Do not add browser fallbacks, compatibility shims, edge-derived parents, existing-node recovery, model-count heuristics, or DOM-state recovery in `WorkspaceCanvas.ts`, `branchTreeLayout.ts`, marker renderers, or event handlers. Fix the API lineage plan, stream timing, or data migration instead.
...
- Fresh multi-model generations with no source/thread node render an API-declared temporary `branchOrigin` root marker, using the configured branch icon and referenced by every generated media output through API-assigned `generatedBy.branchOriginNodeId`; the marker is draggable lineage chrome rather than chat context, so it is not selectable or context-chip eligible. When no reference group anchors the first output, the branch-origin-to-first-media group is centered as one unit so the origin marker does not start outside the visible viewport. Branch origin, fork, and continuation markers render as labeled ovals with the marker icon, the marker action label, one-pixel grayscale fading dividers, provider-icon model rows for reasoning and invoked image/video generation, and read-only text projected from the stored AI chat ProseMirror message. Clicking a marker opens the lineage provenance panel for explicitly provided references and branch-fork decision metadata.
- Multi-reasoning media requests render one API-declared temporary `branchFork` marker per reasoning run under the planned lineage source; generated media from that reasoning run persist API-assigned `generatedBy.branchForkNodeId` and edge from that fork, so the canvas shows separate reasoning-model lineages before the image/video variants fan out. When one reasoning run fans out to several image/video models, every media output from that reasoning run shares the same `branchFork` marker instead of creating media-model fork markers. Clicking a `branchFork` opens the generated branch provenance panel scoped to media under that fork so the chat reconstruction uses only the relevant reasoning model response. Fresh standalone runs with no source still render the `branchOrigin` first, then the `branchFork` split marker
...
- Depth spacing uses `settings.mediaBranchLineage.mediaToMediaGap`, plus `branchFanoutExtraGap` for each extra generated media node when a lineage forks; the first segment from a temporary `branchOrigin` marker uses `branchOriginToFirstMediaGap`. Sibling spacing uses `branchRowGap`. The root keeps its anchor, generated media fan out symmetrically around its vertical center, and linear chains stay collinear. Before the first generated frame exists, tree placement uses the configured pre-frame circle size from `settings.mediaNode.inProgressOutlineAnimation.preFrameCircleScale` instead of the hidden full media box, so the visible pending node never sits farther away than the full generated media node will after frame arrival. Final image/video aspect-ratio updates preserve the node center, then re-tidy the tree so resolved media proportions cannot collapse a fork back onto the predecessor center line
...
### Media Library Panel
- Implemented in `mediaLibraryPanel.ts` and `media-library-panel.scss` inside this canvas module; Svelte supplies the independent bottom-right launcher above the zoom badge, and both bottom controls align to the same AI Chat panel gap as the chat toggle when the panel is open.
...
This section documents canvas-thread nodes that can exist in persisted workspace state. The workspace renderer does not create new `aiChatThread` canvas nodes; active chat sessions live in the right-side AI Chat panel.
...
- Render-state reconciliation is planned by `workspaceRenderStatePlan.ts`. When the active AI chat panel emits a stale metadata render while a local canvas commit is still waiting for store acknowledgement, the canvas preserves the locally committed visual node/edge state until the store catches up. This includes generated-media connector edges whose target node may appear in metadata before the edge does.
...
- The detached prompt input is not used for new sessions; active composers live in the AI Chat panel and the screen-fixed canvas composer
...
| `.canvas-generated-media-info-panel` | Expanded generated-media metadata panel anchored beneath the chrome strip and matched to the media node's world width; its content scales only through the normal canvas viewport transform, and it expands to its full content height without internal cropping |
