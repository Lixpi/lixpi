...
- **Zoom-compensated scaling**: Connector stroke width, arrowhead marker sizes, marker offsets, and invisible hit areas use bounded inverse zoom. From the configured lower breakpoint upward, their screen size stays constant; below that breakpoint, world size freezes so overview zooms naturally render them thinner instead of chunky. Connector scaling is controlled by `settings.connector.useZoomCompensatedScaling`; stroke width, marker size, marker offsets, hit-area width, and breakpoint live under `settings.connector.scaling`. Workspace connector pixels, hit testing, and edge bubble menu anchoring all use cached PIXI path data.
