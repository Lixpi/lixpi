...
- Render the editable Asset title through a title-only ProseMirror document above the media node; render editable title and description metadata together at the top of its details panel. The info button sits before the model badge and opens only Asset metadata/details; this path never resumes or mounts the Asset provenance document. Both icon controls use equal chrome boxes and matched visual glyph diameters, separated by a vertical instance of the branch-lineage fading gradient divider. Generated media adds a larger right-aligned, branch-marker-styled user-message pill capped at 20 characters; activating it is the only media-node action that opens the generating chat-turn history in a separate panel.
