...
- Generated media model chrome renders in a screen-space chrome layer above PIXI. The strip projects the media node's world bounds through the live viewport, uses the same bounded screen-size curve as connector chrome, shows the provider icon plus the pretty model title from the model catalog on the left, and keeps the media info button aligned to the right edge of the media node. Its top gap and lower zoom breakpoint live under `settings.imageNode.generatedMediaChrome`. The info button opens a full-media-width block below the image or video that mounts a scoped read-only AI chat ProseMirror projection for the producing turn, including the real chat message, reasoning, selected generated-media node, and generation-details NodeViews. Branch-fork provenance intentionally keeps sibling generated outputs visible. Generated-media provenance does not append the separate media descriptor because the user prompt and response already provide that context. The canvas provenance block expands to its full content height; it does not crop long prompts or reference metadata.
...
Generated media model labels, info buttons, and expanded provenance panels do not live inside the image node shell. They render in `.workspace-generated-media-chrome-layer`, above the PIXI media layer, so stored image sprites cannot cover them.
...
| `.workspace-image-chrome-viewport` | Transformed overlay layer for video surfaces, video controls, and branch provenance info above PIXI media sprites |
| `.workspace-generated-media-chrome-layer` | Screen-space layer for generated media model chrome and media info buttons projected from the live viewport |
...
| `.workspace-generated-image-chrome` | Zoom-compensated generated-media chrome container positioned below the media node at the exact node width |
| `.image-model-badge` | Generated-media provider icon and pretty model title label |
| `.image-info-button` | Right-aligned icon button that expands the generated-media metadata block without changing its icon color or outline when open |
