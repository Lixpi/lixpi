...
- Render the editable Asset title through a title-only ProseMirror document above the media node; render editable title and description metadata together at the top of its details panel. The info button sits before the model badge and opens only Asset metadata/details. Sealed provenance is resumed for candidate outputs as soon as it exists so replay and history read the same immutable per-Asset source as the API; accepting an output changes topology and history visibility, not the provenance source. Both icon controls use equal chrome boxes and matched visual glyph diameters, separated by a vertical instance of the branch-lineage fading gradient divider. Candidate generated media exposes only Accept and Again actions; prompt regeneration belongs exclusively to the branch-lineage controls. The larger right-aligned, branch-marker-styled user-message pill is hidden until acceptance, capped at 20 characters, prefixed by the same reasoning-model provider icon used on branch-lineage messages, and is the only media-node action that opens the generating chat-turn history in a separate panel.
