...
- Multi-model media requests keep one shared pending placement group per `generationRequestId`, then track each image/video media run by `mediaRunId`. The API lineage-plan projection persists the branch markers before provider execution; each planned media slot stays transient until its first media event atomically attaches the preassigned Asset to the Workspace. A reasoning child that emits no media settles only its own marker, while the matrix-level completion settles the shared request after every reasoning child finishes. Trace events attach live run tracking to those API-owned assignments, so one completed variant does not delete the shared lineage/reference placement while siblings are still running. Every generated sibling receives the same branch resolution plus its own run metadata in `generatedBy`, and branch-tree sibling order uses `variantIndex` before `createdAt` for deterministic fanout layout. Marker text normally comes from the authoritative conversation document and falls back to API-persisted marker provenance while that document is still resuming. Its response row prefers the model's conversational preamble, then falls back to non-empty collapsible reasoning/generation text when a provider emits no standalone preamble; generated image/video metadata never creates a response row by itself.
...
5. **Prompt handoff** — `IMAGE_GENERATION_TRACE` / `VIDEO_GENERATION_TRACE` marks the handoff from reasoning model to media model and clears the reference outlines. For images, later partials apply API geometry snapshots to update only the generated node.
