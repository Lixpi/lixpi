...
- AI chat threads have no on-canvas node. A thread is a read-only transcript hosted in the canvas-owned right side panel, and all prompting happens in the bottom-center canvas composer. The former on-canvas thread node and its vertical connection rail have been removed.
...
- **Floating panel resize** — the canvas-owned right side panel uses its `SidePanel` resize handle on the left edge as the horizontal resize target. Dragging that handle changes `--workspace-right-side-panel-width`, keeping the panel right edge fixed and preserving the zoom indicator offset. Dimensions, resize-handle geometry, toggle geometry, and slide timing are configured via `settings.rightSidePanel`.
