...
- The bottom-center canvas AI input creates a pending branch marker immediately for media-enabled submits, right-aligned above the input with the submitted prompt, selected reasoning/media models, and a spinner. While preflight is waiting on the API lineage plan, the marker uses inverse viewport scaling inside the transformed canvas layer so it stays a comfortable screen size at any zoom. When the API publishes the media lineage plan, that same marker is promoted to the API-declared branch origin/fork/line marker and animates both position and scale into its planned canvas position before generated media placeholders or completions attach.
