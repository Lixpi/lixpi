...
function isWorkspaceCanvasDebugEnabled(): boolean {
    try {
        return window.localStorage.getItem('lixpi.debug.workspaceCanvas') === '1'
    } catch {
        return false
    }
}
...
export function createWorkspaceCanvas(options: WorkspaceCanvasOptions) {
    ...
    // Top-level library and AI-thread switch that lives at the top of the
            ...
            onReplaceMedia: (nodeId) => {
                ...
                input.addEventListener('change', async () => {
                    ...
                    const attachedState: CanvasState = {
                        ...
                        nodes: [...committedDetachedState.nodes, replacementNode],
                        edges: originalState.edges,
                    }
                    const committedAttachedState = await onAssetAttach({
                        assetId: data.assetId,
                        nodeId,
                        canvasState: attachedState,
                    })
                    commitTransientCanvasStatePreservingEditors(committedAttachedState)
                })
    ...
    function updateGeneratedMediaChromeLiveTransform(
        ...
        const pendingCircleGeometry = getPendingGeneratedMediaBeforeFrameCircleGeometry(nodeId, position, dimensions)
        if (chromeEl) {
            applyGeneratedMediaChromeGeometry(
                chromeEl,
                pendingCircleGeometry?.position ?? position,
                pendingCircleGeometry?.dimensions ?? dimensions,
                viewport,
                videoControlsOffsetScreen,
            )
        }
    ...
    function buildBranchMarkerTurnProjectionContent(
        ...
        const projection = buildBranchMarkerTurnProjectionFromThreadContent(
            ...
            getBranchMarkerTurnDescriptor(marker),
            {
                ...
                forceGenerationDetailsOpen: true,
                lineageProjectionScope,
                allowLatestTurnFallback: canUseLatestBranchMarkerTurnFallback(marker),
            },
        )
        if (!projection) return null
    ...
    function createAssetDetailsSection(
        ...
        if (node.type === 'capabilityArtifact') {
            ...
            const documentRoles = Object.keys(asset.documents)
            renditionsEl.textContent = documentRoles.length > 0 ? documentRoles.join(' · ') : 'No documents'
        } else {
    ...
    function openGeneratedOutputDetails(
        ...
        if (options.toggle && generatedOutputDetailsTargetsMatch(aiChatPanelState.generatedOutputDetailsTarget, target)) {
    ...
    function closeGeneratedOutputDetails(): void {
        const { generatedOutputDetailsTarget: _removedTarget, ...panelState } = aiChatPanelState
        aiChatPanelState = panelState
        persistAiChatSidebarState()
        syncMediaGenerationTraceButtons(currentCanvasState)
        renderActiveAiChatPanel()
    }
    ...
    function getBranchGeneratedArtifactNodes(
    ...
    function createMediaInfoButton(node: ImageCanvasNode | VideoCanvasNode): HTMLButtonElement {
        ...
        const target: CanvasGeneratedOutputDetailsTarget = { kind: 'output', nodeId: node.nodeId }
        const isExpanded = generatedOutputDetailsTargetsMatch(aiChatPanelState.generatedOutputDetailsTarget, target)
    ...
    function createMediaAcceptButton(node: ImageCanvasNode | VideoCanvasNode): HTMLButtonElement | null {
        ...
        const handleClick = (event: MouseEvent): void => {
            event.preventDefault()
            event.stopPropagation()
            void acceptGeneratedOutput('output-node', node.nodeId)
        }
        const button = html`
            <button
    ...
    function createMediaGenerationTraceControl(
        ...
        const selected = generatedOutputDetailsTargetsMatch(aiChatPanelState.generatedOutputDetailsTarget, {
            kind: 'output',
            nodeId: node.nodeId,
        })
    ...
    function createGeneratedMediaChrome(node: ImageCanvasNode | VideoCanvasNode): HTMLElement {
        const isPendingBeforeFirstFrame = isPendingGeneratedMediaBeforeFirstFrame(node.nodeId)
        ...
        const modelBadge = isPendingBeforeFirstFrame
            ? null
            : createMediaModelBadge(resolveMediaModelBadgeConfig({ modelId, modelProvider }))
        const acceptButton = isPendingBeforeFirstFrame ? null : createMediaAcceptButton(node)
        const rejectButton = isPendingBeforeFirstFrame ? null : createMediaRejectButton(node)
        const regenerationControls = isPendingBeforeFirstFrame ? null : createMediaRegenerationControls(node)
        const chromeEl = html`
            <div className="workspace-generated-media-chrome" data=${{ mediaChromeNodeId: node.nodeId }}>
                ...
                <div className=${isPendingBeforeFirstFrame
                    ? 'workspace-generated-media-actions workspace-generated-media-pending-actions'
                    : 'workspace-generated-media-actions'}>
        ...
        applyGeneratedMediaChromeGeometry(
            chromeEl,
            pendingCircleGeometry?.position ?? nodeWorldPosition,
            pendingCircleGeometry?.dimensions ?? node.dimensions,
            viewport,
    ...
    function getCapabilityArtifactProvenance(node: CapabilityArtifactCanvasNode): Record<string, any> {
        const document = assetDocumentsStore.get(node.assetId, 'provenance')?.doc
        const text = document ? extractContentFromProseMirror(document).text : ''
        if (text) {
            try {
    ...
    function createCapabilityArtifactInfoButton(node: CapabilityArtifactCanvasNode): HTMLButtonElement {
        ...
        ><span innerHTML=${infoLetterIcon}></span></button>` as HTMLButtonElement
    ...
    function createCapabilityArtifactAcceptButton(node: CapabilityArtifactCanvasNode): HTMLButtonElement | null {
        if (!node.generatedBy || isGeneratedOutputAccepted(node)) return null
        const asset = assetsStore.get(node.assetId)
    ...
    function createGeneratedCapabilityArtifactChrome(node: CapabilityArtifactCanvasNode): HTMLElement {
        ...
        const chrome = html`<div className="workspace-generated-media-chrome" data=${{ mediaChromeNodeId: node.nodeId }}>
            ...
            <div className="workspace-generated-media-actions">
                ${createCapabilityArtifactInfoButton(node)}
                ${modelBadge ? html`<div className="media-info-model-separator media-review-action-separator" aria-hidden="true"></div>` : null}
                ${modelBadge}
                ${acceptButton}
        ...
        applyGeneratedMediaChromeGeometry(chrome, getNodeWorldPosition(node), node.dimensions, getLiveViewport(), 0)
        return chrome
    }
    ...
    function getCapabilityArtifactNodeChromeKey(node: CapabilityArtifactCanvasNode): string {
        const asset = assetsStore.get(node.assetId)
        const artifact = assetDocumentsStore.get(node.assetId, 'capabilityArtifact')
        const provenance = assetDocumentsStore.get(node.assetId, 'provenance')
        return [
            node.nodeId,
            node.assetId,
            node.artifactTypeId,
    ...
    function getGeneratedMediaChromeSyncKey({
        ...
        return [
            ...
            pendingIconNodes.map((node) => [
                ...
                node.assetId,
                getGeneratedMediaModelId(node),
                getGeneratedMediaModelProvider(node, getGeneratedMediaModelId(node)),
    ...
    function syncMediaGenerationTraceButtons(canvasState: CanvasState | null): void {
        for (const [nodeId, control] of mediaGenerationTraceButtons) {
            ...
            control.update(traceState.status, generatedOutputDetailsTargetsMatch(aiChatPanelState.generatedOutputDetailsTarget, {
                kind: 'output',
                nodeId,
            }))
        }
    ...
    function syncGeneratedMediaChrome(canvasState: CanvasState | null = currentCanvasState): void {
        ...
        // or a descriptor gets the below-node action chrome, including pre-first-
        // frame placeholders. Pending placeholders keep only their model badge in
        // the circle center while info + active trace controls stay below-left.
        // Video chrome reserves space for the external playback-control row.
    ...
    function clearFinalizingGeneratedImageOutline(nodeId: string): void {
        ...
        if (nodeEl) updatePendingGeneratedMediaBeforeFrameClass(nodeEl, nodeId)
        syncPixiMediaLayer(currentCanvasState)
    ...
    function getPendingGeneratedMediaBeforeFirstFrameNodeIds(): Set<string> {
        ...
        for (const pending of partialImageTracker.values()) {
            if (!pending.hasReceivedFrame && !hasTerminalGeneratedMediaProgress(pending.nodeId)) {
                nodeIds.add(pending.nodeId)
            }
        }
        for (const pending of videoGenerationTracker.values()) {
            if (!pending.hasReceivedFrame && !hasTerminalGeneratedMediaProgress(pending.nodeId)) {
    ...
    function hasTerminalGeneratedMediaProgress(nodeId: string): boolean {
        const node = currentCanvasState?.nodes.find(candidate => candidate.nodeId === nodeId)
        return Boolean(
            node
            && (node.type === 'image' || node.type === 'video')
    ...
    function isPendingGeneratedMediaBeforeFirstFrame(nodeId: string): boolean {
        if (hasTerminalGeneratedMediaProgress(nodeId)) return false
        for (const pending of partialImageTracker.values()) {
    ...
    // Shared with the API layout. Resolved media reserves its title above and
    // action/model strip below. Pending media keeps its final horizontal width
    // while using the compact circle and live progress timeline vertically.
    function getMediaChromeCollisionInsets(node: CanvasNode): { top: number; bottom: number } {
        if (node.type !== 'image' && node.type !== 'video' && node.type !== 'capabilityArtifact') {
    ...
    function getCanvasNodeCollisionRect(
        ...
        if (pendingCircleGeometry) {
            ...
            const chromeInsets = getMediaChromeCollisionInsets(node)
            collisionRect = {
                x: worldPosition.x,
    ...
    function shouldRenderOperationStatusNode(node: OperationStatusCanvasNode): boolean {
        ...
        if (node.operation === 'media-generation' && node.status === 'failed') {
            const isSupersededByReadyOutput = (currentCanvasState?.nodes ?? []).some(candidate => (
    ...
    function getNodesForConnectionManager(nodes: CanvasNode[]): CanvasNode[] {
        ...
        return nodes.filter(shouldRenderCanvasNode).map((node: CanvasNode) => {
            ...
            const pendingVisualGeometry = finalizingGeneratedImageRunKeysByNodeId.has(node.nodeId)
                ? null
    ...
    function getRightPanelSwitchViewportWidth(panelWidth = getRightSidePanelCurrentWidth()): number {
    ...
    function resizeActiveRightPanelModeSwitch(): void {
        ...
        const viewportWidth = switchEl?.clientWidth ?? getRightPanelSwitchViewportWidth()
        activeRightPanelModeSwitch.resize(0, 0, viewportWidth, settings.aiChatThread.panelSwitch.height)
    }
    ...
    function getRightPanelModeSwitchTransitionDuration(previousMode: CanvasRightSidePanelMode, nextMode: CanvasRightSidePanelMode): number {
        ...
        if (previousIndex < 0 || nextIndex < 0) return settings.aiChatThread.panelSwitch.transitionDurationMs

        const travelDistance = Math.max(1, Math.abs(nextIndex - previousIndex))
        const speedup = 1 + (travelDistance - 1) * settings.aiChatThread.panelSwitch.transitionDistanceSpeedupFactor
        return Math.max(
            settings.aiChatThread.panelSwitch.transitionMinDurationMs,
            Math.round(settings.aiChatThread.panelSwitch.transitionDurationMs / speedup)
        )
    }
    ...
    function applyRightPanelModeBody(nextMode: CanvasRightSidePanelMode): void {
        ...
        renderActiveAiChatPanel({ preserveModeSwitch: true })
    }
    ...
    function destroyActiveAiChatPanel(
        ...
        artifactLibraryPanelInstance?.unmount()
    ...
    function persistAiChatSidebarState(): void {
        ...
        if (JSON.stringify(currentCanvasState.aiChatPanel) === JSON.stringify(nextCanvasState.aiChatPanel)) return
        commitCanvasMetadataState(nextCanvasState)
    }
    ...
    function getPromptReferenceCanvasNodeIds(contentJSON: unknown): string[] {
    ...
    function getLatestUserPromptReferenceCanvasNodeIds(content: unknown, threadId: string): string[] {
    ...
    function getLatestUserPromptText(content: unknown, threadId: string): string {
        const root = parseProseMirrorJsonContent(content)
        if (!root) return ''
        const threadNode = findAiChatThreadContentNode(root, threadId)
        const latestUserMessage = [...(threadNode?.content ?? [])]
    ...
    function renderActiveAiChatPanel(
        ...
        if (!aiChatPanelState.isOpen) {
            if (!activeClosingRightSidePanel) destroyActiveAiChatPanel()
        ...
        destroyActiveAiChatPanel(false, Boolean(preservedModeSwitchEl))
        ...
        const generatedOutputDetailsTarget = aiChatPanelState.generatedOutputDetailsTarget
        const generatedOutputDetailsNode = resolveGeneratedOutputDetailsNode(generatedOutputDetailsTarget)
        if (generatedOutputDetailsTarget && !generatedOutputDetailsNode) {
            const { generatedOutputDetailsTarget: _removedTarget, ...panelState } = aiChatPanelState
            aiChatPanelState = panelState
            persistAiChatSidebarState()
        }

        const modeSwitchEl = preservedModeSwitchEl
            ?? html`<div className="workspace-right-panel-mode-switch"></div>` as HTMLDivElement
        if (!preservedModeSwitchEl) {
            ...
            activeRightPanelModeSwitch = createSlidingSwitch<CanvasRightSidePanelMode>(modeSwitchSvg, {
                ...
                x: 0,
                y: 0,
                width: getRightPanelSwitchViewportWidth(),
                height: settings.aiChatThread.panelSwitch.height,
                options: [
                ...
                transition: {
                    durationMs: settings.aiChatThread.panelSwitch.transitionDurationMs,
        ...
        if (showingAiThreads && generatedOutputDetailsNode) {
            ...
            panelEl.appendChild(html`
                <div className="workspace-generated-output-details-empty nopan">
                    Select a media item or lineage marker to view its details.
                </div>
            ` as HTMLDivElement)
            ...
            artifactLibraryPanelInstance?.unmount()
        } else if (topLevelMode === 'artifacts') {
            const artifactHost = html`<div className="workspace-right-panel-artifact-host workspace-right-panel-media-host"></div>` as HTMLDivElement
            panelEl.appendChild(artifactHost)
            mediaLibraryPanelInstance?.unmount()
            destroyCapabilityLibraryPanel()
            ensureArtifactLibraryPanel().mountInto(artifactHost)
        } else {
        ...
        panelEl.appendChild(activeRightSidePanel.element)
        ...
        if (shouldAnimateOpen) void playRightSidePanelOpen(activeRightSidePanel, panelEl)
        hasRenderedInitialAiChatPanelState = true
    ...
    function failDetachedCanvasRun(threadId: string): void {
        ...
        removePendingBranchMarkerForRun(threadId)
    ...
    function createDetachedCanvasThreadEditor({
        ...
        const editor = new ProseMirrorEditor({
            ...
            onAiChatSubmit: async ({
                ...
                try {
                    ...
                    // ContentExtractor.toMessages intentionally serializes prompt-reference
                    // atoms as non-text context, so its scalar text omits their visible label.
                    // The canvas marker and media preflight must preserve the exact submitted
                    ...
                    const explicitMediaReferenceNodeIds = getExistingMediaNodeIds(submittedExplicitContextNodeIds)
        ...
        aiService = new AiInteractionService({
            ...
            conversationAssetId: threadId,
            organizationId: thread.organizationId,
            onError: () => failDetachedCanvasRun(threadId),
        })
    ...
    async function submitCanvasGenerationRun(
        ...
        const explicitContextNodeIds = Array.from(new Set([
            ...(options.explicitContextNodeIds ?? aiChatPanelState.contextChips),
            ...getPromptReferenceCanvasNodeIds(data.contentJSON),
        ]))
    ...
    function recoverPendingBranchMarkerRecordFromOverlay(
        threadId: string,
        generationRun?: MediaGenerationRunMeta,
    ): PendingBranchMarkerRecord | undefined {
        ...
        const placementKey = getGeneratedMediaPlacementKey(threadId, generationRun)
        const reasoningModelId = identity.reasoningModelId ?? generationRun?.reasoningModelId
        const reasoningIndex = identity.reasoningIndex ?? generationRun?.reasoningIndex
        ...
        pendingBranchMarkers.set(placementKey, record)
        setPendingBranchMarkerRecordAliases(threadId, generationRun, record)
        console.info('[CANVAS] recovered screen-fixed branch marker for lineage handoff', {
            threadId,
            generationRequestId: generationRun?.generationRequestId ?? '',
            mediaRunId: generationRun?.mediaRunId ?? '',
            recoveredNodeId: identity.nodeId,
            reasoningIndex: record.reasoningIndex,
    ...
    function applyPendingBranchMarkerScreenProjection(
        ...
        if (debugLoggingEnabled && !branchMarkerHandoffDebugKeys.has(debugKey)) {
    ...
    function getMatchingScreenFixedPendingBranchMarkerRecord(node: BranchMarkerNode): PendingBranchMarkerRecord | null {
        ...
        for (const record of pendingBranchMarkers.values()) {
            ...
            // During a structural rebuild the preflight element is created in the
            // viewport first and moved to the screen-fixed overlay after the loop.
            // Its pending record is still the owner during that short interval.
            if (markerEl) return record
        }
        return null
    }
    ...
    function resolveVisiblePendingBranchMarkerOwner(
        threadId: string,
        generationRun: MediaGenerationRunMeta | undefined,
        fallbackNodeId: string,
        plannedNodeId: string,
    ): string {
        const overlayIdentities = getPendingBranchMarkerOverlayIdentities(
            pendingBranchMarkerOverlayEl,
            threadId,
        )
        const matchedIdentity = findPendingBranchMarkerOverlayIdentity(
            pendingBranchMarkerOverlayEl,
            threadId,
            generationRun?.reasoningIndex,
            generationRun?.reasoningModelId,
        )
        const visibleOwnerNodeId = matchedIdentity?.nodeId ?? fallbackNodeId
        console.info('[CANVAS] branch marker lineage ownership handoff', {
            threadId,
            generationRequestId: generationRun?.generationRequestId ?? '',
            mediaRunId: generationRun?.mediaRunId ?? '',
    ...
    function getDetachedThreadPendingModelStates(thread: AiChatThread, promptText: string): Array<NonNullable<BranchMarkerNode['pendingState']>> {
        ...
        const selectedImageModelIds = uniqueAiModelIds(collapseForMode(
            parseAiModelSelectionAttr(attrs.aiImageModels),
            useMultipleImageModels,
        ))
        const selectedVideoModelIds = uniqueAiModelIds(collapseForMode(
            parseAiModelSelectionAttr(attrs.aiVideoModels),
            useMultipleVideoModels,
        ))
        const hasExplicitMediaFanout = useMultipleImageModels || useMultipleVideoModels
        const imageModelIds = hasExplicitMediaFanout && !useMultipleImageModels
            ? []
            : selectedImageModelIds
        const videoModelIds = hasExplicitMediaFanout && !useMultipleVideoModels
            ? []
            : selectedVideoModelIds
    ...
    function getPendingBranchMarkerRecord(threadId: string, generationRun?: MediaGenerationRunMeta): PendingBranchMarkerRecord | undefined {
        ...
        return recoverPendingBranchMarkerRecordFromCanvasState(threadId, generationRun)
            ?? recoverPendingBranchMarkerRecordFromOverlay(threadId, generationRun)
    }
    ...
    function promotePendingBranchMarkerElement(pendingNodeId: string, node: BranchMarkerNode): HTMLElement | null {
        ...
        // Structural renders used to leave the fixed overlay intact while rebuilding
        // viewport nodes, so a preflight marker could have more than one DOM copy.
        // Promotion is an identity handoff: remove every old/pre-existing copy before
        // registering exactly one resolved marker.
        replaceBranchMarkerDomCopies({
            overlayEl: pendingBranchMarkerOverlayEl,
            viewportEl,
            previousNodeId: pendingNodeId,
    ...
    function syncPlannedBranchMarkerResolution(
        ...
        const screenFixedOwnerNodeId = resolveVisiblePendingBranchMarkerOwner(
            threadId,
            generationRun,
            previousRecord.nodeId,
            plannedNode.nodeId,
        )
        const promotedEl = promotePendingBranchMarkerElement(screenFixedOwnerNodeId, plannedNode)
        ...
        let edges = retiredOwnerNodeIds.size > 0
            ? currentCanvasState.edges.filter((edge: WorkspaceEdge) =>
                !retiredOwnerNodeIds.has(edge.sourceNodeId) && !retiredOwnerNodeIds.has(edge.targetNodeId)
            )
            : currentCanvasState.edges
        ...
        cleanupBranchMarkerArtifacts(retiredOwnerNodeIds)
        ...
        if (promotedEl) {
            moveBranchMarkerIntoCanvasViewport(promotedEl, plannedNode)
            syncBranchMarkerNodeContent(plannedNode, promotedEl)
        ...
        for (const node of nodesById.values()) {
            if (promotedEl && node.nodeId === plannedNode.nodeId) continue
            appendBranchMarkerNodeForKind(node)
        }
        if (promotedEl) syncConnectionsAfterManualNodeAppend()
        refreshBranchMarkersForAiChatThread(threadId)
    }
    ...
    function resolvePendingBranchMarkerWithLineagePlan(
        ...
        const visibleOwnerNodeId = resolveVisiblePendingBranchMarkerOwner(
            threadId,
            generationRun,
            record.nodeId,
            plannedNodeWithPending.nodeId,
        )
        const promotedEl = promotePendingBranchMarkerElement(visibleOwnerNodeId, plannedNodeWithPending)
        ...
        let edges = currentCanvasState.edges.filter((edge: WorkspaceEdge) =>
            !retiredOwnerNodeIds.has(edge.sourceNodeId) && !retiredOwnerNodeIds.has(edge.targetNodeId)
        )
        edges = addBranchMarkerEdgeIfMissing(edges, plannedResolution.branchForkNode)
        edges = addBranchMarkerEdgeIfMissing(edges, plannedResolution.branchLineNode)
    ...
    function applyMediaBranchLineagePlan(
        ...
        console.info('[CANVAS] branch marker lineage plan received', {
            threadId,
            generationRequestId: lineagePlan.generationRequestId,
        ...
        syncPendingBranchMarkerScreenPlacements()
        if (!currentCanvasState
            || !hasCompletePlannedBranchMarkerGeometry(currentCanvasState.nodes, lineagePlan)) return
    ...
    function finishGeneratedMediaRun(threadId: string, generationRun?: MediaGenerationRunMeta): void {
        ...
        if (activeRunKeys.size > 0) {
            console.info('[CANVAS] generated media run finished with siblings still active', {
                threadId,
                generationRequestId: generationRun.generationRequestId,
                mediaRunId: generationRun.mediaRunId,
        ...
        const preflightSettlement = currentCanvasState
            ? removePreflightBranchMarkersForThread(currentCanvasState, threadId)
            : undefined
        ...
        cleanupBranchMarkerArtifacts(removedOrphanedOverlayNodeIds)
        ...
        console.info('[CANVAS] final generated media run settled detached canvas state', {
            threadId,
            generationRequestId: generationRun.generationRequestId,
            mediaRunId: generationRun.mediaRunId,
    ...
    function settleMediaGenerationRun(
        threadId: string,
        generationRun: MediaGenerationRunMeta,
    ): void {
        clearPendingBranchMarkerStateForRun(threadId, generationRun)
    ...
    function clearGeneratingReferenceNodeIdsForPromptHandoff(threadId: string, generationRun?: MediaGenerationRunMeta): void {
        ...
        if (placement && expectedRunKeys.length > 0) {
            ...
            promptHandoffRunKeys.add(getGeneratedMediaRunKey(threadId, generationRun))
            pendingGeneratedImagePlacements.set(placementKey, {
                ...placement,
    ...
    function branchMarkerThreadSnapshotIsComplete(threadId: string, content: unknown): boolean {
        const markers = (currentCanvasState?.nodes ?? [])
            .filter((node: CanvasNode): node is BranchMarkerNode =>
                isBranchMarkerNode(node) && getBranchMarkerThreadId(node) === threadId)
        ...
        return markers.every((marker) => {
            const preview = getBranchMarkerConversationPreviewFromThreadContent(
                ...
                threadId,
                getBranchMarkerTurnDescriptor(marker),
                { allowLatestTurnFallback: canUseLatestBranchMarkerTurnFallback(marker, content) },
            )
            return Boolean(preview?.userMessage && preview.responseText.trim())
        })
    }

    async function refreshPersistedAiChatThreadForBranchMarkers(threadId: string): Promise<boolean> {
        const asset = await assetService.get(threadId, workspaceId)
        if ('error' in asset || !asset.documents.conversation) return false
        ...
        if (fetchedVersion < currentVersion) return false

        rememberAiChatThreadRecord(thread)
        const snapshotIsComplete = branchMarkerThreadSnapshotIsComplete(threadId, snapshot.doc)
        if (snapshotIsComplete) liveAiChatThreadContentOverrides.delete(threadId)
        refreshBranchMarkersForAiChatThread(threadId)
        refreshGeneratedMediaProjectionsForAiChatThread(threadId)
    ...
    function schedulePersistedAiChatThreadRefreshForBranchMarkers(threadId: string): void {
        ...
        const scheduleAttempt = (attempt: number): void => {
            ...
            const timer = window.setTimeout(() => {
                void refreshPersistedAiChatThreadForBranchMarkers(threadId)
                    .then((complete) => {
                        if (complete) {
                            pendingAiChatThreadRefreshTimers.delete(threadId)
                            return
            ...
            pendingAiChatThreadRefreshTimers.set(threadId, [timer])
        }
        scheduleAttempt(0)
    }
    ...
    function getBranchMarkerConversationPreview(node: BranchMarkerNode): BranchMarkerConversationPreview | null {
        ...
        const preview = getBranchMarkerConversationPreviewFromThreadContent(
            ...
            getBranchMarkerTurnDescriptor(node),
            {
                ...
                allowLatestTurnFallback: canUseLatestBranchMarkerTurnFallback(node),
            },
        )
    ...
    function canUseLatestBranchMarkerTurnFallback(
        node: BranchMarkerNode,
        content: unknown = getAiChatThreadContentForBranchMarker(getBranchMarkerThreadId(node) ?? ''),
    ): boolean {
        ...
        return isBranchMarkerGenerationActive(node)
            || isBranchMarkerGenerationGroupActive(node)
            || Boolean(node.pendingState)
            || getBranchMarkerGeneratedArtifactNodes(node).length > 0
            || (userMessageCount === 1 && responseMessageCount <= 1)
    }

    function getBranchMarkerReasoningResponseText(
        node: BranchMarkerNode,
        preview: BranchMarkerConversationPreview | null | undefined,
    ): string {
    ...
    function resizeBranchMarkerNodeFromProseMirror(node: BranchMarkerNode): BranchMarkerNode {
        ...
        return resizeBranchMarkerNodeToDimensions(
            node,
            getBranchMarkerContentDimensions(getBranchMarkerVisiblePromptText(node, preview), {
    ...
    function getBranchMarkerScreenFixedDimensionsForNode(node: BranchMarkerNode): { width: number; height: number } {
        const preview = getBranchMarkerConversationPreview(node)
        const responseText = getBranchMarkerReasoningResponseText(node, preview)
        const estimatedDimensions = getBranchMarkerScreenFixedDimensions(
            getBranchMarkerVisiblePromptText(node, preview),
            {
                responseLine: shouldShowBranchMarkerResponseLine(node, preview),
                responseText,
            },
    ...
    function refreshGeneratedMediaProjectionsForAiChatThread(threadId: string): void {
        const detailsNode = resolveGeneratedOutputDetailsNode(aiChatPanelState.generatedOutputDetailsTarget)
        ...
        generatedOutputDetailsRefreshRaf = requestAnimationFrame(() => {
            ...
            const currentDetailsNode = resolveGeneratedOutputDetailsNode(aiChatPanelState.generatedOutputDetailsTarget)
            ...
            renderActiveAiChatPanel({ preserveModeSwitch: true, animateOpen: false })
        })
    }
    ...
    function rememberStandaloneGeneratedImagePlacement(
        ...
        const promptText = explicitPromptText.trim() || getPromptTextFromMessages(messages)
        const referenceNodeIds = getExistingMediaNodeIds(explicitReferenceNodeIds)
    ...
    function isGeneratedOutputNode(node: CanvasNode): node is ImageCanvasNode | VideoCanvasNode | CapabilityArtifactCanvasNode {
        return (node.type === 'image' || node.type === 'video' || node.type === 'capabilityArtifact')
            && Boolean(node.generatedBy?.branchId)
    }

    function isBranchTreeCanvasNode(node: CanvasNode): node is ImageCanvasNode | VideoCanvasNode | CapabilityArtifactCanvasNode | BranchOriginCanvasNode | BranchForkCanvasNode | BranchLineCanvasNode {
        return isGeneratedOutputNode(node) || node.type === 'branchOrigin' || node.type === 'branchFork' || node.type === 'branchLine'
    }
    ...
    async function analyzeCanvasMediaStill(nodeId: string, stillAssetId: string, analysisAttempt = 0): Promise<void> {
        ...
        try {
            const result = await describeMedia({ assetId: stillAssetId, workspaceId })
    ...
    async function refreshCompletedGeneratedMediaAsset(
        ...
        try {
            ...
            if (attempt === 0
                ...
                && currentNode.assetId === node.assetId
                && result.media?.renditions.original?.status === 'ready') {
                videoNodeHandler?.retryAssetSources(new Set([node.assetId]), { force: true })
                pixiMediaLayer?.retryAssetTextures(new Set([node.assetId]))
                syncPixiMediaLayer(currentCanvasState)
                appendVideoNodeToDOM(currentNode)
            }
            scheduleGeneratedMediaChromeSync()
            syncBranchMarkerNodeContents()
            queueCanvasMediaAnalysis(node.nodeId, getMediaDescriptorStillAssetId(node))
    ...
    function debugGeneratedMediaLifecycle(event: string, details: Record<string, unknown>): void {
        if (debugLoggingEnabled) console.info('[CANVAS][generated-media]', event, details)
    }
    ...
    function appendCapabilityArtifactNodeToDOM(artifactNode: CapabilityArtifactCanvasNode): void {
        viewportEl.querySelector(`[data-node-id="${artifactNode.nodeId}"]`)?.remove()
        const nodeEl = createCapabilityArtifactNode(artifactNode)
        viewportEl.appendChild(nodeEl)
        connectionManager?.registerNodeElement(artifactNode.nodeId, nodeEl as HTMLDivElement)
        syncConnectionsAfterManualNodeAppend()
    }
    ...
    function commitTransientCanvasNodeInsertion(
        ...
        if (replacedPlaceholderNodeId) {
            viewportEl.querySelector(`[data-node-id="${replacedPlaceholderNodeId}"]`)?.remove()
            selectedNodeIds.delete(replacedPlaceholderNodeId)
        }
        ...
        appendCanvasNodeToDOM(insertedNode)
        syncCanvasNodeDomGeometry(nextState.nodes)
        syncConnectionManagerForCurrentCanvasState({ flushPixi: true })
        syncPendingBranchMarkerScreenPlacements()
        lastVisualSyncKey = getCanvasVisualSyncKey(nextState)
        lastNodeStructureKey = getNodeStructureKey(nextState)

        if (insertedNode.type === 'image' || insertedNode.type === 'video') {
            queueCanvasMediaAnalysis(insertedNode.nodeId, getMediaDescriptorStillAssetId(insertedNode))
        }
    }
    ...
    function reconcileApiBranchMarkersWithTemporaryPreflightNodes(
        state: CanvasState,
        ...
        if (plannedMarkers.length === 0) return { state, removedNodeIds: [] }

        const removedNodeIds = new Set<string>()
        for (const plannedMarker of plannedMarkers) {
            const matchingRecords = [...pendingBranchMarkers.entries()]
                .filter(([, record]) => branchMarkerMatchesPendingRecord(plannedMarker, record))
            const temporaryNodeIds = new Set(matchingRecords.map(([, record]) => record.nodeId))
            for (const temporaryNode of state.nodes) {
                if (!isBranchMarkerNode(temporaryNode) || temporaryNode.pendingState?.phase !== 'preflight') continue
            ...
            for (const temporaryNodeId of temporaryNodeIds) {
                if (temporaryNodeId === plannedMarker.nodeId || removedNodeIds.has(temporaryNodeId)) continue
                const temporaryNode = state.nodes.find(node => node.nodeId === temporaryNodeId)
                if (!temporaryNode || !isBranchMarkerNode(temporaryNode) || temporaryNode.pendingState?.phase !== 'preflight') continue
                removedNodeIds.add(temporaryNodeId)
                cleanupBranchMarkerArtifacts([temporaryNodeId])
            }
            for (const [key, record] of matchingRecords) {
                if (record.nodeId === plannedMarker.nodeId) continue
                if (!removedNodeIds.has(record.nodeId)) continue
        ...
        for (const temporaryNode of state.nodes) {
            ...
            if (!threadId || temporaryNode.generationRequestId !== threadId) continue
    ...
    function shouldDeferApiCanvasSnapshotBranchMarkerRender(node: BranchMarkerNode): boolean {
        const reasoningIndex = getBranchMarkerReasoningIndex(node)
        const reasoningModelId = getBranchMarkerReasoningModelId(node)
        const overlayIdentity = findPendingBranchMarkerOverlayIdentity(
            pendingBranchMarkerOverlayEl,
            getBranchMarkerThreadId(node),
            reasoningIndex ?? undefined,
            reasoningModelId || undefined,
        )
        if (!overlayIdentity && !shouldDeferPlannedBranchMarkerViewportRender(node)) return false

        const logKey = `defer-api-snapshot-marker:${node.nodeId}`
        if (!branchMarkerHandoffDebugKeys.has(logKey)) {
            branchMarkerHandoffDebugKeys.add(logKey)
            console.info('[CANVAS] deferred API branch marker snapshot until preflight handoff', {
                nodeId: node.nodeId,
                threadId: getBranchMarkerThreadId(node),
                generationRequestId: node.generationRequestId,
                markerType: node.type,
                overlayNodeId: overlayIdentity?.nodeId ?? '',
            })
        }
        return true
    }
    ...
    function syncApiCanvasSnapshotNodesToDOM(nodeIds: Iterable<string>): void {
        ...
        for (const node of currentCanvasState.nodes) {
            ...
            if (node.type === 'branchOrigin') {
                if (shouldDeferApiCanvasSnapshotBranchMarkerRender(node)) continue
    ...
    function removeApiCanvasRemovedNodesFromDOM(nodeIds: Iterable<string>): void {
        ...
        if (debugLoggingEnabled) console.info('[CANVAS][api-geometry]', 'removed-dom-nodes', {
    ...
    function resolvePendingBranchMarkersAfterApiGeometry(generationRequestId: string | undefined): void {
        if (!currentCanvasState || !generationRequestId) return

        const resolvedGenerationRequestIds = new Set<string>()
        for (const placement of pendingGeneratedImagePlacements.values()) {
            const lineagePlan = placement.lineagePlan
            if (!lineagePlan
                || lineagePlan.generationRequestId !== generationRequestId
                || resolvedGenerationRequestIds.has(lineagePlan.generationRequestId)
                || !hasCompletePlannedBranchMarkerGeometry(currentCanvasState.nodes, lineagePlan)) continue

            const plannedMarker = currentCanvasState.nodes.find((node: CanvasNode): node is BranchMarkerNode =>
                isBranchMarkerNode(node)
                && node.pendingState?.phase !== 'preflight'
                && lineagePlanReferencesBranchMarkerNode(lineagePlan, node)
            )
            const threadId = plannedMarker ? getBranchMarkerThreadId(plannedMarker) : ''
            if (!threadId) continue

            resolvedGenerationRequestIds.add(lineagePlan.generationRequestId)
            resolvePendingBranchMarkersForLineagePlan(threadId, lineagePlan)
            cleanupOrphanPreflightMarkersForThread(threadId)
        }
    }
    ...
    function applyApiCanvasGeometry(canvasGeometry: CanvasGeometryUpdate): void {
        ...
        if (debugLoggingEnabled) console.info('[CANVAS][api-geometry]', 'applied', {
            layoutRevision: canvasGeometry.layoutRevision,
        ...
        if (changed) commitTransientCanvasStatePreservingEditors(result.state)
        ...
        resolvePendingBranchMarkersAfterApiGeometry(canvasGeometry.generationRequestId)
        if (currentCanvasState) {
    ...
    setAiGeneratedImageCallbacks({
        onCapabilityRunEventToCanvas: ({ threadId, event }) => {
            applyCapabilityRunEventToBranchMarkers(threadId, event)
        },
        ...
        onMediaGenerationSkippedToCanvas: ({ threadId, generationRequestId, generationRun }) => {
            ...
            if (generationRun?.requestKind === 'media-generation-matrix') {
                settleMediaGenerationRun(threadId, generationRun)
                return
            }
        ...
        onMediaGenerationRequestCompleteToCanvas: ({ threadId, generationRequestId, generationRun }) => {
            const accepted = shouldAcceptGeneratedMediaEvent(threadId)
            console.info('[CANVAS] media generation request completion received', {
                threadId,
                generationRequestId,
                mediaRunId: generationRun?.mediaRunId,
                accepted,
                pendingPlacementKeys: [...pendingGeneratedImagePlacements.keys()]
                    .filter(key => key === threadId || key.startsWith(`${threadId}:`)),
                preflightNodeIds: currentCanvasState?.nodes
                    .filter((node: CanvasNode) => isBranchMarkerNode(node)
        ...
        onImagePartialToCanvas: (data) => {
            ...
            if (canvasGeometry) {
                if (debugLoggingEnabled) console.info('[CANVAS][api-geometry]', 'image-partial-apply', {
                    runKey,
                    layoutRevision: canvasGeometry.layoutRevision,
        ...
        onImageCompleteToCanvas: (data) => {
            ...
            if (!data.canvasGeometry) {
                ...
                if (existingCompletedImageNode?.type === 'image') {
                    if (debugLoggingEnabled) console.info('[CANVAS][api-geometry]', 'image-complete-existing-final-without-geometry', {
                        runKey,
                        threadId,
                        completedNodeId,
                        generationRequestId: generationRun?.generationRequestId,
            ...
            if (debugLoggingEnabled) console.info('[CANVAS][api-geometry]', 'image-complete-apply', {
                runKey,
    ...
    setAiGeneratedVideoCallbacks({
        ...
        onVideoCompleteToCanvas: (data) => {
            ...
            if (debugLoggingEnabled) console.info('[CANVAS][api-geometry]', 'video-complete-apply', {
                runKey,
                layoutRevision: data.canvasGeometry.layoutRevision,
                removedNodeIds: data.canvasGeometry.removedNodeIds ?? [],
                edgeSnapshotIds: data.canvasGeometry.edgeSnapshots?.map(edge => edge.edgeId) ?? [],
            ...
            <div
                ...
                onmousedown=${(e: MouseEvent) => {
                    ...
                    handleDragStart(e, node.nodeId, {
                        ...
                        // Non-selectable branch markers open details from the
                        // subsequent native click. Opening during mouseup would
                        // mount the side-panel backdrop before click dispatch,
                        // letting that same pointer action close the panel again.
                        onClick: interactionOptions.allowSelection === false
                            ? undefined
                            : interactionOptions.onClick,
                    })
                }}
    ...
    function mountDocumentNodeEditor(
        ...
        try {
            ...
            editorContainer.replaceChildren(errorPlaceholder.dom)
        }
    }
    ...
    function createDocumentNode(node: DocumentCanvasNode, doc: Document | undefined): HTMLElement {
        ...
        if (doc?.content !== undefined) {
            mountDocumentNodeEditor(node, doc, nodeEl, editorContainer)
        } else {
    ...
    function syncLoadedDocumentEditors(): void {
        if (!currentCanvasState) return
        const documentMap = new Map<string, Document>(currentDocuments.map((doc) => [doc.documentId, doc]))
        for (const node of currentCanvasState.nodes) {
            if (node.type !== 'document' || documentEditors.has(node.assetId)) continue
            const doc = documentMap.get(node.assetId)
            if (doc?.content === undefined) continue
            const nodeEl = viewportEl.querySelector(`[data-node-id="${node.nodeId}"]`) as HTMLElement | null
            const editorContainer = nodeEl?.querySelector('.document-node-editor') as HTMLDivElement | null
            if (!nodeEl || !editorContainer) continue
            mountDocumentNodeEditor(node, doc, nodeEl, editorContainer)
        }
    }

    function scheduleCapabilityArtifactHeight(nodeId: string, measuredHeight: number): void {
        const existingFrame = capabilityArtifactHeightFrames.get(nodeId)
        if (existingFrame !== undefined) cancelAnimationFrame(existingFrame)
        const frame = requestAnimationFrame(() => {
            capabilityArtifactHeightFrames.delete(nodeId)
            if (!currentCanvasState) return
            const node = currentCanvasState.nodes.find((candidate): candidate is CapabilityArtifactCanvasNode => (
            ...
            const resizedNodes = currentCanvasState.nodes.map(candidate => candidate.nodeId === nodeId
                ? { ...candidate, dimensions: { width: node.dimensions.width, height } }
                : candidate)
            const nodes = rebalanceGeneratedMediaTrees(resizedNodes, currentCanvasState.edges)
            commitCanvasStatePreservingEditors({ ...currentCanvasState, nodes })
            scheduleGeneratedMediaChromeSync()
        })
        capabilityArtifactHeightFrames.set(nodeId, frame)
    }

    async function refreshCapabilityArtifactNode(node: CapabilityArtifactCanvasNode): Promise<void> {
        try {
            const result = await assetService.refresh(node.assetId, workspaceId)
            ...
            if (!currentCanvasState?.nodes.some(candidate => candidate.nodeId === node.nodeId)) return
            const nodeEl = viewportEl.querySelector(`[data-node-id="${node.nodeId}"]`) as HTMLElement | null
            const host = nodeEl?.querySelector('.capability-artifact-node-host') as HTMLDivElement | null
            if (!host) return
            host.replaceChildren(createErrorPlaceholder({
                message: 'This Artifact could not be loaded',
                retryLabel: 'Retry',
    ...
    function createCapabilityArtifactNode(node: CapabilityArtifactCanvasNode): HTMLElement {
        ensureCapabilityStyles(viewportEl.ownerDocument)
        const { nodeEl, dragOverlay } = createBaseNodeElement(
            node,
            'workspace-capability-artifact-node',
            { assetId: node.assetId, artifactTypeId: node.artifactTypeId },
        )
        dragOverlay.className = 'capability-artifact-drag-overlay nopan'
        const host = html`<div className="capability-artifact-node-host nopan"></div>` as HTMLDivElement
        nodeEl.appendChild(host)
        const asset = assetsStore.get(node.assetId)
        const snapshot = assetDocumentsStore.get(node.assetId, 'capabilityArtifact')
        if (!asset || !snapshot) {
            host.appendChild(createLoadingPlaceholder().dom)
            void refreshCapabilityArtifactNode(node)
            return nodeEl
        }
        ...
        try {
            ...
            const view = frontend.createCanvasNodeView({
                ...
                mountEditor: ({ container, document, schema, plugins }) => {
                    const editor = new ProseMirrorEditor({
                        ...
                        onEditorChange: () => queueMicrotask(() => scheduleCapabilityArtifactHeight(node.nodeId, host.scrollHeight)),
                    })
                    const observer = new ResizeObserver(() => {
                        scheduleCapabilityArtifactHeight(node.nodeId, host.scrollHeight)
                    })
            ...
            capabilityArtifactViews.set(node.nodeId, {
                destroy: () => view.destroy(),
            })
        } catch (error) {
            console.error('Failed to mount Capability Artifact:', error)
            host.replaceChildren(createErrorPlaceholder({
                message: 'This Artifact cannot be rendered',
                retryLabel: 'Retry',
                onRetry: () => renderNodes(),
            }).dom)
        }
    ...
    function applyMediaOperationRecoveryResult(result: MediaGenerationOperationRecoveryResult): void {
        ...
        if (changedGeometryNodeIds.length > 0) {
            syncCanvasNodeDomGeometry(
                currentCanvasState.nodes.filter(node => changedGeometryNodeIds.includes(node.nodeId)),
            )
        }
        syncPixiMediaLayer(currentCanvasState)
        scheduleGeneratedMediaChromeSync()
            ...
            if (node.problem) {
                const details = html`
                    <details className="workspace-media-operation-problem nopan">
                        ...
                        <span className="workspace-media-operation-moderation-context"></span>
                ...
                const moderationContext = [
                    node.problem.moderationStage
                        ? `Safety stage: ${node.problem.moderationStage}`
                        : '',
                    node.problem.moderationCategories?.length
                        ? `Categories: ${node.problem.moderationCategories.join(', ')}`
                        : '',
                ].filter(Boolean).join(' · ')
                ;(details.querySelector('.workspace-media-operation-moderation-context') as HTMLElement).textContent =
                    moderationContext
    ...
    function getBranchMarkerGeneratedArtifactNodes(node: BranchMarkerNode): CapabilityArtifactCanvasNode[] {
        if (node.type === 'branchOrigin') return getBranchGeneratedArtifactNodes('branchOriginNodeId', node.nodeId)
        if (node.type === 'branchFork') return getBranchGeneratedArtifactNodes('branchForkNodeId', node.nodeId)
    ...
    async function stopBranchMarkerGeneration(node: BranchMarkerNode): Promise<void> {
        ...
        try {
            const result = await stopAiChatMessageForThread({
                workspaceId,
                conversationAssetId: threadId,
                ...(projectionGenerationRequestId ? { generationRequestId: projectionGenerationRequestId } : {}),
            })
            if (result.canvasGeometry) applyApiCanvasGeometry(result.canvasGeometry)
    ...
    function applyCapabilityRunEventToBranchMarkers(threadId: string, event: CapabilityRunEvent): void {
        ...
        const run = runs.get(event.runId) ?? {
            ...
            lastSequence: 0,
            steps: new Map<string, BranchMarkerCapabilityProgressStep>(),
        }
        ...
        capabilityProgressRunsByThreadId.set(threadId, runs)
        refreshBranchMarkersForAiChatThread(threadId)
    }
            ...
            candidate.type === 'operationStatus'
                ...
                : {
                    ...
                    status: candidate.generationProgress
                        ...
                        : candidate.mediaGenerationPhase === 'ready' ? 'completed' : undefined,
                }
        )))
    ...
    function renderNodes() {
        ...
        // This is a full structural rebuild. Preflight markers live outside
        // viewportEl, so clearing only the viewport retained their old DOM copies
        // and the loop below created duplicates for the same state nodes.
        ...
        const branchMarkerNodes = currentCanvasState.nodes.filter(
            (node: CanvasNode): node is BranchMarkerNode => isBranchMarkerNode(node),
        )
        const startedPlannedBranchMarkerNodeIds = new Set(
            branchMarkerNodes
                .filter(node => node.pendingState?.phase !== 'preflight')
        ...
        renderActiveAiChatPanel({ animateOpen: shouldAnimatePanelOpenAfterRender })
    ...
    function getAiChatThreadContentWeight(thread: AiChatThread): number {
        if (!thread.content) return 0
    ...
    function getCanvasConversationThreadIds(canvasState: CanvasState | null): Set<string> {
        const threadIds = new Set<string>()
        for (const node of canvasState?.nodes ?? []) {
            if (isBranchMarkerNode(node) && node.conversationAssetId) {
                threadIds.add(node.conversationAssetId)
                continue
            }
            if ((node.type === 'image' || node.type === 'video' || node.type === 'capabilityArtifact')
                && node.generatedBy?.conversationAssetId) {
                threadIds.add(node.generatedBy.conversationAssetId)
            }
        }
        return threadIds
    }

    function mergeIncomingAiChatThreads(
        incomingThreads: AiChatThread[],
        canvasState: CanvasState | null,
        workspaceChanged: boolean,
    ): AiChatThread[] {
        if (workspaceChanged) return incomingThreads

        const nextById = new Map(incomingThreads.map((thread) => [thread.threadId, thread]))
        const retainedThreadIds = getCanvasConversationThreadIds(canvasState)
        for (const threadId of activeCanvasRunIds) retainedThreadIds.add(threadId)
        for (const threadId of detachedAiChatThreadEditors.keys()) retainedThreadIds.add(threadId)
        for (const threadId of liveAiChatThreadContentOverrides.keys()) retainedThreadIds.add(threadId)

        for (const currentThread of currentAiChatThreads) {
            const incomingThread = nextById.get(currentThread.threadId)
            if (!incomingThread) {
                if (retainedThreadIds.has(currentThread.threadId)) nextById.set(currentThread.threadId, currentThread)
                continue
            }

            const currentVersion = getStoredProseMirrorVersion(currentThread)
            const incomingVersion = getStoredProseMirrorVersion(incomingThread)
            const preserveCurrentContent = currentVersion > incomingVersion
                || (currentVersion === incomingVersion
                    && getAiChatThreadContentWeight(currentThread) > getAiChatThreadContentWeight(incomingThread))
                || (!incomingThread.content && Boolean(currentThread.content))
            if (!preserveCurrentContent) continue

            nextById.set(currentThread.threadId, {
                ...incomingThread,
                content: currentThread.content,
                proseMirrorVersion: currentVersion,
                updatedAt: Math.max(incomingThread.updatedAt, currentThread.updatedAt),
            })
        }
        ...
        if (!mediaLibraryPanelInstance) {
            mediaLibraryPanelInstance = createMediaLibraryPanel({
                ...
                onInsertAsset: async (item: AssetMeta) => {
                    ...
                    const committedState = await onAssetAttach({ assetId: item.assetId, nodeId, canvasState: nextState })
                    commitTransientCanvasStatePreservingEditors(committedState)
                    return true
                },
            })
    ...
    function ensureArtifactLibraryPanel() {
        if (!artifactLibraryPanelInstance) {
            artifactLibraryPanelInstance = createArtifactLibraryPanel({
                ...
                onAcceptAsset: async (asset) => {
                    const node = currentCanvasState?.nodes.find(candidate => (
        ...
        return artifactLibraryPanelInstance
    }
    ...
    let observedAssetRevisions = new Map<string, number>()
    let hasObservedInitialAssetsStore = false
    const unsubscribeAssetsStore = assetsStore.subscribe(({ items }) => {
        ...
        if (!hasObservedInitialAssetsStore) {
            ...
            observedAssetRevisions = nextAssetRevisions
            return
        }
        const changedAssetIds = new Set<string>()
        for (const [assetId, revision] of nextAssetRevisions) {
            if (observedAssetRevisions.get(assetId) !== revision) changedAssetIds.add(assetId)
        }
        observedAssetRevisions = nextAssetRevisions
        videoNodeHandler?.retryAssetSources(changedAssetIds)
        pixiMediaLayer?.retryAssetTextures(changedAssetIds)
    ...
    return {
        ...
        commitTransientCanvasNodeInsertion(
            ...
            commitTransientCanvasNodeInsertion(canvasState, nodeId, replacedPlaceholderNodeId)
        },
        ...
        render(newCanvasState: CanvasState | null, newDocuments: Document[], newAiChatThreads: AiChatThread[] = [], newWorkspaceId?: string) {
            ...
            if (pendingVisualCommitBeforeMerge || renderStatePlan.usedPendingVisualState || renderStatePlan.acknowledgedPendingVisualState) {
                if (debugLoggingEnabled) console.info('[CANVAS][render-state]', 'pending-visual-merge', {
            ...
            const mergedAiChatThreads = mergeIncomingAiChatThreads(
                newAiChatThreads,
            ...
            const nextThreadsKey = getAiChatThreadsKey(mergedAiChatThreads)
            ...
            if (
                ...
                || aiChatPanelState.generatedOutputDetailsTarget !== undefined
            ) {
                if (debugLoggingEnabled) console.info('[CANVAS][render-state]', 'decision', {
                    ...
                    generatedOutputDetailsTarget: aiChatPanelState.generatedOutputDetailsTarget
                        ? `${aiChatPanelState.generatedOutputDetailsTarget.kind}:${aiChatPanelState.generatedOutputDetailsTarget.nodeId}`
                        : '',
                })
            }
            ...
            if (transitionPlan.shouldClearVisualContent) {
                clearWorkspaceVisualContent(newDocuments, mergedAiChatThreads)
                ...
                syncBranchMarkerNodeContents()
                if (
                    aiChatPanelState.generatedOutputDetailsTarget
                    && activeAiChatPanelEl
                    && (documentsKeyChanged || threadsKeyChanged)
                ) {
                    renderActiveAiChatPanel({ preserveModeSwitch: true, animateOpen: false })
                } else if (threadsKeyChanged && aiChatPanelState.isOpen) {
                    renderActiveAiChatPanel()
                }
            ...
            lastDocumentsKey = nextDocumentsKey
            lastThreadsKey = nextThreadsKey
            if (currentCanvasState) renderedWorkspaceId = workspaceId
            refreshBranchMarkerPreviewsForLoadedThreads(mergedAiChatThreads)
            reattachDetachedCanvasRunListenersForActiveMarkers()
        ...
        destroy() {
            ...
            documentEditors.clear()
            for (const frame of capabilityArtifactHeightFrames.values()) cancelAnimationFrame(frame)
            capabilityArtifactHeightFrames.clear()
