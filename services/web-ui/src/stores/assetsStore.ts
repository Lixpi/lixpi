...
export const assetsStore = {
    ...
    get: (assetId: string): Asset | undefined => {
        ...
        const unsubscribe = store.subscribe(state => void (result = state.items.get(assetId)))
        unsubscribe()
    ...
    getAll: (): Asset[] => {
        let result: Asset[] = []
        const unsubscribe = store.subscribe(state => void (result = [...state.items.values()]))
        unsubscribe()

        return result
    },
    reset: (): void => void store.set(initialState),
}
