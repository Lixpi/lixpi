...
import { LoadingStatus, type Asset } from '@lixpi/constants'
...
export const assetsStore = {
    ...
    setAssets: (workspaceId: string, assets: Asset[]): void => store.update((state) => ({
        workspaceId,
    ...
    upsert: (asset: Asset): void => store.update((state) => {
        const existing = state.items.get(asset.assetId)
    ...
    remove: (assetId: string): void => store.update((state) => {
        const items = new Map(state.items)
    ...
    get: (assetId: string): Asset | undefined => {
        ...
        const unsubscribe = store.subscribe((state) => { result = state.items.get(assetId) })
        unsubscribe()
        return result
    },
    getAll: (): Asset[] => {
        let result: Asset[] = []
        const unsubscribe = store.subscribe((state) => { result = [...state.items.values()] })
        unsubscribe()
        return result
    },
