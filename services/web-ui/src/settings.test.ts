...
describe('settings - grouped configuration', () => {
	...
	it('keeps branch lineage and chat-thread layout knobs in grouped subsections', () => {
		...
		expect(settings.mediaNode.image.styles.defaultBoxShadow).not.toBe('none')
		expect(settings.mediaNode.image.defaultInsertionWidth).toBeGreaterThan(0)
		expect(settings.mediaNode.image.styles.borderRadius).toBeGreaterThanOrEqual(0)
	...
	it('keeps migrated behavior and rail hit-target values in their sections', () => {
		...
		expect(settings.mediaNode.generatedMediaChrome.iconSize).toBe(34)
		expect(settings.mediaNode.generatedMediaChrome.topGap).toBe(6)
		expect(settings.mediaNode.generatedMediaChrome.zoomScaling.minZoom).toBe(0.4)
		expect(settings.mediaNode.resizeHandle.size).toBe(24)
		expect(settings.mediaNode.resizeHandle.offset).toBe(6)
		expect(settings.mediaNode.resizeHandle.minSize).toBe(10)
		expect(settings.mediaNode.resizeHandle.zoomScaling.minZoom).toBe(0.4)
		...
		expect(settings.mediaNode.useZoomCompensatedResizeHandleScaling).toBe(true)
	})
