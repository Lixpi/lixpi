...
The bottom settings row summarizes the active model and its primary configuration when one model is selected. With multiple selected models, it shows only the multi-model label and provider icons. The entire row opens the menu. Image and video setting sections render provider and option-set groups from the API configuration matrix, and the inactive media section is hidden.
