...
`promptReferencePickerPlugin/` owns two async pickers per AI prompt editor. `@` defaults to Media and switches among Media, Artifacts, Capabilities, Tools, and Skills with the shared SVG sliding switch. `/` is hard-locked to top-level Capability modules and contains no formatting or upload commands. Every `/` selection uses the same compact `capability-module` badge; module-specific parameter forms do not replace or extend that badge. Required module arguments are derived from the authoritative prompt text by the API. Both pickers ignore stale responses, keep settled rows mounted while a debounced query is pending, and retain their anchor side while the settled result count resizes the popup. The category navigation remains fixed while only the bounded results container scrolls; wheel input is excluded from canvas pan/zoom. A visible-state class lets each host lift the popup above adjacent chrome without permanently changing the editor's stacking order. They support cursor pagination and arrow/Enter/Tab/Escape navigation, close on capture-phase mouse presses outside the popup, and remove their document listener when the editor is destroyed. Presses inside the popup remain available to category and row controls. Each picker replaces the typed query with a typed `prompt_reference` atom. The atom renders as a borderless existing SVG icon and semibold colored name. Composer badges keep their taxonomy color; dark submitted-message and branch-marker surfaces override the Capability token with a lighter orange accent instead of flattening the badge to white. Left/Right skips directly across the non-selectable atom. Media and Capability-module atoms use the shared context-preview card shell, placement, portal, scale, dismissal, and cleanup behavior. Capability description sheets keep the same single-column reading order in a 1200px landscape card and use the media preview card's title and body typography. Registered Artifact rows and chips delegate their copy to the package-owned frontend definition by `artifactTypeId`; their generic host always supplies three non-overlapping columns for the registered existing-SVG icon, copy, and Artifact badge. Display names are cosmetic; stable Asset, Artifact type, module, Tool, or Skill identities survive renames and are reauthorized from the authoritative conversation snapshot after submit.
