...
export class InteractivePreviewPopover {
    ...
    private scheduleClose = (event: PointerEvent | FocusEvent): void => {
        ...
        if (nextTarget instanceof Node
            && (this.options.root.contains(nextTarget) || this.options.popover.contains(nextTarget))) return
        this.cancelScheduledClose()
    ...
    private handleDocumentPointerDown = (event: PointerEvent): void => {
        ...
        if (target instanceof Node
            && (this.options.root.contains(target) || this.options.popover.contains(target))) return
        this.close()
    }
}
