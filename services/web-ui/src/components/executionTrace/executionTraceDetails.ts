...
const POSITIVE_VALUE_TAGS = new Set([
    'complete',
    'completed',
    'passed',
    'true',
])
const CAUTION_VALUE_TAGS = new Set([
    'failed',
    'false',
    'pending',
    'skipped',
...
export const formatExecutionTraceModelId = (modelId: string): string => {
    const parts = modelId.split(':')

    return parts.length > 1 ? parts.slice(1).join(':') : modelId
}

export const formatExecutionTraceHandleRole = (role: string): string => {
    return role
        .split(/[-_\s]+/u).filter(Boolean).map((part, index) => index === 0 ? `${part.charAt(0).toUpperCase()}${part.slice(1)}` : part)
        .join(' ')
}

export const formatExecutionTraceFieldLabel = (label: string): string => {
    const words = label
    ...
    const mappedLabel = TRACE_FIELD_LABELS.get(
        words.toLowerCase(),
    )
...
export const formatExecutionTraceDisplayValue = (value: string): string => {
    if (!/^(?:0?\.\d+|1\.0+)$/u.test(
        value.trim(),
    ))
        return value
...
export const formatExecutionTraceTagLabel = (label: string): string => formatExecutionTraceFieldLabel(label)

export const getExecutionTraceTagPillVariant = (
    label: string,
    index = 0,
): TagPillVariant => {
    const normalizedLabel = label.trim().toLowerCase()

    if (POSITIVE_VALUE_TAGS.has(normalizedLabel))
        return 'explicit'

    if (CAUTION_VALUE_TAGS.has(normalizedLabel))
        return 'auto'

    return TRACE_CATEGORICAL_TAG_VARIANTS[index % TRACE_CATEGORICAL_TAG_VARIANTS.length]
}

export const getExecutionTraceTagPillColors = (
    label: string,
    index = 0,
): TagPillColors => {
    const normalizedLabel = label.trim().toLowerCase()
    ...
    if (POSITIVE_VALUE_TAGS.has(normalizedLabel))
        return TRACE_TAG_POSITIVE_COLORS

    if (
        normalizedLabel === 'failed'
        || normalizedLabel === 'false'
    )
        return TRACE_TAG_DANGER_COLORS

    if (
        normalizedLabel === 'pending'
        || normalizedLabel === 'skipped'
    )
        return TRACE_TAG_CAUTION_COLORS

    return TRACE_CATEGORICAL_TAG_COLORS[index % TRACE_CATEGORICAL_TAG_COLORS.length]!
}

export const getExecutionTraceTagValues = (value: string): string[] => {
    const values = value
        .split(',').map(item => item.trim()).filter(Boolean)

    if (
        values.length === 0
        || values.some(item => item.length > TRACE_TAG_MAX_LABEL_LENGTH)
    )
        return []
    ...
    if (SINGLE_VALUE_TAGS.has(
        singleValue.toLowerCase(),
    ))
        return values
...
export const isExecutionTraceInternalIdValue = (value: string): boolean => TRACE_UUID_PATTERN.test(value)

export const getExecutionTraceDisplayFacts = (facts: readonly ExecutionTraceFact[]): ExecutionTraceFact[] => {
    const groupedFacts = new Map<string, ExecutionTraceFact & { count: number }>()

    for (const fact of facts) {
        if (isExecutionTraceInternalIdValue(fact.value))
            continue
    ...
    return [...groupedFacts.values()].map(
        ({
            count,
            ...fact
        }) =>
...
export const formatExecutionTraceDuration = (
    startedAt?: number,
    completedAt?: number,
): string => {
    if (
...
export const formatExecutionTraceTokenUsage = (usage: ExecutionTraceModelCall['tokenUsage']): string => {
    if (!usage)
        return ''
...
export const getExecutionTraceKey = (trace: ExecutionTrace | null | undefined): string => {
    if (!trace)
        return ''
...
export const isRenderableExecutionTrace = (trace: unknown): trace is ExecutionTrace => {
    if (
        !trace
        || typeof trace !== 'object'
    )
...
class ExecutionTraceDetail implements ExecutionTraceDetailInstance {
    ...
    private renderHandleSection(
        title: string,
        handles: readonly ExecutionTraceHandle[],
    ): HTMLElement {
        return html`
            <section className="execution-trace-section">
                <h4 className="execution-trace-section-title">${title}</h4>
    ...
    private renderHandle(handle: ExecutionTraceHandle): HTMLElement {
        ...
        if (
            ...
            const preview = createMediaPromptReferencePreview(
                ...
                previewRenderer,
                {
                    inlinePopover: this.options.inlinePopover,
    ...
    private renderModelCall(modelCall: ExecutionTraceModelCall): HTMLElement {
        ...
        return html`
            <li
                className="execution-trace-model-call"
                data-model-call-role=${modelCall.role}
            >
                ${modelHeader}
                ${
                    modelCall.purpose
    ...
    private renderValueTag(
        ...
        const selectedFill = colors.fillActive
            ?? colors.fill
            ?? 'transparent'
        const selectedStroke = colors.strokeActive
            ?? colors.stroke
            ?? 'transparent'
        const tag = html`
            <svg
...
export const createExecutionTraceDetail = (options: CreateExecutionTraceDetailOptions): ExecutionTraceDetailInstance =>
    new ExecutionTraceDetail(options)
