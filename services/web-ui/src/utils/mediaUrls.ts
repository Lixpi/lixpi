...
const parseUrl = (url: string): {
    ...
    try {
        ...
        return {
            parsed: isAbsolute ? new URL(url) : new URL(
                url,
                getUrlBase(),
            ),
            isAbsolute,
...
const stringifyUrl = (
    parsed: URL,
    isAbsolute: boolean,
): string => (isAbsolute ? parsed.toString() : `${parsed.pathname}${parsed.search}${parsed.hash}`)

export const buildAssetRenditionPath = (
    assetId: string,
    rendition: string,
): string => `/api/assets/${encodeURIComponent(assetId)}/renditions/${encodeURIComponent(rendition)}`
...
export const stripAuthTokenFromUrl = (url: string): string => {
...
export const setAuthTokenOnUrl = (
    url: string,
    token: string,
): string => {
    if (!token)
...
export const resolveMediaUrl = (
    ...
    if (url.startsWith('/api/')) {
        ...
        return options.token ? setAuthTokenOnUrl(
            sourceUrl,
...
export const resolveAuthenticatedMediaUrl = async (
    ...
    const token = options.token || (isApiEndpoint(url)
