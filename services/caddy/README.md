...
Builds, dependency maintenance, formatting and tests run through Docker. The shared [Go Testing and Tooling guide](../../documentation/code-quality/testing/GO.md) documents the `caddy` runner domain. [The version registry](../../versions-registry/README.md) generates Go requirements in [go.mod](go.mod) and image tags in the [Dockerfile](Dockerfile); Go maintains checksums in [go.sum](go.sum). The Dockerfile uses the repository root as its build context and ships a static executable with CA roots.
