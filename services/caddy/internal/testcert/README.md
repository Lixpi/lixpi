...
The package imports Go's `testing` API and is used only by test files. Production packages must not import it. It doesn't exercise a public authority or Route53; [`main_test.go`](../../cmd/lixpi-caddy/main_test.go) separately runs Caddy's real internal CA. The shared [Go Testing and Tooling guide](../../../../documentation/code-quality/testing/GO.md) documents the Docker test runner.
