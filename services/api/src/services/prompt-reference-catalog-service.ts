...
export class PromptReferenceCatalogService {
    ...
    async listModules(
        ...
        if (!requesterHasWorkspaceScope(requester, workspace))
    ...
    > {
        ...
        const items = await this.listModules(
            workspace,
            requester,
            meta.normalizedName,
        )
    ...
    async list(input: ListPromptReferencesInput): Promise<PromptReferenceCatalogPage> {
        if (!requesterHasWorkspaceScope(input.requester, input.workspace))
            throw new Error('WORKSPACE_ACCESS_DENIED')

        const scopedInput = {
            ...input,
            requester: scopeAssetRequesterToWorkspace(input.requester, input.workspace),
        }
        ...
        if (
            ...
            throw new Error('INVALID_PROMPT_REFERENCE_LIMIT')
        ...
        if (
            ...
            return await this.listCategory({
                ...scopedInput,
                query,
                limit,
            })
        ...
        const recents = await PromptReferenceRecentModel.list({
            userId: scopedInput.requester.userId,
        ...
        const {
            items: recentItems,
            staleKeys,
        } = await this.resolveRecents(scopedInput, recents)
    ...
    private async listCategory(input: ListPromptReferencesInput & {
        query: string
        limit: number
        ...
        if (input.category === 'artifacts')
            return await this.listArtifactCategory(input)
    ...
    private async listArtifactCategory(
        ...
        while (
            ...
            for (const record of page.items) {
                if (placements.has(record.assetId))
                    continue

                const item = await this.resolveArtifactCatalogItem(
                    input,
                    record.assetId,
                    'library',
                )
    ...
    private async listCanvasArtifactItems(
        ...
        const groups = await Promise.all(
            [...placements].map(async ([assetId, nodeIds]) => {
                ...
                const items = await Promise.all(
                    nodeIds.map(
                        async nodeId => await this.resolveArtifactCatalogItem(
                            input,
                            assetId,
                            'canvas',
                            nodeId,
    ...
    private async resolveArtifactCatalogItem(
        ...
        const asset = resolvedAsset ?? await AssetModel.get({
            assetId,
            requester: input.requester,
        })
        ...
        if (
            !registered
            || registered.shared.schemaVersion !== asset.artifact.schemaVersion
        )
            return null
        ...
        const referencedAssets = await Promise.all(
            referencedAssetIds.map(
                async referencedAssetId => await AssetModel.get({
        ...
        if (referencedAssets.some(
            referencedAsset =>
                'error' in referencedAsset || !isAssetAvailableInWorkspaceScope(referencedAsset, input.workspace) || referencedAsset.states.lifecycle !== 'active',
        ))
            return null
        ...
        return {
            ...
            referenceThumbnailAssetIds: referencedAssetIds.slice(0, 4),
        }
    }
    ...
    private async listMediaCategory(
        ...
        while (
            ...
            for (const record of page.items) {
                if (placementsByAsset.has(record.assetId))
                    continue

                if (!isAssetAvailableInWorkspaceScope(record, input.workspace))
                    continue

                items.push(
                    toMediaCatalogItem(record, 'library'),
                )
            }
    ...
    private async listCanvasMediaItems(
        ...
        const assets = await Promise.all(
            [...placementsByAsset.keys()].map(
                async assetId => await AssetModel.get({
                    assetId,
                    requester: input.requester,
                }),
            ),
        ...
        return assets.flatMap((asset): PromptReferenceCatalogItem[] => {
            ...
            return placements.flatMap(nodeId => {
                const item = toMediaCatalogItemFromAsset(
                    asset,
                    'canvas',
                    nodeId,
                )
    ...
    private decodeMediaCursor(
        ...
        try {
            ...
            ) as MediaCatalogCursor

            if (
                !parsed
                || parsed.category !== 'media'
                || parsed.query !== query
                || !Number.isSafeInteger(parsed.canvasOffset)
                || parsed.canvasOffset < 0
    ...
    private async resolveRecents(
        ...
        for (const recent of recents) {
            ...
            if (item)
                ...
                staleKeys.push(recent.referenceKey)
        }
    ...
    private async resolveRecent(
        ...
        if (
            recent.referenceType === 'tool'
            || recent.referenceType === 'skill'
        ) {
            const record = await CapabilityModel.authorize({
            ...
            if (
                'error' in record
                || record.kind !== recent.referenceType
        ...
        if (recent.referenceType === 'capability-artifact') {
            const page = await this.listArtifactCategory({
                ...input,
                category: 'artifacts',
                query: '',
                limit: 20,
        ...
        const placement = (input.workspace.canvasState?.nodes ?? []).find(
            node =>
                'assetId' in node && node.assetId === asset.assetId && ['image', 'video', 'audio', 'document', 'mediaDocument'].includes(node.type),
        )

        return toMediaCatalogItemFromAsset(
            asset,
            placement ? 'canvas' : 'library',
            placement?.nodeId,
        )
    }

    private encodeModuleCursor(
        offset: number,
        query: string,
    ): string {
        return Buffer.from(
    ...
    private decodeArtifactCursor(
        cursor: string | undefined,
        query: string,
    ): ArtifactCatalogCursor {
        if (!cursor)
            return {
                category: 'artifacts',
                query,
