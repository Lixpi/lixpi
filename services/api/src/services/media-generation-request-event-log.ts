...
const getMediaGenerationRequestEventSubjectPrefix = (
    workspaceId: string,
...
export const getMediaGenerationRequestEventSubject = (
    workspaceId: string,
    generationRequestId: string,
): string => `${getMediaGenerationRequestEventSubjectPrefix(workspaceId, generationRequestId)}.events`
...
export const getMediaGenerationRequestReplaySubject = (
    workspaceId: string,
    generationRequestId: string,
): string => `${getMediaGenerationRequestEventSubjectPrefix(workspaceId, generationRequestId)}.>`
...
export class MediaGenerationRequestEventLog {
    ...
    async append({
        ...
        const generationRun = typeof event.payload.generationRun === 'number'
            ...
            : undefined
        const subject = event.status === 'MEDIA_GENERATION_PROGRESS'
        ...
        ).byteLength > MAX_MEDIA_GENERATION_REQUEST_EVENT_BYTES)
        ...
        if (typeof streamSequence !== 'number')
            throw new Error('MEDIA_REQUEST_EVENT_ACK_SEQUENCE_MISSING')
    ...
    async replay({
        ...
        events: MediaGenerationRequestEventEnvelope[]
        hasMore: boolean
        streamName: string
        subject: string
    }> {
        ...
        return {
            events,
            hasMore: (events.at(-1)?.streamSequence ?? startStreamSequence - 1) < last.seq,
            streamName,
            subject,
        }
    }

    async getLatestRunProgress({
        workspaceId,
        generationRequestId,
        generationRun,
    }: {
        ...
        const message = await this.natsService.getJetStreamMessage<MediaGenerationRequestEventEnvelope>(
            ...
            { last_by_subj: getMediaGenerationRequestProgressSubject(
                workspaceId,
                generationRequestId,
                generationRun,
            ) },
    ...
    async purgeRequest({
        ...
        await this.natsService.purgeJetStreamSubject(
            ...
            if (
                candidate.code === 404
                || /not found|no stream/iu.test(candidate.message ?? '')
            )
                return
