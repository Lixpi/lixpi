...
const listOrganizationIds = async (): Promise<string[]> => {
    ...
    return [
        ...new Set(
            (result?.items ?? []).map((item: { organizationId?: unknown }) => item.organizationId).filter(
                (organizationId: unknown): organizationId is string => typeof organizationId === 'string' && Boolean(organizationId),
            ),
        ),
    ]
}
...
const containsEmbeddedAsset = (
    ...
    if (record.attrs?.assetId === assetId)
        return true
...
const isCurrentDocumentSurface = async ({
    ...
    if (
        !hostAsset
        || hostAsset.organizationId !== organizationId
        || !hostAsset.documents[role]
    )
        return false
...
const dispatchMaintenanceMessage = async ({
    ...
    if (subject === NATS_SUBJECTS.ASSET_MAINTENANCE_SUBJECTS.DELETE_ASSET) {
        ...
        if (
            !result.deleted
            && result.reason === 'REFERENCES_REMAIN'
        ) {
        ...
        if (
            !result.deleted
            && result.reason !== 'ASSET_NOT_DELETING'
        )
    ...
    if (subject === NATS_SUBJECTS.ASSET_MAINTENANCE_SUBJECTS.DELETE_BLOB) {
        if (!data.blobHash)
            throw new Error('BLOB_HASH_REQUIRED')
    ...
    if (subject === NATS_SUBJECTS.ASSET_MAINTENANCE_SUBJECTS.REBUILD_PROVENANCE) {
        ...
        try {
            ...
            if (retryAttempt >= MAX_PROVENANCE_RETRY_ATTEMPTS) {
                debugError('Asset provenance rebuild exhausted:', error)
            ...
            if ((error as { message?: unknown })?.message !== 'PROVENANCE_PROJECTION_NOT_READY')
                debugError('Asset provenance rebuild deferred:', error)
        }
    ...
    if (subject === NATS_SUBJECTS.ASSET_MAINTENANCE_SUBJECTS.RETRY_RENDITION) {
        ...
        try {
            ...
            if ((error as { message?: string })?.message !== RENDITION_WORKER_UNAVAILABLE)
                throw error

            debugWarn(`Rendition worker unavailable; retrying asset ${data.assetId} in 30s`)
    ...
    if (subject === NATS_SUBJECTS.ASSET_MAINTENANCE_SUBJECTS.CLEANUP_WORKSPACE_REFERENCE) {
        if (
            ...
            throw new Error('INVALID_WORKSPACE_REFERENCE_CLEANUP_PAYLOAD')
    ...
    if (subject === NATS_SUBJECTS.ASSET_MAINTENANCE_SUBJECTS.CLEANUP_ASSET_SURFACE) {
        if (
            ...
            throw new Error('INVALID_ASSET_SURFACE_CLEANUP_PAYLOAD')
        ...
        try {
            ...
            if (Date.now() >= nextUnregisteredObjectCollectionAt) {
                await deleteUnregisteredContentAddressedObjects({
                    ...
                    olderThan: Date.now() - 24 * 60 * 60 * 1000,
                    isRegistered: async (organizationId, blobHash) => Boolean(await BlobModel.get({
                        organizationId,
                        blobHash,
                    })),
                })
            ...
            debugError('Asset maintenance worker poll failed:', error)
        }
