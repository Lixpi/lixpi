...
export class GeneratedOutputReviewService {
    ...
    async review({
        ...
        if (!requester.editableWorkspaceIds.includes(request.workspaceId))
        ...
        if (prepared.affectedNodes.length === 0) {
            ...
            if (markerGainedGeneratedOutputs)
            ...
            if (orphanRemoval.removedNodeIds.length === 0)
                return { error: 'GENERATED_OUTPUT_NOT_FOUND' }

            const canvasGeometry: CanvasGeometryUpdate = {
                generationRequestId: workspace.canvasState.nodes.find(
                    node =>
        ...
        for (const node of prepared.affectedNodes) {
            ...
            if (asset.organizationId !== workspace.organizationId)
                return { error: 'ORGANIZATION_BOUNDARY_VIOLATION' }

            if (
                request.action === 'accept'
                && asset.generatedOutputReview?.status === 'accepted'
            ) {
                assets.push(asset)
            ...
            if (request.action === 'reject') {
                if (
                    ...
                    if (generationRequestId)
                        unfinishedGenerationRequestIds.add(generationRequestId)
                }
            } else {
                if (!outputReady)
        ...
        if (
            ...
            const preservedLineageNodeIds = request.action === 'supersede'
                && request.preserveLineage
                ? new Set(
                    request.scope === 'branch-lineage'
                        ? [request.nodeId]
            ...
            for (const node of prepared.affectedNodes) {
                ...
                if (!detached)
                    return { error: 'STALE_CANVAS_STATE' }
            }
            ...
            const canvasGeometry: CanvasGeometryUpdate = {
                ...
                nodes: finalWorkspace.canvasState.nodes.map(
                    node => ({
                        nodeId: node.nodeId,
        ...
        const geometryNodeIds = new Set(
            mutation.geometryNodes.map(node => node.nodeId),
        )
        const affectedNodeIds = new Set(
            mutation.affectedNodes.map(node => node.nodeId),
        )
        ...
        const canvasGeometry: CanvasGeometryUpdate = {
            ...
            nodeSnapshots,
            edgeSnapshots: persisted.canvasState.edges.filter(
                edge => snapshotNodeIds.has(edge.sourceNodeId) || snapshotNodeIds.has(edge.targetNodeId),
            ),
