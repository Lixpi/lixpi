...
const purgeDocumentSubjects = async (asset: Asset): Promise<void> => {
    ...
    if (!(await natsService.getJetStreamStreamInfoOrNull(streamName)))
        return
...
const buildAssetBlobRemovalOperations = async (asset: Asset) => {
    ...
    for (const pointer of pointers) {
        ...
        if (!blob)
            throw new Error(`ASSET_BLOB_NOT_FOUND:${pointer.blobHash}`)

        const reference = await BlobModel.getReference(blob.blobKey, pointer.referenceKey)

        if (!reference)
            throw new Error(`ASSET_BLOB_REFERENCE_NOT_FOUND:${pointer.referenceKey}`)
...
const deleteAccessProjections = async (
    asset: Asset,
    accessRows: AssetAccessList[],
): Promise<void> => {
    const operations = accessRows.flatMap(
        access => [
            {
            ...
            ...(access.principalId !== asset.ownerUserId
                ? [
                    ...
                    ...(buildAssetSearchRecord(
                        asset,
                        buildAssetPrincipalScopeKey(access.principalId),
                    )
                        ? [{
                            type: 'delete' as const,
...
const AssetMaintenance = {
    deleteAsset: async ({
        ...
        if (
            ...
            return {
                deleted: false,
                reason: 'ASSET_NOT_DELETING',
            }

        if (await hasReferences(assetId))
            return {
                deleted: false,
        ...
        if (finalOperations.length > 100)
            return {
                deleted: false,
                reason: 'ASSET_BLOB_REFERENCE_LIMIT_EXCEEDED',
            }
