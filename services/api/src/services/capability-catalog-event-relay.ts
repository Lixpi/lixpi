...
export const shouldRelayCapabilityCatalogInvalidation = (
    ...
    if (
        payload.scope === 'user'
        && payload.scopeOwnerId === requester.userId
    )
        return true

    if (
        payload.scope === 'organization'
        && requester.organizationIds.includes(payload.scopeOwnerId)
    )
        return true
...
export const ensureCapabilityCatalogEventRelay = (requester: CapabilityRequesterContext): void => {
        ...
        try {
            for await (const message of subscription) {
                ...
                try {
                    payload = JSON.parse(
                        message.string(),
                    ) as Partial<CapabilityCatalogInvalidation>
                } catch {
                    continue
                }
                ...
                if (!shouldRelayCapabilityCatalogInvalidation(payload as CapabilityCatalogInvalidation, currentRequester))
                    continue

                connection.publish(
                    getCapabilityUserEventSubject(requester.userId, NATS_SUBJECTS.CAPABILITY_SUBJECTS.CATALOG.CATALOG_CHANGED),
                    message.data,
                )
            }
        } finally {
