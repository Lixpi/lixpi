...
const refreshRelayAuthorization = async ({
    ...
    if (requester.workspaceIds.length === 0)
        return
...
export const ensureAiInteractionEventRelay = ({
    ...
    const targetSubject = getAiInteractionResponseSubject(
        userId,
        scopeId,
        pipelineId,
    )
            ...
            for await (const message of subscription) {
                try {
                    if (Date.now() - relay.authorizationRefreshedAt >= REQUESTER_REFRESH_INTERVAL_MS)
                        await refreshRelayAuthorization({
                            relay,
                            userId,
                            scopeId,
                            pipelineId,
                    ...
                    debugError(
                        'AI interaction event authorization failed:',
                        {
                            userId,
                            scopeId,
                            pipelineId,
