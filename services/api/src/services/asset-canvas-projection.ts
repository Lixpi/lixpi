...
): CanvasNode | undefined => (nodeId ? nodes.find(node => node.nodeId === nodeId) : undefined)
...
const markerDimensions = (
    ...
    const preview = node.conversationAssetId
        && context.proseMirrorThreadContent
        ? getMarkerConversationPreview(node, context.proseMirrorThreadContent)
        : null
    const promptText = preview?.userText
        || node.provenance?.promptText
        || node.pendingState?.promptText
        || ''
    const responseText = preview?.responseText
        || node.provenance?.reasoningResponseText
        || ''
...
const isGenerationRequestReservationNode = (
    node: CanvasNode,
    generationRequestId: string,
): boolean => {
    if (node.type === 'operationStatus')
        return node.generationRequestId === generationRequestId
...
const findClearVisiblePosition = (
    ...
    const clampX = (x: number): number => Math.min(
        maxX,
        Math.max(minX, x),
    )
    const clampY = (y: number): number => Math.min(
        maxY,
        Math.max(minY, y),
    )
    const xCandidates = [
...
const fallbackPosition = (
    ...
    if (visibleBounds)
        return findClearVisiblePosition(
            nodes,
            dimensions,
            index,
            visibleBounds,
    ...
    return {
        x: Math.min(
            maxX,
            Math.max(minX, proposed.x),
        ),
        y: Math.min(
            maxY,
            Math.max(minY, proposed.y),
        ),
    }
}
...
const upsertNode = (
    ...
    return {
        nodes: nodes.map((node, nodeIndex) => nodeIndex === index ? merged : node),
        changed: true,
    }
}
...
const addEdge = (
    ...
    if (existingEdgeIndex >= 0) {
        ...
        if (
            existingEdge.sourceHandle === 'right'
            && existingEdge.targetHandle === 'left'
        )
            return {
                edges,
                changed: false,
...
const ensureMarkers = (
    ...
    for (const marker of markers) {
        ...
        if (marker.type !== 'branchOrigin') {
            const edgeResult = addEdge(
                edges,
                marker.parentBranchNodeId,
                marker.nodeId,
            )
...
const reconcileLineagePlan = (
    ...
    const markerIds = new Set(
        markers.map(marker => marker.nodeId),
    )
    ...
    const plannedParentByOutputNodeId = new Map(
        plan.runAssignments.flatMap(
            assignment => (
    ...
    const removedEdgeIds = (state.edges ?? []).filter(edge => !retainedEdges.includes(edge)).map(edge => edge.edgeId)
    ...
    const existingNodeIds = new Set(
        markerResult.state.nodes.map(node => node.nodeId),
    )
    let edges = markerResult.state.edges ?? []
    let changed = markerResult.changed
        || staleMarkerIds.size > 0
        || removedEdgeIds.length > 0

    for (const [outputNodeId, parentNodeId] of plannedParentByOutputNodeId) {
        if (
            !existingNodeIds.has(outputNodeId)
            || !existingNodeIds.has(parentNodeId)
        )
            continue
...
const getApiNodePreFrameRect = (
    ...
    if (
        ...
        || (node.generationProgress && ['completed', 'failed', 'cancelled'].includes(node.generationProgress.status))
        || node.mediaGenerationPhase !== 'pending-before-first-frame'
    )
        return null

    return getGeneratedMediaPreFrameRect(
        position,
        node.dimensions,
...
const rebalance = (
    ...
    const contentProjectedNodes = state.nodes.map((node): CanvasNode => {
        ...
        if (
            ...
            || node.provenance.reasoningResponseText === reasoningResponseText
        )
            return node
    ...
    const layoutNodes = resizedNodes.filter(
        node => (
    ...
    const balancedLayoutNodes = rebalanceBranchTreesAndResolve(
        layoutNodes,
        state.edges ?? [],
        {
            ...
            getNodeCollisionRect: (node, position) => {
                ...
                const chromeInsets = node.type === 'image'
                    || node.type === 'video'
                    || node.type === 'capabilityArtifact'
    ...
    const balancedByNodeId = new Map(
        balancedLayoutNodes.map(node => [node.nodeId, node]),
    )
...
const keepFreshLineageMarkersInsideVisibleArea = (
    ...
    const requestMarkers = state.nodes.filter(
        (node): node is MarkerNode => isMarkerNode(node) && node.generationRequestId === plan.generationRequestId,
    )
...
const geometryDiff = (
    ...
    const beforeById = new Map(
        before.nodes.map(node => [node.nodeId, node]),
    )
...
export const buildAssetCanvasGeometryUpdate = ({
    ...
    const geometryNodeIds = new Set(
        geometryNodes.map(node => node.nodeId),
    )
    const removed = new Set(removedNodeIds)
    const nodeSnapshots = state.nodes.filter(
        node =>
    ...
    const snapshotIds = new Set(
        nodeSnapshots.map(node => node.nodeId),
    )
    const edgeSnapshots = (state.edges ?? []).filter(
        edge =>
...
export const upsertMediaLineagePlanToCanvas = async (params: {
    ...
    if (
        !result.canvasState
        || result.canvasStateUpdatedAt === null
    )
        return null
...
export const settleMediaGenerationRequestOnCanvas = async (params: {
    ...
    const result = await Workspace.mutateCanvasState({
        ...
        mutate: canvasState => {
            ...
            removedNodeIds = canvasState.nodes.filter(node => {
                if (!persistedPendingIds.has(node.nodeId))
                    return false
...
export const settleFailedGeneratedMediaRunOnCanvas = async (params: {
    ...
    for (let attempt = 0; attempt < 5; attempt += 1) {
        ...
        if (!isRemovablePendingNode)
            return null

        const failedOperationNode = workspace.canvasState.nodes.find(
        ...
        const failedDimensions = failedOperationNode?.dimensions ?? {
            width: 360,
            height: 104,
        }
        ...
        const stateWithFailedNode: CanvasState = {
            ...
            nodes: workspace.canvasState.nodes.filter(node => node.nodeId !== replacedOperationNodeId).map(
                node => node.nodeId === pendingNodeId ? failedNode : node,
            ),
            edges: (workspace.canvasState.edges ?? []).filter(
                edge => edge.sourceNodeId !== replacedOperationNodeId && edge.targetNodeId !== replacedOperationNodeId,
            ),
        }
        ...
        try {
            ...
            if ('error' in detached) {
                if (detached.error === 'STALE_CANVAS_STATE')
                    continue
...
export const projectGeneratedAssetNode = ({
    ...
    if (!assignment)
        throw new Error('Generated Asset canvas projection requires a lineage assignment')

    const existingLineageParent = findNode(canvasState.nodes, assignment.lineageParentNodeId)
    const markerResult = ensureMarkers(
        canvasState,
        existingLineageParent
            && isMarkerNode(existingLineageParent)
            ? []
            : markerNodesFromAssignment(
                assignment,
                conversationAssetId,
                canvasState,
            ),
    ...
    const safeAspectRatio = Number.isFinite(aspectRatio)
        && aspectRatio > 0
        ? aspectRatio
        : 1
    ...
    const activeGenerationProgress = existing?.generationProgress
        ...
        : undefined
    const generationProgress = !pendingBeforeFirstFrame
    ...
    const withoutSameAsset = markerResult.state.nodes.filter(
        candidate => candidate.nodeId === nodeId || !('assetId' in candidate) || candidate.assetId !== assetId,
    )
    const nodeResult = upsertGeneratedMediaNode(withoutSameAsset, node)
    const edgeResult = addEdge(
        markerResult.state.edges ?? [],
        lineageParentNodeId,
        nodeId,
    )
...
export const projectGeneratedArtifactNode = ({
    ...
    if (!assignment)
        throw new Error('Generated Artifact canvas projection requires a lineage assignment')
...
export const detachReviewedGeneratedOutputsFromCanvas = ({
    ...
    const affectedNodes = canvasState.nodes.filter((candidate): candidate is GeneratedOutputNode => {
        if (
            ...
            && candidate.type !== 'capabilityArtifact'
        )
            return false

        if (!candidate.generatedBy?.branchId)
            return false

        if (scope === 'output-node')
    ...
    const affectedNodeIds = new Set(
        affectedNodes.map(node => node.nodeId),
    )
    const detachedLineageParentIds = new Set(
        affectedNodes.flatMap(
            node =>
                [
    ...
    const edgesWithoutDetachedLineage = (canvasState.edges ?? []).filter(
        edge => !(affectedNodeIds.has(edge.targetNodeId) && detachedLineageParentIds.has(edge.sourceNodeId)),
    )
    ...
    const markerTypeByNodeId = new Map(
        detachedNodes.filter((node): node is MarkerNode => isMarkerNode(node)).map(node => [node.nodeId, node.type]),
    )
    ...
    for (const candidate of detachedNodes) {
        ...
        if (candidate.generatedBy?.branchOriginNodeId)
            referencedOriginNodeIds.add(candidate.generatedBy.branchOriginNodeId)

        if (candidate.generatedBy?.branchForkNodeId)
            referencedForkNodeIds.add(candidate.generatedBy.branchForkNodeId)

        if (candidate.generatedBy?.branchLineNodeId)
            referencedLineNodeIds.add(candidate.generatedBy.branchLineNodeId)
        ...
        if (
            ...
            referencedOriginNodeIds.add(lineageParentNodeId)

        if (
            lineageParentNodeId
            && lineageParentType === 'branchFork'
        )
            referencedForkNodeIds.add(lineageParentNodeId)

        if (
            lineageParentNodeId
            && lineageParentType === 'branchLine'
        )
            referencedLineNodeIds.add(lineageParentNodeId)
    }
    ...
    const nodesWithoutOrphanMarkers = detachedNodes.filter(candidate => {
        ...
        if (remove)
            removedMarkerNodeIds.add(candidate.nodeId)

        return !remove
    })
    const edgesWithoutOrphanMarkers = edgesWithoutDetachedLineage.filter(
        edge => !removedMarkerNodeIds.has(edge.sourceNodeId) && !removedMarkerNodeIds.has(edge.targetNodeId),
    )
    const removedEdgeIds = (canvasState.edges ?? []).filter(edge => !edgesWithoutOrphanMarkers.some(candidate => candidate.edgeId === edge.edgeId)).map(
        edge => edge.edgeId,
    )
    const next = rebalance({
        ...canvasState,
...
export const removeOrphanBranchLineageMarkerFromCanvas = ({
    ...
    const removedEdgeIds = (canvasState.edges ?? []).filter(edge => edge.sourceNodeId === nodeId || edge.targetNodeId === nodeId).map(
        edge => edge.edgeId,
    )
...
export const removeGeneratedOutputCandidateFromCanvas = ({
    ...
    const markerTypeByNodeId = new Map(
        withoutCandidate.filter((node): node is MarkerNode => isMarkerNode(node)).map(node => [node.nodeId, node.type]),
    )
    ...
    const nodes = withoutCandidate.filter(candidate => {
        ...
        if (remove)
            removedNodeIds.add(candidate.nodeId)

        return !remove
    })
    const edges = (canvasState.edges ?? []).filter(edge => !removedNodeIds.has(edge.sourceNodeId) && !removedNodeIds.has(edge.targetNodeId))
    const removedEdgeIds = (canvasState.edges ?? []).filter(edge => !edges.some(candidate => candidate.edgeId === edge.edgeId)).map(
        edge => edge.edgeId,
    )
...
const removeGenerationRequestProjectionNode = ({
    ...
    const hasRemainingGeneratedOutput = canvasState.nodes.some(
        node =>
            node.nodeId !== assetNodeId
            && isGeneratedOutputForRequest(
                node,
                generationRequestId,
                conversationAssetId,
            ),
    ...
    const removedEdgeIds = (canvasState.edges ?? []).filter(edge => !retainedEdgeIds.has(edge.edgeId)).map(edge => edge.edgeId)

    return {
        canvasState: rebalance({
            ...canvasState,
            nodes,
            edges,
        }).state,
...
export const removeMediaGenerationRequestFromCanvas = async ({
    ...
    for (let operation = 0; operation < 100; operation += 1) {
        ...
        const assetNode = workspace.canvasState.nodes.find(
            node => isGeneratedOutputForRequest(
                node,
                generationRequestId,
                conversationAssetId,
            ),
        ...
        if (assetNode) {
            ...
            if (isUnboundReservation) {
                ...
                if (
                    ...
                    || persisted.canvasStateUpdatedAt === null
                )
            ...
            if (!asset)
                throw new Error(`GENERATION_REQUEST_ASSET_NOT_FOUND:${assetNode.assetId}`)
        ...
        return {
            generationRequestId,
            layoutRevision,
            nodes: workspace.canvasState.nodes.map(
                node => ({
                    nodeId: node.nodeId,
                    position: node.position,
                    dimensions: node.dimensions,
...
export const logCanvasProjectionError = (
    context: string,
    error: unknown,
): void => void err(`[asset-canvas-projection] ${context}:`, error)
