...
export const getAssetRequesterContext = async (userId: string): Promise<AssetRequesterContext> => {
    ...
    const workspaceRecords = await Promise.all(
        workspaces.map(
            async workspace => await Workspace.getWorkspace({
                userId,
    ...
    const editableWorkspaceIds = workspaceRecords.flatMap(workspace => {
        ...
        const canEdit = workspace.accessList?.some(
            entry => entry.userId === userId && (entry.accessLevel === 'owner' || entry.accessLevel === 'editor'),
        )
