...
export const getAssetRequesterContext = async (userId: string): Promise<AssetRequesterContext> => {
    ...
    const workspaceRecords = await Promise.all(workspaces.map(async (workspace) =>
        await Workspace.getWorkspace({ userId, workspaceId: workspace.workspaceId })))
    const editableWorkspaceIds = workspaceRecords.flatMap((workspace) => {
        if ('error' in workspace) return []
        const canEdit = workspace.accessList?.some((entry) =>
            entry.userId === userId && (entry.accessLevel === 'owner' || entry.accessLevel === 'editor'))
