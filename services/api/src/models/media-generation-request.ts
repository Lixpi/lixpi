...
const requestsTableName = (): string => getDynamoDbTableStageName('MEDIA_GENERATION_REQUESTS', ORG_NAME, STAGE)
const metaTableName = (): string => getDynamoDbTableStageName('MEDIA_GENERATION_REQUESTS_META', ORG_NAME, STAGE)
const accessTableName = (): string => getDynamoDbTableStageName('MEDIA_GENERATION_REQUESTS_ACCESS_LIST', ORG_NAME, STAGE)
...
export const createMediaGenerationRequest = async (request: MediaGenerationRequest): Promise<MediaGenerationRequest> => {
    const accessRows: MediaGenerationRequestAccessList[] = [userPrincipalId(request.userId), workspacePrincipalId(request.workspaceId)]
        .map(principalId => ({
            generationRequestId: request.generationRequestId,
            principalId,
...
export const getAuthorizedMediaGenerationRequest = async ({
    ...
    const request = await getMediaGenerationRequest({ generationRequestId, workspaceId })
    ...
    if (requiredAccess === 'read') {
        const workspaceAccess = await dynamoDBService.getItem({
            tableName: accessTableName(),
            key: { generationRequestId, principalId: workspacePrincipalId(workspaceId) },
            consistentRead: true,
        ...
        if (workspaceAccess) {
            ...
            if (!('error' in workspace)) return request
        }
    }
    const access = await dynamoDBService.getItem({
        tableName: accessTableName(),
        key: { generationRequestId, principalId: userPrincipalId(userId) },
        consistentRead: true,
        origin: 'MediaGenerationRequest.getAuthorized',
    }) as MediaGenerationRequestAccessList | undefined
    if (!access) return { error: 'PERMISSION_DENIED' }
    if (requiredAccess === 'owner' && access.accessLevel !== ACCESS_LEVEL.OWNER) {
...
export const transitionMediaGenerationRequest = async ({
    ...
    if (request.revision !== expectedRevision + 1) throw new Error('MEDIA_REQUEST_REVISION_INCREMENT_REQUIRED')
    await dynamoDBService.transactWrite({
        operations: [
            {
                type: 'put',
                ...
                expressionAttributeNames: { '#revision': 'revision', '#deletingAt': 'deletingAt' },
...
export const deleteWorkspaceMediaGenerationRequests = async (workspaceId: string): Promise<MediaGenerationRequest[]> => {
    ...
    for (const request of requests) {
        ...
        await dynamoDBService.transactWrite({
            operations: [
                { type: 'delete', tableName: requestsTableName(), key: { generationRequestId: request.generationRequestId, workspaceId } },
                { type: 'delete', tableName: metaTableName(), key: { workspaceId, generationRequestId: request.generationRequestId } },
                ...(access?.items ?? []).map((row: MediaGenerationRequestAccessList) => ({
                    type: 'delete' as const,
                    tableName: accessTableName(),
                    key: { generationRequestId: request.generationRequestId, principalId: row.principalId },
                })),
            ],
