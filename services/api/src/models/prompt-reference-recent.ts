...
const recentsTableName = (): string => getDynamoDbTableStageName(
...
export const getPromptReferenceId = (reference: PromptReference): string => {
...
export const buildPromptReferenceKey = (
...
const PromptReferenceRecentModel = {
    ...
    list: async ({
        ...
        if (
            ...
            throw new Error('INVALID_PROMPT_REFERENCE_RECENT_LIMIT')
        ...
        return ((result?.items ?? []) as PromptReferenceRecent[]).filter(item => allowedTypes.has(item.referenceType))
            .slice(0, limit)
    },
    remove: async ({
        userId,
        ...
        await dynamoDBService.transactWrite({
            operations: [...new Set(referenceKeys)].map(
                referenceKey => ({
                    type: 'delete' as const,
                    tableName: recentsTableName(),
                    key: {
