...
const materializeCropsForReference = async (args: {
    ...
    const rand = mulberry32(seedFromString(`${args.styleExtractionRunId}:${args.ref.imageRef}`))
    ...
    const results = await Promise.all(plans.map(async (plan, idx): Promise<StyleSampleRef | undefined> => {
        try {
...
export const materializeSourceCrops = async (state: StyleExtractionState, logger: StageLogger, deps: StyleExtractionDependencies): Promise<Partial<StyleExtractionState>> => {
    return await logger.span('crops', 'sharp', async () => {
        ...
        const indexed: StyleSampleRef[] = allCrops.map((c, idx) => ({ ...c, idx }))
