...
const fetchImageBytes = async (url: string): Promise<Buffer> => {
    ...
    if (objRef) {
        ...
        if (!data)
            throw new Error(`Source image not found in NATS object store: ${url}`)
    ...
    if (
        ...
        if (!response.ok)
            throw new Error(`Failed to fetch source image ${url}: HTTP ${response.status}`)
...
const pickSubCrop = (
    ...
    if (minSide < MIN_CROP_AXIS_PX)
        return null

    const axis = Math.min(
        desiredAxis,
        Math.floor(minSide * 0.9),
    )

    if (axis < MIN_CROP_AXIS_PX)
        return null
...
const materializeCropsForReference = async (args: {
    ...
    const rand = mulberry32(
        seedFromString(`${args.styleExtractionRunId}:${args.ref.imageRef}`),
    )
    ...
    for (const region of args.sceneEntry.regions) {
        plans.push(...planRegionCrops(
            rand,
            region,
            args.ref.imageRef,
            imgWidth,
            imgHeight,
            REGION_CROPS_PER_REGION,
        ))
    }

    // One composition-preserving thumbnail per reference.
    plans.push(
        planCompositionThumbnail(
            args.ref.imageRef,
            imgWidth,
            imgHeight,
        ),
    ...
    const results = await Promise.all(
        plans.map(async (plan, idx): Promise<StyleSampleRef | undefined> => {
            try {
                ...
                if (!args.organizationId)
                    throw new Error('Organization context required for crop Blob storage')

                const stored = await BlobModel.store({
                    organizationId: args.organizationId,
                    bytes: cropBuffer,
                    mimeType: 'image/png',
                    description: `style-extraction-${args.styleExtractionRunId}-${args.ref.imageRef}-${plan.region.label}.png`,
                })
    ...
    return results.filter((r): r is StyleSampleRef => r !== undefined)
}

export const materializeSourceCrops = async (
    ...
    return await logger.span(
        ...
        async () => {
            ...
            const indexed: StyleSampleRef[] = allCrops.map(
                (c, idx) => ({
                    ...c,
                    idx,
                }),
        ...
        {
            inputSummary: `references=${state.references.length} subjects=${state.sceneAssessment?.references.reduce(
                (sum, r) => sum + r.subjects.length,
                0,
            ) ?? 0}`,
