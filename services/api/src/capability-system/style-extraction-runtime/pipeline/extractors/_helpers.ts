...
export const buildExtractorMessages = (
    ...
    blocks.push({
        ...
        text: [
            `User intent: ${intent}`,
            '',
            `Scene assessment (from the upstream router): ${JSON.stringify(
                scene,
                null,
                2,
            )}`,
...
export const runAxisVlm = async <T = Record<string, any>>(args: {
    ...
    const schema = wrapAxisSchema(
        args.extractor.axis,
        args.extractor.description,
        args.fieldsSchema,
    )
