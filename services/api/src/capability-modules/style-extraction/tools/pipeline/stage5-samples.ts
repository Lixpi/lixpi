...
const renderAppliedMediumProbe = async (args: {
    ...
    const syntheticState: ProviderState = {
        ...
        eventMeta: { userId: state.input.userId, workspaceId: state.input.workspaceId, organizationId: state.input.organizationId, aiChatThreadId: state.input.styleExtractionRunId },
...
export const generateSamples = async (state: StyleExtractionState, logger: StageLogger, deps: StyleExtractionDependencies): Promise<Partial<StyleExtractionState>> => {
