...
export async function resolveStyleExtractionInput(
    ...
    for (const assetId of sourceAssetIds) {
        ...
        if ('error' in asset || asset.media?.kind !== 'image') {
        ...
        if (!blob) throw new CapabilityError('CAPABILITY_ACTION_FAILED', `Image Asset ${assetId} content is unavailable`)
        imageBlocks.push({ type: 'input_image', image_url: `nats-obj://${blob.bucketName}/${blob.objectKey}` })
    }

    const analysis = await resolveModel(analysisModelId)
    const imageModelId = typeof input.imageModelId === 'string' && input.imageModelId ? input.imageModelId : undefined
    ...
    const intent = typeof input.intent === 'string' && input.intent.trim() ? input.intent.trim() : prompt
    return {
        ...
        messages: [{
            role: 'user',
            content: [{ type: 'input_text', text: prompt }, ...imageBlocks],
        }],
        sourceAssetIds,
