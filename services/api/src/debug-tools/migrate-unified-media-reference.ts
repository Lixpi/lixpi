...
async function main(): Promise<void> {
    ...
    process.stdout.write(`${JSON.stringify({ phase: 'preflight', ...preflight }, null, 2)}\n`)
    if (!process.argv.includes('--apply')) return
    if (preflight.quarantined.length > 0) throw new Error('Migration has quarantined records; repair them before applying.')
