...
const parseDocument = (content: unknown): Record<string, unknown> => {
    ...
    if (
        ...
        throw new Error('Legacy document content is not JSON')
...
const titleFreeDocument = (content: unknown): Record<string, unknown> => {
    ...
    if (
        ...
        && (doc.content[0] as any)?.type === 'documentTitle'
    )
        doc.content.shift()
...
if (
    ...
    throw new Error('Usage: --input <old.zip> --output <rev2.zip>')
...
if (!manifestEntry)
    throw new Error('Input archive is missing manifest.json')

const oldManifest = JSON.parse(
    manifestEntry.getData().toString('utf8'),
) as any

if (oldManifest.exportVersion !== 1)
    throw new Error(`Expected export version 1, got ${oldManifest.exportVersion}`)

if (
    !oldManifest.workspace?.canvasState
    || !Array.isArray(oldManifest.workspace.files)
)
...
for (const file of files.values()) {
    for (const field of renditionLinkFields) {
        if (file[field])
            rootByRelatedFileId.set(file[field]!, file.id)
    }
}

for (const fileId of files.keys()) {
    if (rootByRelatedFileId.has(fileId))
        continue
...
for (const node of canvasNodes) {
    ...
    if (assetId)
        legacyNodeIdToAssetId.set(node.nodeId, assetId)
}
...
const addReference = (
    ...
    if (existing) {
        existing.nodeIds = [...new Set([
            ...(existing.nodeIds ?? []),
            ...nodeIds,
        ])]
        existing.surfaceIds = [...new Set([
            ...(existing.surfaceIds ?? []),
            ...surfaceIds,
        ])]
...
const newestLegacyDescriptor = (nodes: any[]): ContentDescriptor | undefined =>
    nodes.map(node => normalizeLegacyDescriptor(node?.descriptor)).filter((descriptor): descriptor is ContentDescriptor => Boolean(descriptor))
        .sort((left, right) => right.updatedAt - left.updatedAt)[0]

const remapLegacyJson = (value: unknown): unknown => {
    if (Array.isArray(value))
        return value.map(remapLegacyJson)
    ...
    if (
        legacyStorageId
        && legacyIdToAssetId.has(legacyStorageId)
    ) {
        ...
        if (
            ...
            target.src = `/api/assets/${assetId}/renditions/original`

        if ('imageUrl' in source)
            target.imageUrl = `/api/assets/${assetId}/renditions/original`

        if ('videoUrl' in source)
            target.videoUrl = `/api/assets/${assetId}/renditions/original`

        if ('posterUrl' in source)
            target.posterUrl = `/api/assets/${assetId}/renditions/poster`

        if ('frameUrl' in source)
            target.frameUrl = `/api/assets/${assetId}/renditions/representativeFrame`
    }

    for (const [key, child] of Object.entries(source)) {
        if (['fileId', 'posterFileId', 'frameFileId', 'src', 'posterSrc', 'referenceId', 'workspaceId', 'descriptor', 'imageUrl', 'videoUrl', 'posterUrl', 'frameUrl'].includes(key))
            continue

        if (
            ['aiChatThreadId', 'conversationAssetId', 'threadId', 'documentId'].includes(key)
            && typeof child === 'string'
        ) {
...
for (const document of oldManifest.documents ?? []) {
    ...
    const snapshot = Buffer.from(
        JSON.stringify(
            remapLegacyJson(
                titleFreeDocument(document.content),
            ),
        ),
        'utf8',
    )
    ...
    addReference(
        assetId,
        documentNodes.map((node: any) => node.nodeId),
        [
            `document#${assetId}`,
        ],
    )
...
for (const thread of oldManifest.aiChatThreads ?? []) {
    ...
    const snapshot = Buffer.from(
        JSON.stringify(
            remapLegacyJson(
                titleFreeDocument(thread.content),
            ),
        ),
        'utf8',
    )
...
for (const entry of oldZip.getEntries().filter(candidate => candidate.entryName.startsWith('images/') && !candidate.isDirectory)) {
    ...
    const matchingId = [...files.keys()]
        .sort((left, right) => right.length - left.length).find(fileId => filename === fileId || filename.startsWith(`${fileId}.`))

    if (matchingId)
        archivedObjectByFileId.set(
            matchingId,
            entry.getData(),
        )
}

const renditionFileId = (
    root: LegacyFile | undefined,
    nodes: any[],
    ...
    if (name === 'canonical')
        return root?.canonicalFileId ?? node?.canonicalFileId

    if (name === 'preview')
        return root?.previewFileId ?? node?.previewFileId

    if (name === 'thumbnail')
        return root?.thumbnailFileId ?? node?.thumbnailFileId

    if (name === 'poster')
        return root?.posterFileId ?? node?.posterFileId

    if (name === 'representativeFrame')
        return root?.frameFileId ?? node?.frameFileId
...
for (const rootFileId of rootFileIds) {
    ...
    for (const name of renditionNames) {
        ...
        if (!bytes) {
            if (name === 'original')
                throw new Error(`Version-1 archive is missing required object for ${fileId}`)
    ...
    if (renditions.original?.status !== 'ready')
        throw new Error(`Version-1 archive has no original bytes for ${rootFileId}`)

    const requestedRenditions = [...ASSET_REQUIRED_RENDITIONS[kind]]

    if (
        file.modelSafe === false
        && !requestedRenditions.includes('canonical')
    )
        requestedRenditions.push('canonical')
            ...
            ? {
                lineage: {
                    ...
                    sourceAssetIds: [...new Set(
                        sourceNodeIds.flatMap((nodeId: string) => legacyNodeIdToAssetId.get(nodeId) ?? []),
                    )],
    ...
    if (relatedNodes.length > 0)
        addReference(
            assetId,
            relatedNodes.map((node: any) => node.nodeId),
        )
}
...
for (const hostAsset of assets) {
    for (const role of ['content', 'conversation'] as const) {
        ...
        if (!bytes)
            throw new Error(`Converter produced missing document bytes ${pointer.blobHash}`)

        for (const embeddedAssetId of collectEmbeddedAssetIds(
            JSON.parse(
                bytes.toString('utf8'),
            ),
            role,
            ...
            if (embeddedAssetId === hostAsset.assetId)
                throw new Error('Converter produced a self-referential editable document')

            const embeddedAsset = assets.find(asset => asset.assetId === embeddedAssetId)

            if (!embeddedAsset)
                throw new Error(`Converter produced dangling embedded Asset ID ${embeddedAssetId}`)
            ...
            if (!hadWorkspaceReference)
                embeddedAsset.referenceCount += 1
        }
    }
}
...
for (const node of canvasState.nodes ?? []) {
    if (
        ['image', 'video', 'audio', 'mediaDocument', 'document'].includes(node.type)
        && !node.assetId
    )
...
if (assetIds.size !== assets.length)
    throw new Error('Converter produced duplicate Asset IDs')
...
for (const asset of assets) {
    for (const lineageId of [
        ...
        if (
            lineageId
            && !assetIds.has(lineageId)
        )
            throw new Error(`Converter produced dangling lineage Asset ID ${lineageId}`)
    }

    for (const [role, pointer] of Object.entries(asset.documents)) {
        if (
            pointer
            && !blobMeta.has(pointer.blobHash)
        )
            throw new Error(`Converter produced missing document Blob ${pointer.blobHash}`)
    ...
    for (const rendition of Object.values(asset.media?.renditions ?? {})) {
        if (
            ...
            && (!rendition.blobHash || !blobMeta.has(rendition.blobHash))
        )
