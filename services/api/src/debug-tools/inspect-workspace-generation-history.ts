...
function readFlag(name: string, aliases: string[]): string | undefined {
    for (const flag of [name, ...aliases]) {
        ...
        if (equalsArgument) return equalsArgument.slice(flag.length + 1)
...
function parseArgs(): DebugArgs {
    ...
    return { workspaceId, ...(generationRequestId ? { generationRequestId } : {}) }
}
...
function summarizeReference(reference: unknown): JsonRecord {
...
function summarizeDocument(document: unknown): JsonRecord {
    ...
    const visit = (value: unknown, path: Array<string | number>): void => {
        ...
        if (type === 'prompt_reference' || type === 'promptReference' || type === 'aiPromptReference') {
            userReferences.push({
        ...
        if (type === 'aiGeneratedImage' || type === 'aiGeneratedVideo') {
            generatedMedia.push({
                path: compactPath(path),
                type,
                assetId: attrs.assetId,
...
function summarizeCanvasNode(node: CanvasNode): JsonRecord {
...
function summarizeCheckpoint(value: unknown): JsonRecord | null {
...
function summarizeRequest(request: MediaGenerationRequest, checkpoint: unknown): JsonRecord {
    return {
        ...
        unresolvedBindings: request.unresolvedBindings.map(binding => ({
        ...
        resolvedReferences: request.resolvedReferences.map(reference => ({
    ...
    for (const role of roles) {
        ...
        documents[role] = {
            ...
            eventLog: {
                ...
                kinds: events.reduce<Record<string, number>>((result, event) => {
...
async function main(): Promise<void> {
    ...
    try {
        ...
        const outputAssets = (await Promise.all([...outputAssetIds].map(assetId => (
            getAssetRecord(assetId, 'inspect-workspace-generation-history')
        ))))
            .filter((asset): asset is Asset => Boolean(asset))
