...
const loadWorkspaceModel = async (): Promise<WorkspaceModel> => {
...
const parseArgs = (): RepairArgs => {
    const workspaceId = readFlag(
        '--workspace',
        ['--workspaceId'],
...
const getMarkerReasoningRunId = (node: BranchMarkerNode): string | undefined => {
...
const getMarkerReasoningModelId = (node: BranchMarkerNode): string | undefined => {
    return node.type === 'branchFork'
        || node.type === 'branchLine'
...
const getMarkerReasoningIndex = (node: BranchMarkerNode): number | undefined => {
    return node.type === 'branchFork'
        || node.type === 'branchLine'
        ? node.reasoningIndex
...
const matchesGenerationRequest = (
    node: BranchMarkerNode,
    generationRequestId?: string,
): boolean => !generationRequestId || node.generationRequestId === generationRequestId

const findPendingMarkers = (
...
const summarizeMarker = (node: BranchMarkerNode): MarkerSummary => {
    return {
        nodeId: node.nodeId,
        type: node.type,
...
const removePendingStateFromMarkers = (
    canvasState: CanvasState,
    generationRequestId?: string,
): {
    canvasState: CanvasState
...
const main = async (): Promise<void> => {
    ...
    const canvasState = {
        viewport: workspace.canvasState?.viewport ?? {
            x: 0,
            y: 0,
            zoom: 1,
        },
    ...
    const repairedCanvasState = {
        viewport: repairedWorkspace?.canvasState?.viewport ?? {
            x: 0,
            y: 0,
            zoom: 1,
        },
