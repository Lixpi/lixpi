...
const tableName = (name: 'WORKSPACES' | 'AI_CHAT_THREADS'): string => {
    const legacyNames = {
        WORKSPACES: 'Workspaces',
        AI_CHAT_THREADS: 'AI-Chat-Threads',
    } as const

    return formatStageResourceName(
        legacyNames[name],
        requireEnv('ORG_NAME'),
        requireEnv('STAGE'),
    )
...
const getGeneratedBy = (node: CanvasNode): AnyRecord => ((node as AnyRecord).generatedBy ?? {}) as AnyRecord
...
const collectCanvasMedia = (canvasState: CanvasState | undefined): CanvasMediaRecord[] => {
...
const collectThreadMediaFromNode = (
    node: AnyRecord,
    threadId: string,
    ...
    node.content.forEach(
        (child: unknown, index: number) => void collectThreadMediaFromNode(
            child as AnyRecord,
            threadId,
            [...path, 'content', index],
            records,
...
const collectThreadMedia = (threads: AnyRecord[]): ThreadMediaRecord[] => {
    const records: ThreadMediaRecord[] = []

    for (const thread of threads) {
        collectThreadMediaFromNode(
            thread.content as AnyRecord,
            thread.threadId,
            ['content'],
            records,
        )
...
const indexThreadRecords = (threadRecords: ThreadMediaRecord[]): Map<string, ThreadMediaRecord[]> => {
    const indexed = new Map<string, ThreadMediaRecord[]>()

    const add = (
        key: string | undefined,
        record: ThreadMediaRecord,
    ): void => {
        if (!key)
            return
...
const findThreadRecord = (
    ...
    const candidateKeys = [
        canvasRecord.mediaRunId ? {
            kind: 'mediaRunId',
            key: `mediaRunId:${canvasRecord.mediaRunId}`,
        } : undefined,
        canvasRecord.responseId ? {
            kind: 'responseId',
            key: `responseId:${canvasRecord.responseId}`,
        } : undefined,
        canvasRecord.generationRequestId
            && canvasRecord.reasoningRunId
            && canvasRecord.mediaModelId
            ? {
                kind: 'generationRun',
    ...
    for (const {
        ...
        const matchingType = records.find(
            record =>
...
const compareRecords = (
    canvasRecord: CanvasMediaRecord,
    threadRecord?: ThreadMediaRecord,
    matchKind = 'none',
): MediaComparison => {
    if (!threadRecord) {
        return {
    ...
    if (
        canvasRecord.type === 'video'
        && canvasRecord.posterFileId !== threadRecord.posterFileId
    )
...
const collectFileIds = (
    canvasRecords: CanvasMediaRecord[],
    threadRecords: ThreadMediaRecord[],
): string[] => {
...
const inspectObjectStore = async (
    nats: NATS_Service,
    workspaceId: string,
    fileIds: string[],
): Promise<AnyRecord> => {
            ...
            JSON.stringify(
                {
                    ...
                    comparisons,
                    staleThreadRecords,
                    objectStore: await inspectObjectStore(
                        nats,
                        args.workspaceId,
                        fileIds,
                    ),
