...
router.get('/workspaces/:workspaceId/objects/:objectKey', authenticateRequest, async (req: any, res: any) => {
    const { workspaceId, objectKey } = req.params
    if (!isTransientMediaObjectKey(objectKey)) return res.status(400).json({ error: 'INVALID_TRANSIENT_MEDIA_KEY' })
    const mimeType = getTransientMediaMimeType(objectKey)
    if (!mimeType) return res.status(415).json({ error: 'UNSUPPORTED_TRANSIENT_MEDIA_TYPE' })

    const workspace = await Workspace.getWorkspace({ userId: req.user.userId, workspaceId })
    ...
    const bytes = await natsService.getObject(getTransientMediaBucketName(workspace.organizationId), objectKey)
    if (!bytes) return res.status(404).json({ error: 'TRANSIENT_MEDIA_NOT_FOUND' })

    const buffer = Buffer.from(bytes)
    res.setHeader('Content-Type', mimeType)
    res.setHeader('Cache-Control', 'private, no-store')
    const range = req.headers.range
    if ((mimeType.startsWith('audio/') || mimeType.startsWith('video/')) && range) {
