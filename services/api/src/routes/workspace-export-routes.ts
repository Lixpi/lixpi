...
const getWorkspaceReferenceAssetIds = async (workspaceId: string): Promise<string[]> => {
    ...
    return ((result?.items ?? []) as AssetReference[]).filter(reference => reference.type === 'workspace' && reference.workspaceId === workspaceId).map(
...
const collectExportAssets = async ({
    ...
    while (pending.length) {
        ...
        if ('error' in asset)
            throw new Error(`UNEXPORTABLE_REFERENCED_ASSET:${assetId}:${asset.error}`)

        assets.set(asset.assetId, asset)

        for (const lineageAssetId of getAssetLineageIds(asset)) {
            if (!assets.has(lineageAssetId))
                pending.push(lineageAssetId)
        }
        ...
        for (const role of ['content', 'conversation', 'capabilityArtifact'] as const) {
            ...
            for (const referencedAssetId of collectReferencedAssetIds(snapshot?.doc)) {
                if (!assets.has(referencedAssetId))
                    pending.push(referencedAssetId)
            }
        }
    }
    ...
    for (const hostAsset of assets) {
        for (const role of ['content', 'conversation', 'capabilityArtifact'] as const) {
            ...
            for (const embeddedAssetId of collectEmbeddedAssetIds(snapshot?.doc, role)) {
                ...
                const matchingSurfaces = (allReferencesByAssetId.get(embeddedAssetId) ?? []).flatMap(reference => reference.surfaceIds ?? []).filter(
                    surfaceId => surfaceId.startsWith(surfacePrefix),
                )
...
const buildPortableAssets = async (
    assets: Asset[],
    exportedAt: number,
): Promise<{
    assets: Asset[]
    virtualBlobs: Map<string, {
        bytes: Buffer
        meta: Revision2BlobManifestEntry
    }>
}> => {
    const virtualBlobs = new Map<string, {
        bytes: Buffer
        meta: Revision2BlobManifestEntry
    }>()
...
const validateRevision2Manifest = (
    manifest: unknown,
    zip: AdmZip,
): Revision2Manifest => {
    if (
    ...
    if (candidate.exportVersion !== 2)
        throw new Error('REVISION_2_ARCHIVE_REQUIRED')

    if (
        !candidate.workspace?.canvasState
    ...
    for (const asset of candidate.assets) {
        if (
            ...
            throw new Error('DUPLICATE_OR_INVALID_ASSET_ID')
        ...
        if (asset.states.lifecycle === 'deleting')
            throw new Error(`DELETING_ASSET_NOT_PORTABLE:${asset.assetId}`)
    ...
    for (const blob of candidate.blobs) {
        if (
            !isSha256(blob?.blobHash)
            || blobHashes.has(blob.blobHash)
        )
            throw new Error('DUPLICATE_OR_INVALID_BLOB_HASH')
        ...
        if (
            blob.sourceBlobHash
            && !isSha256(blob.sourceBlobHash)
        )
            throw new Error(`INVALID_SOURCE_BLOB_HASH:${blob.blobHash}`)

        blobHashes.add(blob.blobHash)
        totalBlobBytes += blob.byteSize

        if (totalBlobBytes > MAX_ARCHIVE_BYTES)
            throw new Error('ARCHIVE_BLOBS_TOO_LARGE')
        ...
        if (
            ...
            throw new Error(`MISSING_BLOB:${blob.blobHash}`)

        if (entry.header.size !== blob.byteSize)
            throw new Error(`BLOB_SIZE_MISMATCH:${blob.blobHash}`)
        ...
        if (sha256(bytes) !== blob.blobHash)
            throw new Error(`BLOB_HASH_MISMATCH:${blob.blobHash}`)
    }

    for (const blob of candidate.blobs) {
        if (
            blob.sourceBlobHash
            && !blobHashes.has(blob.sourceBlobHash)
        )
    ...
    for (const asset of candidate.assets) {
        ...
        if (
            asset.artifact
            && (!artifactDefinition || asset.documents.capabilityArtifact?.schemaVersion !== asset.artifact.schemaVersion || artifactDefinition.shared.schemaVersion !== asset.artifact.schemaVersion)
        )
        ...
        for (const [role, pointer] of Object.entries(asset.documents)) {
            ...
            if (!blobHashes.has(pointer.blobHash))
                throw new Error(`MISSING_DOCUMENT_BLOB:${asset.assetId}`)

            const blob = candidate.blobs.find(entry => entry.blobHash === pointer.blobHash)

            if (blob?.byteSize !== pointer.byteSize)
                throw new Error(`DOCUMENT_SIZE_MISMATCH:${asset.assetId}:${role}`)
            ...
            if (role === 'capabilityArtifact')
                artifactDefinition.shared.assertInitialDocument(doc)
        ...
        for (const [name, rendition] of Object.entries(asset.media?.renditions ?? {})) {
            ...
            if (
                rendition?.status === 'ready'
                && (!rendition.blobHash || !blobHashes.has(rendition.blobHash))
            )
            ...
            if (rendition?.status === 'ready') {
                ...
                if (
                    blob?.byteSize !== rendition.byteSize
                    || blob.mimeType !== rendition.mimeType
                )
        ...
        for (const lineageAssetId of getAssetLineageIds(asset)) {
            if (!assetIds.has(lineageAssetId))
                throw new Error(`MISSING_LINEAGE_ASSET:${lineageAssetId}`)
        }
    }
    ...
    for (const reference of candidate.references) {
        if (!assetIds.has(reference.assetId))
            throw new Error(`REFERENCE_ASSET_MISSING:${reference.assetId}`)

        const key = `${reference.assetId}#${reference.referenceKey}`

        if (referenceKeys.has(key))
            throw new Error(`DUPLICATE_REFERENCE:${key}`)
    ...
    if (referenceWorkspaceIds.size > 1)
        throw new Error('MULTIPLE_SOURCE_WORKSPACES_NOT_PORTABLE')
    ...
    for (const assetId of getCanvasAssetIds(candidate.workspace.canvasState)) {
        if (!assetIds.has(assetId))
            throw new Error(`CANVAS_ASSET_MISSING:${assetId}`)

        if (!referenceAssetIds.has(assetId))
            throw new Error(`CANVAS_ASSET_REFERENCE_MISSING:${assetId}`)
    }
    ...
    for (const node of candidate.workspace.canvasState.nodes ?? []) {
        ...
        for (const field of ['fileId', 'posterFileId', 'frameFileId', 'src', 'posterSrc', 'referenceId', 'aiChatThreadId']) {
            if (field in record)
                throw new Error(`LEGACY_CANVAS_FIELD:${field}`)
        }
        ...
        if (
            ...
            if (!reference)
                throw new Error(`CANVAS_ASSET_REFERENCE_MISSING:${node.assetId}:${node.nodeId}`)
        }
    }
...
const remapAssetIdsInJson = (
    value: unknown,
    assetIdMap: Map<string, string>,
): unknown => {
    if (Array.isArray(value))
        return value.map(item => remapAssetIdsInJson(item, assetIdMap))

    if (typeof value === 'string') {
        let remapped = value

        for (const [sourceAssetId, targetAssetId] of assetIdMap) {
            remapped = remapped.replaceAll(`/api/assets/${encodeURIComponent(sourceAssetId)}/`, `/api/assets/${encodeURIComponent(targetAssetId)}/`)
        }
...
const remapSurfaceId = (
    surfaceId: string,
    assetIdMap: Map<string, string>,
): string =>
    surfaceId
        .split('#').map(part => assetIdMap.get(part) ?? part)
        .join('#')

const normalizeImportedAsset = (source: Asset): Asset => {
    const renditions = source.media
        ? Object.fromEntries(
            Object.entries(source.media.renditions).map(
                ([name, rendition]) => [
                    name,
                    rendition?.status === 'pending'
                        ? {
                            ...rendition,
                            status: 'failed',
                            errorCode: 'IMPORT_PENDING_RENDITION_NOT_PORTABLE',
                        }
                        : rendition,
                ],
            ),
    ...
    return {
        ...
        ...(media ? { media } : {}),
        states: {
            lifecycle: mediaState === 'failed'
                || mediaState === 'cancelled'
                ? 'failed'
                : 'active',
            media: mediaState,
    ...
    async (req: any, res: any) => {
        try {
            ...
            while (pendingBlobHashes.length) {
                ...
                if (!blob) {
                    if (portable.virtualBlobs.has(blobHash))
                        continue
                ...
                if (blob.sourceBlobHash)
                    pendingBlobHashes.push(blob.sourceBlobHash)
            }
            ...
            for (const blob of storedBlobs.values()) {
                ...
                if (
                    !bytes
                    || bytes.byteLength !== blob.byteSize
                    || sha256(bytes) !== blob.blobHash
                )
            ...
            debugError('Revision-2 workspace export failed:', error)
    ...
    async (req: any, res: any) => {
        ...
        try {
            ...
            if (entry.header.size > 10 * 1024 * 1024)
                return res.status(400).json({ error: 'MANIFEST_TOO_LARGE' })

            const manifest = validateRevision2Manifest(
                JSON.parse(
            ...
            const assetIdMap = new Map<string, string>(
                manifest.assets.map(asset => [asset.assetId, uuid()]),
            )
            ...
            for (const sourceAsset of manifest.assets) {
                for (const role of Object.keys(sourceAsset.documents) as AssetDocumentRole[]) {
                    ...
                    if (!sourceBytes)
                        throw new Error(`MISSING_DOCUMENT_BLOB:${sourceAsset.assetId}:${role}`)
            ...
            for (const blobMeta of manifest.blobs) {
                ...
                if (stored.blobHash !== blobMeta.blobHash)
            ...
            for (const sourceAsset of manifest.assets) {
                ...
                if (
                    normalized.media
                    && normalized.states.media !== 'ready'
                )
                    importedMediaAssetIds.push(assetId)
            }
            ...
            for (const sourceAsset of manifest.assets) {
                ...
                for (const role of ['content', 'capabilityArtifact'] as const) {
                    ...
                    for (const embeddedAssetId of collectEmbeddedAssetIds(document, role)) {
                        if (embeddedAssetId === hostAssetId)
            ...
            for (const previousAssetId of previousAssetIds) {
                if (createdAssetIds.includes(previousAssetId))
                    continue
                ...
                if (!workspaceReferenceRemoved) {
                    debugError(
                        'Post-import workspace reference cleanup queued:',
                        {
                            previousAssetId,
                            cleanupError,
                        },
                    ...
                    await enqueueWorkspaceReferenceCleanup({
                        ...
                        cleanupPending = true
                        debugError(
                            'Unable to queue post-import workspace reference cleanup:',
                            {
                                previousAssetId,
                                error,
                            },
                ...
                const previousAsset = await AssetModel.get({
                    assetId: previousAssetId,
                    requester,
                })
                ...
                if (
                    ...
                    if ('error' in detached) {
                        await enqueueWorkspaceReferenceCleanup({
                            ...
                            cleanupPending = true
                            debugError(
                                'Unable to queue post-import catalog cleanup:',
                                {
                                    previousAssetId,
                                    error,
                                },
            ...
            for (const blobHash of touchedBlobHashes) {
                await enqueueBlobDeletion({
                    ...
                    cleanupPending = true
                    debugError(
                        'Unable to queue imported staging Blob cleanup:',
                        {
                            blobHash,
                            error,
                        },
            ...
            for (const assetId of importedMediaAssetIds) {
                await enqueueRenditionRetry({
                    ...
                    cleanupPending = true
                    debugError(
                        'Unable to queue imported Asset rendition reconstruction:',
                        {
                            assetId,
                            error,
                        },
            ...
            return res.json({
                success: true,
                importedAssets: assetIdMap.size,
                ...(cleanupPending ? { cleanupPending: true } : {}),
            })
        } catch (error) {
            debugError('Revision-2 workspace import failed:', error)
