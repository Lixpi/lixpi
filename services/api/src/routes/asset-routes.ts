...
router.post(
    ...
    async (req: any, res: any) => {
        ...
        if (workspace.deletingAt)
            return res.status(409).json({ error: 'WORKSPACE_DELETING' })
        ...
        try {
            ...
            if (error instanceof AssetFileRejectedError)
                return res.status(422).json({ error: error.reason })

            debugError('Asset upload failed:', error)
...
router.post(
    '/workspaces/:workspaceId/import-url',
    authenticateRequest,
    async (req: any, res: any) => {
        const { userId } = req.user
        const { workspaceId } = req.params
...
router.get(
    '/:assetId/renditions/:renditionName',
    authenticateRequest,
    async (req: any, res: any) => {
        ...
        if ('error' in result)
            return res.status(result.error === 'NOT_FOUND' ? 404 : 403).json(result)
        ...
        if (
            ...
            return res.status(404).json({ error: 'RENDITION_NOT_FOUND' })
        ...
        if (!data)
            return res.status(404).json({ error: 'BLOB_OBJECT_NOT_FOUND' })
        ...
        if (
            (result.media?.kind === 'audio' || result.media?.kind === 'video')
            && range
        ) {
...
router.get(
    '/:assetId/documents/:role/snapshot',
    authenticateRequest,
    async (req: any, res: any) => {
        ...
        if ('error' in asset)
            return res.status(asset.error === 'NOT_FOUND' ? 404 : 403).json(asset)

        const snapshot = await AssetDocumentService.loadSnapshot(asset, role)

        if (!snapshot)
            return res.status(404).json({ error: 'DOCUMENT_SNAPSHOT_NOT_FOUND' })
