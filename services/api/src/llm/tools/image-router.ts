...
export class ImageRouter {
    ...
    async execute(
        ...
        const mediaModelId = imageProvider
            && imageModel
        ...
        if (
            !imageProvider
            || !imageModel
            || (!prompt && !state.capabilityMediaExecutionPlan)
        ) {
            err(
        ...
        const recordRunStatus = async (
            ...
            const problem = error === undefined ? undefined : this.registry.getDefinition(imageProvider).normalizeProblem(
                error,
                {
        ...
        if (state.capabilityMediaExecutionPlan) {
            if (!this.capabilityMediaStrategies)
                throw new Error('CAPABILITY_MEDIA_STRATEGY_REGISTRY_REQUIRED')

            const strategy = this.capabilityMediaStrategies.get(state.capabilityMediaExecutionPlan)

            try {
                await recordRunStatus('running')
                const context = buildCapabilityMediaExecutionContext(
                    state,
                    generationRun,
                    prompt,
                )
                const imagePublisher = this.natsService
                    && generationRun
                    ? new ImagePublisher(
                        this.natsService,
        ...
        if (referenceImages.length === 0)
            warn(
                `[ImageRouter] No reference images attached for ${instanceKey}. If you expected the model to see workspace reference images, check the upstream extractReferenceImages() / messages payload.`,
            )
        ...
        try {
            ...
            return {
                ...
                imageUsage: generatedImages.length > 0
                    ? {
                        generatedCount,
                        size: imageSize,
                        quality: state.imageGenerationConfig?.quality ?? 'auto',
                    }
                    : undefined,
            }
        } catch (e: any) {
            ...
            const problem = await recordRunStatus('failed', e).catch(persistenceError => {
                ...
                return state.durableGenerationRequestId
                    ? this.registry.getDefinition(imageProvider).normalizeProblem(
                        e,
                        {
                            generationRequestId: state.durableGenerationRequestId,
                            modelId: generationRun?.mediaModelId,
...
const buildCapabilityMediaExecutionContext = (
    ...
    if (
        ...
        throw new Error('CAPABILITY_MEDIA_EXECUTION_IDENTITY_REQUIRED')
    ...
    if (!plan)
        throw new Error('CAPABILITY_MEDIA_EXECUTION_PLAN_REQUIRED')
    ...
    info(
        `[ImageRouter] capability media prompt authority ${
            JSON.stringify({
    ...
    return {
        ...
        sharedState: {
            ...
            capabilityInstructions: state.capabilityUsagePrompt?.trim()
                ...
                : [],
            capabilityReferences: (state.capabilityReferenceImages ?? []).map(
                (imageUrl, index) => ({
                    imageUrl,
            ...
            capabilityOutputs: (state.capabilityToolResults ?? []).map(
                result => ({
...
const resolveCapabilityEditTargetAssetId = (state: ProviderState): string | undefined => {
    ...
    if (resolution?.operationKind !== 'edit_existing')
        return undefined
...
const resolveCapabilityAuthoritativePrompt = (
    ...
    if (providerSafeUserPrompt)
        return {
            prompt: providerSafeUserPrompt,
            source: 'provider-safe-user-prompt',
        }
    ...
    return {
        prompt: generatedMediaPrompt.trim(),
        source: 'generated-media-prompt',
    }
}
