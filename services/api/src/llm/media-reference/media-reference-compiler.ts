...
const buildSemanticDescriptor = (asset: Asset): string => {
    ...
    if (!descriptor)
        return `${asset.depictionMedium} ${asset.media?.kind ?? 'media'} reference`

    return [descriptor.summary, ...descriptor.entityTags, ...descriptor.styleTags].map(value => value.trim()).filter(Boolean)
        .join('; ')
}
...
export const createMediaReferenceBindings = ({
    ...
    if (uniqueAssets.length > MEDIA_REFERENCE_MAX_BINDINGS)
...
export const segmentMediaPrompt = (node: ProseMirrorJsonNode): MediaPromptSegment[] => {
    ...
    const visit = (candidate: ProseMirrorJsonNode): void => {
        if (candidate.type === 'text')
            return appendText(candidate.text ?? '')

        if (candidate.type === 'hard_break')
            return appendText('\n')

        if (
            candidate.type === PROMPT_REFERENCE_NODE_TYPE
...
const replaceFreeFormMatches = ({
    ...
    const variants = bindings
        .flatMap(getMediaReferenceBindingVariants)
    ...
    for (let startIndex = 0; startIndex < tokenMatches.length; startIndex++) {
        for (let wordCount = Math.min(maximumWords, tokenMatches.length - startIndex); wordCount >= 1; wordCount--) {
            ...
            if (replacements.some(replacement => start < replacement.to && end > replacement.from))
                continue

            const phrase = text.slice(start, end)

            if (!isIdentifyingMediaReferencePhrase(phrase))
                continue
            ...
            const persistedResolution = resolvedReferences.find(
                resolution => normalizeMediaReferenceVariant(resolution.originalText) === normalizeMediaReferenceVariant(phrase),
            )
            const persistedResolutionIsCurrentCandidate = persistedResolution
                ...
                || (match.kind === 'ambiguous' && match.unresolved?.candidates.some(candidate => candidate.assetId === persistedResolution.assetId))
            )

            if (
                persistedResolution
                && persistedResolutionIsCurrentCandidate
            ) {
                const binding = bindings.find(candidate => candidate.assetId === persistedResolution.assetId)

                if (!binding)
                    throw new Error(`MEDIA_REFERENCE_RESOLUTION_ASSET_NOT_BOUND:${persistedResolution.assetId}`)
...
export const sanitizeMediaReferenceText = (
    text: string,
    bindings: MediaReferenceBinding[],
): string => {
    let safe = text
    ...
    for (const binding of bindings) {
        ...
        for (const variant of variants) {
            safe = safe.replace(
                new RegExp(`\\b${escapeRegularExpression(variant)}\\b`, 'giu'),
                binding.alias,
            )
        }
    }
...
export const compileMediaReferenceIntent = ({
    ...
    intent: ProviderSafeMediaIntent
    ...
    const safePrompt = segments.map(segment => {
        if (segment.kind === 'reference') {
            ...
            if (!binding)
                throw new Error(`MEDIA_REFERENCE_NOT_AUTHORIZED:${segment.assetId}`)
        ...
        const compiled = replaceFreeFormMatches({
            text: segment.text,
            offset: segment.from,
            bindings,
            resolvedReferences,
