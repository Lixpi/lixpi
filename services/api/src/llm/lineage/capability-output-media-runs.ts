...
export const resolveDurableMediaRuns = (runs: readonly MediaGenerationRun[] | undefined): PreassignedCapabilityMediaRun[] | undefined => {
    if (!runs?.length)
        return undefined
...
export const resolveCapabilityOutputMediaRuns = async (assetIds: readonly string[]): Promise<PreassignedCapabilityMediaRun[]> => {
    return await Promise.all(
        assetIds.map(async (assetId, index) => {
            const asset = await getAssetRecord(assetId)

            if (!asset)
                throw new Error(`Capability output Asset ${assetId} was not found`)
            ...
            if (
                !lineage?.reasoningRunId
                || !lineage.reasoningModelId
                || !lineage.mediaRunId
                || !lineage.mediaModelId
            )
