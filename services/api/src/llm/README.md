...
For a plain scalar request with both default media selectors configured, API request routing selects video only for an explicit video/clip/animation/filming request or an authorized source-video continuation; otherwise it selects image. Scene actions that fit in a still do not change modality. The durable request, pending Asset, lineage assignment, and provider tools therefore contain only that selected scalar modality. A Google reasoning run that skips tool selection for an explicit video-generation request is retried with `generate_video` as the only allowed function; a second miss fails the run explicitly instead of publishing a zero-assignment lineage plan. Character Creator is image-only: request routing selects the Tool before provider execution, retains the selected reasoning/image axes, and removes every video model and video option before matrix normalization or scalar provider setup. Explicit matrix requests still enumerate every requested model axis allowed by the selected Tool.
...
- Usage sends normalized units and a route-aware `pricingLookup`; the metering backend resolves provider cost and customer resale policy from its pinned pricing snapshot.
- The pre-call spend gate names one pricing key and one modality, and they always describe the same thing. `usage/usage-estimator.ts` derives the modality from what the run's own model is, so a reasoning run gates as `tokens` even with image or video generation enabled; the media calls it triggers are separate paid calls through transient media providers, each admitted against its own media model.
- That gate sends an upper-bound, dimensioned estimate: prompt plus reserved completion tokens, one image with its size/quality selectors, or video seconds/tokens with its resolution selector. Missing route references or selectors fail closed; Lixpi never sends provider or resale money.
...
- `usage/usage-log.ts` owns the shape of both metering log lines, `[Metrics] usage check` and `[Metrics] usage confirm`, so a run's estimate and measured usage read as a pair. The check line shows how the estimate decomposed and flags any safety factor or placeholder that went into it.
