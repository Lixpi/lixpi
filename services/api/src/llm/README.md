...
- Mirrors Anthropic streamed tool-input `prompt` deltas into the same `<image_prompt>` / `<video_prompt>` collapsible path used by text-tag prompts, so the ProseMirror transcript can update while the tool call is still being generated instead of waiting for `finalMessage()`.
