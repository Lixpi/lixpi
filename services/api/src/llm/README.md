...
`CapabilityMediaDagRunner` executes the 26 base panels and conditional prop panel with provider-scoped concurrency. Each panel receives one semantic candidate and at most one targeted correction. Accepted dependency panels become canonical or adjacent-angle anchors. Structured VLM assessment always runs; photographic face-bearing panels also request the NEX YuNet/SFace workload. Transport retries are bounded separately from the 52–54 semantic ceiling. Required failures terminate the concrete matrix run, while an optional prop failure leaves a traced blank cell.

The Sharp compositor reads the owned SVG layout resource, normalizes accepted panels, derives eye, mouth, and feet crops, adds deterministic labels, swatches, design notes, and source-coverage disclosure, and verifies a 3840x2560 PNG. Only that PNG returns through the existing image publisher and preassigned-Asset settlement path. A cleanup ledger removes every transient source crop and candidate after success, error, or cancellation.
