...
export const assertMessageInputKindsSupported = (
    ...
    for (const message of messages) {
        ...
        for (const block of message.content) {
            ...
            if (
                inputKind
                && !capabilities.supportedInputKinds.includes(inputKind)
            )
...
export const parseNatsObjectRef = (ref: string): {
    bucket: string
    key: string
} | undefined => {
    if (!ref.startsWith('nats-obj://'))
        return undefined
...
const downscaleImageIfNeeded = async (
    ...
    if (hasAlpha) {
        ...
        for (const quality of [92, 85, 78, 70, 60]) {
            const out = await working.clone().jpeg({
                quality,
                mozjpeg: true,
            }).toBuffer()
...
const normalizeDataUrlBlock = async (block: Record<string, any>): Promise<Record<string, any>> => {
    ...
    const {
        data: outData,
        mimeType: outMime,
    } = await normalizeAndDownscale(raw, mediaType)
    ...
    return {
        ...block,
        image_url: `data:${outMime};base64,${newB64}`,
    }
}
    ...
    for (const block of content) {
        ...
        if (
            ...
            const {
                data: outData,
                mimeType: outMime,
            } = await normalizeAndDownscale(buf, detected)
            const b64 = outData.toString('base64')
            resolved.push({
                ...block,
                image_url: `data:${outMime};base64,${b64}`,
            })
...
const convertContentBlockToAnthropic = (block: Record<string, any>): Record<string, any> | null => {
    ...
    if (blockType === 'input_image')
        return convertImageBlockToAnthropic(block)

    if (blockType === 'input_audio')
        throw new Error('ATTACHMENT_INPUT_KIND_UNSUPPORTED:ANTHROPIC:audio')
...
const convertContentForAnthropic = (content: string | Array<Record<string, any>>): string | Array<Record<string, any>> => {
    if (typeof content === 'string')
        return content
    ...
    for (const block of content) {
        if (
            ...
            throw new Error('ATTACHMENT_BLOCK_INVALID:ANTHROPIC')

        const c = convertContentBlockToAnthropic(block)

        if (!c)
            throw new Error('ATTACHMENT_BLOCK_CONVERSION_FAILED:ANTHROPIC')
...
const convertContentForOpenAI = (content: string | Array<Record<string, any>>): string | Array<Record<string, any>> => {
    if (typeof content === 'string')
        return content
    ...
    for (const block of content) {
        if (
            ...
            throw new Error('ATTACHMENT_BLOCK_INVALID:OPENAI')
...
const convertContentForGoogle = (content: string | Array<Record<string, any>>): string | Array<Record<string, any>> => {
    if (typeof content === 'string')
        return content
    ...
    for (const block of content) {
        if (
            ...
            throw new Error('ATTACHMENT_BLOCK_INVALID:GOOGLE')
...
export const convertAttachmentsForProvider = (
    ...
    if (targetFormat === 'ANTHROPIC')
        return convertContentForAnthropic(content)

    if (targetFormat === 'OPENAI')
        return convertContentForOpenAI(content)

    if (targetFormat === 'GOOGLE')
        return convertContentForGoogle(content)
