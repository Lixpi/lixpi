...
const readModerationMetadata = (error: unknown): {
    ...
    const nestedError = candidate.error
        ...
        : {}
    const details = [candidate.moderation_details, nestedError.moderation_details].find(value => value && typeof value === 'object') as Record<string, unknown> | undefined
    const rawStage = details?.moderation_stage ?? details?.stage
    const stage = rawStage === 'input'
        || rawStage === 'output'
        || rawStage === 'unknown'
        ? rawStage
        : undefined
    const rawCategories = details?.categories
        ?? candidate.safety_violations
        ?? nestedError.safety_violations
    const explicitCategories = Array.isArray(rawCategories)
        ...
        : rawCategories
            ...
            ? Object
                .entries(rawCategories as Record<string, unknown>)
...
export const normalizeProviderProblem = ({
    ...
    const capabilityFailureCode = /\b((?:CHARACTER|CAPABILITY)_[A-Z0-9_]+)\b/u.exec(typeof candidate?.message === 'string' ? candidate.message : '')?.[1]
...
export const assertValidMediaProviderDefinition = (definition: MediaProviderDefinition): void => {
    if (
        !definition.provider
        || typeof definition.constructor !== 'function'
    )
        throw new Error('MEDIA_PROVIDER_CONSTRUCTOR_REQUIRED')

    if (
        definition.mediaCapabilities.length === 0
        && definition.provider !== 'Anthropic'
    )
        throw new Error(`MEDIA_PROVIDER_CAPABILITY_REQUIRED:${definition.provider}`)

    if (
        definition.mediaCapabilities.includes('image')
        && !definition.imageReferenceAdapter
    )
    ...
    if (
        !definition.retentionNotes
        || !definition.sensitiveDataNotes
        || definition.documentationUrls.length === 0
        || definition.documentationUrls.some(url => !url.startsWith('https://'))
        || !/^\d{4}-\d{2}-\d{2}$/u.test(definition.reviewedAt)
    ...
    if (definition.provider === 'OpenAI') {
        ...
        if (settings.moderation !== 'low')
            throw new Error('OPENAI_LOW_MODERATION_PROFILE_REQUIRED')
    }

    if (definition.provider === 'Google') {
        const textSettings = definition.moderation.settings(
            'veo-provider-profile-audit',
            'text',
            { regionProfile: 'standard' },
        )
        const imageSettings = definition.moderation.settings(
            'veo-provider-profile-audit',
            'image-conditioned',
            { regionProfile: 'standard' },
        )
        ...
        if (
            textSettings.personGeneration !== 'allow_all'
            || imageSettings.personGeneration !== 'allow_adult'
        )
    ...
    if (
        ...
        && (definition.verification.strategy !== 'provider-hosted-session' || definition.verification.derivativeReuse !== 'documented-lineage')
    )
