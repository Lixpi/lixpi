...
export class BytePlusProvider extends BaseProvider {
    ...
    private async runSeedanceGeneration(state: ProviderState): Promise<VideoUsage & { responseId?: string }> {
        ...
        if (!prompt)
            throw new Error('Seedance: missing prompt in user message')
        ...
        const verifiedAssetUris = (state.providerSafeMediaIntent?.bindings ?? []).flatMap(binding => {
            ...
            const verification = binding.subjectIdentity.providerVerifications.find(
                candidate => (
            ...
            if (!verification)
                throw new Error(`BYTEPLUS_PROVIDER_ASSET_HANDLE_REQUIRED:${binding.assetId}`)

            return [`asset://${verification.subjectHandle}`]
        })
        const existingFrames = [state.videoFirstFrameImage, ...(state.videoReferenceImages ?? [])].filter((value): value is string => Boolean(value))
        ...
        info(
            `[BytePlus:${this.instanceKey}] Seedance submit ${
                JSON.stringify(
                    {
                        model: modelVersion,
                        ratio: payload.ratio,
        ...
        try {
            const created = await this.retryTransport(
                'video-create',
                async () => await createVideoGenerationTask(
                    this.clientConfig,
                    payload,
                    this.signal,
                ),
            ...
            if (!taskId)
                throw new BytePlusModelArkError('ModelArk did not return a task id')

            await this.videoPub.pending()

            const task = await pollVideoGenerationTask(
                this.clientConfig,
                taskId,
                {
                    ...
                    retrieve: async (
                        ...
                        await this.retryTransport(
                            'video-poll',
                            async () => await retrieveVideoGenerationTask(
                                config,
                                id,
                                signal,
                            ),
            ...
            if (task.status !== 'succeeded') {
                ...
                throw new BytePlusModelArkError(`Seedance task ${taskId} ${task.status}: ${detail}`, { code: task.error?.code })
            }

            const videoUrl = task.content?.video_url

            if (!videoUrl)
                throw new BytePlusModelArkError('Seedance task succeeded but returned no video_url')

            // Output URLs are cleaned after 24h — download immediately.
            const videoBuffer = await this.retryTransport('video-download', async () => await downloadVideo(videoUrl, this.signal))
            ...
            if (payload.return_last_frame) {
                ...
                if (lastFrameUrl) {
                    try {
                        frameBuffer = await this.retryTransport('last-frame-download', async () => await downloadLastFrame(lastFrameUrl, this.signal))
                    } catch (error) {
            ...
            const containerFormat = task.output_format === 'mp4'
                || task.output_format === 'mov'
                ? task.output_format
            ...
            info(
                `[BytePlus:${this.instanceKey}] Seedance complete ${
                    JSON.stringify(
                        {
                            taskId,
                            durationSeconds,
