...
vi.mock('@clack/prompts', () => ({
    ...
    log: { info: vi.fn(), step: vi.fn(), success: vi.fn(), warn: vi.fn() },
}))
...
describe('shared grouped wizard', () => {
    ...
    const fixture = [
        ...
        'NATS_REGULAR_USER_PASSWORD=keep-regular-password',
    ...
    beforeEach(() => {
        ...
        vi.spyOn(fs, 'readFileSync').mockImplementation((file, options) => {
            if (file === awsPath) return awsOriginal as never
            if (String(file).startsWith('/workspace/.env.')) return original as never
        ...
        vi.mocked(prompts.select).mockImplementation(async options => {
            if (selects.has(options.message)) return selects.get(options.message) as never
            if (options.message === 'Generate a new configuration or edit an existing one?') return mode
            if (options.message === 'Select a configuration to edit') return selectedFile
            if (options.message.startsWith('How do you want to update ')) return updateMode
        ...
        vi.mocked(prompts.confirm).mockImplementation(async options => {
            if (confirms.has(options.message)) return confirms.get(options.message) as never
            if (options.message.startsWith('Edit ')) return groups.has(options.message.slice(5, -1))
            return options.initialValue ?? false
        })
        vi.mocked(prompts.text).mockImplementation(async options => {
            if (texts.has(options.message)) return texts.get(options.message) as never
            if (options.message.startsWith('Developer name')) return 'test'
    ...
    it('offers new or edit first and lists existing files before group questions', async () => {
        ...
        expect(vi.mocked(prompts.select).mock.calls[1][0].options).toEqual([
            ...
            { value: '.env.test-local', label: '.env.test-local' },
        ])
    ...
    it('only asks for a missing active field inside a selected group', async () => {
        ...
        expect(prompts.select).toHaveBeenCalledWith(expect.objectContaining({
            ...
            options: [{ value: 'override', label: 'Set new value' }],
        }))
    ...
    it('can keep an empty applicable field without exposing unrelated raw variables', async () => {
        ...
        expect(prompts.select).toHaveBeenCalledWith(expect.objectContaining({
            ...
            options: [{ value: 'keep', label: 'Keep empty' }, { value: 'override', label: 'Override value' }],
        }))
    ...
    it('uses the same subgroup branches when creating a fresh configuration', async () => {
        ...
        expect(nkeys.createUser).toHaveBeenCalledTimes(3)
    })
