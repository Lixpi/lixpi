...
const table = (
    ...
    name: getDynamoDbTableStageName(
        resource,
        ORG_NAME,
        STAGE,
    ),
...
const getLegacyStorageDefinitions = (): Record<string, TableDefinition> => ({
    ...
    legacyMediaLibraryItemsAccessListTable: {
        name: formatStageResourceName(
            'Media-Library-Items-Access-List',
            ORG_NAME,
            STAGE,
        ),
        attributes: [
...
export const createDynamoDbTables = async (opts?: { provider?: aws.Provider }) => {
    ...
    const legacyTables = !opts?.provider
        && legacyStorageRemovalStage !== 'remove'
        ? Object.fromEntries(
            Object.entries(
                getLegacyStorageDefinitions(),
            ).map(
