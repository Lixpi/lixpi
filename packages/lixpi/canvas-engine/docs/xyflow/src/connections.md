...
Engine ConnectionManager uses its own cancellable handle and menu gestures over XYFlow node/port geometry. Explicit ports supply bounds; cancellation must not become an empty-space reconnect drop. See [engine connectors](../../EDGES-AND-CONNECTIONS.md).
