...
**Transform callback frequency** — `onPanZoom` fires on every animation frame during active pan/zoom. Any DOM work in this callback must be cheap. The engine defers expensive operations (edge re-rendering, menu repositioning) to `requestAnimationFrame` to avoid layout thrashing.
...
ViewportController owns the XYPanZoom instance and explicit input-lock leases. ViewportBridge synchronizes supplied DOM roots and rendering targets; persistence acknowledgments do not drive the live transform. See [rendering lifecycle](../../RENDERING-ENGINE.md).
