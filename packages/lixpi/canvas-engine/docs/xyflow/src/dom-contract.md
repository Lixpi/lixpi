...
The React/Svelte wrapper styles use a container structure with these classes and layers. A custom system consumer supplies its own scene structure:
...
A wrapper container needs an explicit size. The wrapper's measurement path can report error004 when size is missing. The canvas engine separately requires a positioned, sized root.
...
The upstream style family includes:
...
The wrappers check their stylesheet contract and can report error013. Engine consumers import `@lixpi/canvas-engine/styles/interaction`, not wrapper skins.
