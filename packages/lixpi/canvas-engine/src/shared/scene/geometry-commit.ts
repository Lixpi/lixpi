...
export const computeGeometryChanges = (
    nodes: readonly EngineNode[],
    proposed: ReadonlyMap<string, CanvasEngineRect>,
    options: GeometryCommitOptions,
): NodeGeometryChange[] => {
    ...
    for (const node of topoSortByParent(nodes)) {
        ...
        const next = proposed.get(node.nodeId) ?? {
            ...bounds,
            x: bounds.x + (nextParent
                && originalParent
                ? nextParent.x - originalParent.x
                : 0),
            y: bounds.y + (nextParent
                && originalParent
                ? nextParent.y - originalParent.y
                : 0),
        }
    ...
    if (options.collisions) {
        ...
        for (const node of topoSortByParent(nodes)) {
            ...
            const key = node.parentId
                ? keys.get(node.parentId)!
                : policy?.collisionGroup
                    ? `group:${policy.collisionGroup}`
                    : `node:${node.nodeId}`
            keys.set(node.nodeId, key)
            const bounds = world.get(node.nodeId)!
            const measurement = policy?.measure({
                ...node,
                parentId: undefined,
                position: {
                    x: bounds.x,
        ...
        for (const box of boxes) {
            ...
            for (const id of groups.get(box.id)!.nodes) {
                ...
                world.set(
                    ...
                    {
                        ...bounds,
                        x: bounds.x + moved.x - box.x,
                        y: bounds.y + moved.y - box.y,
                    },
                )
            }
        }
    ...
    for (const node of nodes) {
        ...
        if (
            ...
            && bounds.width === node.dimensions.width
            && bounds.height === node.dimensions.height
        )
            continue
