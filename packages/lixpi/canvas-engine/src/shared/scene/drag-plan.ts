...
export const computeDragPlan = <Node extends CanvasPositionedNode>(input: DragPlanInput<Node>): {
    ...
    while (pending.length) {
        ...
        if (!input.isContainer?.(node))
            continue

        for (const child of input.nodes) {
            if (
                child.parentId !== id
                || selected.has(child.nodeId)
                || input.canDrag?.(child) === false
            )
                continue
    ...
    return {
        resolvedNodeId: input.primaryNodeId,
        draggedNodeIds: Array.from(selected),
