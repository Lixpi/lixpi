...
export const assertCanvasBounds = (
    ...
    if (
        ...
        throw new CanvasDiagnosticError({
            code: 'invalid-geometry',
            nodeId,
            message: `Canvas bounds must be finite with nonnegative dimensions${nodeId ? `: ${nodeId}` : ''}`,
        })
}

export const validateScene = (scene: SceneSnapshot): void => {
    const nodes = new Map(
    ...
    for (const node of scene.nodes) {
        if (
            ...
            throw new CanvasDiagnosticError({
                code: 'duplicate-node',
                nodeId: node.nodeId,
                message: 'Scene node IDs must be nonempty and unique',
            })
        ...
        if (
            ...
            throw new CanvasDiagnosticError({
                code: 'invalid-parent',
                nodeId: node.nodeId,
                message: `Missing parent ${node.parentId} for node ${node.nodeId}`,
            })
        ...
        for (const port of node.ports) {
            if (
                ...
                throw new CanvasDiagnosticError({
                    code: 'invalid-port',
                    nodeId: node.nodeId,
                    message: `Invalid or duplicate port on node ${node.nodeId}`,
                })
        ...
        while (!resolved.has(current.nodeId)) {
            if (ancestors.has(current.nodeId))
                throw new CanvasDiagnosticError({
                    code: 'cyclic-parent',
                    nodeId: node.nodeId,
                    message: `Cyclic parenting at node ${current.nodeId}`,
                })
    ...
    for (const edge of scene.edges) {
        ...
        if (
            ...
            throw new CanvasDiagnosticError({
                code: 'invalid-edge',
                edgeId: edge.edgeId,
                message: `Invalid edge identity or endpoints: ${edge.edgeId}`,
            })
