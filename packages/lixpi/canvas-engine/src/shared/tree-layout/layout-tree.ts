...
export const layoutTree = (
    ...
    const depthGapForChildren = (
        node: TreeLayoutNode,
        children: TreeLayoutNode[],
    ): number => {
        const configuredFanoutGap = options.getBranchFanoutDepthGap?.(node, children)
        const fanoutDepthGap = configuredFanoutGap !== undefined
            && Number.isFinite(configuredFanoutGap)
    ...
    for (const node of nodes) {
        ...
        if (
            nodeX === undefined
            || nodeCenterY === undefined
        )
            continue // unreachable orphan
