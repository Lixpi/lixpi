...
export type TextureInput =
    & { mipmaps?: boolean }
    & (
        | { kind: 'pixels'; size: CanvasEngineSize; rgba: Uint8Array | Uint8ClampedArray }
        | { kind: 'image'; source: ImageBitmap | HTMLCanvasElement | OffscreenCanvas }
    )
...
export type MaterialBinding =
    | { kind: 'uniform'; name: string; binding: number; type: 'f32' | 'vec2f' | 'vec3f' | 'vec4f' | 'mat3f' | 'mat4f'; value: number | Float32Array }
    | { kind: 'texture'; name: string; binding: number; samplerBinding: number; texture: ResourceHandle<'texture'>; sampling: 'nearest' | 'linear' }

export type MaterialProgram = {
    abi: 'canvas-material-v1'
    webgl: { vertex: string; fragment: string }
    webgpu: { vertex: string; fragment: string }
    bindings: readonly MaterialBinding[]
}

export type PathPaint = {
    fill?: { color: string; alpha?: number }
    stroke?: { color: string; alpha?: number; width: number; cap?: 'butt' | 'round' | 'square'; join?: 'miter' | 'round' | 'bevel'; dash?: readonly number[] }
}

export type VectorShape = PathPaint & {
    path: string
    holes?: readonly string[]
    projection?: { x: number; y: number; zoom: number; snapResolution?: number }
}
