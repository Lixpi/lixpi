...
export type RectangleOverlayOptions = {
...
export class RectangleOverlay {
    ...
    setZoom(zoom: number): void {
        ...
        if (
            ...
            throw new RangeError('Overlay zoom must be finite and positive')
    ...
    private paint(): void {
        ...
        const visible = this.bounds !== null
            && Object.values(this.bounds).every(Number.isFinite)
            && this.bounds.width > 0
        ...
        if (visible)
            resources.updatePath(
                this.path,
                [{
                    path: roundedRectanglePath(this.bounds!, radius / this.zoom),
                    fill: this.filled ? { color: fill } : undefined,
                    stroke: {
                        color: stroke,
