...
export class PixiDrawingResources implements DrawingResources {
    ...
    setViewport(viewport: CanvasViewport): void {
        if (
            ...
            throw new RangeError('Viewport coordinates must be finite and zoom must be positive')
    ...
    createGroup(options: {
        ...
        if (options.layer.kind === 'layer')
            parent = this.registry.get<Layer>(options.layer, 'layer')[options.space]
        else {
            const group = this.registry.get<Group>(options.layer, 'group')

            if (group.space !== options.space)
                throw new Error('Nested drawing groups must use the same coordinate space')
    ...
    updateGroup(
        ...
        if (values.some(value => value !== undefined && !Number.isFinite(value)))
            throw new RangeError('Group transforms must be finite')
        ...
        if (transform.order !== undefined)
            container.zIndex = transform.order
    ...
    private display(handle: ResourceHandle<'group' | 'mesh' | 'path'>): Container {
        if (handle.kind === 'group')
            return this.registry.get<Group>(handle, 'group').container

        if (handle.kind === 'mesh')
            return this.registry.get<PixiMeshResource>(handle, 'mesh').container

        return this.registry.get<Graphics>(handle, 'path')
    }

    setVisible(
        handle: ResourceHandle<'group' | 'mesh' | 'path'>,
        visible: boolean,
    ): void {
        const container = this.display(handle)

        if (container.renderable === visible)
            return
        ...
        try {
            return this.registry.add(
                'texture',
                {
                    ...
                    update: (value: TextureInput) => resource.update(value),
                },
                () => {
                    resource.destroy()
                    releaseInput()
    ...
    updateTexture(
        ...
        if (!resource.update)
            throw new Error('Capture textures are read-only')
    ...
    createMesh(
        ...
        const mesh = new PixiMeshResource(
            data,
            this.paint(paint),
            this.retire,
        )
        ...
        const handle = this.registry.add(
            ...
            {
                parent: group,
                dependencies: paint ? [paint] : [],
            },
        )
        this.meshes.set(handle, mesh)
        this.meshPaint.set(handle, paint)
        this.samples.set(mesh.container, paint?.kind === 'texture'
            ? [paint]
            : paint
                ? this.materialTextures.get(paint)!
                : [])
    ...
    updatePath(
        ...
        for (const shape of shapes) {
            ...
            if (shape.stroke) {
                ...
                graphics
                    .beginPath()
                    .path(dash ? dashVectorPath(path, dash) : path)
                    .stroke(stroke)
            }
        }
    ...
    capture(input: CaptureSpec): CaptureResource {
        ...
        const handle = this.registry.add(
            'capture',
            record,
            () => void this.captures.delete(handle),
        )
        const textureHandle = this.registry.add(
            'texture',
            { texture },
            () => {
    ...
    updateCapture(
        ...
        if (
            capture.texture.width !== capture.spec.bounds.width
            || capture.texture.height !== capture.spec.bounds.height
            || capture.texture.source.resolution !== (capture.spec.resolution ?? 1)
        )
            capture.texture.resize(
                capture.spec.bounds.width,
                capture.spec.bounds.height,
                capture.spec.resolution ?? 1,
            )
    ...
    private captureSpec(input: CaptureSpec): CaptureSpec {
        ...
        if (
            ![width, height, x, y, input.resolution ?? 1].every(Number.isFinite)
            || width <= 0
            || height <= 0
        ...
        for (const bounds of input.sampleBounds ?? []) {
            if (
                ...
                throw new RangeError('Capture sample bounds must be finite and positive')
        }

        return {
            ...input,
            bounds: { ...input.bounds },
            sampleBounds: input.sampleBounds?.map(bounds => ({ ...bounds })),
    ...
    private screenSampleBounds(capture: Capture): readonly CanvasEngineRect[] {
        ...
        if (capture.spec.space === 'screen')
            return regions

        return regions.map(
            bounds => ({
                x: bounds.x * this.viewport.zoom + this.viewport.x,
                y: bounds.y * this.viewport.zoom + this.viewport.y,
                width: bounds.width * this.viewport.zoom,
                height: bounds.height * this.viewport.zoom,
            }),
        )
    }

    private overlaps(
        first: CanvasEngineRect,
        second: CanvasEngineRect,
    ): boolean {
    ...
    invalidateCaptures(bounds?: CanvasEngineRect): void {
        for (const capture of this.captures.values()) {
            if (
                !bounds
                || this.screenSampleBounds(capture).some(region => this.overlaps(bounds, region))
            )
                capture.dirty = true
        }
    }
    ...
    private captureChanged(capture: Capture): boolean {
        ...
        for (const [container, change] of this.changes) {
            if (!regions.some(bounds => this.overlaps(change.bounds, bounds)))
                continue

            if (!includes.some(parent => change.ancestors.has(parent) || this.contains(container, parent)))
                continue

            if (excludes.some(parent => change.ancestors.has(parent)))
                continue
    ...
    renderCaptures(renderer: Renderer): void {
        ...
        const byTexture = new Map(
            Array.from(
                ...
                capture => [capture.handle, capture],
            ),
        )
        const visit = (capture: Capture) => {
            if (visiting.has(capture))
                throw new Error('Cyclic canvas capture dependency')

            if (dependencies.has(capture))
                return
            ...
            for (const [container, textures] of this.samples) {
                if (!this.samplesCapture(capture, container))
                    continue
        ...
        for (const capture of ordered) {
            if (capture.spec.enabled === false)
                continue

            if (
                !capture.dirty
                && !this.captureChanged(capture)
                && !Array.from(dependencies.get(capture)!).some(input => refreshed.has(input))
            )
                continue
    ...
    private renderCapture(
        ...
        const visit = (node: Container) => {
            ...
            if (
                !included
                || excludes.some(parent => this.contains(parent, node))
            )
    ...
    release(handle: ResourceHandle): void {
        if (handle.kind === 'layer')
            throw new Error('Canvas layers are borrowed resources')

        if (
            handle.kind === 'texture'
            && this.captureTextures.has(handle as ResourceHandle<'texture'>)
        )
            throw new Error('Capture textures are borrowed resources')

        if (handle.kind === 'capture')
            this.captures.delete(handle as ResourceHandle<'capture'>)
        ...
        if (
            ...
            for (const [group, mask] of this.masks) {
                if (
                    ...
                    this.setMask(group, null)
            }

            for (const capture of this.captures.values()) {
                const retained = (item: CanvasLayer | ResourceHandle<'group'>) => item.kind === 'layer' || !this.contains(
                    removed,
                    this.display(item),
                )
                ...
                if (
                    include.length !== capture.spec.include.length
                    || exclude.length !== capture.spec.exclude.length
                ) {
                    capture.spec = {
                        ...capture.spec,
                        include,
