...
export const validateMesh = (data: MeshData): void => {
    if (
        data.positions.length % 2 !== 0
        || data.uvs.length !== data.positions.length
    )
        throw new RangeError('Mesh positions and UVs must contain matching pairs')

    if (data.indices.length % 3 !== 0)
        throw new RangeError('Mesh indices must describe triangles')
...
export const validateTexture = (input: TextureInput): void => {
    ...
    if (
        input.kind === 'pixels'
        && input.rgba.length !== size.width * size.height * 4
    )
        throw new RangeError('Texture pixels must contain four channels per pixel')
}

export const validateMaterialBindings = (bindings: readonly MaterialBinding[]): void => {
    ...
    const reserveName = (name: string) => {
        if (
            ...
            throw new Error(`Invalid or reserved material name ${name}`)
    ...
    const reserve = (slot: number) => {
        if (
            ...
            throw new RangeError(`Invalid or duplicate material binding ${slot}`)
    ...
    for (const binding of bindings) {
        ...
        if (binding.kind === 'texture') {
            ...
            if (
                binding.sampling !== 'linear'
                && binding.sampling !== 'nearest'
            )
                throw new Error('Unsupported material sampling')
        ...
        if (
            !(binding.type in uniformLengths)
            || values.length !== uniformLengths[binding.type]
        )
...
export const validateMaterial = (program: MaterialProgram): void => {
    if (program.abi !== 'canvas-material-v1')
        throw new Error('Unsupported canvas material ABI')

    if (
        !program.webgl.vertex.trim()
        || !program.webgl.fragment.trim()
        || !program.webgpu.vertex.trim()
        || !program.webgpu.fragment.trim()
    )
