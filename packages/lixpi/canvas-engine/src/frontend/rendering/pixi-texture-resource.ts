...
function copyPixels(input: Uint8Array | Uint8ClampedArray, output: Uint8Array = new Uint8Array(input.length)): Uint8Array {
...
export class PixiTextureResource {
    ...
    constructor(input: TextureInput) {
        ...
        const source = input.kind === 'pixels'
            ? new BufferImageSource({ width: input.size.width, height: input.size.height, resource: copyPixels(input.rgba), format: 'rgba8unorm', alphaMode: 'premultiplied-alpha' })
            : new ImageSource({ resource: input.source, width: input.source.width, height: input.source.height })
    ...
    update(input: TextureInput): void {
        if (this.destroyed) throw new Error('Texture is disposed')
        validateTexture(input)
        if (input.kind !== this.kind) throw new Error('Texture updates must preserve the source kind')
        const source = this.texture.source
        if (input.mipmaps !== undefined) source.autoGenerateMipmaps = input.mipmaps
