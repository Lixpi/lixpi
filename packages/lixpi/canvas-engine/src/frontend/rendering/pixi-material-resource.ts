...
const uniformTypes = { f32: 'f32', vec2f: 'vec2<f32>', vec3f: 'vec3<f32>', vec4f: 'vec4<f32>', mat3f: 'mat3x3<f32>', mat4f: 'mat4x4<f32>' } as const

type MaterialInstance = { shader: Shader; transform: UniformGroup }
...
export class PixiMaterialResource {
    ...
    constructor(program: MaterialProgram, private readonly texture: (handle: ResourceHandle<'texture'>) => Texture) {
        ...
        this.gpu = new GpuProgram({
            vertex: { source: program.webgpu.vertex, entryPoint: 'mainVertex' },
            fragment: { source: program.webgpu.fragment, entryPoint: 'mainFragment' },
        })
    ...
    private validateGpuBindings(bindings: readonly MaterialBinding[]): void {
        ...
        const expected = new Map<string, { group: number; binding: number; type: string }>([['canvas_transform', { group: 0, binding: 0, type: 'mat3x3<f32>' }]])
        for (const binding of bindings) {
            expected.set(binding.name, { group: 1, binding: binding.binding, type: binding.kind === 'uniform' ? uniformTypes[binding.type] : 'texture_2d<f32>' })
            if (binding.kind === 'texture') expected.set(`${binding.name}_sampler`, { group: 1, binding: binding.samplerBinding, type: 'sampler' })
        }
        if (declared.length !== expected.size) throw new Error('Material shader bindings do not match its declared resources')
        for (const binding of declared) {
            const entry = expected.get(binding.name)
            if (!entry || entry.group !== binding.group || entry.binding !== binding.binding || entry.type !== binding.type) throw new Error(`Unexpected material shader binding ${binding.name}`)
        }
    }
    ...
    update(bindings: readonly MaterialBinding[]): void {
        ...
        for (const binding of bindings) {
            ...
            if (previous && (previous.kind !== binding.kind || (previous.kind === 'uniform' && binding.kind === 'uniform' && previous.type !== binding.type))) {
                throw new Error('Material updates cannot change binding types')
            }
            if (binding.kind === 'texture') textures.set(binding.name, this.texture(binding.texture))
        }
        for (const binding of bindings) {
            if (binding.kind === 'uniform') {
                ...
                if (group) {
                    ...
                    this.uniforms.set(binding.name, new UniformGroup({ [binding.name]: { type: uniformTypes[binding.type], value } }))
                }
            } else {
        ...
        this.bindings = bindings.map(binding => binding.kind === 'uniform' && typeof binding.value !== 'number' ? { ...binding, value: new Float32Array(binding.value) } : { ...binding })
        for (const instance of this.instances) this.apply(instance.shader)
    }

    createInstance(): MaterialInstance {
        const transform = new UniformGroup({ canvas_transform: { type: 'mat3x3<f32>', value: new Float32Array([1, 0, 0, 0, 1, 0, 0, 0, 1]) } })
        const resources: Record<string, unknown> = { canvas_transform: transform }
        for (const binding of this.bindings) {
            if (binding.kind === 'uniform') resources[binding.name] = this.uniforms.get(binding.name)!
            else {
        ...
        const shader = new Shader({ gpuProgram: this.gpu, glProgram: this.gl, resources })
        const instance = { shader, transform }
    ...
    private apply(shader: Shader): void {
        for (const binding of this.bindings) {
            if (binding.kind === 'uniform') shader.resources[binding.name] = this.uniforms.get(binding.name)!
            else {
