...
export class CanvasMedia implements EngineMedia {
    ...
    constructor(private readonly options: CanvasMediaOptions) {
        ...
        if (
            !Number.isInteger(this.maxTextures)
            || this.maxTextures < 0
            || !Number.isFinite(this.maxBytes)
            || this.maxBytes < 0
        )
            throw new RangeError('Media cache limits must be non-negative')
    ...
    scoped(signal: AbortSignal): EngineMedia {
        return {
            acquireImage: request => this.acquireImage({
                ...request,
                signal: AbortSignal.any([request.signal, signal]),
            }),
            acquirePlayback: request => this.acquirePlayback({
                ...request,
                signal: AbortSignal.any([request.signal, signal]),
            }),
        }
    ...
    async acquireImage(request: Parameters<EngineMedia['acquireImage']>[0]): Promise<ImageLease> {
        ...
        const entry = this.entries.get(key)
            ?? this.reusableImage(request.media, rendition)
            ?? this.createEntry(
                key,
                request.media,
                rendition,
        ...
        const aborted = new Promise<never>((_, reject) => void (rejectAbort = reject))
        const release = () => {
            if (released)
                return
        ...
        try {
            ...
            return {
                ...
                intrinsicSize: { ...value.intrinsicSize },
                renditionId: entry.rendition.id,
                release,
            }
        } catch (error) {
            release()
    ...
    private reusableImage(
        media: MediaDescriptor,
        desired: MediaRendition,
        ...
        return Array.from(
            ...
            entry =>
                entry.value && entry.media.key === media.key && entry.media.version === media.version && entry.value.intrinsicSize.width >= size.width && entry.value.intrinsicSize.height >= size.height,
        )
    ...
    private async load(entry: CacheEntry): Promise<CachedImage> {
        ...
        try {
            ...
            try {
                signal.throwIfAborted()
                bitmap = await this.decoder.decode(
                    {
                        url: source.url,
                        ...source.request,
                    },
    ...
    private evict(): void {
        if (
            this.entries.size <= this.maxTextures
        ...
        const idle = Array.from(
            this.entries.values(),
        ).filter(entry => entry.value && entry.interests === 0)
    ...
    async acquirePlayback(request: Parameters<EngineMedia['acquirePlayback']>[0]): Promise<{
        url: string
        release: Dispose
    }> {
        const signal = AbortSignal.any([request.signal, this.controller.signal])
        ...
        if (!request.media.renditions.some(rendition => rendition.id === request.renditionId))
            throw new Error('Unknown playback rendition')
        ...
        try {
            ...
            if (Object.keys(source.request?.headers ?? {}).length > 0)
                throw new Error('Playback sources must be directly usable by a native media element')
