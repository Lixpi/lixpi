...
worker.onmessage = async (event: MessageEvent<ImageDecodeRequest>) => {
    ...
    try {
        ...
        controller.signal.throwIfAborted()
        worker.postMessage({ requestId, bitmap } satisfies ImageDecodeResponse, [bitmap])
        bitmap = null
    } catch (error) {
        if (!controller.signal.aborted) {
            worker.postMessage({ requestId, error: error instanceof Error ? error.message : String(error) } satisfies ImageDecodeResponse)
        }
    } finally {
        bitmap?.close()
