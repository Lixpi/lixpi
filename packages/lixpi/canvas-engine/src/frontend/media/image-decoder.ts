...
export class ImageDecoder {
    ...
    constructor(options: ImageDecoderOptions = {}) {
        ...
        if (!Number.isInteger(this.maxWorkers) || this.maxWorkers < 1) throw new RangeError('maxWorkers must be a positive integer')
        this.workerFactory = options.workerFactory ?? (() => new Worker(new URL('./image-decode-worker.ts', import.meta.url), { type: 'module' }))
    }

    async decode(source: string | ImageSourceRequest, signal?: AbortSignal): Promise<ImageBitmap> {
        if (this.destroyed) throw new DOMException('Image decoder destroyed', 'AbortError')
        ...
        return new Promise<ImageBitmap>((resolve, reject) => {
            const abort = () => {
                ...
                try {
                    worker.postMessage({ kind: 'cancel', requestId } satisfies ImageDecodeRequest)
                } catch {
    ...
    private nextWorker(): Worker {
        if (this.workers.length < this.maxWorkers) {
            ...
            worker.onmessage = (event: MessageEvent<ImageDecodeResponse>) => {
                ...
                else entry.resolve(data.bitmap)
            }
            worker.onerror = () => {
                for (const [id, entry] of this.pending) {
                    if (entry.worker === worker) this.take(id)?.reject(new Error('Image decode worker crashed'))
                }
    ...
    destroy(): void {
        ...
        this.destroyed = true
        for (const id of this.pending.keys()) this.take(id)?.reject(new DOMException('Image decoder destroyed', 'AbortError'))
