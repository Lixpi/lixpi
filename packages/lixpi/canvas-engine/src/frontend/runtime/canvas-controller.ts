...
export type CanvasInteractionOptions = {
    ...
    onSelectionChange?: (nodeIds: ReadonlySet<string>, fromMarquee: boolean) => void
}
...
export class CanvasController {
    ...
    constructor(private readonly options: CanvasOptions) {
        ...
        this.interaction = options.interaction === false ? false : { ...options.interaction, marquee: options.interaction?.marquee ? { ...options.interaction.marquee } : undefined }
        ...
        try {
            ...
            this.renderer = new CanvasRenderer({ ...options.renderer, root: this.pane, mediaResolver: options.mediaResolver, onError: options.onError })
            this.lifetime.own(() => this.renderer.destroy())
            this.scene = new CanvasScene({ ...options, renderer: this.renderer, root: options.overlayRoot ?? this.pane, geometry: this.geometry })
            this.lifetime.own(() => this.scene.destroy())
            this.transforms = new NodeTransformController({ root: options.root, gestures: this.gestures, overrides: this.geometry, getViewport: () => this.scene.viewport })
            this.lifetime.own(() => this.gestures.destroy())
            this.lifetime.own(() => {
                for (const handle of this.handles.values()) handle.destroy()
                this.handles.clear()
            })
            if (this.interaction) this.installInteraction(this.interaction)
            if (options.connectors) this.installConnections(options.connectors)
    ...
    private installInteraction(options: CanvasInteractionOptions): void {
        this.installViewport({
            ...
            onTransformChange: ([x, y, zoom]) => {
                this.scene.setViewport({ x, y, zoom })
                this.refreshPresentation()
                this.emit({ kind: 'viewport', sceneKey: this.scene.scene.sceneKey, viewport: { x, y, zoom } })
            },
        })
        this.installSelectionOverlay({
            marquee: options.marquee ?? { borderColor: 'currentColor', backgroundColor: 'transparent' },
            onGroupPointerDown: event => {
                const first = this.selection.nodeIds.values().next().value
                if (first) this.startDrag(event, first)
            },
        })
        this.installMarquee({
            ...
            onChange: bounds => {
                ...
                const selected = getIntersectingNodeIds(this.scene.scene.nodes, bounds, node => this.scene.getNodeGeometry(node.nodeId)!.selectionBounds)
                this.setSelected(selected, true)
            },
            onEnd: moved => {
                this.selectionOverlay!.setMarquee(null)
                if (!moved) this.setSelected([])
        ...
        this.installKeyboard({
            ...
            onDelete: () => {
                ...
                this.emit({ kind: 'delete', sceneKey: this.scene.scene.sceneKey, nodeIds, edgeIds })
                return true
            },
        })
        this.options.root.addEventListener('mousedown', this.pointerDown, true)
        this.lifetime.own(() => this.options.root.removeEventListener('mousedown', this.pointerDown, true))
    }

    installConnections(options: CanvasConnectionOptions): CanvasConnectionControls {
        this.assertAlive()
        if (this.connections) throw new Error('Canvas connection input is already installed')
        const lifetime = this.lifetime.child()
        try {
            const surface = this.renderer.createScope()
            lifetime.own(() => surface.destroy())
            const drawing = new ConnectorRenderer({ ...options.renderer, surface })
            lifetime.own(() => drawing.destroy())
            this.connections = new ConnectionManager({
                isCentered: () => true,
                isNodeTarget: target => Boolean(target.closest('[data-canvas-node-id]')),
                ...options.policy,
                ...
                getTransform: () => {
                    const { x, y, zoom } = this.scene.viewport
                    return [x, y, zoom]
                },
                panBy: async delta => {
                    if (!this.viewport) return false
                    const viewport = this.viewport.getViewport()
                    return this.viewport.setViewport({ ...viewport, x: viewport.x + delta.x, y: viewport.y + delta.y })
                },
                onEdgesChange: edges => {
                    try {
                        if (options.onEdgesChange) options.onEdgesChange(edges)
                        else this.connectionChange(edges)
                    } catch (error) {
                        this.options.onError(error)
                    }
                },
            ...
            this.connections.syncEdges(this.scene.scene.edges.map(edge => ({ edgeId: edge.edgeId, sourceNodeId: edge.source.nodeId, sourceHandle: edge.source.portId, targetNodeId: edge.target.nodeId, targetHandle: edge.target.portId, pathType: edge.path, data: edge.data })))
            this.refreshPresentation()
    ...
    installViewport(options: Omit<ViewportControllerOptions, 'root' | 'viewport'>): ViewportController {
        this.assertAlive()
        if (this.viewport) throw new Error('Canvas viewport input is already installed')
        const viewport = new ViewportController({ ...options, root: this.options.root, viewport: this.scene.viewport })
    ...
    installSelectionOverlay(options: Omit<SelectionOverlayOptions, 'root'>): SelectionOverlay {
        this.assertAlive()
        if (this.selectionOverlay) throw new Error('Canvas selection overlay is already installed')
        const overlay = new SelectionOverlay({ ...options, root: this.scene.worldElement })
    ...
    installMarquee(options: Omit<MarqueeControllerOptions, 'root' | 'gestures' | 'getWorldPoint'>): MarqueeController {
        this.assertAlive()
        if (this.marquee) throw new Error('Canvas marquee input is already installed')
        const marquee = new MarqueeController({ ...options, root: this.options.root, gestures: this.gestures, getWorldPoint: (x, y) => this.clientToWorld({ x, y }) })
    ...
    installKeyboard(options: Omit<CanvasKeyboardOptions, 'root'>): CanvasKeyboardController {
        this.assertAlive()
        if (this.keyboard) throw new Error('Canvas keyboard input is already installed')
        const keyboard = new CanvasKeyboardController({ ...options, root: this.options.root })
    ...
    startNodeResize(options: NodeResizeOptions): CanvasGesture {
        return this.startTransform({ ...options, targets: [options.target] }, wrapped => this.transforms.startResize({ ...options, ...wrapped }))
    }

    private startTransform(options: NodeTransformOptions, start: (options: NodeTransformOptions) => CanvasGesture): CanvasGesture {
        this.assertAlive()
        this.cancelInteraction('replaced')
        this.transforming = new Set(options.targets.map(target => target.nodeId))
        try {
            return start({
                ...options,
                onChange: bounds => {
                ...
                onEnd: (event, bounds, moved) => {
                    this.transforming.clear()
                    try {
                        options.onEnd(event, bounds, moved)
                    } finally {
                        this.refreshPresentation()
                    }
                },
    ...
    private assertAlive(): void {
        if (this.destroyed) throw new Error('Canvas controller is disposed')
    }

    private connectionChange = (edges: ConnectionEdge[]): void => {
        const snapshot = this.scene.scene
        const incoming = new Map(edges.map(edge => [edge.edgeId, edge]))
        const removed = snapshot.edges.filter(edge => !incoming.has(edge.edgeId)).map(edge => edge.edgeId)
        const intents: CanvasIntent[] = removed.length ? [{ kind: 'delete', sceneKey: snapshot.sceneKey, nodeIds: [], edgeIds: removed }] : []
        for (const edge of edges) {
            if (!edge.sourceHandle || !edge.targetHandle) continue
            const previous = snapshot.edges.find(candidate => candidate.edgeId === edge.edgeId)
            const source = { nodeId: edge.sourceNodeId, portId: edge.sourceHandle }
            const target = { nodeId: edge.targetNodeId, portId: edge.targetHandle }
            if (!previous) intents.push({ kind: 'connect', sceneKey: snapshot.sceneKey, source, target })
            else if (previous.source.nodeId !== source.nodeId || previous.source.portId !== source.portId || previous.target.nodeId !== target.nodeId || previous.target.portId !== target.portId) intents.push({ kind: 'reconnect', sceneKey: snapshot.sceneKey, edgeId: edge.edgeId, source, target })
        }
    ...
    setScene(snapshot: SceneSnapshot): void {
        ...
        if (changedScene || Array.from(this.transforming).some(id => !snapshot.nodes.some(node => node.nodeId === id))) this.cancelInteraction('scene-change')
        if (changedScene) {
            this.selection.clear()
            this.selectedEdge = null
        } else for (const id of this.selection.nodeIds) if (!snapshot.nodes.some(node => node.nodeId === id)) this.selection.remove(id)
        this.connections?.syncEdges(snapshot.edges.map(edge => ({ edgeId: edge.edgeId, sourceNodeId: edge.source.nodeId, sourceHandle: edge.source.portId, targetNodeId: edge.target.nodeId, targetHandle: edge.target.portId, pathType: edge.path, data: edge.data })))
        this.refreshPresentation()
    }

    setViewport(viewport: CanvasViewport): void {
        if (this.destroyed) return
        this.scene.setViewport(viewport, this.size())
    ...
    clientToWorld = (point: CanvasEnginePoint): CanvasEnginePoint => {
        ...
        return { x: (point.x - bounds.left - viewport.x) / viewport.zoom, y: (point.y - bounds.top - viewport.y) / viewport.zoom }
    }

    setSelected(ids: Iterable<string>, fromMarquee = false): void {
        if (this.destroyed) return
        const available = new Set(this.scene.scene.nodes.map(node => node.nodeId))
        this.selection.replace(Array.from(ids).filter(id => available.has(id)), fromMarquee)
        this.refreshSelection()
        if (this.interaction) this.interaction.onSelectionChange?.(new Set(this.selection.nodeIds), this.selection.fromMarquee)
    }

    private refreshSelection(): void {
        if (this.destroyed) return
        this.scene.setSelected(this.selection.nodeIds)
        const bounds = Array.from(this.selection.nodeIds, id => this.scene.getNodeGeometry(id)?.selectionBounds).filter((rect): rect is CanvasEngineRect => Boolean(rect))
        if (this.interaction) this.selectionOverlay?.setGroup(this.selection.nodeIds.size > 1 && !this.marquee?.active ? unionRectangles(bounds) : null)
        this.refreshHandles()
    }
    ...
    private refreshPresentation(): void {
        ...
        this.connections?.syncNodes(this.scene.scene.nodes.map(node => {
            const geometry = this.scene.getNodeGeometry(node.nodeId)!
            const bounds = geometry.connectorBounds
            return { ...node, parentId: undefined, position: { x: bounds.x, y: bounds.y }, dimensions: { width: bounds.width, height: bounds.height }, ports: node.ports.map(port => ({ ...port, anchor: { x: port.anchor.x + geometry.worldBounds.x - bounds.x, y: port.anchor.y + geometry.worldBounds.y - bounds.y } })) }
        }))
        if (this.connections) {
            for (const node of this.scene.scene.nodes) {
                const root = this.scene.getNodeRoot(node.nodeId)
                if (root) this.connections.registerNodeElement(node.nodeId, root)
            }
        }
    ...
    private refreshHandles(): void {
        ...
        for (const node of nodes) {
            ...
            if (!handles) {
                handles = new NodeHandles({ root: this.scene.worldElement, nodeId: node.nodeId, flowId: this.connections?.flowId ?? '', size: this.interaction.handleSize, onConnect: (event, port, element, isTarget) => this.connections?.onHandlePointerDown(event, { nodeId: node.nodeId, handleId: port.id, handleDomNode: element, isTarget }), onResize: (event, handle) => this.startResize(event, node.nodeId, handle) })
                this.handles.set(node.nodeId, handles)
            }
            const root = this.scene.getNodeRoot(node.nodeId)
            if (root) root.style.pointerEvents = 'auto'
            handles.update(this.scene.getNodeGeometry(node.nodeId)!.worldBounds, this.connections ? node.ports : [], this.selection.has(node.nodeId), this.scene.viewport.zoom)
        }
    }

    private pointerDown = (event: MouseEvent): void => {
        if (event.button !== 0 || event.defaultPrevented || !this.interaction) return
        const target = event.target as Element
        if (this.selectionOverlay?.contains(target)) return
        if (isEditableTarget(target) || target.closest('button, a, input, textarea, select, .nopan') || this.interaction.isInteractiveTarget?.(target)) return
        ...
        const node = this.hitNode(event)
        if (node) this.startDrag(event, node.nodeId)
        else {
    ...
    private hitNode(event: MouseEvent): EngineNode | undefined {
        ...
        const nodes = this.scene.scene.nodes
        if (id) return nodes.find(node => node.nodeId === id)
        const point = this.clientToWorld({ x: event.clientX, y: event.clientY })
        return Array.from(nodes).reverse().find(node => rectangleContainsPoint(this.scene.getNodeGeometry(node.nodeId)!.hitBounds, point))
    }
    ...
    private startDrag(event: MouseEvent, nodeId: string): void {
        ...
        this.cancelInteraction('replaced')
        const nodes = this.scene.scene.nodes
        const plan = computeDragPlan({ nodes, primaryNodeId: nodeId, selectedNodeIds: this.selection.nodeIds, canDrag: node => this.options.registry.get(node.type)?.geometry.movable ?? false })
        ...
        this.transforms.startDrag({
            event,
            targets: plan.draggedNodeIds.map(id => ({ nodeId: id, bounds: this.scene.getNodeGeometry(id)!.worldBounds })),
            threshold: this.interaction.dragThreshold ?? 3,
            lock: () => this.viewport!.lock(),
            onStart: () => {
                if (!this.selection.has(nodeId)) this.setSelected([nodeId])
            },
            onChange: () => this.refreshPresentation(),
            onEnd: (_event, bounds, moved) => {
                this.transforming.clear()
                if (moved) this.commitGeometry(bounds)
                else this.setSelected([nodeId])
                this.refreshPresentation()
            },
            onCancel: () => {
                this.transforming.clear()
    ...
    private startResize(event: MouseEvent, nodeId: string, handle: ResizeHandle): void {
        if (event.button !== 0 || !this.interaction) return
        ...
        this.transforms.startResize({
            event,
            target: { nodeId, bounds: this.scene.getNodeGeometry(nodeId)!.worldBounds },
            handle,
    ...
    private commitGeometry(bounds: ReadonlyMap<string, CanvasEngineRect>): void {
        ...
        const changes = computeGeometryChanges(snapshot.nodes, bounds, {
        ...
        if (changes.length) this.emit({ kind: 'geometry', sceneKey: snapshot.sceneKey, revision: snapshot.revision, changes })
    }
    ...
    installExtension(extension: CanvasExtension): Dispose {
        if (this.destroyed) throw new Error('Canvas controller is disposed')
        if (!extension.id.trim() || this.extensions.has(extension.id)) throw new Error(`Invalid or duplicate canvas extension: ${extension.id}`)
        ...
        try {
            scope.own(extension.mount(Object.assign(context, { overlayRoot: context.contentRoot })))
        } catch (error) {
            dispose()
            throw error
        }
