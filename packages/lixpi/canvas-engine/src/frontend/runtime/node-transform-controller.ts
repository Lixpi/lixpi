...
export type NodeTransformTarget = { nodeId: string; bounds: CanvasEngineRect }
export type NodeTransformOptions = {
    ...
    onEnd: (event: MouseEvent | PointerEvent, bounds: ReadonlyMap<string, CanvasEngineRect>, moved: boolean) => void
...
class NodeTransformSession {
    ...
    constructor(private readonly host: NodeTransformControllerOptions, private readonly options: NodeTransformOptions, private readonly resize?: Pick<NodeResizeOptions, 'handle' | 'constraints'>) {
        this.zoom = host.getViewport().zoom
        if (!Number.isFinite(this.zoom) || this.zoom <= 0) throw new Error('Node transforms require a finite positive zoom')
        if (!Number.isFinite(options.threshold ?? 0) || (options.threshold ?? 0) < 0) throw new Error('Node transform threshold must be finite and nonnegative')
        for (const target of options.targets) {
            assertCanvasBounds(target.bounds, target.nodeId)
            if (this.starts.has(target.nodeId)) throw new Error(`Duplicate transform target: ${target.nodeId}`)
    ...
    private snapshot(): ReadonlyMap<string, CanvasEngineRect> {
        return new Map(Array.from(this.bounds, ([id, bounds]) => [id, { ...bounds }]))
    }
    ...
    private move = (event: MouseEvent | PointerEvent): void => {
        ...
        try {
            if (!this.moved) {
                if (Math.hypot(x, y) < (this.options.threshold ?? 0)) return
                this.moved = true
            ...
            for (const [nodeId, start] of this.starts) {
                const bounds = this.resize
                    ? computeResizedBounds(start, { x: x / this.zoom, y: y / this.zoom }, this.resize.handle, this.resize.constraints)
                    : { ...start, x: start.x + x / this.zoom, y: start.y + y / this.zoom }
                assertCanvasBounds(bounds, nodeId)
                this.bounds.set(nodeId, bounds)
                this.scope.set(nodeId, { position: { x: bounds.x, y: bounds.y }, dimensions: { width: bounds.width, height: bounds.height } })
            }
    ...
    private end = (event: MouseEvent | PointerEvent): void => {
        this.cleanup()
        this.options.onEnd(event, this.snapshot(), this.moved)
    }
...
export class NodeTransformController {
    ...
    startResize(options: NodeResizeOptions): CanvasGesture {
        return new NodeTransformSession(this.options, { ...options, targets: [options.target] }, options).gesture
    }
}
