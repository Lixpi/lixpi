...
export class FrameScheduler {
    ...
    animate(callback: (elapsedMs: number) => void): Dispose {
        ...
        return () => {
            ...
            if (this.animations.size === 0 && !this.dirty && this.frameId !== null) {
    ...
    private schedule(): void {
        if (this.frameId !== null || this.destroyed || this.ticking) return
    ...
    private tick = (time: number): void => {
        ...
        for (const callback of Array.from(this.animations)) {
            if (!this.animations.has(callback) || this.destroyed) continue
            try {
                callback(elapsed)
            } catch (error) {
                this.animations.delete(callback)
        ...
        if (this.dirty && !this.destroyed) {
            const invalidation = { full: this.full, bounds: this.dirtyBounds }
            this.dirty = false
            this.full = false
            this.dirtyBounds = []
            try {
        ...
        this.ticking = false
        if (this.animations.size > 0 || this.dirty) this.schedule()
