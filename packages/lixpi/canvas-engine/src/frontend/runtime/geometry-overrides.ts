...
export type NodeGeometryOverride = { position?: CanvasEnginePoint; dimensions?: CanvasEngineSize }

export class GeometryOverrideScope {
    private readonly values = new Map<string, NodeGeometryOverride>()
    private destroyed = false

    constructor(readonly priority: number, private readonly release: () => void) {}

    set(nodeId: string, value: NodeGeometryOverride): void {
        if (this.destroyed) return
        assertCanvasBounds({ x: value.position?.x ?? 0, y: value.position?.y ?? 0, width: value.dimensions?.width ?? 0, height: value.dimensions?.height ?? 0 }, nodeId)
...
export class GeometryOverrides {
    ...
    createScope(priority = 0): GeometryOverrideScope {
        if (this.destroyed) throw new Error('Geometry overrides are disposed')
        if (!Number.isFinite(priority)) throw new Error('Geometry override priority must be finite')
        const scope = new GeometryOverrideScope(priority, () => this.scopes.delete(scope))
        this.scopes.add(scope)
        return scope
    }

    get(nodeId: string, except?: GeometryOverrideScope): NodeGeometryOverride | undefined {
    ...
    worldPosition<Node extends CanvasPositionedNode>(node: Node, nodes: ReadonlyMap<string, Node>): CanvasEnginePoint {
        return computeWorldPosition(node, nodes, id => this.get(id)?.position)
    }

    worldBounds<Node extends CanvasGeometryNode>(node: Node, nodes: ReadonlyMap<string, Node>): CanvasEngineRect {
        return { ...this.worldPosition(node, nodes), ...(this.get(node.nodeId)?.dimensions ?? node.dimensions) }
    }
