...
export class CanvasScene {
        ...
        this.views = new NodeViews({
            ...
            mountUnknown: (node, context) => {
                const diagnostic: CanvasDiagnostic = {
                    ...
                    nodeId: node.nodeId,
                    message: `Unregistered canvas node type: ${node.type}`,
                }
                ...
                if (options.onDiagnostic)
                    ...
                    options.onError(
                        new CanvasDiagnosticError(diagnostic),
                        node.nodeId,
                    )
        ...
        try {
            while (
                ...
                this.views.sync({
                    ...
                    nodes: next.nodes,
                    presentation: node => this.presentation(node.nodeId),
                })
    ...
    refreshGeometry(): void {
        ...
        this.indexGeometry(
            this.measureGeometry(this.snapshot, id => this.geometry.get(id)),
        )
        this.updateVisibility()
        this.presentAll()
        this.schedulePrefetch()
    ...
    private measureGeometry(
        scene: SceneSnapshot,
        getOverride: (nodeId: string) => NodeGeometryOverride | undefined,
    ): Map<string, MeasuredNodeGeometry> {
        ...
        for (const node of scene.nodes) {
            const worldNode = {
                ...node,
                parentId: undefined,
                position: computeWorldPosition(
                    node,
                    byId,
                    id => getOverride(id)?.position,
                ),
                dimensions: getOverride(node.nodeId)?.dimensions ?? node.dimensions,
            }
            const worldBounds = {
                ...worldNode.position,
                ...worldNode.dimensions,
            }
            assertCanvasBounds(worldBounds, node.nodeId)
            const geometry = this.options.registry.get(node.type)?.geometry
            const measurement = geometry?.measure(worldNode) ?? {
                visualBounds: worldBounds,
                hitBounds: worldBounds,
                selectionBounds: worldBounds,
                collisionBounds: worldBounds,
    ...
    private updateVisibility(): void {
        ...
        this.visible = new Set(
            this.spatial.search({
                minX: -x / zoom - margin,
                minY: -y / zoom - margin,
                maxX: (width - x) / zoom + margin,
                maxY: (height - y) / zoom + margin,
    ...
    private presentation(nodeId: string): NodePresentation {
        ...
        if (context)
            context.contentRoot.style.display = this.visible.has(nodeId) ? '' : 'none'

        return {
            worldBounds,
            viewport: this.view.viewport,
            visible: this.visible.has(nodeId),
            selected: this.selected.has(nodeId),
        }
    }
    ...
    createContext(nodeId?: string): SceneContext {
        if (this.destroyed)
            throw new Error('Canvas scene is disposed')
        ...
        if (nodeId)
            root.dataset.canvasNodeId = nodeId
        ...
        const context = new SceneContext(
            ...
            () => {
                ...
                if (
                    nodeId
                    && this.nodeContexts.get(nodeId) === context
                )
                    this.nodeContexts.delete(nodeId)
            },
        )
    ...
    subscribeScene = (callback: (scene: SceneSnapshot) => void): Dispose => this.subscribe(
        this.sceneSubscribers,
        callback,
        this.snapshot,
    )
    subscribeView = (callback: (view: CanvasView) => void): Dispose => this.subscribe(
    ...
    private schedulePrefetch(): void {
        ...
        const batchSize = Math.max(
            0,
            Math.floor(this.options.prefetchBatchSize ?? 0),
        )
        ...
        const center = {
            x: (this.view.screenBounds.width / 2 - x) / zoom,
            y: (this.view.screenBounds.height / 2 - y) / zoom,
        }
