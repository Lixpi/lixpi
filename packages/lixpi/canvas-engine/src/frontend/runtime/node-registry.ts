...
export class NodeRegistry {
    ...
    register<Data>(registration: NodeRegistration<Data>): this {
        if (!registration.type.trim())
            throw new Error('Node registrations require a nonempty type')

        if (this.registrations.has(registration.type))
            throw new Error(`Node type is already registered: ${registration.type}`)
