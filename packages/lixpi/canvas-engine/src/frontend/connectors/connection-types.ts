...
export type ConnectionEdge<Data = unknown> = SpreadEdge & { pathType?: PathType; data?: Data }
...
export type ConnectionSettings = {
    ...
    autoAlign: { minSlideHeight: number; edgeMargin: number }
    styles: { lineDefaultColor: string; lineFocusColor: string }
}
...
export type ConnectionPolicy<Node extends ConnectionNode> = {
    ...
    targetMarker?: (node: Node | undefined, selected: boolean) => MarkerType
    additionalGeometry?: (node: Node, nodes: readonly Node[]) => readonly NodeConfig[]
    renderedSourceNodeId?: (edge: ConnectionEdge, nodes: readonly Node[]) => string
    ...
    sourceAnchorT?: (edge: ConnectionEdge, element: HTMLElement | undefined) => number | null
    isNodeTarget?: (target: Element) => boolean
    canConnectProximity?: (source: Node, target: Node) => boolean
}
