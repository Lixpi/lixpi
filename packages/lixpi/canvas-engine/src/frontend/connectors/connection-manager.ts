...
export function getEdgeAnchorPositions(edge: ConnectionEdge): { source: 'left' | 'right'; target: 'left' | 'right' } {
...
export class ConnectionManager<Node extends ConnectionNode = ConnectionNode> {
    ...
    private cachedEdgeConfigs: Array<{ edgeConfig: EdgeConfig; isSelected: boolean }> | null = null
    ...
    private getRenderedEdgeSourceNodeId(edge: ConnectionEdge, _nodeById: Map<string, Node>, worldNodeMap: Map<string, NodeConfig>): string {
    ...
    private resolveAnchorT(node: Node | undefined, t: number | undefined): number {
    ...
    public constructor(config: ConnectionManagerConfig<Node>) {
        this.config = { ...config, settings: structuredClone(config.settings) }
        ...
        if (!view) throw new Error('Connection manager requires a browser document')
        this.view = view
    ...
    private portAnchor(nodeId: string, handleId: string | undefined, fallback: AnchorPosition, t: number | undefined, nodes: Map<string, Node>, contentAnchor = false, logicalNodeId = nodeId): EdgeAnchor {
        const node = nodes.get(logicalNodeId)
        const port = node?.ports?.find(port => port.id === handleId)
        if (!port || !node) return this.buildEdgeAnchor(nodeId, fallback, t, nodes)
        ...
        if (nodeId !== logicalNodeId) {
            ...
            const projectedT = contentAnchor && verticalSide ? t : length > 0 ? position / length : 0.5
            return this.buildEdgeAnchor(nodeId, port.direction, projectedT, nodes)
        }
        const y = contentAnchor && verticalSide && t !== undefined
            ? node.dimensions.height * Math.max(0, Math.min(1, t))
            : port.anchor.y
        return {
            nodeId,
            position: port.direction,
            t: 0,
            offset: { x: port.anchor.x - (port.direction === 'right' ? node.dimensions.width : 0), y: y - (port.direction === 'bottom' ? node.dimensions.height : 0) },
        }
    }

    public syncNodes(canvasNodes: Node[]) {
        if (this.destroyed) return
        const nodeIds = new Set(canvasNodes.map(node => node.nodeId))
        for (const id of this.nodeElements.keys()) if (!nodeIds.has(id)) this.nodeElements.delete(id)
        ...
        if (this.connectionInProgress && !canvasNodes.some(node => node.nodeId === this.connectionInProgress?.fromHandle.nodeId)) this.cancelTransientConnection()
        this.nodes = canvasNodes

        const xyNodes: NodeBase[] = sortedNodes.map((n) => ({
            id: n.nodeId,
            data: {},
            ...
            handles: n.ports
                ? n.ports.flatMap(port =>
                    (port.role === 'both' ? ['source', 'target'] as const : [port.role === 'input' ? 'target' : 'source'] as const).map(type => ({
                        id: port.id,
                        type,
    ...
    public registerNodeElement(nodeId: string, nodeElement: HTMLElement) {
        if (this.destroyed) return
        ...
        if (this.nodes.find(node => node.nodeId === nodeId)?.ports) return
        const updates = new Map([
            [nodeId, { id: nodeId, nodeElement: nodeElement as HTMLDivElement }],
        ])
    ...
    public startConnectionFromMenu(nodeId: string) {
        ...
        const onMouseMove = (e: MouseEvent) => {
            ...
            const closestHandle = this.findClosestHandle(rendererPos, fromHandle, this.config.settings.menuConnectionSnapRadius)
        ...
        const onMouseUp = (e: MouseEvent) => {
            ...
            if (toHandle && toNode && isValid) {
                ...
                if (sourceNode && targetNode && this.canAutoAlignTarget(targetNode)) {
        ...
        const cleanup = () => {
            ...
            this.document.removeEventListener('mousedown', onMouseDownCapture, true)
        ...
        this.document.addEventListener('mousedown', onMouseDownCapture, true)
    ...
    private getMenuSnapPoint(nodeId: string, handle: Handle, pointer: { x: number; y: number }) {
        ...
        if (port) return { x: node.internals.positionAbsolute.x + port.anchor.x, y: node.internals.positionAbsolute.y + port.anchor.y }
    ...
    private isMenuConnectionValid(sourceNodeId: string, targetHandle: Handle): boolean {
        ...
        if (this.nodes.find(node => node.nodeId === targetHandle.nodeId)?.ports && targetHandle.type !== 'target') return false
    ...
    public onHandlePointerDown(event: MouseEvent | TouchEvent, meta: HandleMeta) {
        ...
        this.handleGesture = new HandleConnectionGesture(event, {
            ...
            updateConnection: (state: ConnectionInProgress) => {
                if (this.destroyed || this.connectionEpoch !== connectionEpoch) return
                this.connectionInProgress = state
                this.render()
            },

            cancelConnection: () => {
                if (this.connectionEpoch === connectionEpoch) this.cancelTransientConnection()
            },

            isValidConnection: (connection: Connection) => {
                if (this.destroyed || this.connectionEpoch !== connectionEpoch) return false
                // No self-loops
    ...
    public render() {
        ...
        const nodeById = new Map<string, Node>(this.nodes.map((node) => [node.nodeId, node]))
        ...
        const drawingEdgeConfigsForCache: Array<{ edgeConfig: EdgeConfig; isSelected: boolean }> = []
        const addConnectorDatum = (edgeConfig: EdgeConfig, isSelected: boolean): void => {
            if (!this.config.onConnectorGeometry) return
            drawingEdgeConfigsForCache.push({ edgeConfig, isSelected })
        ...
        const spreadEdges = effectiveEdges.map(edge => ({
            ...edge,
        ...
        const spreadTValues = computeConnectorSpread(spreadEdges, this.nodes, { isCentered: this.config.isCentered, ...this.config.settings.autoAlign })
        ...
        for (const e of this.edges) {
            ...
            const target = e.targetHandle == null && this.config.defaultTargetHandle?.(edgeTargetNode) === 'left'
                ? 'left'
                : storedTarget
            ...
            if (this.config.sourceAnchorT) {
                const computedT = this.config.sourceAnchorT(e, this.nodeElements.get(e.sourceNodeId))
                if (computedT !== null && Number.isFinite(computedT)) {
                    sourceT = computedT
            ...
            const edgeConfig: EdgeConfig = {
                id: e.edgeId,
                source: this.portAnchor(renderedSourceNodeId, e.sourceHandle, source, sourceT, nodeById, sourceContentAnchor, e.sourceNodeId),
                target: this.portAnchor(e.targetNodeId, e.targetHandle, target, targetT, nodeById, targetContentAnchor),
        ...
        if (this.connectionInProgress) {
            ...
            const tempEdge: EdgeConfig = {
                ...
                source: this.portAnchor(sourceNodeId, sourceHandleId, sourcePosition, undefined, nodeById),
                target: snappedTargetNode && snappedTargetPosition && this.connectionInProgress.toHandle
                    ? this.portAnchor(snappedTargetNode.nodeId, this.connectionInProgress.toHandle.id ?? undefined, snappedTargetPosition, snappedTargetT, nodeById)
    ...
    public recomputeConnectorGeometry(zoom: number): boolean {
        if (!this.cachedEdgeConfigs || !this.cachedWorldNodeMap || !this.config.onConnectorGeometry) return false
        ...
        const connectorData: ConnectorRenderDatum[] = []
        for (const { edgeConfig, isSelected } of this.cachedEdgeConfigs) {
            const drawingDatum = computeConnectorDatum(
                edgeConfig,
            ...
            if (drawingDatum) connectorData.push(drawingDatum)
        }
    ...
    public checkProximity(
        ...
        for (const candidate of this.nodes) {
            if (candidate.nodeId === nodeId || !this.config.canConnectProximity?.(draggedNode, candidate)) continue
            if (this.edges.some(edge => edge.sourceNodeId === nodeId && edge.targetNodeId === candidate.nodeId)) continue
            const delta = Math.hypot(position.x + dimensions.width - candidate.position.x, position.y + dimensions.height / 2 - candidate.position.y - candidate.dimensions.height / 2)
            if (delta > distance) continue
            distance = delta
            closestCandidate = { sourceNodeId: nodeId, targetNodeId: candidate.nodeId, sourceHandle: 'right', targetHandle: 'left', sourceT: 0.5, targetT: 0.5 }
        }
