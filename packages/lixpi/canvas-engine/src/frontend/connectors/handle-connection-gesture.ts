...
export type HandleConnectionGestureOptions = {
    ...
    onReconnectEnd: (event: MouseEvent | TouchEvent, state: ConnectionInProgress) => void
}
...
export class HandleConnectionGesture {
    ...
    constructor(event: MouseEvent | TouchEvent, private readonly options: HandleConnectionGestureOptions) {
        this.document = options.domNode.ownerDocument
        const view = this.document.defaultView
        if (!view) throw new Error('Connection gestures require a browser document')
        this.view = view
        this.touchId = 'touches' in event ? event.changedTouches[0]?.identifier : undefined
        const point = this.eventPoint(event)
        if (!point) throw new Error('Connection gesture requires a pointer position')
        this.start = point
        this.pointer = point
        try {
            const moveType = this.touchId === undefined ? 'mousemove' : 'touchmove'
            const endType = this.touchId === undefined ? 'mouseup' : 'touchend'
            this.document.addEventListener(moveType, this.move, { passive: false })
            ...
            this.lifetime.own(() => this.document.removeEventListener('keydown', this.keyDown, true))
        ...
        for (const [nodeId, node] of this.options.nodeLookup) {
            for (const type of ['source', 'target'] as const) {
                for (const handle of node.internals.handleBounds?.[type] ?? []) {
                    ...
                    result.push({ ...handle, nodeId, type, x: x + handle.x + handle.width / 2, y: y + handle.y + handle.height / 2 })
                }
            }
        }
        return result
    ...
    private update(): void {
        ...
        const pointer = { x: this.pointer.x - rect.left, y: this.pointer.y - rect.top }
        ...
        if (inside && (!directHandle || options.domNode.contains(directHandle))) {
            for (const handle of handles) {
                if (handle.type === type || handle.nodeId === fromHandle.nodeId) continue
            ...
            if (directHandle) {
                target = handles.find(handle => handle.nodeId === directHandle.dataset.nodeid && handle.id === directHandle.dataset.handleid && handle.type !== type) ?? target
            }
        }
        const source = type === 'source' ? fromHandle : target
        const destination = type === 'source' ? target : fromHandle
        this.connection = source && destination ? { source: source.nodeId, sourceHandle: source.id ?? null, target: destination.nodeId, targetHandle: destination.id ?? null } : null
        ...
        this.state = {
            ...
            to: target && isValid ? { x: target.x * zoom + x, y: target.y * zoom + y } : pointer,
            toHandle: target,
    ...
    private move = (event: Event): void => {
        ...
        if (!point) return
        this.pointer = point
        if (!this.moving && Math.hypot(point.x - this.start.x, point.y - this.start.y) <= 1) return
        event.preventDefault()
        this.moving = true
        try {
            this.update()
            if (!this.ended && !this.panning && this.frame === null) this.frame = this.view.requestAnimationFrame(this.autoPan)
        } catch (error) {
            this.cancel()
            throw error
        }
    ...
    private autoPan = async (): Promise<void> => {
        ...
        const speed = (position: number, size: number) => 15 * (Math.max(0, Math.min(1, (40 - position) / 40)) - Math.max(0, Math.min(1, (position - size + 40) / 40)))
        const delta = { x: speed(this.pointer.x - rect.left, rect.width), y: speed(this.pointer.y - rect.top, rect.height) }
        this.panning = true
        try {
            if ((delta.x || delta.y) && await this.options.panBy(delta) && !this.ended) this.update()
        } catch (error) {
            this.cancel()
        ...
        if (!this.ended) this.frame = this.view.requestAnimationFrame(this.autoPan)
    }

    private end = (event: Event): void => {
        const point = this.eventPoint(event as MouseEvent | TouchEvent)
        if (this.ended || !point) return
        this.pointer = point
        if (this.moving) {
            try {
                this.update()
        ...
        this.cleanup()
        try {
            if (this.state?.isValid && this.connection) this.options.onConnect(this.connection)
            if (this.moving && this.state) this.options.onReconnectEnd(event as MouseEvent | TouchEvent, this.state)
        } finally {
    ...
    private touchCancel = (event: TouchEvent): void => {
        if (this.eventPoint(event)) this.cancel()
    }
