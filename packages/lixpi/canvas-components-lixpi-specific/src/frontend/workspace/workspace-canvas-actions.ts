...
export type CanvasIngestResult = { assetId: string; kind: MediaKind }
export type CanvasIngestReply = CanvasIngestResult | { error: string } | null
export type CanvasUploadRequest = { workspaceId: string; onStart: () => boolean }
export type WorkspaceCanvasActionScope = { workspaceId: string; organizationId: string; revision: number }
...
export type WorkspaceCanvasActionsPorts = {
    ...
    createDocument: (request: { workspaceId: string; organizationId: string; title: string }) => Promise<{ assetId: string }>
    ...
    attach: (workspaceId: string, request: { assetId: string; nodeId: string; prepare: () => CanvasState }) => Promise<CanvasState>
    insertPlaceholder: (node: Omit<OperationStatusCanvasNode, 'position'>) => void
    failPlaceholder: (nodeId: string, message: string) => void
    prepareInsertion: (node: WorkspaceCanvasInsertionNode, placeholderNodeId?: string) => CanvasState
    commitDocument: (state: CanvasState) => void
    commitMedia: (state: CanvasState, nodeId: string, placeholderNodeId?: string) => void
    closeUploadMenu: () => void
    reportError: (message: string, error: unknown) => void
}

type Action = { scope: WorkspaceCanvasActionScope; revision: number }
...
export class WorkspaceCanvasActions {
    ...
    async createDocument(): Promise<void> {
        ...
        try {
            ...
            const state = await this.ports.attach(action.scope.workspaceId, {
                assetId: document.assetId,
                nodeId: node.nodeId,
                prepare: () => {
            ...
            if (this.isCurrent(action)) this.ports.commitDocument(state)
        } catch (error) {
    ...
    async uploadFile(file: File): Promise<void> {
        ...
        if (file.size > MAX_UPLOAD_FILE_SIZE) {
            ...
            if (this.isCurrent(action)) this.ports.failPlaceholder(placeholderId, 'File is too large.')
            return
        }
        await this.ingest(action, file.name, 'Upload failed', onStart => this.ports.uploadFile({ workspaceId: action.scope.workspaceId, file, onStart }), false)
    }
    ...
    async importUrl(value: string): Promise<void> {
        ...
        await this.ingest(action, title, 'File URL import failed', onStart => this.ports.importUrl({ workspaceId: action.scope.workspaceId, url, onStart }), true)
    }

    private capture(): Action | null {
        const scope = this.ports.readScope()
        return this.disposed || !scope ? null : { scope: { ...scope }, revision: this.revision }
    }
    ...
    private assertCurrent(action: Action): void {
        if (!this.isCurrent(action)) throw new Error('WORKSPACE_CHANGED_DURING_CANVAS_MUTATION')
    }
    ...
    private async ingest(action: Action, title: string, failureMessage: string, request: (onStart: () => boolean) => Promise<CanvasIngestReply>, closeMenu: boolean): Promise<void> {
        ...
        try {
            ...
            if (!this.isCurrent(action) || !result) return
            if ('error' in result) {
                if (placeholderId) this.ports.failPlaceholder(placeholderId, result.error || failureMessage)
                return
            }
            if (closeMenu) this.ports.closeUploadMenu()
            if (!this.isCurrent(action)) return
            await this.addAsset(action, result, placeholderId)
        } catch (error) {
            this.ports.reportError(failureMessage, error)
            if (placeholderId && this.isCurrent(action)) this.ports.failPlaceholder(placeholderId, failureMessage)
        }
    }

    private async addAsset(action: Action, result: CanvasIngestResult, placeholderId: string | undefined): Promise<void> {
        ...
        const dimensions = result.kind === 'audio'
            ? { width: 360, height: 96 }
            : { width, height: width / (result.kind === 'document' ? 0.7727 : 1) }
        const node = { nodeId, type: result.kind === 'document' ? 'mediaDocument' : result.kind, assetId: result.assetId, dimensions } as WorkspaceCanvasInsertionNode
        const state = await this.ports.attach(action.scope.workspaceId, {
        ...
        if (this.isCurrent(action)) this.ports.commitMedia(state, nodeId, placeholderId)
    }
}
