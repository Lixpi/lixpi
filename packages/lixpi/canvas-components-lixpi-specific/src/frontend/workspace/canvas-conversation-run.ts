...
export type CanvasConversationRunPorts = {
    ...
    preflight: (
        placement: CanvasGenerationPreflight,
...
export class CanvasConversationRun {
    ...
    constructor(
        ...
        const editor = ports.mountEditor({
            ...
            onSegment: (event, options) => {
                ...
                if ((event.conversationAssetId || event.threadId) !== thread.threadId)
                    return
        ...
        if (!scope.isCurrent())
            throw new DOMException('Canvas conversation was replaced', 'AbortError')
    ...
    private receivingChanged(
        ...
        if (
            this.scope.isCurrent()
            && !receiving
            && !this.ports.hasPendingPlacement(threadId)
        )
    ...
    private async submit(submission: CanvasConversationSubmit): Promise<void> {
        ...
        try {
            ...
            const promptParts = getBranchMarkerPromptParts(
                {
                    type: 'doc',
                    content: submittedData.contentJSON,
                },
                promptText,
            )
            ...
            const mediaNodeIds = new Set(
                nodes.filter(node => node.type === 'image' || node.type === 'video').map(node => node.nodeId),
            )
            ...
            if (mediaBranchCandidateSnapshot) {
                const placementAnchorNodeId = mediaBranchCandidateSnapshot.candidates.find(
                    candidate => candidate.candidateId === mediaBranchCandidateSnapshot.activeTargetCandidateId,
                )?.nodeId
            ...
            if (!this.transport)
                throw new Error('Canvas conversation transport is not initialized')
