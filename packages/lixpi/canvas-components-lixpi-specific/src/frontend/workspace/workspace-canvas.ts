...
export { type WorkspaceCanvasOptions } from './workspace-canvas-contracts.ts'
...
export class LixpiWorkspaceCanvas {
    ...
    constructor(
        private readonly options: WorkspaceCanvasOptions,
        private readonly host: WorkspaceCanvasHost,
    ) {
        try {
            ...
            this.getPromptReferenceCatalogClient = (organizationId = String(this.host.workspace.organizationId() ?? '')): WorkspacePromptCatalog => {
                if (
                    !this.promptReferenceCatalogClient
                    || organizationId !== this.promptReferenceCatalogOrganizationId
                    || this.workspaceId !== this.promptReferenceCatalogWorkspaceId
                ) {
            ...
            this.initialMediaAnalysisState = this.normalizedInitialCanvasState
                ...
                : {
                    state: this.normalizedInitialCanvasState,
                    changed: false,
                }
            ...
            this.nodeShells = new WorkspaceNodeShells({
                ...
                getBounds: node => ({
                    ...this.getNodeWorldPosition(node),
                    ...node.dimensions,
                }),
                ...
                consumeSuppressedClick: () => this.nodeGestures.consumeNodeClick(),
                ...
                togglePlayback: nodeId => {
                    if (this.canvasMediaLayer?.playback.hasEntry(nodeId))
            ...
            this.generationContext = new WorkspaceGenerationContext({
            ...
            this.documentNodes = new WorkspaceDocumentNodes(
                this.nodeShells,
                {
                    mountEditor: this.mountDocumentEditor,
                    onError: (error, nodeId) => console.error(
                        'Failed to create document editor:',
                        {
                            nodeId,
                            error,
                        },
            ...
            this.referenceProjection = new WorkspaceReferenceProjection({
                ...
                getDocumentTitles: () => new Map(
                    this.currentDocuments.map(document => [document.documentId, document.title]),
                ),
            ...
            this.branchMarkerPresentation = new WorkspaceBranchMarkerPresentation({
                ...
                getZoomScale: () => this.getBranchMarkerReviewZoomScale(
                ...
                openDetails: nodeId => this.outputDetails.open(
                    {
                        kind: 'branch-marker',
                        nodeId,
                    },
                ...
                log: (event, detail) => console.info(
                    '[CANVAS][branch-marker-info]',
                    event,
                    detail,
                ),
            })
            ...
            this.context = new WorkspaceCanvasContext({
                ...
                applyLocalPanelState: state => {
                    ...
                    if (this.currentCanvasState)
                        this.currentCanvasState = setAiChatPanelState(this.currentCanvasState, state)
                },
            ...
            this.libraries = new WorkspaceCanvasLibraries({
                ...
                insertNode: node => this.insertNodeAtViewportCenterInternal(
                    node,
                    {},
                    false,
                ),
            ...
            this.mediaTrackers = new WorkspaceMediaTrackers({
                readScope: () => !this.rendererDestroyed
                    && this.workspaceId
                    ? {
            ...
            this.generationVisuals = new WorkspaceGenerationVisuals({
                ...
                onFinalized: nodeId => {
                    ...
                    if (nodeEl)
                        this.updatePendingGeneratedMediaBeforeFrameClass(nodeEl, nodeId)
            ...
            this.domNodes = new WorkspaceDomNodes({
                shells: this.nodeShells,
                document: node => this.documentNodes.create(
                    node,
                    this.currentDocuments.find(document => document.documentId === node.assetId),
                ),
            ...
            this.mediaGeometry = new WorkspaceMediaGeometry({
                ...
                toParentRelativePosition: (
                    ...
                    parentId,
                    nodesById,
                ) => this.workspaceGeometry.toParentRelativePosition(
                    position,
                    parentId,
                    nodesById,
                ),
                ...
                commit: (state, options) => {
                    if (options.preserveEditors)
                        this.commitCanvasStatePreservingEditors(state)
                    else
                        this.commitCanvasState(state)
                },
            ...
            this.canvasMediaLayer = createWorkspaceMediaLayer({
                    ...
                    geometry: type => ({
                        measure: projected => {
                            ...
                            const pending = this.generationVisuals.isFinalizing(node.nodeId)
                                ? null
                                : this.getPendingGeneratedMediaBeforeFrameVisualGeometry(
                                    node.nodeId,
                                    node.position,
                                    node.dimensions,
                                )
                        ...
                        resize: {
                            min: {
                                width: type === 'image' ? 50 : 200,
                                height: type === 'image' ? 1 : 150,
                            },
                ...
                marker: {
                    paths: this.connectorIcon.pathData,
                    width: this.connectorIcon.width,
                    reference: {
                        x: 48,
                        y: 128,
                    },
            ...
            this.markerHandoff = new WorkspaceBranchMarkerHandoff({
                ...
                hasElement: nodeId => Boolean(
                    this.findBranchMarkerNodeEl(nodeId),
                ),
            ...
            this.apiCanvasGeometry = new WorkspaceApiCanvasGeometry({
                ...
                preserveUntilAcknowledged: state => void (this.pendingLocalCanvasVisualCommit = createPendingCanvasVisualCommit(state)),
                log: (event, details) => {
            ...
            this.mediaOperationRecovery = new WorkspaceMediaOperationRecovery({
                ...
                apply: (result, progressOnly) => {
                    if (progressOnly)
                        this.operationStatusNodes.applyProgress(result)
                    else
                        this.operationStatusNodes.applyRecovery(result)
                },
            ...
            this.conversationProjection = new WorkspaceConversationProjection<AiChatThread>({
                readScope: () => !this.rendererDestroyed
                    ...
                    : null,
                getThreads: () => this.currentAiChatThreads,
                setThreads: threads => void (this.currentAiChatThreads = threads),
                getNodes: () => this.currentCanvasState?.nodes ?? [],
                ...
                now: Date.now,
                reportError: (error, threadId) => console.error(
                    '[CANVAS] Conversation refresh failed:',
                    {
                        threadId,
                        error,
                    },
            ...
            this.threadState = new WorkspaceCanvasThreadState({
                ...
                merge: (
                    ...
                    state,
                    workspaceChanged,
                ) => this.conversationProjection.merge(
                    threads,
                    state,
                    workspaceChanged,
                ),
            ...
            this.canvasGenerationSubmission = new CanvasGenerationSubmission({
                ...
                createId: this.host.createId,
                now: Date.now,
                createConversation: request => this.host.assets.create({
                    ...request,
                ...
                install: request => {
                    const thread: AiChatThread = {
                        ...request.thread,
                        status: request.thread.status === 'none' ? 'idle' : request.thread.status,
                    }
                    ...
                    this.createDetachedCanvasThreadEditor({
                        ...request,
                        thread,
                    })
            ...
            this.mediaAnalysis = new WorkspaceMediaAnalysis({
                ...
                loadWorkspaceAssets: async capturedWorkspaceId => void (await this.host.assets.loadWorkspaceAssets(capturedWorkspaceId)),
                refreshVideo: node => {
                    this.canvasMediaLayer?.retryAssetTextures(
                        new Set([node.assetId]),
                    )
                    this.syncCanvasMediaLayer(this.currentCanvasState)
            ...
            this.outputReview = new WorkspaceOutputReview({
                ...
                readArtifactReplay: node => this.host.capabilities.frontend.require(node.artifactTypeId).buildReplaySubmitData(
                    { provenance: this.assets.getArtifactProvenance(node) },
                ),
            ...
            this.callbacks.own(
                this.canvasRuntime.scene.subscribeScene(scene => {
                    if (
                        this.rendererDestroyed
                        || scene.sceneKey === this.projectionSceneKey
                    )
                        return
            ...
            this.videoChrome = new WorkspaceVideoChrome({
                ...
                getBounds: node => this.canvasMediaLayer?.getNodeBounds(node.nodeId) ?? {
                    ...this.getNodeWorldPosition(node),
                    ...node.dimensions,
                },
                getViewport: this.getLiveViewport,
                ...
                startResize: this.handleResizeStart,
                togglePlayback: nodeId => void this.canvasMediaLayer?.playback.toggle(nodeId),
            })
            ...
            this.outputChrome = new WorkspaceOutputChrome({
                ...
                getState: () => this.currentCanvasState,
                getViewport: this.getLiveViewport,
                getBounds: (node, nodesById) => ({
                    ...this.getNodeWorldPosition(node, nodesById),
                    ...(this.liveNodeOverrides.get(node.nodeId)?.dimensions ?? node.dimensions),
                }),
                getPendingBounds: (nodeId, bounds) => {
                    const circle = this.getPendingGeneratedMediaBeforeFrameCircleGeometry(
                        nodeId,
                        bounds,
                        bounds,
                    )
                ...
                isSelected: nodeId => this.outputDetails.targetsMatch(
                ...
                mountTitle: (node, host) => this.canvasAssetViews.mountMetadata(
                    node,
                    host,
                    'node',
                ),
                queueAnalysis: node => this.queueCanvasMediaAnalysis(
                    node.nodeId,
                    this.getMediaDescriptorStillAssetId(node),
                ),
                onOpenDetails: nodeId => this.outputDetails.open(
                    {
                        kind: 'output',
                        nodeId,
                    },
                ...
                onAccept: nodeId => void this.outputDetails.accept('output-node', nodeId),
                onReject: nodeId => void this.deleteCanvasNodes(
                    new Set([nodeId]),
                ),
                onRegenerate: node => void this.outputDetails.regenerate({
                    scope: 'output-node',
                ...
                onSync: event => {
                    if (this.debugLoggingEnabled)
                        console.info(
                            '[CANVAS][generated-media-chrome]',
                            event.rebuilt ? 'sync-rebuild' : 'sync-skip-same-key',
                            event,
                        )
                },
            })
            ...
            this.nodeDeletion = new WorkspaceNodeDeletion({
                ...
                readState: () => this.currentCanvasState,
                getAsset: assetId => this.host.assets.read(assetId),
                clearSelection: () => this.selectionController.setNodes(
                    new Set(),
                ),
            ...
            this.interactions = new WorkspaceCanvasInteractions({
                ...
                clientToWorld: (clientX, clientY) => this.canvasRuntime.clientToWorld({
                    x: clientX,
                    y: clientY,
                }),
            ...
            this.generationHandlers = new WorkspaceGenerationHandlers(
                this.generationEvents,
                {
                    ...
                    refreshAsset: async (assetId, sourceWorkspaceId) => {
                        ...
                        if ('error' in refreshed)
                            throw new Error(`GENERATED_ASSET_REFRESH_FAILED:${refreshed.error}`)
                    },
                    ...
                    hasNodeElement: nodeId => Boolean(
                        this.viewportEl.querySelector(`[data-node-id="${nodeId}"]`),
                    ),
            ...
            this.branchMarkerGeneration = new WorkspaceBranchMarkerGeneration({
                ...
                settleRequest: (
                    ...
                    generationRequestId,
                    options,
                ) => this.settleMediaGenerationRequest(
                    threadId,
                    generationRequestId,
                    undefined,
                    options,
            ...
            this.operationStatusNodes = new WorkspaceOperationStatusNodes({
                ...
                getWorkspaceId: () => this.workspaceId,
                getState: () => this.currentCanvasState,
                replaceState: state => void (this.currentCanvasState = state),
                captureAdmission: this.captureSceneAdmission,
                ...
                ensureRecovery: node => void this.mediaOperationRecovery.ensure(node),
                addContext: this.context.add,
                getComposer: () => this.globalCanvasComposer,
            })
            ...
            this.panZoomConfig = {
                ...defaultPanZoomConfig(transform => {
                    ...
                    if (this.persistedViewportApplied)
                    ...
                    if (zoomChanged) {
                        this.branchMarkerPresentation.updateZoom(
                            this.getBranchMarkerReviewZoomScale(vp.zoom),
                        )
            ...
            this.rendering = new WorkspaceCanvasRendering({
                getWorkspaceId: () => this.workspaceId,
                setWorkspaceId: workspaceId => void (this.workspaceId = workspaceId),
                getRenderedWorkspaceId: () => this.renderedWorkspaceId,
                setRenderedWorkspaceId: workspaceId => void (this.renderedWorkspaceId = workspaceId),
                ...
                setPendingVisualCommit: commit => void (this.pendingLocalCanvasVisualCommit = commit),
                getState: () => this.currentCanvasState,
                setState: state => void (this.currentCanvasState = state),
                setDocuments: documents => void (this.currentDocuments = documents),
                setThreads: threads => void (this.currentAiChatThreads = threads),
                getPanelState: () => this.aiChatPanelState,
                getKeys: () => ({
                ...
                setKeys: keys => {
                    if (keys.nodeStructure !== undefined)
                        this.lastNodeStructureKey = keys.nodeStructure

                    if (keys.visual !== undefined)
                        this.lastVisualSyncKey = keys.visual

                    if (keys.documents !== undefined)
                        this.lastDocumentsKey = keys.documents

                    if (keys.threads !== undefined)
                        this.lastThreadsKey = keys.threads
                },
                ...
                markPersistedViewportApplied: () => void (this.persistedViewportApplied = true),
                isDebugEnabled: () => this.debugLoggingEnabled,
                debug: (event, details) => console.info(
                    '[CANVAS][render-state]',
                    event,
                    details,
                ),
            ...
            this.canvasRuntime.installKeyboard({
                ...
                onDelete: () => {
                    ...
                    if (!this.selectionController.selectedEdgeId)
                        return false
            ...
            if (
                this.initialStaleMediaAnalysisReset
                && this.currentCanvasState
            ) {
            ...
            this.unsubscribeAssetsStore = this.host.assets.subscribe(({ items }) => {
                ...
                const nextAssetRevisions = new Map(
                    Array.from(items, ([assetId, asset]) => [assetId, asset.revision]),
                )
                ...
                for (const [assetId, revision] of nextAssetRevisions) {
                    if (this.observedAssetRevisions.get(assetId) !== revision)
                ...
                if (
                    detailsAssetId
                    && changedAssetIds.has(detailsAssetId)
            ...
            this.unsubscribeWorkspaceStore = this.host.workspace.subscribe(({
                loadingStatus,
                error,
            }) => {
                if (this.rendererDestroyed)
                ...
                if (loadingStatus === LoadingStatus.error) {
                    ...
                    this.workspaceLoadingOutline?.setErrorMessage(
                        this.getWorkspaceLoadErrorMessage(error),
                    )
    ...
    private resetStaleAnalyzingMediaDescriptors = (canvasState: CanvasState): {
        state: CanvasState
        changed: boolean
    } => ({
        state: canvasState,
    ...
    private isCurrentScene = (
        originWorkspaceId: string,
        originSceneKey: string,
    ): boolean => !this.rendererDestroyed && this.workspaceId === originWorkspaceId && this.canvasRuntime.scene.scene.sceneKey === originSceneKey

    private deleteCanvasNodes = async (nodeIds: ReadonlySet<string>): Promise<void> => void (await this.nodeDeletion.deleteCanvasNodes(nodeIds))

    private getCanvasNodesById = (nodes: CanvasNode[] = this.currentCanvasState?.nodes ?? []): Map<string, CanvasNode> => new Map(
    ...
    private getNodeWorldPosition = (
        ...
        x: number
        y: number
    } => this.liveNodeOverrides.worldPosition(node, nodesById)

    private getNodeWorldRect = (
        node: CanvasNode,
        nodesById: Map<string, CanvasNode> = this.getCanvasNodesById(),
    ): Rect => this.liveNodeOverrides.worldBounds(node, nodesById)
    ...
    private syncCanvasNodeDomGeometry = (nodes: CanvasNode[]): void => {
        ...
        for (const node of nodes) {
            ...
            this.canvasMediaLayer?.setNodeLiveTransform(
                node.nodeId,
                position,
                dimensions,
            )
            ...
            this.updateGeneratedMediaChromeLiveTransform(
                node.nodeId,
                position,
                dimensions,
                this.getLiveViewport(),
            )
        }
    ...
    private findBranchMarkerNodeEl = (nodeId: string): HTMLElement | null =>
        this.viewportEl.querySelector(`[data-node-id="${nodeId}"]`) as HTMLElement | null

    private findBranchMarkerNodeElForNode = (node: BranchMarkerNode): HTMLElement | null => this.findBranchMarkerNodeEl(node.nodeId)
    ...
    private updateGeneratedMediaChromeLayout = (viewport: Viewport = this.getLiveViewport()): void => void this.outputChrome.layout(viewport)
    ...
    private resetGeneratedMediaChromeSyncKey = (): void => void this.outputChrome.invalidate()

    private scheduleGeneratedMediaChromeSync = (): void => void this.outputChrome.schedule()

    private syncGeneratedOutputNodeFooters = (canvasState: CanvasState | null): void => void this.outputChrome.updateState(canvasState)

    private syncGeneratedMediaChrome = (canvasState: CanvasState | null = this.currentCanvasState): void => void this.outputChrome.sync(canvasState)

    private syncGeneratingMediaNodes = (canvasState: CanvasState | null = this.currentCanvasState): void =>
        void this.generationVisuals.sync(canvasState)

    private clearFinalizingGeneratedImageOutline = (nodeId: string): void => void this.generationVisuals.clearCompletion(nodeId)

    private getPendingGeneratedMediaBeforeFirstFrameNodeIds = (): Set<string> => this.generationVisuals.pendingNodeIds()

    private isGeneratedMediaCanvasNodeWaitingForFrame = (node: CanvasNode): node is ImageCanvasNode | VideoCanvasNode =>
        this.generationVisuals.isWaitingForFrame(node)

    private isPendingGeneratedMediaBeforeFirstFrame = (nodeId: string): boolean => this.generationVisuals.isPending(nodeId)

    private updatePendingGeneratedMediaBeforeFrameClass = (
        nodeEl: HTMLElement,
        nodeId: string,
    ): void => void this.generationVisuals.updateHitArea(nodeEl, nodeId)
    ...
    private getCanvasVisibleAreaForApiProjection = (): {
        width: number
        height: number
    } | undefined => this.workspaceGeometry.getCanvasVisibleAreaForApiProjection()

    private getCenteredInsertionPosition = (dimensions: {
        width: number
        height: number
    }): {
        x: number
        y: number
    } => this.workspaceGeometry.getCenteredInsertionPosition(dimensions)
    ...
    ): Rect => this.workspaceGeometry.getCanvasNodeCollisionRect(node, worldPosition)

    private resolveTopLevelNodeCollisions = (nodes: CanvasNode[]): CanvasNode[] => this.workspaceGeometry.resolveTopLevelNodeCollisions(nodes)
    ...
    private createGeneratedMediaRebalancePipeline = (): GeneratedMediaRebalancePipeline =>
        this.workspaceGeometry.createGeneratedMediaRebalancePipeline()

    private rebalanceGeneratedMediaTrees = (
        nodes: CanvasNode[],
        edges: WorkspaceEdge[],
    ): CanvasNode[] => {
    ...
    ): CanvasGeometry | null => this.workspaceGeometry.getPendingGeneratedMediaBeforeFrameCircleGeometry(
        nodeId,
        position,
        dimensions,
    )

    private getPendingGeneratedMediaBeforeFrameCircleInset = (dimensions: {
        width: number
        height: number
    }): {
        x: number
        y: number
        size: number
    } => this.workspaceGeometry.getPendingGeneratedMediaBeforeFrameCircleInset(dimensions)
    ...
    private getPendingGeneratedMediaBeforeFrameVisualGeometry = (
        ...
        const circleGeometry = this.getPendingGeneratedMediaBeforeFrameCircleGeometry(
            nodeId,
            position,
            dimensions,
        )
    ...
    private getLiveViewport = (): Viewport => ({
        x: this.lastTransform[0],
        y: this.lastTransform[1],
        zoom: this.lastTransform[2],
    })

    private ensureActiveRightSidePanel = (): void => void this.rightPanel.ensure()
    ...
    private persistAiChatSidebarState = (): void => {
        ...
        if (JSON.stringify(this.currentCanvasState.aiChatPanel) === JSON.stringify(nextCanvasState.aiChatPanel))
            return
    ...
    private createGlobalCanvasComposer = (): void => {
        ...
        this.globalCanvasComposer = new WorkspacePromptComposer({
            ...
            mountContextTray: () => {
                ...
                return {
                    element,
                    destroy: () => this.context.releaseTray(element),
                }
            },
            appearance: {
            ...
            onSubmit: data => void this.submitCanvasGenerationRun(data),
        })
        ...
        this.canvasMediaLayer?.setGlassTargets([
            ...(this.options.glassTargets ?? []),
            {
                ...
                element: this.globalCanvasComposer.input.element,
            },
        ])
        this.context.refresh()
    }

    private teardownDetachedCanvasRun = (threadId: string): void => void this.detachedAiChatThreadEditors.teardown(threadId)
    ...
    private scheduleDetachedCanvasRunTeardown = (threadId: string): void => void this.detachedAiChatThreadEditors.defer(threadId, 1500)

    private settleDetachedCanvasRun = (threadId: string): void => void this.detachedAiChatThreadEditors.settle(threadId)
    ...
    private createDetachedCanvasThreadEditor = ({
        ...
        return this.detachedAiChatThreadEditors.mount(
            threadId,
            scope =>
                new CanvasConversationRun(
                    scope,
                    {
                    ...
                    },
                    {
                        ...
                        now: () => Date.now(),
                        publishContent: content => this.onAiChatThreadContentChange?.({
                            workspaceId: runWorkspaceId,
                            threadId,
                            content,
                        }),
                        rememberContent: (
                            ...
                            content,
                            streaming,
                        ) => this.conversationProjection.rememberContent(
                            id,
                            content,
                            streaming,
                        ),
                        ...
                        preflight: (
                            ...
                            if (regeneration?.mode === 'existing-prompt') {
                                ...
                                this.insertPendingBranchMarkerForCanvasRun(
                                    threadId,
                                    placement.promptText,
                                    data,
                                )
                            }
                        },
    ...
    private getActiveDetachedCanvasRunThreadIds = (): string[] => {
        ...
        const threadsById = new Map(
            this.currentAiChatThreads.map(thread => [thread.threadId, thread]),
        )
        ...
        if (this.currentCanvasState) {
            for (const node of this.currentCanvasState.nodes) {
                ...
                if (!this.threadState.isDetached(threadId))
                    continue

                if (this.detachedAiChatThreadEditors.isSettled(threadId))
                    continue
                ...
                if (
                    !this.isBranchMarkerGenerationActive(node)
                    && !this.threadState.hasInProgressContent(thread)
                )
                    continue
        ...
        for (const thread of this.currentAiChatThreads) {
            if (!this.threadState.isDetached(thread.threadId))
                continue

            if (this.detachedAiChatThreadEditors.isSettled(thread.threadId))
                continue

            if (thread.workspaceId !== this.workspaceId)
                continue

            if (this.threadState.hasCanvasProjection(thread.threadId))
                continue

            if (!this.threadState.isRecentUpdate(thread))
                continue

            if (!this.threadState.hasRecoverableTurn(thread))
                continue
    ...
    private reattachDetachedCanvasRunListenersForActiveMarkers = (): void => {
        ...
        for (const threadId of this.getActiveDetachedCanvasRunThreadIds()) {
            if (this.detachedAiChatThreadEditors.isActive(threadId))
                continue

            if (this.detachedAiChatThreadEditors.has(threadId))
                continue
    ...
    private submitPersistedDetachedCanvasThreadMessage = (threadId: string): void =>
        void this.detachedAiChatThreadEditors.get(threadId)?.submitPersisted()
    ...
    ): Promise<void> => void (await this.canvasGenerationSubmission.submit(data, options))

    private isBranchMarkerGenerationCancelled = (node: BranchMarkerNode): boolean => this.generationPlacements.isBranchMarkerGenerationCancelled(node)

    private getBranchMarkerUiPhase = (node: BranchMarkerNode): BranchMarkerUiPhase | undefined =>
        this.generationPlacements.getBranchMarkerUiPhase(node)

    private isBranchMarkerPendingForUi = (node: BranchMarkerNode): boolean => this.generationPlacements.isBranchMarkerPendingForUi(node)

    private getGeneratedMediaPlacementKey = (
        threadId: string,
        generationRun?: MediaGenerationRunMeta,
    ): string => this.generationPlacements.getGeneratedMediaPlacementKey(threadId, generationRun)

    private getGeneratedMediaRunKey = (
        threadId: string,
        generationRun?: MediaGenerationRunMeta,
    ): string => this.generationPlacements.getGeneratedMediaRunKey(threadId, generationRun)

    private deletePendingBranchMarkerAliasesForNodeId = (nodeId: string): void =>
        void this.generationPlacements.deletePendingBranchMarkerAliasesForNodeId(nodeId)
    ...
    private hasStartedGeneratedMediaForBranchMarkerNode = (nodeId: string): boolean => {
        for (const tracker of this.partialImageTracker.values()) {
            if (tracker.sourceNodeId === nodeId)
                return true
        }
    ...
    private debugBranchMarkerHandoff = (
        ...
        if (this.branchMarkerHandoffDebugKeys.has(key))
            return

        this.branchMarkerHandoffDebugKeys.add(key)
        console.info(
            '[CANVAS][branch-marker-handoff]',
            event,
            {
    ...
    private restoreDetachedCanvasPreflightMarkersForActiveThreads = (): void =>
        void this.preflightMarkers.restoreDetachedCanvasPreflightMarkersForActiveThreads()

    private insertPendingBranchMarkerForCanvasRun = (
        placementKey: string,
        promptText: string,
        data: AiPromptComposerSubmitData,
    ): void => void this.preflightMarkers.insertPendingBranchMarkerForCanvasRun(
        placementKey,
        promptText,
        data,
    )
    ...
    ): PendingGeneratedImagePlacement | undefined => this.generationPlacements.ensurePendingGeneratedMediaPlacementForApiRun(
        threadId,
        generationRun,
        seed,
    )
    ...
    ): void => void this.generationPlacements.setPendingGeneratedMediaPlacement(
        threadId,
        generationRun,
        placement,
    )

    private removePendingBranchMarkerForRun = (
        threadId: string,
        generationRun?: MediaGenerationRunMeta,
    ): void => void this.markerHandoff.removePendingBranchMarkerForRun(threadId, generationRun)

    private settleBranchMarkersForGenerationRequest = (
        generationRequestId: string,
        options: BranchMarkerSettlementOptions = {},
    ): void => void this.generationSettlement.settleBranchMarkersForGenerationRequest(generationRequestId, options)
    ...
    private settleMediaGenerationRequest = (
        ...
        generationRun?: MediaGenerationRunMeta,
        options: BranchMarkerSettlementOptions = {},
    ): void => void this.generationSettlement.settleMediaGenerationRequest(
        threadId,
        generationRequestId,
        generationRun,
        options,
    ...
    private clearPendingGeneratedMediaPlacementsForThread = (threadId: string): void =>
        void this.generationSettlement.clearPendingGeneratedMediaPlacementsForThread(threadId)

    private findCanvasNodeById = (nodeId: string | undefined): CanvasNode | undefined => this.lineageProjection.findCanvasNodeById(nodeId)

    private getExistingMediaNodeIds = (nodeIds: Iterable<string | null | undefined>): string[] =>
        this.lineageProjection.getExistingMediaNodeIds(nodeIds)
    ...
    private appendBranchMarkerNodeToDOM = (
        ...
        if (placed.type === 'branchFork')
            this.appendCanvasNodeToDOM(placed as BranchForkCanvasNode)
        else if (placed.type === 'branchLine')
            this.appendCanvasNodeToDOM(placed as BranchLineCanvasNode)
    }

    private setGeneratingReferenceNodeIds = (
        threadId: string,
        referenceNodeIds: Iterable<string>,
    ): void => {
    ...
    private clearGeneratingReferenceNodeIds = (threadId: string): void => {
        if (!this.generationVisuals.removeReferences(threadId))
            return

        this.syncGeneratingMediaNodes()
    }

    private clearGeneratingReferenceNodeIdsForPromptHandoff = (
        threadId: string,
        generationRun?: MediaGenerationRunMeta,
    ): void => {
        ...
        const expectedRunKeys = placement?.lineagePlan?.runAssignments.map(assignment => assignment.mediaRunId ?? assignment.reasoningRunId).filter(
            (runKey): runKey is string => Boolean(runKey),
        )
            ?? []
        ...
        if (
            ...
            promptHandoffRunKeys.add(
                this.getGeneratedMediaRunKey(threadId, generationRun),
            )
            this.pendingGeneratedImagePlacements.set(
                placementKey,
                {
                    ...placement,
            ...
            if (expectedRunKeys.some(runKey => !promptHandoffRunKeys.has(runKey)))
                return
        }
    ...
    private clearGeneratingReferencesOnFirstPixels = (
        threadId: string,
        generationRun?: MediaGenerationRunMeta,
    ): void => void this.clearGeneratingReferenceNodeIdsForPromptHandoff(threadId, generationRun)

    private clearGeneratingReferencesAfterPromptHandoff = (
        threadId: string,
        generationRun?: MediaGenerationRunMeta,
    ): void => void this.clearGeneratingReferenceNodeIdsForPromptHandoff(threadId, generationRun)

    private getPersistedAiChatThread = (threadId: string): AiChatThread | undefined => this.conversationProjection.get(threadId)

    private schedulePersistedAiChatThreadRefreshForBranchMarkers = (threadId: string): void => void this.conversationProjection.schedule(threadId)

    private getAiChatThreadContentForProjection = (threadId: string): unknown => this.conversationProjection.content(threadId)

    private getGeneratedMediaHistoryContent = (node: ImageCanvasNode | VideoCanvasNode): unknown =>
        this.workspaceHistory.getGeneratedMediaHistoryContent(node)

    private isBranchMarkerGenerationActive = (node: BranchMarkerNode): boolean => this.branchActivity.isBranchMarkerGenerationActive(node)

    private isBranchMarkerGenerationGroupActive = (node: BranchMarkerNode): boolean => this.branchActivity.isBranchMarkerGenerationGroupActive(node)

    private getBranchMarkerConversationPreview = (node: BranchMarkerNode): BranchMarkerConversationPreview | null =>
        this.workspaceHistory.getBranchMarkerConversationPreview(node)
    ...
    private refreshGeneratedMediaProjectionsForAiChatThread = (threadId: string): void => {
        ...
        if (
            detailsThreadId !== threadId
            || !this.rightPanel.element
        )
            return

        if (this.generatedOutputDetailsRefreshRaf !== null)
            return
        ...
        this.generatedOutputDetailsRefreshRaf = this.window.requestAnimationFrame(() => {
            ...
            if (
                currentDetailsThreadId !== threadId
                || !this.rightPanel.element
            )
                return
    ...
    private buildWorkspaceContextTitlesByNodeId = (nodes: CanvasNode[]): Record<string, string> =>
        this.referenceProjection.buildWorkspaceContextTitlesByNodeId(nodes)

    private isGeneratedMediaNode = (node: CanvasNode): node is ImageCanvasNode | VideoCanvasNode => (node.type === 'image' || node.type === 'video') && Boolean(
        node.generatedBy?.branchId,
    )

    private pruneOrphanBranchMarkers = (
        nodes: CanvasNode[],
        edges: WorkspaceEdge[],
    ): {
        nodes: CanvasNode[]
        ...
        for (const node of nodes) {
            ...
            if (node.generatedBy?.branchOriginNodeId)
                referencedOriginNodeIds.add(node.generatedBy.branchOriginNodeId)

            if (node.generatedBy?.branchForkNodeId)
                referencedForkNodeIds.add(node.generatedBy.branchForkNodeId)

            if (node.generatedBy?.branchLineNodeId)
                referencedLineNodeIds.add(node.generatedBy.branchLineNodeId)
        }
        ...
        const prunedNodes = nodes.filter((node: CanvasNode) => {
            ...
            if (shouldRemove)
                removedMarkerNodeIds.add(node.nodeId)

            return !shouldRemove
        })

        if (removedMarkerNodeIds.size === 0)
            return {
                nodes,
                edges,
            }
    ...
    private resolveGeneratedMediaTreeState = (
        nodes: CanvasNode[],
        edges: WorkspaceEdge[],
    ): {
        nodes: CanvasNode[]
    ...
    private updatePendingGeneratedImageReferencesFromWorkspaceContext = (
        ...
        const forcedChipNodeIds = resolution.selections.filter((selection: WorkspaceContextSelection) => selection.role === 'forced-chip').map(
            (selection: WorkspaceContextSelection) => selection.nodeId,
        )
        ...
        this.setPendingGeneratedMediaPlacement(
            threadId,
            generationRun,
            {
                ...placement,
        ...
        this.setGeneratingReferenceNodeIds(
            this.getGeneratedMediaPlacementKey(threadId, generationRun),
            referenceNodeIds,
        )
    }

    private handleWorkspaceContextResolution = (
        threadId: string | undefined,
        resolution: WorkspaceContextResolution,
        generationRun?: MediaGenerationRunMeta,
    ): void => {
        this.patchWorkspaceContextImprovedDescriptors(resolution.improvedDescriptors)
        this.updatePendingGeneratedImageReferencesFromWorkspaceContext(
            threadId,
            resolution,
            generationRun,
        )
    ...
    private queueCanvasMediaAnalysis = (
        nodeId: string,
        stillAssetId: string | undefined,
    ): void => void this.mediaAnalysis.queue(nodeId, stillAssetId)

    private getMediaDescriptorStillAssetId = (node: ImageCanvasNode | VideoCanvasNode): string | undefined => node.assetId || undefined

    private debugGeneratedMediaLifecycle = (
        event: string,
        details: Record<string, unknown>,
    ): void => {
        if (this.debugLoggingEnabled)
            console.info(
                '[CANVAS][generated-media]',
                event,
                details,
            )
    }
    ...
    private preserveActiveGeneratedMediaTrackersInState = (state: CanvasState | null): CanvasState | null =>
        this.mediaTrackers.preserveActiveGeneratedMediaTrackersInState(state)
    ...
    private syncConnectionManagerForCurrentCanvasState = (options: { flushRenderer?: boolean } = {}): void => {
        ...
        if (options.flushRenderer)
            this.canvasMediaLayer?.renderNow()
    }

    private syncConnectionsAfterManualNodeAppend = (): void => void this.syncConnectionManagerForCurrentCanvasState({ flushRenderer: true })

    private appendCanvasNodeToDOM = (node: CanvasNode): void => {
        if (!this.currentCanvasState?.nodes.some(candidate => candidate.nodeId === node.nodeId))
            return
    ...
    private syncExistingOperationStatusNodeToDOM = (node: OperationStatusCanvasNode): void => {
        ...
        if (
            this.currentCanvasState
            && !this.visibility.shouldRenderOperation(node, this.currentCanvasState)
        )
    ...
    private replaceUploadPlaceholderInternal = (
        ...
        const placeholderNode = this.currentCanvasState.nodes.find(
            (candidate: CanvasNode): candidate is OperationStatusCanvasNode =>
        ...
        const nodes = this.resolveTopLevelNodeCollisions(
            this.currentCanvasState.nodes.map(
                (candidate: CanvasNode): CanvasNode => (candidate.nodeId === placeholderNodeId ? preparedNode : candidate),
            ),
        )
        const edges = this.currentCanvasState.edges.map(
            (edge: WorkspaceEdge): WorkspaceEdge => ({
                ...edge,
        ...
        if (commit) {
            ...
            if (
                preparedNode.type === 'image'
                || preparedNode.type === 'video'
            )
                this.queueCanvasMediaAnalysis(
                    preparedNode.nodeId,
                    this.getMediaDescriptorStillAssetId(preparedNode),
                )
        }
    ...
    private markUploadPlaceholderFailedInternal = (
        placeholderNodeId: string,
        message: string,
    ): CanvasState | null => {
        if (!this.currentCanvasState)
    ...
    private commitTransientCanvasNodeInsertionToScene = (
        ...
        if (
            insertedNode.type === 'image'
            || insertedNode.type === 'video'
        )
            this.queueCanvasMediaAnalysis(
                insertedNode.nodeId,
                this.getMediaDescriptorStillAssetId(insertedNode),
            )
    }
    ...
    private pruneApiCanvasRemovedGeneratedMediaTrackers = (nodeIds: Iterable<string>): void =>
        void this.mediaTrackers.pruneApiCanvasRemovedGeneratedMediaTrackers(nodeIds)

    private applyApiCanvasGeometry = (canvasGeometry: CanvasGeometryUpdate): void =>
        void this.apiCanvasGeometry.applyApiCanvasGeometry(canvasGeometry)

    private suspendPanZoomForNodePointer = (nodeId: string): void => void this.nodeGestures.suspendPanLock(nodeId)

    private releasePanZoomForNodePointer = (): void => void this.nodeGestures.releasePanLock()

    private selectNode = (nodeId: string | null) => void this.selectionController.selectNode(nodeId)
    ...
    private scheduleTransformSideEffects = () => {
        if (this.transformSideEffectsRaf !== null)
            return
    ...
    private updateResizeHandles = (zoom: number) => void this.nodeShells.setZoom(zoom)

    private getCurrentViewportZoom = (): number => this.getLiveViewport().zoom

    private handleDragStart = (
        event: MouseEvent,
        nodeId: string,
        options: DragStartOptions = {},
    ) => void this.nodeGestures.startDrag(
        event,
        nodeId,
        options,
    )
    ...
    ) => void this.nodeGestures.startResize(
        event,
        nodeId,
        handlePosition,
    )
    ...
    private applyCapabilityArtifactHeight = (
        nodeId: string,
        ...
        const node = this.currentCanvasState.nodes.find(
            (candidate): candidate is CapabilityArtifactCanvasNode => (
        ...
        if (Math.abs(node.dimensions.height - height) <= 1)
            return

        const resizedNodes = this.currentCanvasState.nodes.map(
            candidate =>
    ...
    private createCapabilityArtifactNode = (node: CapabilityArtifactCanvasNode): HTMLElement => {
        return new WorkspaceCapabilityNode(
            node,
            this.nodeShells,
            {
                ...
                onError: (error, nodeId) => console.error(
                    'Failed to mount Capability Artifact:',
                    {
                        nodeId,
                        error,
                    },
    ...
    private getReferenceResolutionMediaKind = (assetId: string): MediaPromptReference['mediaKind'] | undefined =>
        this.referenceProjection.getReferenceResolutionMediaKind(assetId)
    ...
    private createBranchMarkerReferenceResolution = (operation: OperationStatusCanvasNode) => {
        ...
        const view = createBranchReferenceResolution({
            ...
            renderReference: createCanvasPromptReferenceRenderer({
                document: this.paneEl.ownerDocument,
                previewRenderer: this.context.getPromptReferencePreviewRenderer({ inlinePopover: true }),
                inlinePopover: true,
            }),
            resolveReference: request => this.host.generation.resolveReference({
                ...request,
                workspaceId: this.workspaceId,
            }),
        })
    ...
    private getBranchMarkerGeneratedOutputNodes = (node: BranchMarkerNode): GeneratedOutputCanvasNode[] =>
        this.workspaceHistory.getBranchMarkerGeneratedOutputNodes(node)

    private stopBranchMarkerGeneration = async (node: BranchMarkerNode): Promise<void> => void (await this.branchMarkerGeneration.stop(node))
    ...
    private applyCapabilityRunEventToBranchMarkers = (
        threadId: string,
        event: CapabilityRunEvent,
    ): void => {
        if (!this.capabilityProgressRuns.apply(threadId, event))
            return
    ...
    private createBranchMarkerGlobalProgress = (
        ...
        const state = buildBranchMarkerProgress({
            ...
            capabilityRuns: [...(this.capabilityProgressRuns.get(
    ...
    private renderNodes = () => {
        ...
        this.ensureConnectionManager()
        const shouldAnimatePanelOpenAfterRender = this.aiChatPanelState.isOpen
            && !this.rightPanel.element
            && this.rightPanel.hasRendered

        if (!this.rightPanel.isClosing)
            this.destroyActiveAiChatPanel(false)

        this.documentNodes.syncDocuments(this.currentDocuments)
        this.syncCanvasMediaLayer(this.currentCanvasState)

        const existingNodeIds = new Set(
            this.currentCanvasState.nodes.map((node: CanvasNode) => node.nodeId),
        )
    ...
    private clearWorkspaceVisualContent = (
        newDocuments: Document[],
        newAiChatThreads: AiChatThread[],
    ): void => {
        this.generationPlacements.clear()
        this.mediaAnalysis.clear()
        this.outputReview.clear()
    ...
    private initializePanZoom = () => {
        const initialViewport = this.currentCanvasState?.viewport || {
            x: 0,
            y: 0,
            zoom: 1,
        }
        ...
        this.panZoom = this.canvasRuntime.installViewport({
            ...
            minZoom: 0.1,
            maxZoom: 2,
            onDraggingChange: (dragging: boolean) => void this.paneEl.classList.toggle('is-dragging', dragging),
        })
    ...
    insertNodeAtViewportCenter = (
        ...
        return this.insertNodeAtViewportCenterInternal(
            node,
            statePatch,
            commit,
        )
    ...
    replaceUploadPlaceholder = (
        ...
        return this.replaceUploadPlaceholderInternal(
            placeholderNodeId,
            node,
            commit,
        )
    ...
    commitTransientCanvasNodeInsertion = (
        ...
        this.commitTransientCanvasNodeInsertionToScene(
            canvasState,
            nodeId,
            replacedPlaceholderNodeId,
        )
    }
    ...
    render = (
        newCanvasState: CanvasState | null,
        newDocuments: Document[],
        newAiChatThreads: AiChatThread[] = [],
    ...
    toggleMediaLibrary = () => {
        ...
        if (
            this.aiChatPanelState.isOpen
            && this.aiChatPanelState.topLevelMode === 'media'
        ) {
            void this.closeAiChatPanel()
    ...
    destroy = () => {
        ...
        destroyWorkspaceCanvasResources([
            ...
            () => void (this.globalCanvasComposer = null),
            ...
            () => void (this.workspaceLoadingOutline = null),
            () => this.workspaceLoadingOutline?.destroy(),
            () => void (this.canvasMediaLayer = null),
            () => this.canvasMediaLayer?.destroy(),
            () => void (this.mediaChromeViewportEl = null),
            () => this.mediaChromeViewportEl?.remove(),
            () => this.videoChrome?.destroy(),
            () => this.mediaGenerationProgressInstances?.clear(),
            () => destroyWorkspaceCanvasResources(
                Array.from(this.mediaGenerationProgressInstances?.values() ?? [], progress => () => progress.destroy()),
            ),
            ...
            () => {
                if (
                    this.outputChrome
                    && this.canvasAssetViews
                )
                    this.destroyGeneratedMediaChromeControls()
            },
            () => void (this.viewportBridge = null),
            () => void (this.connectionManager = null),
...
export const createWorkspaceCanvas = (
    options: WorkspaceCanvasOptions,
    host: WorkspaceCanvasHost,
): LixpiWorkspaceCanvas => new LixpiWorkspaceCanvas(options, host)
