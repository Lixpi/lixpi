...
export type WorkspaceMediaGeometryPorts = {
    ...
    getActiveGesture: () => { draggingNodeId: string | null; resizingNodeId: string | null }
    getWorldPosition: (node: CanvasNode, nodesById: Map<string, CanvasNode>) => { x: number; y: number }
    toParentRelativePosition: (worldPosition: { x: number; y: number }, parentId: string, nodesById: Map<string, CanvasNode>) => { x: number; y: number }
    rebalance: (nodes: CanvasNode[], edges: WorkspaceEdge[]) => CanvasNode[]
    commit: (state: CanvasState, options: { preserveEditors: boolean }) => void
...
export class WorkspaceMediaGeometry {
    ...
    handleVideoIntrinsicSize = (size: IntrinsicMediaSize): void => {
        ...
        if (!state || this.hasActiveGesture(size.nodeId)) return
        ...
        const nextState = this.resizeNode(state, node, aspectRatio)
        if (nextState) this.ports.commit(nextState, { preserveEditors: false })
    }
    ...
    handleImageIntrinsicSize = (size: IntrinsicMediaSize & { preserveNodeGeometry?: boolean }): void => {
        ...
        this.ports.clearImageCompletion(size.nodeId)
        if (size.preserveNodeGeometry || this.hasActiveGesture(size.nodeId)) return

        const nextState = this.resizeNode(state, node, aspectRatio)
        if (nextState) this.ports.commit(nextState, { preserveEditors: true })
    }
    ...
    private resizeNode(
        ...
        const previousAspectRatio = 'aspectRatio' in node && typeof node.aspectRatio === 'number' ? node.aspectRatio : 0
        ...
        if (!aspectChanged && !widthChanged && !heightChanged) return null

        const nodesById = new Map(state.nodes.map(candidate => [candidate.nodeId, candidate]))
        ...
        const position = node.parentId
            ? this.ports.toParentRelativePosition(nextWorldPosition, node.parentId, nodesById)
            : nextWorldPosition
        const nodes = state.nodes.map(candidate =>
            candidate.nodeId === node.nodeId
                ? { ...node, aspectRatio, position, dimensions }
                : candidate
        )
        const resolvedNodes = node.generatedBy
            ? this.ports.rebalance(nodes, state.edges)
            : nodes
        return { ...state, nodes: resolvedNodes }
    }
}
