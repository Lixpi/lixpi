...
export type WorkspaceBranchMarkerGenerationPorts = {
    ...
    getScene: () => { workspaceId: string; sceneKey: string }
    ...
    settleRequest: (threadId: string, generationRequestId: string, options: { preserveGeometry: boolean }) => void
    clearPlacements: (threadId: string) => void
    settleMarkers: (generationRequestId: string, options: { preserveGeometry: boolean }) => void
    settleConversation: (threadId: string) => void
    scheduleTeardown: (threadId: string) => void
    refreshMarkers: (threadId: string) => void
    stopConversation: (request: { workspaceId: string; conversationAssetId: string; generationRequestId?: string }) => Promise<{ canvasGeometry?: CanvasGeometryUpdate }>
...
export class WorkspaceBranchMarkerGeneration {
    ...
    stop = async (node: BranchMarkerNode): Promise<void> => {
        ...
        if (generationRequestId) this.ports.cancelledRequests.add(generationRequestId)
        this.removeActiveMedia(node)
        if (generationRequestId) {
            this.ports.settleRequest(threadId, generationRequestId, { preserveGeometry: true })
        } else {
        ...
        try {
            ...
            if (!this.ports.isCurrentScene(scene.workspaceId, scene.sceneKey)) return
            if (result.canvasGeometry) this.ports.applyGeometry(result.canvasGeometry)
            if (!this.ports.isCurrentScene(scene.workspaceId, scene.sceneKey)) return
            await this.ports.refreshConversation(threadId)
        } catch (error) {
            this.ports.reportError('[CANVAS] failed to stop branch-marker generation', {
                nodeId: node.nodeId,
                threadId,
                error,
            })
    ...
    private belongsToMarker(mediaNode: GeneratedMediaNode, markerNodeId: string): boolean {
    ...
    private trackerBelongsToMarker(tracker: PendingGeneratedMediaTracker, markerNodeId: string): boolean {
        if (tracker.sourceNodeId === markerNodeId) return true
        const mediaNode = this.ports.getState()?.nodes.find((node: CanvasNode): node is GeneratedMediaNode => (
    ...
    private getActiveMediaNodeIds(node: BranchMarkerNode): Set<string> {
        ...
        const trackerNodeIds = new Set<string>([
            ...Array.from(this.ports.imageTrackers.values(), tracker => tracker.nodeId),
            ...Array.from(this.ports.videoTrackers.values(), tracker => tracker.nodeId),
        ])
        ...
        for (const tracker of this.ports.imageTrackers.values()) {
            if (this.trackerBelongsToMarker(tracker, node.nodeId)) nodeIds.add(tracker.nodeId)
        }
