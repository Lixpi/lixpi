...
export type WorkspaceBranchMarkerPresentationPorts = {
    ...
    getPromptParts: (node: BranchMarkerNode, preview: BranchMarkerConversationPreview | null) => BranchMarkerPromptPart[]
    getPromptPreviewRenderer: () => PromptReferencePreviewRenderer
    showResponseLine: (node: BranchMarkerNode, preview: BranchMarkerConversationPreview | null) => boolean
    createProgress: (node: BranchMarkerNode, preview: BranchMarkerConversationPreview | null, responseText: string) => HTMLElement | null
    destroyProgress: (nodeId: string) => void
    createReferenceResolution: (operation: NonNullable<ReturnType<typeof getMediaGenerationReferenceResolutionForMarker>>) => { element: HTMLElement; destroy: () => void } | null
...
export class WorkspaceBranchMarkerPresentation {
    ...
    create = (node: BranchMarkerNode): HTMLElement => {
        const { nodeEl, dragOverlay, own } = this.ports.shells.createBranchMarker(node, () => this.handleInfoClick(node.nodeId))
        own(() => this.destroyNode(node.nodeId))
        dragOverlay.before(this.createContent(node))
        this.syncActions(node, nodeEl)
        return nodeEl
    }

    sync = (node: BranchMarkerNode, element?: HTMLElement): void => {
        ...
        if (dragOverlay) dragOverlay.before(content)
        else nodeElement.append(content)
        this.syncActions(node, nodeElement)
    }

    syncAll = (): void => {
        for (const node of this.ports.getState()?.nodes ?? []) {
            if (this.isMarker(node)) this.sync(node)
        }
    }
    ...
    clear(): void {
        const cleanup = new Lifetime()
        for (const nodeId of new Set([...this.contents.keys(), ...this.actions.keys()])) {
    ...
    private createContent(node: BranchMarkerNode): HTMLDivElement {
        ...
        const progress = this.ports.createProgress(node, preview, responseText)
        ...
        const content = new BranchMarkerContent({
            ...
            mediaModels: this.ports.models.getTooltipEntries(node).map(({ label, entry }) => ({
                ...entry,
                label,
            ...
            progress: progress ? { element: progress, destroy: () => this.ports.destroyProgress(node.nodeId) } : null,
    ...
    private syncActions(node: BranchMarkerNode, element: HTMLElement): void {
        ...
        const controls = new BranchMarkerActions({
            ...
            onStop: () => {
                void this.ports.stop(node)
            },
            onAcceptAll: () => {
                void this.ports.accept(node.nodeId)
            },
            onRegenerate: mode => {
                void this.ports.regenerate({ scope: 'branch-lineage', mode, targetNodeId: node.nodeId, outputNodes: outputs })
            },
        })
        this.actions.set(node.nodeId, controls)
        controls.setZoomScale(this.ports.getZoomScale())
        if (controls.stopControl) element.appendChild(controls.stopControl)
    ...
    private handleInfoClick(nodeId: string): void {
        ...
        if (!node || !this.isMarker(node)) {
            this.ports.log('info-click-missing-node', { nodeId })
            return
        }
        this.ports.log('info-click', {
            nodeId: node.nodeId,
    ...
    private getTypeLabel(node: BranchMarkerNode): string {
        if (this.ports.getUiPhase(node) === 'preflight') return 'Preparing branch'
