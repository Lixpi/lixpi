...
export class WorkspaceCanvasInteractions {
    ...
    constructor(private readonly ports: WorkspaceCanvasInteractionsPorts) {
        this.menu = new WorkspaceCanvasMenu({
            ...
            actions: {
                ...
                onDeleteNode: nodeId => void ports.deleteNodes(
                    new Set([nodeId]),
                ),
    ...
    destroy(): void {
        ...
        this.ports.pane.removeEventListener(
            'mousedown',
            this.handleMouseDown,
            true,
        )
    ...
    private downloadMedia = (nodeId: string): void => {
        ...
        if (!['mediaDocument', 'audio', 'image', 'video'].includes(node.type))
            return
    ...
    private hitTest(point: {
        ...
        for (let index = state.nodes.length - 1; index >= 0; index--) {
            ...
            if (!['image', 'video', 'document', 'branchOrigin', 'branchFork', 'branchLine'].includes(node.type))
                continue

            const worldRect = this.ports.getWorldRect(node, nodesById)
            const pendingCircle = this.ports.getPendingCircle(
                node.nodeId,
                worldRect,
                node.dimensions,
            )
            const rect = pendingCircle ? {
                ...pendingCircle.position,
                ...pendingCircle.dimensions,
            } : worldRect

            if (rectangleContainsPoint(rect, point))
                return node
        }
    ...
    private handleClick = (event: MouseEvent): void => {
        if (
            ...
            || this.ports.gestures.consumePaneClick()
        )
            return

        if (!this.ports.selection.isCanvasBackgroundTarget(event.target))
            return
    ...
    private handlePointerDown = (event: PointerEvent): void => {
        ...
        if (nodeId)
            this.ports.suspendPanZoom(nodeId)
    }

    private handleMouseMove = (event: MouseEvent): void => {
        if (this.ports.gestures.resizingNodeId)
            return
        ...
        this.hitTest(
            this.ports.clientToWorld(event.clientX, event.clientY),
        )
        this.ports.pane.style.cursor = ''
    }
