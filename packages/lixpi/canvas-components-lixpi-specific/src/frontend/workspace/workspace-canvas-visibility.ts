    ...
    getVisibleNodes(state: CanvasState): CanvasNode[] {
        ...
        for (const node of state.nodes) {
            ...
            if (ownership.suppressedNodeIds.has(node.nodeId)) {
                this.reportSuppressedMarker(node, markers, startedPlannedMarkerIds, ownership.visibleOwnerBySuppressedNodeId)
                continue
            }
            if (node.type === 'operationStatus') void this.ports.ensureOperation(node)
            if (node.type !== 'operationStatus' || this.shouldRenderOperation(node, state)) visible.push(node)
        }
        return visible
    }

    shouldRenderOperation(node: OperationStatusCanvasNode, state: CanvasState): boolean {
        if (isMediaGenerationReferenceResolutionOperation(node)) return false
        if (node.operation === 'media-generation' && node.status === 'failed') {
            const superseded = state.nodes.some(candidate => (
