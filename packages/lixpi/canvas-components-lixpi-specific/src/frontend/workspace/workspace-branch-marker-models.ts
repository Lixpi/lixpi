...
export class WorkspaceBranchMarkerModels {
    ...
    getReasoningModel(node: BranchMarkerNode): BranchMarkerModelEntry | null {
        const entries = this.uniqueEntries(
            this.getReasoningModelDescriptors(node)
                .map(descriptor => this.getModelEntry(descriptor.modelId, descriptor.modelProvider ?? ''))
    ...
    getDetails(node: BranchMarkerNode): BranchMarkerModelDetail[] {
        ...
        for (const descriptor of getBranchMarkerMediaModelCircleDescriptors(node, this.ports.getCanvasNodes())) {
        ...
        return Array.from(descriptorsByLabel.entries())
            .map(([label, descriptors]) => this.createDetail(label, descriptors))
            .filter((detail): detail is BranchMarkerModelDetail => Boolean(detail))
    }

    getDescriptors(node: BranchMarkerNode) {
        return getBranchMarkerMediaModelCircleDescriptors(node, this.ports.getCanvasNodes())
    }
    ...
    getTooltipEntries(node: BranchMarkerNode): Array<{ label: string; entry: BranchMarkerModelEntry }> {
        return getBranchMarkerMediaModelCircleDescriptors(node, this.ports.getCanvasNodes())
            .map((descriptor) => {
    ...
    private splitModelId(modelId: string): { provider: string; model: string } {
    ...
    private findModelMeta(modelId: string, modelProvider: string): BranchMarkerModelCatalogEntry | null {
        const { provider, model } = this.splitModelId(modelId)
    ...
    private getModelEntry(modelId: string, modelProvider = ''): BranchMarkerModelEntry | null {
    ...
    private createDetail(label: string, descriptors: BranchMarkerModelDescriptor[]): BranchMarkerModelDetail | null {
        const entries = this.uniqueEntries(
            descriptors
                .map(descriptor => this.getModelEntry(descriptor.modelId, descriptor.modelProvider ?? ''))
                .filter((entry): entry is BranchMarkerModelEntry => Boolean(entry)),
