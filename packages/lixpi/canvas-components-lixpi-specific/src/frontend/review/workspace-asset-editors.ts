...
export type WorkspaceAssetEditorPorts = {
    ...
    updateMetadata: (assetId: string, revision: number, patch: { title: string; descriptor?: Asset['descriptor'] }) => Promise<Asset | { error: string }>
    onChanged: () => void
    onError: (error: unknown) => void
}

export function buildAssetMetadataEditorDocument(asset: Asset, mode: 'node' | 'details'): ProseMirrorJsonNode {
    const title = asset.title.trim()
    const description = asset.descriptor?.summary?.trim() ?? ''
    return {
        type: 'doc',
        content: [
            { type: 'documentTitle', ...(title ? { content: [{ type: 'text', text: title }] } : {}) },
            ...(mode === 'details' ? [{ type: 'paragraph', ...(description ? { content: [{ type: 'text', text: description }] } : {}) }] : []),
        ],
    }
}

export function readAssetMetadataEditorDocument(value: ProseMirrorJsonNode): { title: string; description?: string } {
    const title = value.content?.find(node => node.type === 'documentTitle')
    const description = value.content?.find(node => node.type === 'paragraph')
    return { title: collectProseMirrorText(title).trim(), ...(description ? { description: collectProseMirrorText(description).trim() } : {}) }
}
...
export class WorkspaceAssetMetadataEditor {
    ...
    constructor(private readonly assetId: string, host: HTMLElement, mode: 'node' | 'details', private readonly ports: WorkspaceAssetEditorPorts) {
        ...
        try {
            const editor = ports.mountEditor({
                ...
                onChange: content => {
                    if (!this.lifetime.signal.aborted) this.draft = content
                },
            })
            ...
            const blur = (event: FocusEvent) => {
                ...
                if (NodeType && next instanceof NodeType && host.contains(next)) return
                this.requested = true
                void this.commit()
            }
    ...
    private async commit(): Promise<void> {
        if (this.committing || this.lifetime.signal.aborted) return
        ...
        try {
            while (this.requested && !this.lifetime.signal.aborted) {
                ...
                if (metadata.title === current.title && !descriptionChanged) continue
                const descriptor = current.descriptor && metadata.description !== undefined
                    ...
                    : undefined
                const updated = await this.ports.updateMetadata(current.assetId, current.revision, { title: metadata.title, ...(descriptor ? { descriptor } : {}) })
                if (this.lifetime.signal.aborted) return
                if ('error' in updated) this.ports.onError(new Error(updated.error))
                else this.ports.onChanged()
            }
        } catch (error) {
            if (!this.lifetime.signal.aborted) this.ports.onError(error)
        } finally {
...
export class WorkspaceAssetContentEditor {
    ...
    constructor(host: HTMLElement, content: ProseMirrorJsonNode, authority: Omit<NonNullable<WorkspaceAssetEditorRequest['authority']>, 'onLeaseStateChange'>, mount: WorkspaceAssetEditorPorts['mountEditor']) {
        try {
            const editor = mount({
                host,
                content,
                ...
                authority: {
                    ...
                    onLeaseStateChange: state => {
                        ...
                        if (description) host.setAttribute('aria-description', description)
                        else host.removeAttribute('aria-description')
                    },
                },
            })
            this.lifetime.own(() => editor.destroy())
