...
export type WorkspaceAssetDetailsPorts = WorkspaceAssetEditorPorts & {
    ...
    getContentDocument: (assetId: string) => {
...
export class WorkspaceAssetDetails {
    ...
    constructor(
        ...
        private readonly ports: WorkspaceAssetDetailsPorts,
    ) {
        ...
        try {
            ...
            if (documentStorage) {
                ...
                renditions.textContent = Object.entries(asset.media?.renditions ?? {}).map(
                    ([name, rendition]) => `${name}: ${rendition?.status ?? 'missing'}`,
                )
                    .join(' · ') || 'No media renditions'
            ...
            if (seed === undefined)
                this.find('.canvas-asset-seed-row').remove()
            else {
                const tooltip = createHelpTooltip({
                    icon: questionMarkCircleIcon,
                    hideDelayMs: ports.tooltipHideDelayMs,
                    label: 'Seed details',
            ...
            this.scope = createPureDropdown({
                ...
                onSelect: option => void this.changeScope(option.scope as Asset['scope']),
            })
    ...
    private async changeScope(scope: Asset['scope']): Promise<void> {
        ...
        try {
            const ownerId = scope === 'workspace'
                ? this.ports.workspaceId
                : scope === 'user'
                    ? this.ports.userId
                    : current.organizationId
            const updated = await this.ports.changeScope(
                current.assetId,
                current.revision,
                scope,
                ownerId,
            ...
            if ('error' in updated) {
                this.scope.update(
                    this.scopeOption(current.scope),
                )
                this.error(`Scope update failed: ${updated.error}`)
            } else {
                this.scope.update(
                    this.scopeOption(updated.scope),
                )
