...
const uniqueAiModelIds = (modelIds: Array<string | undefined>): AiModelId[] => {
...
export class WorkspaceOutputReview {
    ...
    private admitReviewResponse(
        scope: ReviewScope,
        response: GeneratedOutputReviewResponse,
    ): boolean {
        if (!this.isCurrent(scope))
            return false

        if (response.workspaceId === scope.workspaceId)
            return true
    ...
    private applyReviewGeometry(
        scope: ReviewScope,
        geometry: CanvasGeometryUpdate,
        pruneChips: boolean,
    ): void {
        if (!this.isCurrent(scope))
            return
        ...
        if (pruneChips)
            this.ports.removeContextChips(geometry.removedNodeIds ?? [])
    ...
    isGeneratedOutputReviewReady(node: GeneratedOutputCanvasNode): boolean {
        return isGeneratedOutputReadyForReview(
            node,
            this.ports.readAsset(node.assetId),
        )
    }

    async acceptGeneratedOutput(
        ...
        if (!this.admitReviewResponse(scopeSnapshot, result))
            return

        this.applyReviewGeometry(
            scopeSnapshot,
    ...
    async rejectGeneratedOutput(
        ...
        if ('error' in result) {
            if (result.error === 'GENERATED_OUTPUT_NOT_FOUND')
                return 'not-found'
    ...
    getGeneratedMediaTrace(node: ImageCanvasNode | VideoCanvasNode): ImageGenerationTrace | VideoGenerationTrace | null {
        ...
        const usesSealedProvenance = Boolean(
            this.ports.readProvenance(node.assetId),
        )
        ...
        const visit = (value: unknown): void => {
            ...
            if (
                ...
                if (
                    imageTrace
                    && typeof imageTrace === 'object'
                )
                    traces.push(imageTrace as ImageGenerationTrace)

                if (
                    videoTrace
                    && typeof videoTrace === 'object'
                )
                    traces.push(videoTrace as VideoGenerationTrace)
            }
        ...
        const matchesRun = (trace: ImageGenerationTrace | VideoGenerationTrace): boolean => {
            ...
            if (
                generatedBy.mediaRunId
                && run?.mediaRunId === generatedBy.mediaRunId
            )
                return true
        ...
        return usesSealedProvenance
            && traces.length === 1
            ? traces[0]!
            : null
    }

    getGeneratedMediaReplayDescriptor(node: ImageCanvasNode | VideoCanvasNode): GeneratedMediaReplayDescriptor | null {
    ...
    getGeneratedArtifactReplayDescriptor(node: CapabilityArtifactCanvasNode): GeneratedArtifactReplayDescriptor | null {
        if (!node.generatedBy)
            return null
    ...
    getGeneratedOutputReplayDescriptor(node: GeneratedOutputCanvasNode): GeneratedOutputReplayDescriptor | null {
    ...
    buildRegenerationSubmitData(
        ...
        const reasoningModels = uniqueAiModelIds(
            descriptors.map(descriptor => descriptor.reasoningModelId),
        )
        const mediaDescriptors = descriptors.filter((descriptor): descriptor is GeneratedMediaReplayDescriptor => descriptor.kind === 'media')
        const capabilityInputs = descriptors.reduce<CapabilityReplaySubmitData['capabilityInputs']>(
            (merged, descriptor) =>
                descriptor.kind === 'artifact'
                    ? {
                        ...merged,
                        ...descriptor.capabilityInputs,
                    }
                    : merged,
            {},
        )
        ...
        const imageModels = uniqueAiModelIds(
            imageDescriptors.map(descriptor => descriptor.mediaModelId),
        )
        const videoModels = uniqueAiModelIds(
            videoDescriptors.map(descriptor => descriptor.mediaModelId),
        )
        ...
        return {
            ...
            mediaGenerationMode: firstVideo
                && !firstImage
                ? 'video'
                : 'image',
    ...
    async regenerateGeneratedOutputs(request: GeneratedOutputRegenerationRequest): Promise<void> {
        ...
        if (
            ...
            for (const outputNode of outputNodes) {
                if (!this.isCurrent(scopeSnapshot))
                    return
        ...
        await Promise.all(
            outputNodes.map(async node => {
                if (this.ports.readProvenance(node.assetId))
                    return
                ...
                if ('error' in result) {
                    this.ports.reportError(
                        '[CANVAS][generated-output-review] Unable to load sealed provenance:',
                        {
        ...
        const descriptors = outputNodes.map(node => this.getGeneratedOutputReplayDescriptor(node)).filter(
            (descriptor): descriptor is GeneratedOutputReplayDescriptor => Boolean(descriptor),
        )

        if (descriptors.length !== outputNodes.length) {
            this.ports.reportError(
                '[CANVAS][generated-output-review] Generation history is not ready for regeneration.',
                {
                    targetNodeId,
        ...
        const sourceMediaNode = replaysExistingPrompt
            && mediaDescriptors.length === 1
            ? mediaDescriptors[0]!.node
            : undefined

        if (
            replaysExistingPrompt
            && (!lineageParentNodeId || !lineageParentType || !branchId)
        ) {
            this.ports.reportError(
                '[CANVAS][generated-output-review] Branch lineage is unavailable.',
                {
                    targetNodeId,
                    lineageParentNodeId,
                    lineageParentType,
                    ...
                    : replaysExistingPrompt
                        ...
                        ? {
                            regeneration: {
                                ...
                                replayPrompts: mediaDescriptors.map(
                                    descriptor => ({
