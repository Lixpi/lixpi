...
export type WorkspaceHistoryMount = { host: HTMLElement; signal: AbortSignal }
...
export type WorkspaceOutputDetailsPorts = {
    ...
    mountMediaHistory: (request: WorkspaceHistoryMount & { target: WorkspaceMediaHistoryTarget; onProgress: (progress: MediaGenerationProgressInstance) => void }) => WorkspaceHistoryView | null
...
function isMarker(node: GeneratedOutputCanvasNode | BranchMarkerNode): node is BranchMarkerNode {
...
export class WorkspaceOutputDetails {
    ...
    constructor(private readonly body: HTMLElement, node: GeneratedOutputCanvasNode | BranchMarkerNode, private readonly ports: WorkspaceOutputDetailsPorts) {
        try {
            ...
            if (isMarker(node)) {
                ...
                this.appendHistory()
                if (media) this.mountMedia(media)
                if (!this.projection) {
                    this.mountBranch({
                        marker: node,
                        lineageProjectionScope: node.type === 'branchOrigin' ? 'branch-origin' : node.type === 'branchFork' ? 'branch-fork' : 'media-run',
                    })
                }
                ...
                if (node.generatedBy || branch) this.appendHistory()
                if (node.generatedBy) this.mountMedia({ node, lineageProjectionScope: 'media-run', limitProjectionToSelectedMedia: true })
                if (!this.projection && branch) this.mountBranch(branch)
            } else {
                this.appendMetadata(node)
                if (node.generatedBy) {
                    this.appendHistory()
                    this.adoptProjection(ports.mountArtifactHistory({ host: this.history!, node, signal: this.lifetime.signal }))
                }
            }
            let fallback: MediaGenerationProgressInstance | null = null
            if (progressNode && !this.progress) {
            ...
            if (this.history && !this.projection && !fallback) this.history.remove()
        } catch (error) {
            try {
    ...
    sync(state: CanvasState): void {
        if (this.lifetime.signal.aborted || !this.progressNodeId) return
        ...
        if (progress) this.progress?.update(progress)
    }
    ...
    private appendHistory(): void {
        ...
        this.history = html`<section className="canvas-generated-media-history-section workspace-generated-output-details-history">
    ...
    private adoptProjection(projection: WorkspaceHistoryView | null): void {
        ...
        if (projection) this.lifetime.own(() => projection.destroy())
    }

    private mountMedia(target: WorkspaceMediaHistoryTarget): void {
        this.adoptProjection(this.ports.mountMediaHistory({
            host: this.history!,
    ...
    private mountBranch(target: WorkspaceBranchHistoryTarget): void {
        this.adoptProjection(this.ports.mountBranchHistory({ host: this.history!, signal: this.lifetime.signal, target }))
    }
    ...
    private appendMetadata(node: GeneratedOutputCanvasNode): void {
        ...
        if (asset) {
            const editor = new WorkspaceAssetMetadataEditor(node.assetId, title, 'details', this.ports.assets)
        ...
        if (definition) {
            ...
            if (document) {
                ...
                this.append(host)
                const view = definition.createGeneratedOutputInfoView({ container: host, document })
                this.lifetime.own(() => view.destroy())
            }
        } else if (node.type === 'image' || node.type === 'video') {
            const descriptor = this.descriptor(this.ports.getDescriptor(node))
            if (descriptor) this.append(descriptor)
        }
        if (asset) {
            const details = new WorkspaceAssetDetails(asset, node.type === 'capabilityArtifact', this.ports.assets)
    ...
    private descriptor(descriptor: MediaDescriptor | undefined): HTMLElement | null {
        if (!descriptor || descriptor.source !== 'analysis') return null
        const html = createDocumentHtml(this.body.ownerDocument)
        if (descriptor.status === 'analyzing') {
            const spinnerStyle = { animationDelay: `${-(this.ports.now() % descriptorSpinnerPeriodMs)}ms` }
            return html`<div className="canvas-media-descriptor is-analyzing">
        ...
        if (descriptor.status === 'failed' || !descriptor.summary) {
            return html`<div className="canvas-media-descriptor is-failed">
        ...
        return html`<div className="canvas-media-descriptor">${
            tags.length
                ? html`<div className="canvas-media-descriptor-tags">
