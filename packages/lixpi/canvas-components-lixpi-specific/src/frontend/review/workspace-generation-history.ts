...
export type WorkspaceHistoryProjection = { threadId: string; content: ProseMirrorJsonNode }
export type WorkspaceHistoryProgressRequest = { id: string; state: MediaGenerationProgressState; showSummaryWhenCollapsedItemIds: readonly string[] }
...
export type WorkspaceGenerationHistoryOptions = {
    ...
    signal?: AbortSignal
    media?: { node: MediaNode; limitToSelectedMedia: boolean; onProgress: (progress: MediaGenerationProgressInstance) => void }
}

export function mountWorkspaceMediaHistory(
    request: { host: HTMLElement; node: MediaNode; lineageProjectionScope: AiLineageProjectionScope; limitToSelectedMedia: boolean; signal?: AbortSignal; onProgress: (progress: MediaGenerationProgressInstance) => void },
    ports: WorkspaceGenerationHistoryPorts,
): WorkspaceGenerationHistory | null {
    if (request.signal?.aborted || !request.node.generatedBy) return null
    const locator = getGeneratedMediaProjectionLocator(request.node)
    if (!locator) return null
    const projection = buildGeneratedMediaTurnProjectionFromThreadContent(ports.getMediaContent(request.node), locator, {
    ...
    return projection
        ? new WorkspaceGenerationHistory({
            ...
            media: { node: request.node, limitToSelectedMedia: request.limitToSelectedMedia, onProgress: request.onProgress },
        }, ports)
        : null
}
...
export class WorkspaceGenerationHistory {
    ...
    constructor(private readonly options: WorkspaceGenerationHistoryOptions, private readonly ports: WorkspaceGenerationHistoryPorts) {
        ...
        try {
            ...
            if (reasoningModelId) {
                const header = html`<div className="canvas-generated-media-reasoning-model">
    ...
    private referenceSources(reference: ImageGenerationTraceReference): string[] {
        if (this.lifetime.signal.aborted || !reference.nodeId) return []
        const node = this.ports.getNode(reference.nodeId)
        if (node?.type === 'image') return [this.ports.renditionPath(node.assetId, 'preview')]
        if (node?.type === 'video') return [this.ports.renditionPath(node.assetId, 'representativeFrame'), this.ports.renditionPath(node.assetId, 'poster')]
        return []
    }

    private referenceTile(reference: ImageGenerationTraceReference): HTMLElement | null {
        if (this.lifetime.signal.aborted || !reference.nodeId) return null
        const node = this.ports.getNode(reference.nodeId)
        if (!node) return null
    ...
    private createProgress(request: WorkspaceHistoryProgressRequest): MediaGenerationProgressInstance {
        if (this.lifetime.signal.aborted) throw new Error('History projection is disposed')
        ...
        const progress = createMediaGenerationProgress({
            ...
            state: resolveMediaGenerationHistoryProgress({ projectedState: request.state, liveState, matchesLiveTarget }),
        ...
        const view: MediaGenerationProgressInstance = { element: progress.element, update: state => progress.update(state), destroy }
        if (matchesLiveTarget) media.onProgress(view)
        return view
    }
}
