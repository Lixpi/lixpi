...
class WorkspaceContextTray {
    ...
    constructor(private readonly ports: WorkspaceContextTrayPorts, kind: 'canvas' | 'chat', refresh: () => void) {
        ...
        this.element = html`<div className=${`workspace-ai-chat-panel-context-chips ${className}`} role="list" aria-label=${label} contenteditable="false"></div>` as HTMLDivElement
        this.element.hidden = true
        this.lifetime.own(() => this.element.remove())
        this.lifetime.own(() => this.content.destroy())
        const frame = ports.requestFrame(() => {
            if (!this.lifetime.signal.aborted && this.element.isConnected) refresh()
        })
        this.lifetime.own(() => ports.cancelFrame(frame))
    }

    render(nodes: readonly CanvasNode[], environment: ContextPreviewEnvironment): void {
        ...
        try {
            for (const node of nodes) {
                const preview = createContextPreviewTile({ node, getNode: () => this.ports.getNode(node.nodeId) ?? node, environment })
                this.content.own(() => preview.destroy())
                const label = `Remove ${getContextPreviewAccessibleLabel(node, environment)} from context`
                const chip = html`<div className="workspace-ai-chat-panel-context-chip workspace-ai-chat-panel-context-chip-explicit" data=${{ nodeId: node.nodeId, contextKind: 'explicit', contextRole: 'forced-chip' }} role="listitem">
                    ${preview.dom}
                    <button type="button" className="workspace-ai-chat-panel-context-chip-remove" aria-label=${label} innerHTML=${xCircleIcon}></button>
...
export class WorkspaceContextTrays {
    ...
    create(kind: 'canvas' | 'chat'): HTMLDivElement {
        if (this.destroyed) throw new Error('Workspace context trays are disposed')
        const tray = new WorkspaceContextTray(this.ports, kind, () => this.refresh())
    ...
    refresh(): void {
        ...
        const nodes = this.ports.getContextNodeIds().map(id => this.ports.getNode(id)).filter((node): node is CanvasNode => Boolean(node))
        ...
        for (const [element, tray] of this.entries) {
            ...
            else tray.render(nodes, environment)
        }
    }
