...
const isMidSideAnchorNode = (node: CanvasNode | undefined): boolean =>
    node?.type === 'image' || node?.type === 'video' || isBranchLineageMarkerNode(node)

export const computeSpreadTValues = (
    edges: WorkspaceEdge[],
    nodes: CanvasNode[],
    settings: WorkspaceConnectionManagerConfig['settings'],
) => {
    return computeConnectorSpread(
        edges,
        nodes,
        {
...
const additionalGeometry = (
    ...
    return descriptors.map((_descriptor, index) => {
        ...
        return {
            id: getBranchMarkerMediaModelCircleNodeId(node.nodeId, index),
            shape: 'rect',
            ...rectangle,
        }
    })
...
const renderedSourceNodeId = (
    ...
    if (
        !isBranchMarkerNodeForMediaModelCircles(source)
        || !isGeneratedMediaNodeForMediaModelCircles(target)
    )
        return edge.sourceNodeId

    const index = getBranchMarkerMediaModelCircleIndexForGeneratedMedia(
        source,
        [...nodes],
        target,
    )
...
const sourceAnchorT = (
    ...
    if (
        center < nodeRect.top
        || center > nodeRect.bottom
    )
        return null

    return Math.max(
        0,
        Math.min(1, (center - nodeRect.top) / nodeRect.height),
    )
}

const workspacePolicy = (): ConnectionPolicy<CanvasNode> => {
    return {
        isCentered: isMidSideAnchorNode,
        ...
        isNodeTarget: target => Boolean(
            target.closest('.workspace-document-node, .workspace-image-node, .workspace-ai-chat-thread-node'),
        ),
    }
}

const projectConnectionNode = (node: EngineNode): CanvasNode => {
...
export const createWorkspaceConnectionPolicy = (): ConnectionPolicy<EngineNode> => {
    const policy = workspacePolicy()
    const projectOptional = (node: EngineNode | undefined) => (node ? projectConnectionNode(node) : undefined)

    return {
        isCentered: node => policy.isCentered(
            projectOptional(node),
        ),
        isReconnectionCentered: node => policy.isReconnectionCentered!(
            projectOptional(node),
        ),
        defaultTargetHandle: node => policy.defaultTargetHandle!(
            projectOptional(node),
        ),
        targetMarker: (node, selected) => policy.targetMarker!(
            projectOptional(node),
            selected,
        ),
        additionalGeometry: (node, nodes) => additionalGeometry(
            projectConnectionNode(node),
            nodes.map(projectConnectionNode),
        ),
        renderedSourceNodeId: (edge, nodes) => renderedSourceNodeId(
            edge,
            nodes.map(projectConnectionNode),
        ),
        sourceAnchorT: (edge, element) => sourceAnchorT(
            {
                ...edge.data as WorkspaceEdge | undefined,
                ...edge,
            },
            element,
        ),
