...
export type WorkspaceWireAttachment = { nodeId: string; handle: 'left' | 'right'; t: number }
export type WorkspaceEdgeAnchorPositions = { sourceT?: number; targetT?: number }

function fraction(value: number | undefined): number {
    return value !== undefined && Number.isFinite(value) ? Math.max(0, Math.min(1, value)) : 0.5
}
...
export class WorkspaceEdgePorts {
    ...
    ): { nodes: EngineNode<Data>[]; edges: EngineEdge<WorkspaceEdge>[] } {
        const byId = new Map(nodes.map(node => [node.nodeId, node]))
        const ports = new Map<string, CanvasPort[]>()
        const attachments = new Map<string, Map<string, WorkspaceWireAttachment>>()
        const add = (node: EngineNode<Data>, portId: string, handle: 'left' | 'right', t: number, role: CanvasPort['role']) => {
            const sideT = fraction(t)
            ports.get(node.nodeId)!.push({ id: portId, role, direction: handle, anchor: { x: handle === 'left' ? 0 : node.dimensions.width, y: node.dimensions.height * sideT } })
            attachments.get(node.nodeId)!.set(portId, { nodeId: node.nodeId, handle, t: sideT })
        }
        for (const node of nodes) {
            ports.set(node.nodeId, [])
            attachments.set(node.nodeId, new Map())
            add(node, 'left', 'left', 0.5, 'input')
            add(node, 'right', 'right', 0.5, 'both')
        }
        const projected: EngineEdge<WorkspaceEdge>[] = []
        for (const edge of edges) {
            const source = byId.get(edge.sourceNodeId), target = byId.get(edge.targetNodeId)
            if (!source || !target) throw new Error(`Cannot project edge ${edge.edgeId}: an endpoint is missing`)
            ...
            const targetSide = edge.targetHandle === undefined
                ...
                ? 'left'
                : 'right'
            add(source, sourcePort, sourceSide, fraction(override?.sourceT ?? edge.sourceT), 'output')
            add(target, targetPort, targetSide, fraction(override?.targetT ?? edge.targetT), 'input')
            projected.push({
                edgeId: edge.edgeId,
                source: { nodeId: source.nodeId, portId: sourcePort },
                target: { nodeId: target.nodeId, portId: targetPort },
                path: edge.pathType ?? this.defaultPath,
                data: edge,
            })
        }
        this.attachments = attachments
        return { nodes: nodes.map(node => ({ ...node, ports: ports.get(node.nodeId)! })), edges: projected }
    }
    ...
    reconnect(edge: WorkspaceEdge, source: EdgeEndpoint, target: EdgeEndpoint): WorkspaceEdge | undefined {
        const sourceAttachment = this.resolve(source), targetAttachment = this.resolve(target)
        if (!sourceAttachment || !targetAttachment) return undefined
        ...
        return {
            ...edge,
            ...(!unchangedSource ? { sourceNodeId: sourceAttachment.nodeId, sourceHandle: sourceAttachment.handle, sourceT: sourceAttachment.t } : {}),
            ...(!unchangedTarget ? { targetNodeId: targetAttachment.nodeId, targetHandle: targetAttachment.handle, targetT: targetAttachment.t } : {}),
        }
    }
