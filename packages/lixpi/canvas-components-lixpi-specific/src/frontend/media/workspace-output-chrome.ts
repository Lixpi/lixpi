...
export type WorkspaceOutputChromePorts = {
    ...
    getBounds: (node: GeneratedOutputCanvasNode, nodesById: Map<string, CanvasNode>) => CanvasEngineRect
    getPendingBounds: (nodeId: string, bounds: CanvasEngineRect) => CanvasEngineRect | null
    getPendingNodeIds: () => ReadonlySet<string>
    getAsset: (assetId: string) => Asset | undefined
    getDocumentVersion: (assetId: string, role: 'capabilityArtifact' | 'provenance') => number | undefined
    ...
    createModelBadge: (options: { modelId: string; modelProvider: string; iconOnly: boolean }) => HTMLElement | null
    mountTitle: (node: GeneratedOutputCanvasNode, host: HTMLElement) => () => void
    ...
    onSync?: (event: { rebuilt: boolean; mediaCount: number; pendingCount: number; videoCount: number }) => void
}
...
export class WorkspaceOutputChrome {
    ...
    schedule(): void {
        if (this.lifetime.signal.aborted || this.frame !== null) return
        const version = this.frameVersion
        this.frame = this.ports.requestFrame(() => {
            if (version !== this.frameVersion) return
            this.frame = null
    ...
    sync(state = this.ports.getState()): void {
        ...
        const media = nodes.filter((node): node is MediaNode => (node.type === 'image' || node.type === 'video') && Boolean(this.ports.getAsset(node.assetId) || node.generatedBy || node.generationProgress?.mediaModelId))
        const artifacts = nodes.filter((node): node is CapabilityArtifactCanvasNode => node.type === 'capabilityArtifact' && Boolean(this.ports.getAsset(node.assetId)))
        const pending = nodes.filter((node): node is MediaNode => (node.type === 'image' || node.type === 'video') && pendingIds.has(node.nodeId))
        const videos = nodes.filter((node): node is VideoCanvasNode => node.type === 'video' && this.ports.getAsset(node.assetId)?.media?.renditions.original?.status === 'ready')
        for (const node of media) {
            const descriptor = this.ports.getDescriptor(node)
            if (descriptor?.status === 'ready' && descriptor.version !== MEDIA_DESCRIPTOR_VERSION) this.ports.queueAnalysis(node)
        }
        const key = [
            media.map(node => this.mediaKey(node)).join('\u001e'),
            artifacts.map(node => this.artifactKey(node)).join('\u001e'),
            pending.map(node => [node.nodeId, node.type, node.assetId, getGeneratedMediaModelId(node), getGeneratedMediaModelProvider(node, getGeneratedMediaModelId(node)), node.generationProgress?.status ?? ''].join('\u001f')).join('\u001e'),
            videos.map(node => {
        ...
        this.ports.onSync?.({ rebuilt: key !== this.syncKey, mediaCount: media.length, pendingCount: pending.length, videoCount: videos.length })
        if (key === this.syncKey) {
            this.updateState(state)
            this.layout(this.ports.getViewport(), state)
            return
        }
        this.clear()
        try {
            ...
            for (const node of [...media, ...artifacts]) this.mountFooter(node, state, pendingIds.has(node.nodeId))
            this.layout(this.ports.getViewport(), state)
            this.syncKey = key
        } catch (error) {
            try {
    ...
    updateState(state = this.ports.getState()): void {
        ...
        for (const node of state?.nodes ?? []) {
            ...
            this.footers.get(node.nodeId)?.update({ progressActive: this.ports.isProgressActive(node), selected: this.ports.isSelected(node.nodeId) })
        }
    }

    layout(viewport = this.ports.getViewport(), state = this.ports.getState()): void {
        if (this.lifetime.signal.aborted) return
        const nodesById = new Map((state?.nodes ?? []).map(node => [node.nodeId, node]))
        for (const node of state?.nodes ?? []) {
            if (node.type !== 'image' && node.type !== 'video' && node.type !== 'capabilityArtifact') continue
            this.update(node.nodeId, this.ports.getBounds(node, nodesById), viewport)
        }
    }

    update(nodeId: string, bounds: CanvasEngineRect, viewport: CanvasViewport): void {
        if (this.lifetime.signal.aborted) return
        this.footers.get(nodeId)?.setGeometry(this.ports.getPendingBounds(nodeId, bounds) ?? bounds, viewport, this.ports.video.outsideOffsetScreen(nodeId, viewport))
        const icon = this.pendingIcons.get(nodeId)
        if (icon) {
            const zoom = Number.isFinite(viewport.zoom) ? Math.max(viewport.zoom, 0.01) : 1
            const scale = scaleCanvasChromeToScreenForZoom(1, zoom, getAdaptiveBoundedZoomScalingOptions(this.ports.settings.zoomScaling))
            applyStyle(icon, {
        ...
        this.ports.video.update(nodeId, bounds, viewport)
    }

    clear(): void {
        this.frameVersion += 1
        if (this.frame !== null) this.ports.cancelFrame(this.frame)
        this.frame = null
        this.syncKey = null
        this.footers.clear()
    ...
    private mountPendingIcon(node: MediaNode): void {
        ...
        const badge = this.ports.createModelBadge({ modelId, modelProvider: getGeneratedMediaModelProvider(node, modelId), iconOnly: true })
        const html = createDocumentHtml(this.ports.document)
        const icon = html`<div className="workspace-generated-media-pending-icon" data=${{ pendingMediaIconNodeId: node.nodeId }}>${badge}</div>` as HTMLElement
        this.pendingIcons.set(node.nodeId, icon)
        this.pendingElement.appendChild(icon)
    }

    private mountFooter(node: GeneratedOutputCanvasNode, state: CanvasState | null, pending: boolean): void {
        ...
        const reviewContext = { node, asset, nodes: state?.nodes ?? [], edges: state?.edges ?? [] }
        const chrome = new GeneratedOutputNodeChrome({
            ...
            modelBadge: pending ? null : this.ports.createModelBadge({ modelId, modelProvider, iconOnly: false }),
    ...
    private mediaKey(node: MediaNode): string {
        const asset = this.ports.getAsset(node.assetId)
        const descriptor = this.ports.getDescriptor(node)
        const descriptorKey = descriptor ? [descriptor.status ?? '', descriptor.source ?? '', descriptor.summary ?? '', ...(descriptor.entityTags ?? []), ...(descriptor.styleTags ?? []), String(descriptor.version ?? '')].join('\u001f') : ''
        return [
            node.nodeId,
            node.type,
            node.assetId,
