...
export type WorkspaceGenerationVisualsPorts = {
    ...
    getPendingInset: (dimensions: { width: number; height: number }) => { x: number; y: number; size: number }
    completionTimeoutMs: number
    setTimer: (callback: () => void, delayMs: number) => number
    clearTimer: (handle: number) => void
}

type Completion = { runKey: string; cancel: Dispose }
...
export class WorkspaceGenerationVisuals {
    ...
    markFrameDecoded(nodeId: string): void {
        if (!this.disposed) this.decodedImages.add(nodeId)
    }
    forgetDecodedFrame(nodeId: string): void {
        this.decodedImages.delete(nodeId)
    }

    setReferences(key: string, nodeIds: Iterable<string>): void {
        if (this.disposed) return
        const values = new Set(nodeIds)
        if (values.size) this.references.set(key, values)
    ...
    sync(state: CanvasState | null = this.ports.getState()): void {
        ...
        if (this.ports.alwaysOn()) {
            for (const node of state?.nodes ?? []) {
                ...
                targets.set(node.nodeId, { direction: pending ? 'clockwise' : 'counterclockwise', shape: pending ? 'preFrameCircle' : 'node' })
            }
            this.ports.setTargets(targets)
            return
        }
        for (const partial of this.ports.images.values()) {
            targets.set(partial.nodeId, {
                direction: 'clockwise',
        ...
        for (const pending of this.ports.videos.values()) {
            targets.set(pending.nodeId, { direction: 'clockwise', shape: pending.hasReceivedFrame ? 'node' : 'preFrameCircle' })
        }
        for (const node of state?.nodes ?? []) {
            if (!targets.has(node.nodeId) && this.isWaitingForFrame(node)) targets.set(node.nodeId, { direction: 'clockwise', shape: 'preFrameCircle' })
        }
        for (const references of this.references.values()) {
            for (const nodeId of references) if (!targets.has(nodeId)) targets.set(nodeId, { direction: 'counterclockwise' })
        }
        this.ports.setTargets(targets)
    }

    keepCompletion(runKey: string, previous: PendingGeneratedMediaTracker, node: Pick<ImageCanvasNode, 'nodeId' | 'assetId'>): void {
        if (this.disposed) return
        for (const [nodeId, completion] of this.completions) {
            if (completion.runKey === runKey || nodeId === node.nodeId) this.clearCompletion(nodeId)
        }
        setGeneratedMediaTracker(this.ports.images, runKey, {
            ...previous,
            nodeId: node.nodeId,
            assetId: node.assetId || previous.assetId,
            hasReceivedFrame: false,
        })
        const completion: Completion = { runKey, cancel: () => {} }
        this.completions.set(node.nodeId, completion)
        try {
            const timer = this.ports.setTimer(() => {
                if (this.completions.get(node.nodeId) === completion) this.clearCompletion(node.nodeId)
            ...
            if (this.ports.images.get(runKey)?.nodeId === node.nodeId) this.ports.images.delete(runKey)
            throw error
        }
        this.sync()
    }
    ...
    clearCompletion(nodeId: string): void {
        ...
        if (this.ports.images.get(completion.runKey)?.nodeId === nodeId) this.ports.images.delete(completion.runKey)
        try {
            completion.cancel()
        } finally {
            if (!this.disposed) this.ports.onFinalized(nodeId)
        }
    }

    pendingNodeIds(): Set<string> {
        const ids = new Set<string>()
        if (this.disposed) return ids
        for (const tracker of [this.ports.images, this.ports.videos]) {
            for (const node of tracker.values()) if (!node.hasReceivedFrame && !this.hasTerminalProgress(node.nodeId)) ids.add(node.nodeId)
        }
        for (const node of this.ports.getState()?.nodes ?? []) if (this.isWaitingForFrame(node)) ids.add(node.nodeId)
        return ids
    }
    ...
    isWaitingForFrame(node: CanvasNode): node is ImageCanvasNode | VideoCanvasNode {
        if (this.disposed || (node.type !== 'image' && node.type !== 'video')) return false
        if (node.generationProgress && ['completed', 'failed', 'cancelled'].includes(node.generationProgress.status)) return false
        if (node.type === 'image' && this.decodedImages.has(node.nodeId)) return false
    ...
    isPending(nodeId: string): boolean {
        if (this.disposed || this.hasTerminalProgress(nodeId)) return false
        for (const tracker of [this.ports.images, this.ports.videos]) {
            for (const pending of tracker.values()) if (pending.nodeId === nodeId) return !pending.hasReceivedFrame
        }
        const node = this.ports.getState()?.nodes.find(candidate => candidate.nodeId === nodeId)
        return node ? this.isWaitingForFrame(node) : false
    }

    updateHitArea(element: HTMLElement, nodeId: string): void {
        if (this.disposed) return
