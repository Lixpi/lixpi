...
export type WorkspaceMediaSourcePorts = {
    ...
    resolveAssetRendition: (request: {
        assetId: string
        renditionId: string
        version: string
        signal: AbortSignal
    }) => Promise<SourceLease>
    resolveTransientSource: (
        url: string,
        signal: AbortSignal,
    ) => Promise<SourceLease>
}
...
export class WorkspaceMediaSources implements MediaSourceResolver {
    ...
    describeAsset(
        ...
        return {
            key: assetId,
            kind,
            version: JSON.stringify([this.retries.get(assetId) ?? 0, ready.map(
                rendition => [rendition.name, rendition.blobHash, rendition.updatedAt, rendition.mimeType, rendition.width, rendition.height],
            )]),
            dimensions: media.width
                && media.height
                ? {
                    width: media.width,
                    height: media.height,
    ...
    resolve: MediaSourceResolver['resolve'] = async (
        descriptor,
        renditionId,
        signal,
    ) => {
        ...
        if (!descriptor.renditions.some(rendition => rendition.id === renditionId))
            throw new Error(`Undeclared media rendition: ${renditionId}`)

        const source = (descriptor as WorkspaceMediaDescriptor).source

        if (!source)
            throw new Error('Workspace media descriptor is missing its source')

        const lease = source.kind === 'asset'
            ? await this.ports.resolveAssetRendition({
