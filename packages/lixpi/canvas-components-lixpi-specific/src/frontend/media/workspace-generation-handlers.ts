...
export type WorkspaceGenerationHandlersPorts = {
    ...
    refreshAsset: (assetId: string, workspaceId: string) => Promise<void>
    reloadWorkspace: (workspaceId: string) => Promise<unknown>
    applyCapabilityRunEventToBranchMarkers: (threadId: string, event: CapabilityRunEvent) => void
    handleWorkspaceContextResolution: (threadId: string, resolution: WorkspaceContextResolution, generationRun?: MediaGenerationRunMeta) => void
    setGeneratingReferenceNodeIds: (key: string, nodeIds: Iterable<string>) => void
    clearGeneratingReferenceNodeIds: (key: string) => void
    clearGeneratingReferencesAfterPromptHandoff: (threadId: string, generationRun?: MediaGenerationRunMeta) => void
    clearGeneratingReferencesOnFirstPixels: (threadId: string, generationRun?: MediaGenerationRunMeta) => void
    ...
    setTransientImageSource: (nodeId: string, source: string | null) => void
    renderNow: () => void
    removeSelection: (nodeId: string) => void
    rebalanceGeneratedMediaTrees: (nodes: CanvasNode[], edges: WorkspaceEdge[]) => CanvasNode[]
    commitTransientCanvasStatePreservingEditors: (state: CanvasState) => void
    appendCanvasNodeToDOM: (node: CanvasNode) => void
    appendBranchMarkerNodeToDOM: (nodes: CanvasNode[], marker: BranchForkCanvasNode | BranchLineCanvasNode | undefined) => void
    hasNodeElement: (nodeId: string) => boolean
    debugLoggingEnabled: boolean
    debugGeneratedMediaLifecycle: (event: string, details: Record<string, unknown>) => void
    log: (level: 'info' | 'error', ...details: unknown[]) => void
}

function isBranchMarkerNode(node: CanvasNode): node is BranchOriginCanvasNode | BranchForkCanvasNode | BranchLineCanvasNode {
...
export class WorkspaceGenerationHandlers {
    ...
    constructor(events: CanvasGenerationEvents, private readonly ports: WorkspaceGenerationHandlersPorts) {
        try {
            this.releases.push(events.subscribeImages({
                onCapabilityRunEventToCanvas: ({ workspaceId: eventWorkspaceId, threadId, event }) => {
                ...
                onMediaBranchResolvedToCanvas: ({ workspaceId: eventWorkspaceId, threadId, resolution, generationRun }) => {
                    const scope = this.capture(eventWorkspaceId)
                    if (!scope) return
                    const workspaceId = scope.workspaceId
                    if (!this.shouldAcceptGeneratedMediaEvent(threadId, undefined, generationRun)) return
                    ...
                    if (!this.isCurrent(scope)) return
                    this.ports.setGeneratingReferenceNodeIds(this.ports.placements.getGeneratedMediaPlacementKey(threadId, generationRun), referenceNodeIds)
                    if (!this.isCurrent(scope)) return

                    this.ports.log('info', '[CANVAS] image branch VLM resolution', {
                        threadId,
                ...
                onMediaLineagePlannedToCanvas: ({ workspaceId: eventWorkspaceId, threadId, lineagePlan, generationRun }) => {
                    const scope = this.capture(eventWorkspaceId)
                    if (!scope) return
                    const workspaceId = scope.workspaceId
                    if (!this.shouldAcceptGeneratedMediaEvent(threadId, undefined, generationRun)) return

                    this.ports.settlement.applyMediaBranchLineagePlan(threadId, lineagePlan, generationRun)
                    if (!this.isCurrent(scope)) return
                },

                onWorkspaceContextResolvedToCanvas: ({ workspaceId: eventWorkspaceId, threadId, resolution, generationRun }) => {
                    const scope = this.capture(eventWorkspaceId)
                    if (!scope) return
                    const workspaceId = scope.workspaceId
                    if (!this.shouldAcceptGeneratedMediaEvent(threadId, undefined, generationRun)) return

                    this.ports.handleWorkspaceContextResolution(threadId, resolution, generationRun)
                    if (!this.isCurrent(scope)) return
                },
                ...
                onMediaGenerationSkippedToCanvas: ({ workspaceId: eventWorkspaceId, threadId, generationRequestId, generationRun }) => {
                    const scope = this.capture(eventWorkspaceId)
                    if (!scope) return
                    const workspaceId = scope.workspaceId
                    if (!this.shouldAcceptGeneratedMediaEvent(threadId)) return
                    ...
                    this.ports.settlement.settleMediaGenerationRequest(threadId, generationRequestId, generationRun)
                    if (!this.isCurrent(scope)) return
                },

                onCanvasGeometryResolvedToCanvas: ({ workspaceId: eventWorkspaceId, canvasGeometry }) => {
                    const scope = this.capture(eventWorkspaceId)
                    if (!scope) return
                    const workspaceId = scope.workspaceId
                ...
                onMediaGenerationRequestCompleteToCanvas: ({ workspaceId: eventWorkspaceId, threadId, generationRequestId, generationRun }) => {
                    const scope = this.capture(eventWorkspaceId)
                    if (!scope) return
                    const workspaceId = scope.workspaceId
                    const accepted = this.shouldAcceptGeneratedMediaEvent(threadId)
                    this.ports.log('info', '[CANVAS] media generation request completion received', {
                        threadId,
                        generationRequestId,
                        ...
                        preflightNodeIds: this.currentCanvasState?.nodes
                ...
                onMediaBranchResolutionErrorToCanvas: ({ workspaceId: eventWorkspaceId, threadId, generationRun }) => {
                    const scope = this.capture(eventWorkspaceId)
                    if (!scope) return
                    const workspaceId = scope.workspaceId
                    if (!this.shouldAcceptGeneratedMediaEvent(threadId, undefined, generationRun)) return
                ...
                onImageGenerationTraceToCanvas: ({ workspaceId: eventWorkspaceId, threadId, generationRun }) => {
                    const scope = this.capture(eventWorkspaceId)
                    if (!scope) return
                    const workspaceId = scope.workspaceId
                    if (!this.shouldAcceptGeneratedMediaEvent(threadId, undefined, generationRun)) return
                    ...
                    if (this.ports.placements.getApiMediaRunLineageAssignment(generationRun)) {
                        const existingImageNode = this.ports.trackers.findGeneratedMediaNodeForRun('image', threadId, generationRun)
                        if (!this.isCurrent(scope)) return
                        if (existingImageNode?.type === 'image') {
                            this.ports.trackers.rememberPartialImageTrackerForNode(threadId, generationRun, existingImageNode)
                            if (!this.isCurrent(scope)) return
                            ...
                            this.ports.debugGeneratedMediaLifecycle('image-generation-trace-waiting-for-api-geometry', {
                    ...
                    this.ensureImageGenerationPlaceholderForRun({ threadId, generationRun })
                    if (!this.isCurrent(scope)) return
                    this.ports.clearGeneratingReferencesAfterPromptHandoff(threadId, generationRun)
                    if (!this.isCurrent(scope)) return
                },

                onImageErrorToCanvas: ({ workspaceId: eventWorkspaceId, threadId, error, generationRun }) => {
                    const scope = this.capture(eventWorkspaceId)
                    if (!scope) return
                    const workspaceId = scope.workspaceId
                    if (!this.shouldAcceptGeneratedMediaEvent(threadId, undefined, generationRun)) return
                    ...
                    const outputNodeId = generationRun?.lineageAssignment
                        ...
                        : existing?.nodeId
                    const failureResult = this.currentCanvasState && generationRequestId
                        ? applyMediaGenerationStreamFailureToOperationNodes(this.currentCanvasState, {
                            generationRequestId,
                        ...
                        : undefined
                    if (failureResult) this.ports.applyMediaOperationRecoveryResult(failureResult)
                    if (!this.isCurrent(scope)) return
                    if (!existing || !this.currentCanvasState) {
                        if (!failureResult?.changed) this.ports.handoff.removePendingBranchMarkerForRun(threadId, generationRun)
                        if (!this.isCurrent(scope)) return
                ...
                onImagePartialToCanvas: (data) => {
                    ...
                    if (canvasGeometry) {
                        if (this.ports.debugLoggingEnabled) {
                            this.ports.log('info', '[CANVAS][api-geometry]', 'image-partial-apply', {
                                runKey,
                        ...
                        const imageNode = (expectedNodeId ? this.getCurrentCanvasMediaNode(expectedNodeId) : undefined)
                            ?? this.ports.trackers.findGeneratedMediaNodeForRun('image', threadId, generationRun)
                        if (imageNode?.type !== 'image') {
                            this.ports.log('error', '[CANVAS][api-geometry] image partial geometry did not materialize image node', {
                                runKey,
                                threadId,
                                expectedNodeId,
                                generationRequestId: generationRun?.generationRequestId,
                        ...
                        if (!this.isCurrent(scope)) return
                        const tracker = this.ports.trackers.rememberPartialImageTrackerForNode(threadId, generationRun, imageNode)
                        if (!this.isCurrent(scope)) return
                        setGeneratedMediaTracker(this.ports.trackers.images, runKey, {
                            ...tracker,
                        ...
                        this.ports.debugGeneratedMediaLifecycle('image-partial-api-geometry-update', {
                            runKey,
                            threadId,
                        ...
                        this.ports.handoff.clearPendingBranchMarkerStateForRun(threadId, generationRun)
                        if (!this.isCurrent(scope)) return
                        if (hasFrame) this.ports.clearGeneratingReferencesOnFirstPixels(threadId, generationRun)
                        if (!this.isCurrent(scope)) return
                    ...
                    if (lineageAssignment && (imageUrl || assetId)) {
                        ...
                        if (
                            ...
                            this.ports.debugGeneratedMediaLifecycle('image-partial-duplicate-without-geometry', {
                                runKey,
                                threadId,
                        ...
                        this.ports.log('error', '[CANVAS][api-geometry] missing image partial geometry; refusing local canvas topology mutation', {
                            runKey,
                            threadId,
                            partialIndex: data.partialIndex,
                    ...
                    if (lineageAssignment) {
                        const existing = this.ports.trackers.images.get(runKey)
                        if (!this.isCurrent(scope)) return
                        this.ports.debugGeneratedMediaLifecycle('empty-image-partial-api-heartbeat', {
                            runKey,
                            threadId,
                    ...
                    if (!this.isCurrent(scope)) return
                    if (existing && !this.getCurrentCanvasMediaNode(existing.nodeId)) {
                        this.ports.debugGeneratedMediaLifecycle('drop-stale-image-tracker-before-partial', {
                            runKey,
                            threadId,
                            nodeId: existing.nodeId,
                    ...
                    if (existing) {
                        if (!imageUrl && !assetId) {
                            this.ports.debugGeneratedMediaLifecycle('empty-image-partial-refresh-outline', {
                                runKey,
                                threadId,
                                nodeId: existing.nodeId,
                        ...
                        setGeneratedMediaTracker(this.ports.trackers.images, runKey, updatedTracker)
                        this.ports.debugGeneratedMediaLifecycle('image-partial-update', {
                            runKey,
                            threadId,
                            nodeId: existing.nodeId,
                        ...
                        if (imageUrl && this.currentCanvasState) {
                            ...
                            this.ports.commitTransientCanvasStatePreservingEditors({ ...this.currentCanvasState, nodes: resolvedNodes })
                            if (!this.isCurrent(scope)) return
                        }
                        return
                    }
                ...
                onImageCompleteToCanvas: (data) => {
                    ...
                    if (!data.canvasGeometry) {
                        ...
                        if (existingCompletedImageNode?.type === 'image') {
                            if (this.ports.debugLoggingEnabled) {
                                this.ports.log('info', '[CANVAS][api-geometry]', 'image-complete-existing-final-without-geometry', {
                                    runKey,
                                    threadId,
                                    completedNodeId,
                                    generationRequestId: generationRun?.generationRequestId,
                            ...
                            const completionTracker = this.ports.trackers.images.get(runKey)
                                ?? this.ports.trackers.rememberPartialImageTrackerForNode(threadId, generationRun, existingCompletedImageNode)
                            if (!this.isCurrent(scope)) return
                            ...
                            if (pendingNodeId) this.ports.setTransientImageSource(pendingNodeId, null)
                            if (!this.isCurrent(scope)) return
                        ...
                        this.ports.log('error', '[CANVAS][api-geometry] missing image completion geometry; refusing local canvas topology mutation', {
                            runKey,
                            threadId,
                    ...
                    if (this.ports.debugLoggingEnabled) {
                        this.ports.log('info', '[CANVAS][api-geometry]', 'image-complete-apply', {
                            runKey,
                    ...
                    if (completedImageNode?.type !== 'image') {
                        this.ports.log('error', '[CANVAS][api-geometry] image completion geometry did not materialize final node', {
                            runKey,
                            threadId,
                            completedNodeId,
                            generationRequestId: generationRun?.generationRequestId,
                    ...
                    const completionTracker = this.ports.trackers.images.get(runKey)
                        ?? this.ports.trackers.rememberPartialImageTrackerForNode(threadId, generationRun, completedImageNode)
                    if (!this.isCurrent(scope)) return
                    if (!this.ports.visuals.isFinalizing(completedImageNode.nodeId)) {
                        this.ports.visuals.keepCompletion(runKey, completionTracker, completedImageNode)
                        if (!this.isCurrent(scope)) return
                    }
                    if (pendingNodeId) this.ports.setTransientImageSource(pendingNodeId, null)
                    if (!this.isCurrent(scope)) return
            ...
            this.releases.push(events.subscribeVideos({
                onVideoPendingToCanvas: (data) => {
                    ...
                    if (!this.isCurrent(scope)) return
                    if (!lineageAssignment) {
                        this.ports.log('error', '[CANVAS] Missing API media lineage assignment for video pending', { threadId, generationRun })
                        if (!this.isCurrent(scope)) return
                        this.ports.handoff.removePendingBranchMarkerForRun(threadId, generationRun)
                        if (!this.isCurrent(scope)) return
                        return
                    ...
                    if (!canvasGeometry) {
                        this.ports.log('error', '[CANVAS][api-geometry] missing video pending geometry; refusing local canvas topology mutation', {
                            runKey,
                            threadId,
                            generationRequestId: generationRun?.generationRequestId,
                            mediaRunId: generationRun?.mediaRunId,
                    ...
                    if (videoNode?.type !== 'video') {
                        this.ports.log('error', '[CANVAS][api-geometry] video pending geometry did not materialize video node', {
                            runKey,
                            threadId,
                            nodeId,
                            generationRequestId: generationRun?.generationRequestId,
                    ...
                    this.ports.trackers.rememberVideoGenerationTrackerForNode(threadId, generationRun, videoNode)
                    if (!this.isCurrent(scope)) return
                    this.ports.handoff.clearPendingBranchMarkerStateForRun(threadId, generationRun)
                    if (!this.isCurrent(scope)) return
                ...
                onVideoGeneratingToCanvas: ({ workspaceId: eventWorkspaceId, threadId }) => {
                    const scope = this.capture(eventWorkspaceId)
                    if (!scope) return
                    const workspaceId = scope.workspaceId
                    if (!this.shouldAcceptGeneratedMediaEvent(threadId)) return
                ...
                onVideoGenerationTraceToCanvas: ({ workspaceId: eventWorkspaceId, threadId, generationRun }) => {
                    const scope = this.capture(eventWorkspaceId)
                    if (!scope) return
                    const workspaceId = scope.workspaceId
                    if (!this.shouldAcceptGeneratedMediaEvent(threadId, undefined, generationRun)) return
                ...
                onVideoCompleteToCanvas: (data) => {
                    ...
                    const completedNodeId = generationRun?.lineageAssignment
                        ...
                        : ''
                    if (!data.canvasGeometry) {
                        this.ports.log('error', '[CANVAS][api-geometry] missing video completion geometry; refusing local canvas topology mutation', {
                            runKey,
                            threadId,
                            completionAssetId: assetId,
                            generationRequestId: generationRun?.generationRequestId,
                    ...
                    if (this.ports.debugLoggingEnabled) {
                        this.ports.log('info', '[CANVAS][api-geometry]', 'video-complete-apply', {
                            runKey,
                            layoutRevision: data.canvasGeometry.layoutRevision,
                            removedNodeIds: data.canvasGeometry.removedNodeIds ?? [],
                            edgeSnapshotIds: data.canvasGeometry.edgeSnapshots?.map(edge => edge.edgeId) ?? [],
                    ...
                    if (completedVideoNode?.type !== 'video') {
                        this.ports.log('error', '[CANVAS][api-geometry] video completion geometry did not materialize final node', {
                            runKey,
                            threadId,
                            completedNodeId,
                            generationRequestId: generationRun?.generationRequestId,
    ...
    destroy(): void {
        ...
        if (errors.length) throw new AggregateError(errors, 'Canvas generation handler cleanup failed')
    }

    private capture(eventWorkspaceId?: string): HandlerScope | null {
        const scope = this.closed ? null : this.ports.readScope()
        if (!scope || (eventWorkspaceId && eventWorkspaceId !== scope.workspaceId)) return null
        return { ...scope }
    }
    ...
    ensureImageGenerationPlaceholderForRun({
        ...
        if (!scope) return
        if (!this.currentCanvasState) return undefined
        ...
        if (!this.isCurrent(scope)) return
        if (existingImageNode?.type === 'image') {
            if (this.ports.trackers.hasGeneratedImageFrame(existingImageNode) && !imageUrl && !assetId) {
                this.ports.handoff.clearPendingBranchMarkerStateForRun(threadId, generationRun)
                if (!this.isCurrent(scope)) return
                return undefined
            }
            const tracker = this.ports.trackers.rememberPartialImageTrackerForNode(threadId, generationRun, existingImageNode)
            if (!this.isCurrent(scope)) return
            this.ports.debugGeneratedMediaLifecycle('reattach-image-placeholder', {
                runKey,
                threadId,
        ...
        if (!lineageAssignment) {
            if (failOnMissingLineage) {
                this.ports.log('error', '[CANVAS] Missing API media lineage assignment for image placeholder', { threadId, generationRun })
                if (!this.isCurrent(scope)) return
                this.ports.handoff.removePendingBranchMarkerForRun(threadId, generationRun)
                if (!this.isCurrent(scope)) return
            } else {
                this.ports.debugGeneratedMediaLifecycle('skip-image-placeholder-missing-lineage', {
                    runKey,
                    threadId,
                    generationRequestId: generationRun?.generationRequestId ?? '',
                    mediaRunId: generationRun?.mediaRunId ?? '',
        ...
        if (!this.isCurrent(scope)) return
        const branchOriginNode = this.ports.lineage.ensureBranchOriginForGeneratedMedia(threadId, generationRun, imageHeight)
        if (!this.isCurrent(scope)) return
        ...
        if (!edgeSourceNode) {
            if (failOnMissingLineage) {
                this.ports.log('error', '[CANVAS] Missing API media lineage parent for image placeholder', {
                    threadId,
                ...
                this.ports.debugGeneratedMediaLifecycle('skip-image-placeholder-missing-parent', {
                    runKey,
                    threadId,
                    lineageParentNodeId: lineageAssignment.lineageParentNodeId,
                    generationRequestId: generationRun?.generationRequestId ?? '',
        ...
        if (!this.isCurrent(scope)) return
        const nodeId = getPendingGeneratedMediaNodeId(lineageAssignment)
        this.ports.trackers.images.set(runKey, {
            nodeId,
        ...
        if (!this.isCurrent(scope)) return
        pruneGeneratedMediaTrackerAliases(this.ports.trackers.images, runKey, nodeId)
        const tracker = this.ports.trackers.images.get(runKey)
        if (!this.isCurrent(scope)) return
        if (!tracker) return undefined
        this.ports.debugGeneratedMediaLifecycle('create-image-placeholder', {
            runKey,
            threadId,
            nodeId,
            assetId: tracker.assetId,
        ...
        const existingNodes = this.ports.lineage.addBranchLineageMarkerNodesIfMissing(this.currentCanvasState.nodes, branchOriginNode, branchForkNode, branchLineNode)
        if (!this.isCurrent(scope)) return
        ...
        if (!this.isCurrent(scope)) return
        if (imageUrl) this.ports.clearGeneratingReferencesOnFirstPixels(threadId, generationRun)
        if (!this.isCurrent(scope)) return
        return tracker
    }
    ...
    prepareGeneratedImageCompletionTextureHandoff(
        ...
        this.ports.visuals.keepCompletion(runKey, previousTracker, {
            nodeId: completedNodeId,
            assetId: completedAssetId,
        })
        if (!this.isCurrent(scope)) return
    ...
    shouldAcceptGeneratedMediaEvent(
        ...
        const workspaceId = scope.workspaceId
        const generationRequestId = this.getGenerationRequestId(generationRun)
        if (generationRequestId && this.ports.placements.cancelledRequests.has(generationRequestId)) return false
