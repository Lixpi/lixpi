...
const safeZoom = (viewport: CanvasViewport): number => (Number.isFinite(viewport.zoom) ? Math.max(viewport.zoom, 0.01) : 1)
...
export class WorkspaceVideoChrome {
    ...
    outsideOffsetScreen(
        ...
        return scaleCanvasChromeToScreenForZoom(
            settings.canvas.bottomInset,
            zoom,
            scaling,
        )
            + scaleCanvasChromeToScreenForZoom(
...
class VideoChromeEntry {
    ...
    constructor(
        ...
        private readonly options: WorkspaceVideoChromeOptions,
    ) {
        const html = createDocumentHtml(options.document)
        this.element = html`
            <div
                className="workspace-video-chrome"
                data=${{ videoChromeNodeId: nodeId }}
            >
        ...
        try {
            ...
            this.lifetime.own(() => {
                if (video.parentNode !== this.surface)
                    return
                ...
                if (originalNextSibling?.parentNode === originalParent)
                    originalNextSibling.before(video)
                else
            ...
            this.listen('mouseleave', () => void (this.surface.style.cursor = ''))
            this.listen('mousedown', event => {
                ...
                if (corner)
                    ...
                    options.startDrag(event, nodeId)
            })
            ...
            const svg = select(this.host)
                ...
                .attr('height', '100%')
            this.svg = svg.node()!
            this.controls = createVideoControls(
                svg,
                {
    ...
    private layout(
        ...
        const insetX = scaleCanvasChromeWorldSizeForZoom(
            baseInset,
            zoom,
            scaling,
        )
        ...
        return {
            insetX,
            width,
            top: bounds.height + scaleCanvasChromeWorldSizeForZoom(
                settings.canvas.bottomInset,
                zoom,
                scaling,
            ),
    ...
    update(
        ...
        applyStyle(
            this.element,
            {
                ...
                height: `${layout.top + layout.height}px`,
            },
        )
        applyStyle(
            this.surface,
    ...
    private resizeCorner(event: MouseEvent): WorkspaceResizeCorner | null {
        ...
        const {
            size,
            offset,
        } = settings.useZoomCompensatedScaling
            ? getResizeHandleScaledSizes(
                zoom,
                {
