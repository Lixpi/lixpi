...
class ArtifactLibraryPanel implements ArtifactLibraryPanelInstance {
    ...
    constructor(private readonly options: ArtifactLibraryPanelOptions) {
        ...
        this.rootEl = this.html`
            <div className="media-library-panel media-library-panel-embedded artifact-library-panel nopan nowheel">
    ...
    private async load(): Promise<void> {
        ...
        try {
            ...
            const attachable = metadata.filter(item => isAttachableToWorkspace(item, this.options.workspaceId)).filter(
                item => Boolean(item.artifactTypeId),
            )
                .sort((left, right) => right.updatedAt - left.updatedAt)
            const entries = await Promise.all(
                attachable.map(async meta => await this.loadEntry(meta)),
            )
            ...
            this.browserEl.replaceChildren(
                this.html`<div className="media-library-state media-library-state-error">Could not load Artifacts.</div>` as HTMLElement,
            )
        }
    }
    ...
    private async loadEntry(meta: AssetMeta): Promise<ArtifactLibraryEntry | null> {
        ...
        if (
            !definition
            || !snapshot
            || definition.schemaVersion !== asset.artifact.schemaVersion
        )
            return null
        ...
        return {
            meta,
            displayMetadata: definition.buildCatalogMetadata(snapshot.doc),
        }
    }
    ...
    private render(): void {
        ...
        try {
            ...
            if (this.selectedAssetId)
                void this.renderInspector(this.selectedAssetId)
            else {
            ...
            this.options.onError(error)
            this.browserEl.replaceChildren(
                this.html`<div className="media-library-state media-library-state-error">Could not display Artifacts.</div>` as HTMLElement,
            )
        }
    }
    ...
    private createRow(entry: ArtifactLibraryEntry): HTMLElement {
        ...
        const host = this.html`
            ...
            ><div className="artifact-library-item-host"></div><button
                    type="button"
                    className="capability-library-row-action"
                    data-action="inspect"
                >Details</button></article>
        ` as HTMLElement
    ...
    private async renderInspector(assetId: string): Promise<void> {
        ...
        try {
            ...
            if (
                ...
                this.inspectorEl.replaceChildren(
                    this.html`<div className="media-library-state media-library-state-error">Artifact unavailable.</div>` as HTMLElement,
                )
            ...
            if (
                ...
                this.inspectorEl.replaceChildren(
                    this.html`<div className="media-library-state media-library-state-error">Artifact definition unavailable.</div>` as HTMLElement,
                )
            ...
            const view = frontend.createGeneratedOutputInfoView({
                container: infoHost,
                document: snapshot.doc,
            })
            lifetime.own(() => view.destroy())
            await this.mountHistory(
                asset,
                detail,
                lifetime,
            )
            ...
            this.options.onError(error)
            this.inspectorEl.replaceChildren(
                this.html`<div className="media-library-state media-library-state-error">Could not load Artifact details.</div>` as HTMLElement,
            )
        }
    }
    ...
    private createInspector(asset: Asset): HTMLElement {
        const detail = this.html`
            <section
                className="media-library-detail artifact-library-detail"
                data-side-panel-no-drag="true"
            >
            <button
                type="button"
    ...
    private async accept(
        ...
        await runLibraryAction(
            ...
            () => this.options.onAcceptAsset!(asset),
            accepted => {
    ...
    private async updateTitle(
        ...
        await runLibraryAction(
            this.viewLifetime.signal,
            () => this.options.assets.updateMetadata(
                asset.assetId,
                asset.revision,
                { title },
            ),
    ...
    private async updateScope(
        ...
        await runLibraryAction(
            this.viewLifetime.signal,
            () => this.options.assets.changeScope(
                asset.assetId,
                asset.revision,
                scope,
                scopeOwnerId,
...
export const createArtifactLibraryPanel = (options: ArtifactLibraryPanelOptions): ArtifactLibraryPanelInstance => new ArtifactLibraryPanel(options)
