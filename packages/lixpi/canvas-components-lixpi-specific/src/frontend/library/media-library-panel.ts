...
export type MediaLibraryPanelOptions = WorkspaceLibraryPorts & {
    ...
    attestSubjectIdentity: (assetId: string, revision: number, classification: SubjectIdentityClassification) => Promise<LibraryAssetResult>
...
class MediaLibraryPanel implements MediaLibraryPanelInstance {
    ...
    private renditionUrl: ((assetId: string, rendition: string) => string) | null = null
    private isMounted = false
    private loadSequence = 0

    constructor(private readonly options: MediaLibraryPanelOptions) {
        this.html = createDocumentHtml(options.document)
        this.rootEl = this.html`<div className="media-library-panel media-library-panel-embedded media-library-panel-images nopan nowheel">
    ...
    private async load(): Promise<void> {
        ...
        try {
            ...
            this.browserEl.replaceChildren(this.html`<div className="media-library-state media-library-state-error">Could not load media.</div>` as HTMLElement)
        }
    }
    ...
    private render(): void {
        ...
        try {
            ...
            this.destroyInspectorResources()
            this.options.onError(error)
            this.browserEl.replaceChildren(this.html`<div className="media-library-state media-library-state-error">Could not display media.</div>` as HTMLElement)
        }
    }

    private buildAssetRow(asset: AssetMeta): HTMLElement {
        const thumbEl = asset.thumbnailBlobHash
            ? this.html`<img className="capability-library-row-thumb" src=${this.getAssetRenditionUrl(asset.assetId, 'thumbnail')} alt="" />` as HTMLElement
            : this.html`<div className="capability-library-row-thumb-placeholder" aria-hidden="true"></div>` as HTMLElement
        const metadata = [
        ...
        >
            ...
            <button type="button" className="capability-library-row-action capability-library-row-action-primary" data-action="insert" data-side-panel-no-drag="true">Add</button>
    ...
    private async insertAsset(asset: AssetMeta): Promise<void> {
        ...
        if (!this.isMounted || lifetime.signal.aborted) return
        try {
    ...
    private async renderAssetInspector(assetId: string): Promise<void> {
        ...
        try {
            ...
            if ('error' in asset) {
                this.inspectorEl.replaceChildren(this.html`<div className="media-library-state media-library-state-error">Asset unavailable: ${asset.error}</div>` as HTMLElement)
                return
            }
            const detail = this.buildAssetInspector(asset)
            this.inspectorEl.replaceChildren(detail)
            await this.mountAssetDocuments(asset, detail, lifetime)
        } catch (error) {
            if (!lifetime.signal.aborted && sequence === this.inspectorSequence) {
                this.options.onError(error)
                this.inspectorEl.replaceChildren(this.html`<div className="media-library-state media-library-state-error">Could not load Asset details.</div>` as HTMLElement)
            }
        }
    }
    ...
    private buildAssetInspector(asset: Asset): HTMLElement {
        const lifetime = this.viewLifetime
        const detail = this.html`<section className="media-library-detail" data-side-panel-no-drag="true">
        ...
        const subjectIdentityControl = mountAssetSubjectIdentityControl({
            host: identityMount,
            asset,
            attestSubjectIdentity: (assetId, revision, classification) => this.options.attestSubjectIdentity(assetId, revision, classification),
            onUpdated: updated => {
                this.refreshAssetState(stateEl, updated)
            },
            onError: message => {
                stateEl.textContent = `Subject identity update failed: ${message}`
            },
        })
        ...
        this.listen(detail.querySelector('.media-library-detail-remove')!, 'click', () => void this.removeAsset(asset, stateEl))
        this.listen(titleInput, 'change', () => void this.updateAssetTitle(asset, titleInput, stateEl))
        this.listen(scopeSelect, 'change', () => void this.updateAssetScope(asset, scopeSelect, stateEl))
        return detail
    }

    private async removeAsset(asset: Asset, stateEl: HTMLElement): Promise<void> {
        await runLibraryAction(this.viewLifetime.signal, () => this.options.removeFromLibrary(asset.assetId), result => {
            if (result?.error) {
    ...
    private async updateAssetTitle(asset: Asset, titleInput: HTMLInputElement, stateEl: HTMLElement): Promise<void> {
        ...
        await runLibraryAction(signal, async () => {
            const current = await this.options.assets.get(asset.assetId, this.options.workspaceId)
            if (signal.aborted || 'error' in current) return
            const updated = await this.options.assets.updateMetadata(current.assetId, current.revision, { title })
            ...
            } else this.refreshAssetState(stateEl, result.updated)
        }, this.options.onError)
    }
    ...
    private async updateAssetScope(asset: Asset, scopeSelect: HTMLSelectElement, stateEl: HTMLElement): Promise<void> {
        ...
        await runLibraryAction(signal, async () => {
            const current = await this.options.assets.get(asset.assetId, this.options.workspaceId)
            if (signal.aborted || 'error' in current) return
            const scopeOwnerId = scope === 'workspace' ? this.options.workspaceId : scope === 'user' ? this.options.userId : current.organizationId
            if (!scopeOwnerId) return
            const updated = await this.options.assets.changeScope(current.assetId, current.revision, scope, scopeOwnerId)
            return { current, updated }
        }, result => {
            if (!result) return
            if ('error' in result.updated) {
    ...
    private async mountAssetDocuments(asset: Asset, detail: HTMLElement, lifetime: Lifetime): Promise<void> {
        if (asset.documents.content) {
            await this.options.assets.resumeDocument({ organizationId: asset.organizationId, assetId: asset.assetId, role: 'content' })
            if (lifetime.signal.aborted) return
            ...
            if (snapshot) {
                const editor = new WorkspaceAssetContentEditor(
                    ...
                    { organizationId: asset.organizationId, workspaceId: this.options.workspaceId, assetId: asset.assetId, role: 'content', baseVersion: snapshot.version },
