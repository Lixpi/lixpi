...
class CapabilityLibraryPanel implements CapabilityLibraryPanelInstance {
    ...
    constructor(private readonly config: CapabilityLibraryPanelConfig) {
        ...
        this.element = this.html`<div className="media-library-panel capability-library-panel nopan nowheel">
    ...
    async load(): Promise<void> {
        ...
        try {
            ...
            if (loadSequence !== this.loadSequence) return
            this.browserElement.replaceChildren(this.html`<div className="media-library-state media-library-state-error">Could not load Capabilities.</div>` as HTMLElement)
        }
    }
    ...
    private render(): void {
        ...
        this.browserElement.replaceChildren(this.html`<div className="media-library-browser-intro">
            <h2>Capabilities</h2>
        ...
        else {this.inspectorElement.appendChild(this.html`<div className="capability-library-inspector-empty">
    ...
    private buildRow(item: CapabilityCatalogItem): HTMLElement {
        ...
        const select = (): void => {
            void this.select(item.capabilityId)
        }
    ...
    private async select(capabilityId: string): Promise<void> {
        ...
        try {
            ...
            if (detailsSequence !== this.detailsSequence) return
            this.inspectorElement.replaceChildren(this.html`<div className="media-library-state media-library-state-error">Could not load Capability details.</div>` as HTMLElement)
        }
    }

    private buildInspector(details: CapabilityDetails): HTMLElement {
        const inspector = this.html`<section className="capability-library-inspector-card" data-side-panel-no-drag="true">
        ...
        this.listen(inspector.querySelector<HTMLElement>('.capability-library-back')!, 'click', () => {
            this.selected = null
            this.render()
        })
        this.listen(inspector.querySelector<HTMLElement>('.capability-library-attach')!, 'click', () => this.attach(details))
        return inspector
    }

    private listen<Type extends keyof HTMLElementEventMap>(element: HTMLElement, type: Type, listener: (event: HTMLElementEventMap[Type]) => void): void {
