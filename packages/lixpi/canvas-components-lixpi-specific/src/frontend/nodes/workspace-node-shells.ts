...
export type WorkspaceNodeShellsOptions = {
    ...
    startDrag: (event: MouseEvent, nodeId: string, options: Pick<WorkspaceNodeInteractionOptions, 'allowSelection' | 'onClick'>) => void
    startResize: (event: MouseEvent, nodeId: string, corner: WorkspaceResizeCorner) => void
    onCreate: (element: HTMLElement, nodeId: string) => void
    togglePlayback: (nodeId: string) => void
}

type Entry = { shell: NodeShell; lifetime: Lifetime }
export type WorkspaceNodeElements = { nodeEl: HTMLElement; dragOverlay: HTMLElement; own: (dispose: Dispose) => Dispose }
...
export class WorkspaceNodeShells {
    ...
    create(node: CanvasNode, extraClasses?: string, data?: Record<string, string>, interaction: WorkspaceNodeInteractionOptions = {}): WorkspaceNodeElements {
        if (this.destroyed) throw new Error('Workspace node shells are disposed')
        ...
        try {
            shell = new NodeShell({
                ...
                onClick: event => this.click(event, node.nodeId, interaction),
                onDragStart: event => {
                    ...
                    this.options.startDrag(event, node.nodeId, {
                ...
                resize: interaction.renderResizeHandles === false ? undefined : {
                    ...
                    measure: zoom => {
                        ...
                        return settings.useZoomCompensatedScaling
                            ...
                            : { size: settings.size, offset: settings.offset }
                    },
                    onPointerDown: (event, corner) => this.options.startResize(event, node.nodeId, corner as WorkspaceResizeCorner),
                },
            })
        ...
        if (previous?.shell.element.parentNode) previous.shell.element.replaceWith(shell.element)
        this.entries.set(node.nodeId, { shell, lifetime })
        previous?.lifetime.destroy()
        return { nodeEl: shell.element, dragOverlay: shell.dragOverlay, own: dispose => lifetime.own(dispose) }
    }

    createMedia(node: WorkspaceMediaNode): HTMLElement {
        const name = node.type === 'mediaDocument' ? 'media-document' : node.type
        const { nodeEl, dragOverlay } = this.create(node, `workspace-${name}-node`, { assetId: node.assetId })
    ...
    createBranchMarker(node: BranchOriginCanvasNode | BranchForkCanvasNode | BranchLineCanvasNode, onClick: () => void): WorkspaceNodeElements {
        const kind = node.type === 'branchOrigin' ? 'origin' : node.type === 'branchFork' ? 'fork' : 'line'
        const data = {
        ...
        const elements = this.create(node, `workspace-branch-${kind}-node`, data, { renderResizeHandles: false, allowSelection: false, onClick })
        elements.dragOverlay.className = `branch-${kind}-drag-overlay nopan`
        return elements
    }

    private click(event: MouseEvent, nodeId: string, interaction: WorkspaceNodeInteractionOptions): void {
        event.stopPropagation()
        if (this.options.consumeSuppressedClick()) return
        const target = event.target as HTMLElement | null
        if (target && (target.isContentEditable || target.closest('.ProseMirror, .ai-chat-thread-wrapper'))) return
        if (event.metaKey || event.ctrlKey) {
            if (interaction.allowSelection !== false) this.options.toggleSelection(nodeId)
            else interaction.onClick?.()
            return
        }
        if (interaction.allowSelection !== false) this.options.select(nodeId)
        interaction.onClick?.()
    }
    ...
    replace(nodeId: string, mount: () => HTMLElement): HTMLElement {
        ...
        try {
            ...
            this.remove(nodeId)
            if (previous) this.entries.set(nodeId, previous)
            throw error
        }
        if (previous?.shell.element.parentNode) previous.shell.element.replaceWith(element)
        previous?.lifetime.destroy()
        return element
    }
