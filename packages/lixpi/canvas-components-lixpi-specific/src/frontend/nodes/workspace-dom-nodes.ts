...
export type WorkspaceDomNodesOptions = {
    ...
    updateBranch: (node: BranchMarker, element: HTMLElement) => void
}
...
class WorkspaceDomNode implements WorkspaceDomNodeView {
    ...
    constructor(private node: CanvasNode, private readonly options: WorkspaceDomNodesOptions) {
        try {
    ...
    private create(node: CanvasNode): HTMLElement {
        if (node.type === 'document') return this.options.document(node)
        if (node.type === 'capabilityArtifact') return this.options.capability(node)
        if (node.type === 'operationStatus') return this.options.operation(node)
        if (node.type === 'branchOrigin' || node.type === 'branchFork' || node.type === 'branchLine') return this.options.branch(node)
    ...
    private needsReplacement(previous: CanvasNode, node: CanvasNode): boolean {
        if (node.type === 'document' || node.type === 'capabilityArtifact') {
            return !('assetId' in previous) || previous.assetId !== node.assetId
        }
        if (node.type !== 'operationStatus') return false
        const { position: _position, dimensions: _dimensions, parentId: _parentId, ...content } = node
        const { position: _previousPosition, dimensions: _previousDimensions, parentId: _previousParentId, ...previousContent } = previous
