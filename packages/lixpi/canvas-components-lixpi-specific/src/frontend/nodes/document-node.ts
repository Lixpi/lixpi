...
export type WorkspaceDocumentLeaseState = { readOnly: boolean; holderWorkspaceId?: string; expiresAt?: number }
...
export class WorkspaceDocumentNodes {
    ...
    constructor(private readonly shells: WorkspaceNodeShells, private readonly options: WorkspaceDocumentNodesOptions) {}

    create(node: DocumentCanvasNode, document: WorkspaceDocument | undefined): HTMLElement {
        if (this.destroyed) throw new Error('Workspace document nodes are disposed')
        const shell = this.shells.create(node, undefined, { assetId: node.assetId })
        const view = new WorkspaceDocumentNode(node, shell.nodeEl, document, this.options)
        this.views.set(node.nodeId, view)
        shell.dragOverlay.className = 'document-drag-overlay nopan'
        shell.own(() => {
            if (this.views.get(node.nodeId) === view) this.views.delete(node.nodeId)
            view.destroy()
        })
        return shell.nodeEl
    }
    ...
    syncDocuments(documents: readonly WorkspaceDocument[]): void {
        ...
        for (const view of this.views.values()) view.setDocument(byId.get(view.assetId))
    }
...
class WorkspaceDocumentNode {
    ...
    constructor(private readonly node: DocumentCanvasNode, private readonly element: HTMLElement, document: WorkspaceDocument | undefined, private readonly options: WorkspaceDocumentNodesOptions) {
        this.assetId = node.assetId
        const html = createDocumentHtml(element.ownerDocument)
        this.container = html`<div className="document-node-editor nopan" data-help-tooltip="aria-description"></div>` as HTMLDivElement
    ...
    setDocument(document: WorkspaceDocument | undefined): void {
        if (this.lifetime.signal.aborted || this.mounted) return
        ...
        try {
            const editor = this.options.mountEditor({
                ...
                onLeaseStateChange: state => {
                    if (!content.signal.aborted) this.setLease(state)
                },
            })
    ...
    private setLease(state: WorkspaceDocumentLeaseState): void {
        ...
        if (description) this.container.setAttribute('aria-description', description)
        else this.container.removeAttribute('aria-description')
    }
