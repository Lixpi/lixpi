...
export const getStashedViewportStorageKey = (workspaceId: string): string => `${STASH_KEY_PREFIX}${workspaceId}`

export const encodeStashedViewport = (viewport: ViewportSnapshot): string => JSON.stringify({ viewport })

export const parseStashedViewport = (raw: string | null): ViewportSnapshot | null => {
    if (!raw)
        return null
...
export const shouldApplyStashedViewport = (
    stashedViewport: ViewportSnapshot,
    serverViewport: ViewportSnapshot | null | undefined,
): boolean => !viewportsMatch(stashedViewport, serverViewport)
