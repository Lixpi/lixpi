...
export class WorkspaceViewportPersistence {
    ...
    constructor(
        private readonly session: WorkspaceCanvasSession,
        private readonly ports: WorkspaceViewportPersistencePorts,
    ) {
    ...
    sync(
        ...
        if (canvasState)
            this.latestState = canvasState

        if (
            !this.pending
            && canvasState
            && viewportsMatch(viewport, canvasState.viewport)
        )
    ...
    restore(serverViewport: ViewportSnapshot | null | undefined): void {
        ...
        if (
            !viewport
            || !shouldApplyStashedViewport(viewport, serverViewport)
        )
            return
    ...
    stashForUnload(): void {
        ...
        if (
            !inFlight
            && viewportsMatch(viewport, this.lastSubmitted)
        )
            return

        try {
            this.ports.storage.set(
                getStashedViewportStorageKey(this.workspaceId),
                encodeStashedViewport(viewport),
            )
        } catch {
    ...
    private submit(viewport: ViewportSnapshot): boolean {
        if (viewportsMatch(viewport, this.lastSubmitted))
            return true

        const state = this.ports.readCanvasState()
            ?? this.latestState
            ?? this.session.persistence.read()?.canvasState
        ...
        try {
            this.session.persistence.update(
                {
                    ...state,
                    viewport: { ...viewport },
                },
