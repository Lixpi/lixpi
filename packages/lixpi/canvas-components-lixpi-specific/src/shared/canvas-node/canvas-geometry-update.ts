...
const edgeTouchesAnyNode = (
    edge: WorkspaceEdge,
    nodeIds: Set<string>,
): boolean => nodeIds.has(edge.sourceNodeId) || nodeIds.has(edge.targetNodeId)

const buildCompletedGeneratedMediaRunIdentities = (nodes: CanvasNode[]): Set<string> => {
...
const canvasNodesMatch = (
    left: CanvasNode,
    right: CanvasNode,
): boolean => JSON.stringify(left) === JSON.stringify(right)

const isApiOwnedCanvasSnapshotNode = (node: CanvasNode): boolean => {
...
const isEmptyGeneratedMediaSnapshotStaleForExistingFrame = (
    existing: CanvasNode,
    snapshot: CanvasNode,
): boolean => {
    if (
        !isGeneratedMediaCanvasNode(existing)
        || !isPendingGeneratedMediaCanvasNode(snapshot)
    )
        return false
...
export const applyCanvasGeometryUpdateToState = (
    state: CanvasState,
    update: CanvasGeometryUpdate,
): ApplyCanvasGeometryUpdateResult => {
    const geometryByNodeId = new Map(
        update.nodes.map(geometry => [geometry.nodeId, geometry]),
    )
    ...
    const stalePendingSnapshotNodeIds = new Set(
        (update.nodeSnapshots ?? []).filter(snapshot => {
            if (!isPendingGeneratedMediaCanvasNode(snapshot))
                return false
    ...
    if (requestedRemovedNodeIds.size > 0) {
        nodes = state.nodes.filter(node => {
            if (!requestedRemovedNodeIds.has(node.nodeId))
                return true
    ...
    if (
        requestedRemovedNodeIds.size > 0
        || requestedRemovedEdgeIds.size > 0
    ) {
    ...
    const snapshotNodesById = new Map(
        snapshotNodes.map(snapshot => [snapshot.nodeId, snapshot]),
    )
    ...
    if (snapshotNodesById.size > 0) {
        nodes = nodes.map(node => {
            ...
            if (canvasNodesMatch(node, snapshot))
                return node
    ...
    const resolvedNodes = nodes.map(node => {
        ...
        const result = applyNodeGeometry(
            node,
            {
    ...
    const resolvedNodeIds = new Set(
        resolvedNodes.map(node => node.nodeId),
    )
    const missingGeometryNodeIds = update.nodes.filter(geometry => !resolvedNodeIds.has(geometry.nodeId)).map(geometry => geometry.nodeId)

    const upsertedEdgeIds: string[] = []
    let resolvedEdges = edges
    const edgesById = new Map(
        resolvedEdges.map(edge => [edge.edgeId, edge]),
    )
    ...
    for (const snapshot of update.edgeSnapshots ?? []) {
        ...
        if (
            existing
            && workspaceEdgesMatch(existing, snapshot)
        )
            continue
