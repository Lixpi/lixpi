...
export class WorkspaceGenerationSettlement {
    ...
    applyMediaBranchLineagePlan(
        ...
        const placement = this.ports.placements.ensurePendingGeneratedMediaPlacementForApiRun(
            threadId,
            generationRun,
            {
        ...
        const nextPlacement: PendingGeneratedImagePlacement = {
            ...
            activeRunKeys: new Set([
                ...
                ...lineagePlan.runAssignments.map(assignment => assignment.mediaRunId ?? assignment.reasoningRunId).filter(
        ...
        if (
            lineagePlan.regenerationTarget
            && !lineagePlan.regenerationTarget.sourceMediaNodeId
        ) {
        ...
        this.ports.preflight.insertPendingBranchMarkersFromLineagePlan(
            threadId,
            lineagePlan,
            generationRun,
        )
        ...
        this.resolvePendingBranchMarkersForLineagePlan(
            threadId,
            lineagePlan,
            generationRun,
        )
    ...
    resolvePendingBranchMarkersForLineagePlan(
        ...
        for (const assignment of assignments) {
            this.ports.handoff.resolvePendingBranchMarkerWithLineagePlan(
                threadId,
                this.ports.lineage.buildGenerationRunFromLineageAssignment(
                    lineagePlan,
                    assignment,
                    sourceGenerationRun,
                ),
    ...
    cleanupOrphanPreflightMarkersForThread(threadId: string): void {
        ...
        const orphanNodeIds = this.currentCanvasState.nodes.filter(
        ...
        if (orphanNodeIds.length > 0) {
            this.ports.commit({
                ...
                edges: this.currentCanvasState.edges.filter(
                    (edge: WorkspaceEdge) => !orphanNodeIds.includes(edge.sourceNodeId) && !orphanNodeIds.includes(edge.targetNodeId),
                ),
            })
    ...
    finishGeneratedMediaRun(
        ...
        if (generationRun.mediaRunId)
            activeRunKeys.delete(generationRun.mediaRunId)
        ...
        if (activeRunKeys.size > 0) {
            this.ports.log(
                '[CANVAS] generated media run finished with siblings still active',
                {
                    threadId,
            ...
            this.ports.placements.placements.set(
                placementKey,
                {
                    ...placement,
                    activeRunKeys,
        ...
        if (placementKey !== threadId) {
            ...
            for (const key of this.ports.placements.markers.keys()) {
                if (key.startsWith(initialReasoningModelPrefix))
                    this.ports.placements.markers.delete(key)
            }
        }
        ...
        this.ports.log(
            '[CANVAS] final generated media run settled detached canvas state',
            {
                threadId,
                generationRequestId: generationRun.generationRequestId,
                mediaRunId: generationRun.mediaRunId,
    ...
    finishFailedGeneratedMediaRun(
        ...
        if (this.ports.placements.placements.has(placementKey))
            return
    ...
    settleBranchMarkersForGenerationRequest(
        ...
        const nodes = this.currentCanvasState.nodes.map((node: CanvasNode): CanvasNode => {
            ...
            if (!node.pendingState) {
                if (hadTrackedUiPhase)
                    markersToSync.push(node)
            ...
            const positionedSettledNode = !options.preserveGeometry
                && this.ports.isManuallyPositioned(node.nodeId)
                ? {
                    ...settledNode,
    ...
    settleMediaGenerationRequest(
        ...
        const plannedRuns: Array<MediaGenerationRunMeta | undefined> = lineagePlan
            ? this.ports.lineage.getUniqueLineageAssignmentsForMarkers(lineagePlan).map(
                assignment => this.ports.lineage.buildGenerationRunFromLineageAssignment(
                    lineagePlan,
                    assignment,
                    generationRun,
                ),
        ...
        if (plannedRuns.length === 0)
            plannedRuns.push(generationRun)

        for (const plannedRun of plannedRuns) {
            const targetRun = plannedRun ?? generationRun
            this.ports.handoff.clearPendingBranchMarkerStateForRun(
                threadId,
                targetRun,
                options,
            )
