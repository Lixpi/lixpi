...
export const uniqueAiModelIds = (modelIds: Array<string | undefined>): AiModelId[] => {
...
export const splitAiModelId = (modelId: string): {
    provider: string
    model: string
} => {
...
export const getGeneratedMediaModelId = (node: ImageCanvasNode | VideoCanvasNode): string => {
    const generatedBy = node.generatedBy

    if (!generatedBy)
        return String(node.generationProgress?.mediaModelId ?? '')
...
export const getGeneratedMediaModelProvider = (
    ...
    if (node.generationProgress?.mediaModelProvider)
        return node.generationProgress.mediaModelProvider
