...
export class WorkspaceBranchMarkerHandoff {
    ...
    rememberPlannedBranchMarkerRecord(
        ...
        this.ports.placements.setPendingBranchMarkerRecordAliases(
            threadId,
            generationRun,
            plannedRecord,
        )
    ...
    findLineageAssignmentForGenerationRun(
        ...
        return lineagePlan.runAssignments.find(
            assignment =>
                ...
                || (
                    ...
                    && this.ports.lineage.getLineageAssignmentReasoningIndex(
                        lineagePlan,
                        assignment,
                        generationRun,
    ...
    applyPendingStateToSyncedPlannedBranchMarker(
        ...
        if (!pendingState)
            return this.ports.resizeMarker(plannedNode)
    ...
    syncPlannedBranchMarkerResolution(
        ...
        for (const node of currentCanvasState.nodes) {
            if (retiredOwnerNodeIds.has(node.nodeId))
                continue
        ...
        for (const node of nodesById.values()) {
            if (insertedNodeIds.has(node.nodeId))
                continue
        ...
        let edges = retiredOwnerNodeIds.size > 0
            ? currentCanvasState.edges.filter(
                (edge: WorkspaceEdge) => !retiredOwnerNodeIds.has(edge.sourceNodeId) && !retiredOwnerNodeIds.has(edge.targetNodeId),
            )
        ...
        edges = plannedNode.type === 'branchFork'
            || plannedNode.type === 'branchLine'
        ...
        this.rememberPlannedBranchMarkerRecord(
            threadId,
            generationRun,
            previousRecord,
            plannedNode.nodeId,
        ...
        this.ports.debugHandoff(
            'sync-planned-marker-resolution',
            plannedNode,
            {
        ...
        this.ports.log(
            'info',
            '[CANVAS] incremental branch marker ownership handoff',
            {
                threadId,
    ...
    resolvePendingBranchMarkerWithLineagePlan(
        ...
        if (regenerationTarget) {
            const markerNode = currentCanvasState.nodes.find(
            ...
            if (!markerNode) {
                ...
                this.ports.log(
                    'error',
                    '[CANVAS][generated-output-review] API regeneration lineage parent is missing.',
                    {
                        threadId,
                        generationRequestId: generationRun?.generationRequestId,
                        regenerationTarget,
        ...
        if (!record) {
            ...
            if (plannedNode) {
                ...
                this.syncPlannedBranchMarkerResolution(
                    threadId,
                    generationRun,
                    this.ports.placements.createPendingBranchMarkerRecordFromCanvasNode(
                        threadId,
                        generationRun,
                        plannedNode,
                    ),
        ...
        if (
            !pendingNode
            || !isBranchMarkerNode(pendingNode)
            || !pendingNode.pendingState
        ) {
            ...
            if (!plannedResolution.primaryNode)
                return
        ...
        this.ports.debugHandoff(
            'promote-planned-marker',
            plannedNodeWithPending,
            {
        ...
        const supportNodesById = new Map<string, BranchMarkerNode>(
            supportNodes.map(node => [node.nodeId, node]),
        )
        ...
        for (const node of currentCanvasState.nodes) {
            ...
            if (
                retiredOwnerNodeIds.has(node.nodeId)
                || node.nodeId === plannedNodeWithPending.nodeId
            ) {
        ...
        let edges = currentCanvasState.edges.filter(
            (edge: WorkspaceEdge) => !retiredOwnerNodeIds.has(edge.sourceNodeId) && !retiredOwnerNodeIds.has(edge.targetNodeId),
        )
        edges = this.ports.lineage.addBranchMarkerEdgeIfMissing(edges, plannedResolution.branchForkNode)
        edges = this.ports.lineage.addBranchMarkerEdgeIfMissing(edges, plannedResolution.branchLineNode)
        edges = plannedNodeWithPending.type === 'branchFork'
            || plannedNodeWithPending.type === 'branchLine'
    ...
    forgetPendingBranchMarkerRecordForRun(
        threadId: string,
        generationRun?: MediaGenerationRunMeta,
    ): void {
        const placementKey = this.ports.placements.getGeneratedMediaPlacementKey(threadId, generationRun)
        ...
        if (record)
            this.ports.placements.deletePendingBranchMarkerAliasesForNodeId(record.nodeId)
        else
        ...
        if (
            record
            && threadRecord?.nodeId === record.nodeId
        )
            this.ports.placements.markers.delete(threadId)
    }

    removePendingBranchMarkerForRun(
        threadId: string,
        generationRun?: MediaGenerationRunMeta,
    ): void {
        const capturedScope = this.ports.readScope()
        ...
        if (!record) {
            const placementKey = this.ports.placements.getGeneratedMediaPlacementKey(threadId, generationRun)
            const removableGenerationRequestIds = new Set([
                placementKey,
                threadId,
            ])
            const removableNodeIds = currentCanvasState.nodes.filter(
                (node: CanvasNode): node is BranchMarkerNode =>
                        isBranchMarkerNode(node)
            ...
            this.ports.commit({
                ...
                edges: currentCanvasState.edges.filter(
                    (edge: WorkspaceEdge) => !removableNodeIds.includes(edge.sourceNodeId) && !removableNodeIds.includes(edge.targetNodeId),
                ),
            })
        ...
        if (
            !markerNode
            || !isBranchMarkerNode(markerNode)
            || !markerNode.pendingState
        )
            return
        ...
        this.ports.commit({
            ...
            edges: currentCanvasState.edges.filter(
                (edge: WorkspaceEdge) => edge.sourceNodeId !== record.nodeId && edge.targetNodeId !== record.nodeId,
            ),
        })
    ...
    buildPendingBranchMarkerSpecsFromLineagePlan(
        ...
        return assignments.map(assignment => {
            const generationRun = this.ports.lineage.buildGenerationRunFromLineageAssignment(
                lineagePlan,
                assignment,
                sourceGenerationRun,
            )
            ...
            return {
                ...
                pendingState: {
                    ...
                    reasoningModelIds: uniqueAiModelIds([reasoningModelId]),
                    ...(reasoningModelId ? { reasoningModelId } : {}),
                    reasoningIndex: this.ports.lineage.getLineageAssignmentReasoningIndex(
                        lineagePlan,
                        assignment,
                        sourceGenerationRun,
                    ),
