...
export function isMediaGenerationReferenceResolutionOperation(
    node: CanvasNode,
): node is OperationStatusCanvasNode & { operation: 'media-generation'; status: 'action-required' } {
...
function isBranchMarker(node: CanvasNode): node is ReferenceResolutionBranchMarker {
    return node.type === 'branchOrigin' || node.type === 'branchFork' || node.type === 'branchLine'
}

function getMarkerPriority(node: ReferenceResolutionBranchMarker): number {
    if (node.pendingState?.phase === 'preflight') return 0
    if (node.type === 'branchOrigin') return 1
    if (node.type === 'branchFork') return 2
    return 3
}

export function getMediaGenerationReferenceResolutionOwner(
    nodes: readonly CanvasNode[],
    generationRequestId: string,
): ReferenceResolutionBranchMarker | undefined {
    return nodes
        .filter((node): node is ReferenceResolutionBranchMarker => (
...
export function getMediaGenerationReferenceResolutionForMarker(
    nodes: readonly CanvasNode[],
    ...
    if (!operation?.generationRequestId) return undefined
