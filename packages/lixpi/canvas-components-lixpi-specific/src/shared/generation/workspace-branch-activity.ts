...
export type BranchActivityPlacement = { activeRunKeys?: ReadonlySet<string>; lineagePlan?: MediaBranchLineagePlan }
...
export function lineagePlanReferencesBranchMarkerNode(lineagePlan: MediaBranchLineagePlan, node: BranchMarkerNode): boolean {
...
export function getBranchMarkerPlacementKeys(node: BranchMarkerNode): string[] {
...
export function assignmentMatchesBranchMarker(assignment: MediaRunLineageAssignment, node: BranchMarkerNode): boolean {
...
export function isLineageAssignmentActive(assignment: MediaRunLineageAssignment, activeRunKeys: ReadonlySet<string>): boolean {
    return Boolean(
...
export class WorkspaceBranchActivity {
    ...
    isBranchMarkerGenerationActive(node: BranchMarkerNode): boolean {
        if (this.ports.isCancelled(node)) return false
        if (this.ports.hasStartedMedia(node.nodeId)) return false
        if (node.pendingState || this.ports.isPending(node)) return true
    ...
    isBranchMarkerGenerationGroupActive(node: BranchMarkerNode): boolean {
        ...
        const everyGeneratedOutputCompleted = hasGeneratedMediaOutput
            && generatedOutputNodes.every(outputNode =>
