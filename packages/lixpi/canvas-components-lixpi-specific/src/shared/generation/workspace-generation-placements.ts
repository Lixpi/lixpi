...
export class WorkspaceGenerationPlacements {
    ...
    getBranchMarkerUiPhase(node: BranchMarkerNode): BranchMarkerUiPhase | undefined {
        if (this.isBranchMarkerGenerationCancelled(node)) return undefined
        if (this.ports.hasStartedMedia(node.nodeId)) return 'media-placeholder'
    ...
    isBranchMarkerPendingForUi(node: BranchMarkerNode): boolean {
        if (this.isBranchMarkerGenerationCancelled(node)) return false
        const uiPhase = this.getBranchMarkerUiPhase(node)
        return uiPhase === 'preflight' || uiPhase === 'planned-awaiting-media'
    }

    getBranchMarkerUiPhaseNodeIdsForRun(threadId: string, generationRun?: MediaGenerationRunMeta): string[] {
    ...
    markBranchMarkerRunMediaPlaceholderPhase(threadId: string, generationRun?: MediaGenerationRunMeta): void {
        for (const nodeId of this.getBranchMarkerUiPhaseNodeIdsForRun(threadId, generationRun)) {
            if (this.phases.has(nodeId)) this.phases.set(nodeId, 'media-placeholder')
        }
    }

    clearBranchMarkerUiPhasesForRun(threadId: string, generationRun?: MediaGenerationRunMeta): void {
        for (const nodeId of this.getBranchMarkerUiPhaseNodeIdsForRun(threadId, generationRun)) {
            this.phases.delete(nodeId)
        }
    }

    getGeneratedMediaPlacementKey(threadId: string, generationRun?: MediaGenerationRunMeta): string {
        return generationRun?.generationRequestId
            ? `${threadId}:${generationRun.generationRequestId}`
            : threadId
    }

    getGeneratedMediaRunKey(threadId: string, generationRun?: MediaGenerationRunMeta): string {
    ...
    getPendingBranchMarkerReasoningModelKey(placementKey: string, reasoningModelId: string): string {
        return `${placementKey}:reasoning-model:${reasoningModelId}`
    }

    getPendingBranchMarkerReasoningRunKey(placementKey: string, reasoningRunId: string): string {
        return `${placementKey}:reasoning-run:${reasoningRunId}`
    }

    getPendingBranchMarkerReasoningIndexKey(placementKey: string, reasoningIndex: number): string {
        return `${placementKey}:reasoning-index:${reasoningIndex}`
    }

    getPendingBranchMarkerBranchNodeKey(placementKey: string, markerNodeId: string): string {
    ...
    addUniquePendingBranchMarkerKey(keys: string[], key: string | undefined): void {
    ...
    findPendingBranchMarkerRecordByReasoningIndex(
        ...
        return this.markers.get(this.getPendingBranchMarkerReasoningIndexKey(placementKey, reasoningIndex))
    }
    ...
    recoverPendingBranchMarkerRecordFromCanvasState(
        ...
        const record = this.createPendingBranchMarkerRecordFromCanvasNode(threadId, generationRun, matchingNode)
        this.markers.set(placementKey, record)
        this.setPendingBranchMarkerRecordAliases(threadId, generationRun, record)
        return record
    }
    ...
    setPendingBranchMarkerRecordAliases(
        ...
        for (const key of this.getPendingBranchMarkerSpecificKeys(threadId, generationRun)) {
            this.markers.set(key, { ...record, placementKey })
        }
    }

    deletePendingBranchMarkerAliasesForNodeId(nodeId: string): void {
        for (const [key, record] of this.markers.entries()) {
            if (record.nodeId === nodeId) this.markers.delete(key)
        }
    }
    ...
    deletePendingBranchMarkerAliasesForPlacement(placementKey: string): void {
        const placementPrefix = `${placementKey}:`
        for (const key of this.markers.keys()) {
            if (key === placementKey || key.startsWith(placementPrefix)) this.markers.delete(key)
        }
    }

    getPendingBranchMarkerRecord(threadId: string, generationRun?: MediaGenerationRunMeta): PendingBranchMarkerRecord | undefined {
        const placementKey = this.getGeneratedMediaPlacementKey(threadId, generationRun)
        for (const key of this.getPendingBranchMarkerSpecificKeys(threadId, generationRun)) {
        ...
        if (placementRecord && this.pendingBranchMarkerRecordMatchesGenerationRun(placementRecord, generationRun)) {
            return placementRecord
        }

        const threadRecord = placementKey !== threadId ? this.markers.get(threadId) : undefined
        if (threadRecord && this.pendingBranchMarkerRecordMatchesGenerationRun(threadRecord, generationRun)) {
            return threadRecord
        }
    ...
    ensurePendingBranchMarkerRecordForApiRun(
        ...
        for (const key of this.getPendingBranchMarkerSpecificKeys(threadId, generationRun)) {
            ...
            if (existing) {
                this.setPendingBranchMarkerRecordAliases(threadId, generationRun, existing)
                return existing
            }
        }
        ...
        const byReasoningIndex = this.findPendingBranchMarkerRecordByReasoningIndex(placementKey, generationRun?.reasoningIndex)
            ?? (placementKey !== threadId
                ...
                : undefined)
        if (byReasoningIndex) {
            this.setPendingBranchMarkerRecordAliases(threadId, generationRun, byReasoningIndex)
        ...
        const byReasoningModel = this.findPendingBranchMarkerRecordByReasoningModel(placementKey, generationRun?.reasoningModelId)
            ?? (placementKey !== threadId
                ...
                : undefined)
        if (byReasoningModel) {
            this.setPendingBranchMarkerRecordAliases(threadId, generationRun, byReasoningModel)
            return byReasoningModel
        }

        const existing = this.markers.get(placementKey)
        if (existing && this.pendingBranchMarkerRecordMatchesGenerationRun(existing, generationRun)) {
            this.setPendingBranchMarkerRecordAliases(threadId, generationRun, existing)
            return existing
        }
        ...
        this.setPendingBranchMarkerRecordAliases(threadId, generationRun, migrated)
        return migrated
    }
    ...
    getPendingGeneratedMediaPlacement(threadId: string, generationRun?: MediaGenerationRunMeta): PendingGeneratedImagePlacement | undefined {
        return this.placements.get(this.getGeneratedMediaPlacementKey(threadId, generationRun))
    }
    ...
    setPendingGeneratedMediaPlacement(
        ...
        this.placements.set(this.getGeneratedMediaPlacementKey(threadId, generationRun), placement)
    }
