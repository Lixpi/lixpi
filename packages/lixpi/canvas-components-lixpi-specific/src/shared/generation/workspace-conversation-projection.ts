...
export type CanvasConversationProjectionScope = { workspaceId: string; sceneKey: string }
export type WorkspaceConversationProjectionPorts<Thread extends CanvasConversationProjectionRecord> = {
    ...
    canUseLatestTurnFallback: (node: BranchMarkerNode, content: unknown) => boolean
    fetchThread: (request: { workspaceId: string; threadId: string }) => Promise<Thread | null>
    refreshProjection: (threadId: string) => void
    setTimer: (callback: () => void, delay: number) => () => void
    now: () => number
    reportError: (error: unknown, threadId: string) => void
}

type Refresh = { scope: CanvasConversationProjectionScope; cancelTimer?: () => void }

export function readCanvasConversationVersion(record: { proseMirrorVersion?: unknown } | undefined): number {
    const version = record?.proseMirrorVersion
    return typeof version === 'number' && Number.isInteger(version) && version >= 0 ? version : 0
}

function isMarker(node: CanvasNode): node is BranchMarkerNode {
    return node.type === 'branchOrigin' || node.type === 'branchFork' || node.type === 'branchLine'
}

function contentWeight(record: CanvasConversationProjectionRecord): number {
    if (!record.content) return 0
    const messageCount = countProseMirrorNodesByType(record.content, new Set(['aiUserMessage', 'aiResponseMessage', 'aiReasoningSection', 'prompt_reference']))
...
export class WorkspaceConversationProjection<Thread extends CanvasConversationProjectionRecord> {
    ...
    rememberContent(threadId: string, content: object, streaming: boolean | null = null): void {
        if (this.destroyed) return
        if (streaming === true) this.liveContent.set(threadId, content)
        else if (streaming === false) this.liveContent.delete(threadId)
        const thread = this.get(threadId)
        if (thread) this.remember({ ...thread, content, updatedAt: Math.max(thread.updatedAt, this.ports.now()) })
    }

    merge(incoming: Thread[], canvasState: CanvasState | null, workspaceChanged: boolean): Thread[] {
        if (workspaceChanged) return incoming
        const next = new Map(incoming.map(thread => [thread.threadId, thread]))
        const retained = new Set([...this.ports.retainedThreadIds(), ...this.liveContent.keys()])
        for (const node of canvasState?.nodes ?? []) {
            if (isMarker(node) && node.conversationAssetId) retained.add(node.conversationAssetId)
            else if ((node.type === 'image' || node.type === 'video' || node.type === 'capabilityArtifact') && node.generatedBy?.conversationAssetId) {
        ...
        for (const current of this.ports.getThreads()) {
            ...
            if (!incoming) {
                if (retained.has(current.threadId)) next.set(current.threadId, current)
                continue
            }
    ...
    snapshotIsComplete(threadId: string, content: unknown): boolean {
        ...
        return markers.every(marker => {
            const preview = getBranchMarkerConversationPreviewFromThreadContent(content, threadId, getBranchMarkerTurnDescriptor(marker), {
    ...
    async refresh(threadId: string): Promise<void> {
        ...
        await this.attempt(threadId, refresh, 0)
    }
    ...
    schedule(threadId: string): void {
        ...
        const refresh: Refresh = { scope: { ...scope } }
        this.refreshes.set(threadId, refresh)
        this.scheduleAttempt(threadId, refresh, 0)
    }
    ...
    clear(): void {
        ...
        if (errors.length) throw new AggregateError(errors, 'Canvas conversation projection cleanup failed')
    }
    ...
    private scheduleAttempt(threadId: string, refresh: Refresh, attempt: number): void {
        if (!this.isCurrent(threadId, refresh)) return
        ...
        try {
            refresh.cancelTimer = this.ports.setTimer(() => {
                void this.attempt(threadId, refresh, attempt)
            }, delay)
        } catch (error) {
            this.release(threadId)
    ...
    private async attempt(threadId: string, refresh: Refresh, attempt: number): Promise<void> {
        if (!this.isCurrent(threadId, refresh)) return
        refresh.cancelTimer = undefined
        let complete = false
        try {
            const thread = await this.ports.fetchThread({ workspaceId: refresh.scope.workspaceId, threadId })
            if (!this.isCurrent(threadId, refresh)) return
            if (thread) {
                if (thread.threadId !== threadId || thread.workspaceId !== refresh.scope.workspaceId) throw new Error('Conversation refresh returned another workspace or thread')
                if (readCanvasConversationVersion(thread) >= readCanvasConversationVersion(this.get(threadId))) {
                    this.remember(thread)
                    if (!this.isCurrent(threadId, refresh)) return
                    complete = this.snapshotIsComplete(threadId, thread.content)
                    if (complete) this.liveContent.delete(threadId)
                    this.ports.refreshProjection(threadId)
                }
            }
        } catch (error) {
            if (this.isCurrent(threadId, refresh)) this.ports.reportError(error, threadId)
        }
        if (!this.isCurrent(threadId, refresh)) return
        if (complete) this.release(threadId)
        else this.scheduleAttempt(threadId, refresh, attempt + 1)
    }

    private isCurrent(threadId: string, refresh: Refresh): boolean {
        if (this.destroyed || this.refreshes.get(threadId) !== refresh) return false
        const scope = this.ports.readScope()
