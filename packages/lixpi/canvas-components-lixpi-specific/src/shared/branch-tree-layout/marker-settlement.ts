...
const isPreflightBranchMarkerForThread = (
    node: CanvasNode,
    threadId: string,
): boolean => {
    if (
...
export const removePreflightBranchMarkersForThread = (
    state: CanvasState,
    threadId: string,
): PreflightBranchMarkerSettlement => {
    const removedNodeIds = state.nodes.filter(node => isPreflightBranchMarkerForThread(node, threadId)).map(node => node.nodeId)
...
export const getSupersededBranchMarkerNodeIdsForAuthoritativePlan = (args: {
    state: CanvasState
    ...
    if (authoritativeMarkers.length === 0)
        return []

    const authoritativeNodeIds = new Set(
        authoritativeMarkers.map(marker => marker.nodeId),
    )
    const authoritativeThreadIds = new Set(
        authoritativeMarkers.flatMap(marker => (marker.conversationAssetId ? [marker.conversationAssetId] : [])),
    )
    ...
    return args.state.nodes.flatMap(node => {
        ...
        if (authoritativeNodeIds.has(node.nodeId))
            return []
        ...
        return isProvisionalPlanMarker
            || isInitialPreflightMarker
            ? [node.nodeId]
            : []
    })
}
