...
export class WorkspaceLineageProjection {
    ...
    private getNodeWorldRect(node: CanvasNode, nodesById = this.getCanvasNodesById()): Rect {
        return this.ports.getWorldRect(node, nodesById)
    }

    private getGeneratedMediaOutputGap(sourceNode: CanvasNode): number {
        if (sourceNode.type === 'branchOrigin') return this.ports.settings.branchOriginToFirstMediaGap
        if (sourceNode.type === 'branchFork' && !sourceNode.parentBranchNodeId) return this.ports.settings.rootToFirstMediaGap
    ...
    findPendingLineageNode(
        ...
        return pendingNodes.find((node): node is BranchOriginCanvasNode | BranchForkCanvasNode | BranchLineCanvasNode => Boolean(node && node.nodeId === nodeId))
    }
    ...
    addBranchLineageMarkerNodesIfMissing(
        ...
        const additions = markerNodes.filter((node): node is BranchOriginCanvasNode | BranchForkCanvasNode | BranchLineCanvasNode => {
            if (!node || existingNodeIds.has(node.nodeId)) return false
    ...
    addBranchMarkerEdgeIfMissing(edges: WorkspaceEdge[], markerNode: BranchForkCanvasNode | BranchLineCanvasNode | undefined): WorkspaceEdge[] {
    ...
    ensureBranchForkForGeneratedMedia(
        ...
        const parentNode = this.getBranchForkParentNode(threadId, generationRun, branchOriginNode)
        ...
        const position = parentNode && branchForkPlan.parentBranchNodeId
            ? this.getPendingBranchMarkerPositionBeforeGeneratedMedia(
                parentNode,
                dimensions,
                this.getPlannedBranchMarkerSiblingSlot(threadId, generationRun, branchForkPlan.parentBranchNodeId, branchForkNodeId),
            )
            : this.getRootBranchMarkerPositionBeforeGeneratedMedia(
                ...
                dimensions,
                mediaHeight,
                this.getPlannedRootBranchForkSiblingSlot(threadId, generationRun, branchForkNodeId),
            )
    ...
    ensureBranchLineForGeneratedMedia(
        ...
        const siblingSlot = this.getPlannedBranchMarkerSiblingSlot(threadId, generationRun, parentBranchNodeId, branchLineNodeId)
        const position = this.getPendingBranchMarkerPositionBeforeGeneratedMedia(parentNode, dimensions, siblingSlot)
    ...
    ensureBranchMarkerForGeneratedMedia(
        ...
        const branchForkNode = this.ensureBranchForkForGeneratedMedia(threadId, generationRun, branchOriginNode)
        const branchLineNode = branchForkNode
            ? undefined
            : this.ensureBranchLineForGeneratedMedia(threadId, generationRun, branchOriginNode)
    ...
    getReferenceGroupRectForGeneratedMedia(threadId: string, generationRun?: MediaGenerationRunMeta): Rect | undefined {
        const placement = this.ports.placements.getPendingGeneratedMediaPlacement(threadId, generationRun)
        const state = this.ports.readCanvasState()
        if (!placement?.referenceNodeIds?.length || !state) return undefined
    ...
    getNextGeneratedMediaPosition(sourceNode: CanvasNode, mediaHeight: number): { x: number; y: number } {
        const nodes = this.ports.readCanvasState()?.nodes || []
        const edges = this.ports.readCanvasState()?.edges ?? []
        const existingMediaOutputs = this.getGeneratedMediaOutputs(sourceNode, nodes, edges)
    ...
    getGeneratedMediaOutputs(sourceNode: CanvasNode, nodes: CanvasNode[], edges: WorkspaceEdge[]): (ImageCanvasNode | VideoCanvasNode)[] {
    ...
    createGeneratedImageEdge(sourceNode: CanvasNode, imageNodeId: string, responseMessageId?: string): WorkspaceEdge {
        return {
    ...
    getLineageAssignmentReasoningIndex(
        ...
        if (sourceGenerationRun && sourceGenerationRun.reasoningRunId === lineageAssignment.reasoningRunId) {
    ...
    buildGenerationRunFromLineageAssignment(
        ...
        return {
            ...
            reasoningIndex: this.getLineageAssignmentReasoningIndex(lineagePlan, lineageAssignment, sourceGenerationRun),
            lineageAssignment,
        }
    }
