...
export const isGeneratedMediaNode = (node: CanvasNode): node is GeneratedOutputNode =>
    node.type === 'image' || node.type === 'video' || node.type === 'capabilityArtifact'

export const isFailedMediaGenerationOutputNode = (node: CanvasNode): node is FailedMediaGenerationOutputNode => {
...
export const isBranchLineageOutputNode = (node: CanvasNode): node is BranchLineageOutputNode =>
    isGeneratedMediaNode(node) || isFailedMediaGenerationOutputNode(node)

export const getBranchLineageAssignment = (node: BranchLineageOutputNode): MediaRunLineageAssignment | undefined => {
    if (isFailedMediaGenerationOutputNode(node))
        return node.lineageAssignment
...
export const isBranchLineageMarkerNode = (node: CanvasNode | undefined): node is BranchLineageMarkerNode =>
...
export const hasActiveGeneratedOutputLineage = (
    node: CanvasNode,
    nodes: CanvasNode[],
    edges: WorkspaceEdge[],
): boolean => {
    ...
    const incomingMarkerIds = new Set(
        edges.filter(edge => edge.targetNodeId === node.nodeId).map(edge => edge.sourceNodeId),
    )
...
export const getGeneratedMediaLineageMarkerIds = (node: BranchLineageOutputNode): string[] => {
...
export const getGeneratedMediaMidpointMarkerId = (node: BranchLineageOutputNode): string | undefined => {
    const assignment = getBranchLineageAssignment(node)
    const generatedBy = isFailedMediaGenerationOutputNode(node) ? undefined : node.generatedBy
...
export const getStartedLineageMarkerState = (nodes: CanvasNode[]): StartedLineageMarkerState => {
    ...
    for (const node of nodes) {
        ...
        for (const markerId of getGeneratedMediaLineageMarkerIds(node)) {
            ...
            if (
                (markerNode?.type === 'branchFork' || markerNode?.type === 'branchLine')
                && markerNode.parentBranchNodeId
            )
