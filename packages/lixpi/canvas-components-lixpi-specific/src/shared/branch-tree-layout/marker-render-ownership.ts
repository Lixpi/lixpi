...
const getPlannedBranchMarkerIdentities = (lineagePlan: MediaBranchLineagePlan): PlannedBranchMarkerIdentity[] => {
    return [
...
export const hasCompletePlannedBranchMarkerGeometry = (
    nodes: CanvasNode[],
...
const getReasoningIndex = (node: BranchMarkerNode): number | undefined => {
    if (node.pendingState?.reasoningIndex != null)
        return node.pendingState.reasoningIndex

    if (
        node.type === 'branchFork'
        || node.type === 'branchLine'
    )
        return node.reasoningIndex

    return undefined
}

const getReasoningModelId = (node: BranchMarkerNode): string => {
...
export const getSupersededPreflightNodeIdsForPlannedOwner = (
    nodes: BranchMarkerNode[],
    plannedOwner: BranchMarkerNode,
): string[] => {
    if (
        plannedOwner.pendingState?.phase === 'preflight'
        || !plannedOwner.conversationAssetId
    )
        return []
    ...
    return preflightNodes.filter(preflightNode => selectPlannedOwner(preflightNode, plannedNodes)?.nodeId === plannedOwner.nodeId).map(
        node => node.nodeId,
    )
}

export const resolveBranchMarkerRenderOwnership = (
    nodes: BranchMarkerNode[],
    ...
    const threadIds = new Set(
        nodes.map(node => node.conversationAssetId).filter((threadId): threadId is string => Boolean(threadId)),
    )
    ...
    for (const threadId of threadIds) {
        ...
        for (const plannedOwner of plannedNodes) {
            const matchingPreflightNodes = preflightNodes.filter(
                preflightNode => preflightOwnerByNodeId.get(preflightNode.nodeId)?.nodeId === plannedOwner.nodeId,
            )

            if (matchingPreflightNodes.length === 0)
                continue
            ...
            for (const candidate of [plannedOwner, ...matchingPreflightNodes]) {
                if (candidate.nodeId === visibleOwner.nodeId)
                    continue
        ...
        for (const preflightNode of preflightNodes) {
            if (preflightOwnerByNodeId.get(preflightNode.nodeId))
                continue
    ...
    return {
        suppressedNodeIds,
        visibleOwnerBySuppressedNodeId,
    }
}
