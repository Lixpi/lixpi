...
export type WorkspaceNodeDeletionPorts = {
    ...
    }) => Promise<{ request?: { revision: number } }>
...
const isMissingAssetDetachError = (error: unknown): boolean => error instanceof Error && error.message === 'NOT_FOUND'
...
export class WorkspaceNodeDeletion {
        ...
        try {
            ...
            for (const nodeId of requestedNodeIds) {
                ...
                try {
                    ...
                    if (await this.cancelOwningMediaGenerationRun(node, scope))
                        continue
            ...
            if (this.isCurrent(scope))
                this.ports.reportError('[CANVAS][node-deletion] Unable to delete canvas selection:', error)
        } finally {
    ...
    private async cancelOwningMediaGenerationRun(
        node: CanvasNode,
        scope: Scope,
    ): Promise<boolean> {
        if (
        ...
        const operation = (this.ports.readState()?.nodes ?? []).find(
            (candidate): candidate is OperationStatusCanvasNode => (
        ...
        if (
            !cancelRequestId
            || cancelRequestId.startsWith('canvas-')
        )
            return false

        const requestRevision = operation?.requestRevision
            ?? (await this.ports.getRequest({
        ...
        if (
            ...
            this.ports.removeOperation(operation.nodeId, 'media-generation')

        return true
    }

    private async detachCanvasNode(
        nodeId: string,
        scope: Scope,
    ): Promise<void> {
        if (!this.isCurrent(scope))
        ...
        const resolvedTreeState = (isGeneratedOutputCanvasNode(deletedNode) && Boolean(deletedNode.generatedBy?.branchId))
            || isBranchMarkerNode(deletedNode)
        ...
        if (
            assetId
            && this.ports.detachAsset
        ) {
            try {
                const committedState = await this.ports.detachAsset({
                    assetId,
                    nodeId,
                    removedNodeIds,
                ...
                if (!isMissingAssetDetachError(error))
                    throw error

                this.ports.warn(
                    '[CANVAS][node-deletion] Detaching a node whose Asset no longer exists:',
                    {
                        nodeId,
                        assetId,
                    },
