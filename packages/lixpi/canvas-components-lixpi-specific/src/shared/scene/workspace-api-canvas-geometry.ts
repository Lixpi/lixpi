...
export type WorkspaceApiCanvasGeometryPorts = {
    ...
    publishAuthoritative: (snapshot: { canvasState: CanvasState; layoutRevision: number }) => void
...
function reconcileApiBranchMarkersWithTemporaryPreflightNodes(
...
): { state: CanvasState; removedNodeIds: string[]; records: Map<string, PendingBranchMarkerRecord> } {
    const pendingBranchMarkers = new Map(sourceRecords)
    const cleanupBranchMarkerArtifacts = (nodeIds: Iterable<string>) => {
        const removed = new Set(nodeIds)
        for (const [key, record] of pendingBranchMarkers) if (removed.has(record.nodeId)) pendingBranchMarkers.delete(key)
    }
    ...
    if (plannedMarkers.length === 0) return { state, removedNodeIds: [], records: pendingBranchMarkers }
    ...
    if (removedNodeIds.size === 0) return { state, removedNodeIds: [], records: pendingBranchMarkers }
    return {
        state: {
            ...state,
...
export class WorkspaceApiCanvasGeometry {
    ...
    private capture(): GeometryTicket | null {
        ...
        if (scope.workspaceId !== this.scope?.workspaceId || scope.sceneKey !== this.scope?.sceneKey) {
            this.clear()
            this.scope = { ...scope }
        }
    ...
    syncApiCanvasSnapshotNodesToDOM(nodeIds: Iterable<string>): void {
        ...
        for (const node of state.nodes) {
            ...
            if (!ids.has(node.nodeId)) continue
    ...
    resolvePendingBranchMarkersAfterApiGeometry(generationRequestId: string | undefined): void {
        const ticket = this.capture()
        const state = this.ports.readCanvasState()
        if (!ticket || !state || !generationRequestId) return
        ...
        for (const placement of this.ports.placements.placements.values()) {
            ...
            const marker = state.nodes.find((node): node is BranchMarkerNode =>
                isBranchMarkerNode(node)
    ...
    applyApiCanvasGeometry(canvasGeometry: CanvasGeometryUpdate): void {
        ...
        if (canvasGeometry.layoutRevision <= this.lastAppliedApiLayoutRevision) return
        if (canvasGeometry.layoutRevision < this.highestObservedApiLayoutRevision) return

        const ticket = { ...scope, epoch: ++this.epoch }
        this.highestObservedApiLayoutRevision = Math.max(this.highestObservedApiLayoutRevision, canvasGeometry.layoutRevision)
        const preflight = reconcileApiBranchMarkersWithTemporaryPreflightNodes(state, canvasGeometry, this.ports.placements.markers)
        ...
        if (result.fullyApplied) this.lastAppliedApiLayoutRevision = canvasGeometry.layoutRevision
        this.ports.log('applied', {
            layoutRevision: canvasGeometry.layoutRevision,
        ...
        if (result.fullyApplied) {
            this.ports.publishAuthoritative({ canvasState: result.state, layoutRevision: canvasGeometry.layoutRevision })
            if (!this.isCurrent(ticket)) return
        }
        if (!changed) return
        if (preflight.removedNodeIds.length || result.removedNodeIds.length) {
            this.ports.syncMedia(this.ports.readCanvasState())
            if (!this.isCurrent(ticket)) return
        ...
        if (current && snapshotNodeIds.size) {
            this.ports.syncNodeGeometry(current.nodes.filter(node => snapshotNodeIds.has(node.nodeId)))
            if (!this.isCurrent(ticket)) return
        }
        ...
        if (resolved) {
            ...
            if (!this.isCurrent(ticket)) return
            this.ports.log('preserve-until-store-ack', {
                layoutRevision: canvasGeometry.layoutRevision,
