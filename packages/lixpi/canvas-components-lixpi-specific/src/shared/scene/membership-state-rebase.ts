...
export const rebaseCanvasMembershipState = ({
    requestedState,
    currentState,
    operation,
    removedNodeIds = [],
    ...
    const requestedNodeIds = new Set(
        requestedState.nodes.map(node => node.nodeId),
    )
    const nodes = [
    ...
    const requestedEdgeIds = new Set(
        requestedState.edges.map(edge => edge.edgeId),
    )
    const edges = [
