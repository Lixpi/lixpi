...
const sanitizeTopLevelMode = (mode: CanvasRightSidePanelMode | undefined): CanvasRightSidePanelMode => {
    return mode
        && RIGHT_SIDE_PANEL_MODES.includes(mode)
        ? mode
        : 'aiThreads'
}

export const createDefaultAiChatPanelState = (): CanvasAiChatPanelState => {
    return {
        isOpen: false,
...
const sanitizeGeneratedOutputDetailsTarget = (
...
export const sanitizeContextChips = (
    ...
    return Array.from(
        new Set(
            (nodeIds ?? []).filter(nodeId => Boolean(nodeId) && eligibleNodeIds.has(nodeId)),
        ),
    )
}

export const getAiChatPanelState = (canvasState: CanvasState | null | undefined): CanvasAiChatPanelState => {
    ...
    const generatedOutputDetailsTarget = sanitizeGeneratedOutputDetailsTarget(persisted?.generatedOutputDetailsTarget, canvasState.nodes)
...
export const setAiChatPanelState = (
    canvasState: CanvasState,
    panelState: CanvasAiChatPanelState,
): CanvasState => {
    const normalized = getAiChatPanelState({
        ...canvasState,
...
export const getPersistedPanelConversationIds = (panel: unknown): string[] => {
    if (
        !panel
        || typeof panel !== 'object'
        || !('tabs' in panel)
    ...
    return panel.tabs.flatMap((tab: unknown) => {
        ...
        return typeof tab.refId === 'string'
            && tab.refId
            ? [tab.refId]
            : []
    })
