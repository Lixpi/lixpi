...
class NoteView implements NodeView<BoardData> {
    ...
    constructor(
        node: EngineNode<BoardData>,
        context: ComponentContext,
        options: MediaBoardOptions,
        onChange: (text: string) => void,
    ) {
        this.dispose = options.mountEditor(
            context.contentRoot,
            node.data.text ?? '',
            onChange,
        )
...
class BoardPulse {
    ...
    constructor(
        ...
        const group = context.resources.createGroup({
            space: 'screen',
            layer: context.layers.foreground,
        })
        this.lifetime.own(() => context.resources.release(group))

        try {
            const path = context.resources.createPath(
                group,
                [{
                    path: 'M8 8 H100 V16 H8 Z',
                    fill: { color },
                }],
            )
            let elapsed = 0
            this.lifetime.own(
                context.requestFrame(delta => {
                    elapsed += delta
                    context.resources.updatePath(
                        path,
                        [{
                            path: 'M8 8 H100 V16 H8 Z',
                            fill: {
                                color,
                                alpha: 0.5 + Math.sin(elapsed / 500) * 0.3,
                            },
                        }],
                    )
                }),
...
export class MediaBoard {
    ...
    constructor(
            ...
            nodes: [
                {
                    ...
                    data: { text: 'Your editor mounts here' },
                },
                {
                    nodeId: 'photo',
                    type: 'image',
                ...
                },
                {
                    ...
                    data: {
                        media: video,
                        playback: 'playback',
                        poster: options.posterUrl ? 'poster' : undefined,
                    },
                },
            ],
        }
        const registry = new NodeRegistry()
            .register<BoardData>({
                type: 'note',
                geometry,
                mount: (node, context) => new NoteView(
                    node,
                    context,
                    options,
                    text => this.editNote(node.nodeId, text),
                ),
            })
            .register(
                createImageNodeRegistration<BoardData>({
                    type: 'image',
                    geometry,
                    getMedia: node => node.data.media ?? null,
                    radius: 12,
                    onError: options.onError,
                }),
            )
            .register(
                createPlaybackNodeRegistration<BoardData>({
                    type: 'video',
                    kind: 'video',
                    geometry,
                    getContent: node => node.data.media ? {
                        media: node.data.media,
                        playbackRenditionId: node.data.playback ?? null,
                        posterRenditionId: node.data.poster,
                    } : null,
        ...
        this.canvas = new CanvasController({
            ...
            mediaResolver: {
                resolve: async (
                    ...
                    if (!url)
                        throw new Error('No source supplied for this rendition')
            ...
            extensions: [{
                id: 'board-pulse',
                mount: context => new BoardPulse(context, options.color).destroy,
            }],
    ...
    private accept = (intent: CanvasIntent): void => {
        ...
        if (intent.kind === 'geometry') {
            ...
            this.snapshot = {
                ...this.snapshot,
                nodes: this.snapshot.nodes.filter(node => !intent.nodeIds.includes(node.nodeId)),
            }
        else
            return
...
export const mountTwoBoards = (
    firstRoot: HTMLElement,
    secondRoot: HTMLElement,
    options: MediaBoardOptions,
): readonly [MediaBoard, MediaBoard] => {
    const first = new MediaBoard(firstRoot, options)

    try {
        return [first, new MediaBoard(
            secondRoot,
            {
                ...options,
                color: '#cc6600',
