...
export class TravelingOutline {
    ...
    constructor(private readonly options: TravelingOutlineOptions) {
        if (options.surface.signal.aborted)
            throw new Error('Cannot mount an outline in a disposed drawing scope')
        ...
        this.group = resources.createGroup({
            ...
            layer: options.layer ?? layers.foreground,
        })
    ...
    private createEntry(datum: TravelingOutlineDatum): OutlineEntry {
        ...
        const group = resources.createGroup({
            space: this.options.space ?? 'world',
            layer: this.group,
        })
    ...
    private paint(entry: OutlineEntry): void {
        ...
        this.options.surface.resources.updateMesh(
            entry.mesh,
            {
                ...entry.buffers,
    ...
    private syncAnimation(): void {
        if (Array.from(
            this.entries.values(),
        ).some(entry => entry.active && entry.datum.visible))
