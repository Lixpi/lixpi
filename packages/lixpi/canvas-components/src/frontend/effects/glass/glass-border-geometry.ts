...
const getRoundedRectSignedDistance = (
    px: number,
    py: number,
    rect: {
        x: number
    ...
    return Math.hypot(outsideX, outsideY) + Math.min(
        Math.max(qx, qy),
        0,
    ) - radius
}
...
const getRoundedRectNormal = (
    ...
    const epsilon = 0.75
    const dx = getRoundedRectSignedDistance(
        px + epsilon,
        py,
        rect,
    )
    ...
    const dy = getRoundedRectSignedDistance(
        px,
        py + epsilon,
        rect,
    )
...
const getGlassBorderSampleCount = (
    perimeter: number,
    borderWidth: number,
): number => {
    ...
    return Math.max(
        48,
        Math.min(
            GLASS_BORDER_MAX_SAMPLE_COUNT,
            Math.ceil(perimeter / spacing),
        ),
    )
}

// Keep fixed CPU staging arrays; the engine handles GPU buffer reuse.
export const createGlassBorderMeshGeometry = (): GlassBorderMeshGeometry => {
...
export const writeClosedRoundedBorderGeometry = (
    geometry: GlassBorderMeshGeometry,
    ...
    for (let index = 0; index <= sampleCount; index++) {
        ...
        const frame = getRoundedOutlineFrame(
            boundedWidth,
            boundedHeight,
            radius,
            perimeter * progress,
...
const getDisplacementMapScale = (
    viewport: ViewportSize,
    style: GlassBorderStyle,
): number => {
    const maxDimension = Math.max(
        256,
        Math.round(style.displacementMapMaxDimensionPx || 1400),
    )
...
export const getDisplacementSignature = (
    datums: ReadonlyArray<GlassBorderDatum>,
    viewport: ViewportSize,
    style: GlassBorderStyle,
): string => {
    const geometry = datums.map(
        datum =>
                [
                    datum.id,
...
export const getMaterialGeometrySignature = (
    datum: GlassBorderDatum,
    borderWidth: number,
    edgeFeatherFraction: number,
): string => {
...
export const getMaskAndHighlightSignature = (
    datums: ReadonlyArray<GlassBorderDatum>,
    style: GlassBorderStyle,
): string => {
    const geometry = datums.map(
...
const writeLiquidGlassDisplacementMap = (
    rgba: Uint8Array,
    mapWidth: number,
    mapHeight: number,
    mapScale: number,
    ...
    for (const datum of datums) {
        ...
        const outerBounds = {
            minX: Math.max(
                0,
                Math.floor((datum.x - halfBorderWidth - 1) * mapScale),
            ),
            minY: Math.max(
                0,
                Math.floor((datum.y - halfBorderWidth - 1) * mapScale),
            ),
            maxX: Math.min(
                mapWidth - 1,
                Math.ceil((datum.x + datum.width + halfBorderWidth + 1) * mapScale),
            ),
            maxY: Math.min(
                mapHeight - 1,
                Math.ceil((datum.y + datum.height + halfBorderWidth + 1) * mapScale),
            ),
        }
        const centerRect = {
            x: datum.x,
        ...
        for (let mapY = outerBounds.minY; mapY <= outerBounds.maxY; mapY++) {
            ...
            for (let mapX = outerBounds.minX; mapX <= outerBounds.maxX; mapX++) {
                ...
                const centerDistance = getRoundedRectSignedDistance(
                    screenX,
                    screenY,
                    centerRect,
                )
                ...
                if (
                    centerDistance < -halfBorderWidth
                    || centerDistance > halfBorderWidth
                )
                    continue
                ...
                const normal = getRoundedRectNormal(
                    screenX,
                    screenY,
                    centerRect,
                )
                ...
                rgba[offset] = Math.max(
                    0,
                    Math.min(255, 128 + Math.round(vectorX * 127)),
                )
                rgba[offset + 1] = Math.max(
                    0,
                    Math.min(255, 128 + Math.round(vectorY * 127)),
                )
                rgba[offset + 2] = Math.max(
                    0,
                    Math.min(255, 128 + Math.round(rimVolume * 70)),
                )
                rgba[offset + 3] = 255
            }
        }
...
export const bakeGlassDisplacementMap = (
    ...
    writeLiquidGlassDisplacementMap(
        rgba,
        width,
        height,
        scale,
