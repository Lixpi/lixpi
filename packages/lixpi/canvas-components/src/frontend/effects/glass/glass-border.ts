...
const rimShape = (
    ...
    return {
        path: roundedRectPath(
            datum.x - offset,
            datum.y - offset,
            Math.max(1, datum.width + offset * 2),
            Math.max(1, datum.height + offset * 2),
            Math.max(0, datum.radius + offset),
        ),
        stroke: {
            color,
            alpha,
...
export class GlassBorder {
    ...
    constructor(private readonly options: GlassBorderOptions) {
        ...
        if (signal.aborted)
            throw new Error('Cannot mount glass in a disposed drawing scope')
        ...
        try {
            ...
            this.mask = own(
                resources.createPath(this.root, []),
            )
            ...
            this.materialTexture = own(
                resources.createTexture(options.texture),
            )
            this.displacementTexture = own(
                resources.createTexture({
                    kind: 'pixels',
                    size: this.mapSize,
                    rgba: new Uint8Array([128, 128, 128, 255]),
                }),
            )
    ...
    sync(
        datums: readonly GlassBorderDatum[],
        ...
        const visible = style.enabled
            && borderWidth > 0
            && viewport.width > 0
            && viewport.height > 0
            ? datums.filter(
                datum =>
                    datum.visible && datum.x + datum.width + borderWidth >= 0 && datum.y + datum.height + borderWidth >= 0 && datum.x - borderWidth <= viewport.width && datum.y - borderWidth <= viewport.height,
            )
            : []
        ...
        if (visible.length === 0) {
            if (
                this.capture
                && this.captureSpec?.enabled !== false
            ) {
                this.captureSpec = {
                    ...this.captureSpec!,
                    enabled: false,
    ...
    private syncDisplacement(
        datums: readonly GlassBorderDatum[],
        viewport: ViewportSize,
    ): void {
        const signature = `${this.options.style.displacementMapMaxDimensionPx}|${getDisplacementSignature(
            datums,
            viewport,
            this.options.style,
        )}`

        if (signature === this.displacementSignature)
            return

        const pixels = bakeGlassDisplacementMap(
            datums,
            viewport,
            this.options.style,
        )
    ...
    private syncCapture(
        ...
        const sampleBounds = datums.map(
            datum => ({
                x: datum.x - padding,
                y: datum.y - padding,
                width: datum.width + padding * 2,
                height: datum.height + padding * 2,
            }),
        )
        const signature = JSON.stringify([bounds, resolution, sampleBounds.map(
            region => [region.x, region.y, region.width, region.height].map(value => Math.round(value * 10) / 10),
        )])
        ...
        if (signature !== this.captureSignature) {
            ...
            if (this.capture)
                ...
                this.capture = resources.capture(this.captureSpec)
        ...
        if (binding !== this.displacementBinding) {
            this.displace?.()
            this.displace = resources.displace(
                this.refraction,
            ...
            datums.flatMap(
                datum => [
                    ringShape(
                        datum,
                        width,
                        color(style.bodyColor, '#ffffff'),
                        alpha(style.bodyAlpha),
                    ),
                    rimShape(
                        datum,
                        width / 2,
                        color(style.highlightColor, '#ffffff'),
                        alpha(style.highlightAlpha),
                    ),
                    rimShape(
                        datum,
                        -width / 2,
                        color(style.shadowColor, '#415061'),
    ...
    private syncMeshes(datums: readonly GlassBorderDatum[]): void {
        ...
        for (const datum of datums) {
            ...
            const signature = getMaterialGeometrySignature(
                datum,
                Math.max(0, style.widthPx),
            ...
            if (!entry) {
                ...
                const mesh = resources.createMesh(
                    this.material,
                    {
                        ...buffers,
                        version: 0,
    ...
    destroy = (): void => {
        ...
        if (this.capture)
            resources.release(this.capture.handle)
