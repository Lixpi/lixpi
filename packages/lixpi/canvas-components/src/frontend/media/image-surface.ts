...
const mediaIdentity = (media: MediaDescriptor): string => JSON.stringify([media.key, media.version])
...
export class ImageSurface {
    ...
    constructor(private readonly options: ImageSurfaceOptions) {
        ...
        if (signal.aborted)
            throw new Error('Cannot mount an image in a disposed drawing scope')

        this.radius = options.radius ?? 0
        this.placeholderPaint = options.placeholder === undefined ? {
            color: '#e7eaee',
            alpha: 0.85,
        } : options.placeholder
        this.group = resources.createGroup({
            space: 'world',
            layer: options.layer ?? layers.media,
        })

        try {
            this.placeholder = resources.createPath(this.group, [])
            this.imageGroup = resources.createGroup({
                space: 'world',
                layer: this.group,
            })
            this.mask = resources.createPath(this.group, [])
            resources.setMask(this.imageGroup, this.mask)
            this.mesh = resources.createMesh(
                this.imageGroup,
                this.geometry,
                null,
            )
    ...
    setGeometry(
        ...
        if (
            ![bounds.x, bounds.y, bounds.width, bounds.height, viewport.x, viewport.y, viewport.zoom, radius].every(Number.isFinite)
            || bounds.width < 0
            || bounds.height < 0
            || viewport.zoom <= 0
        )
            throw new RangeError('Image bounds, radius and viewport must be finite')

        const changed = bounds.x !== this.bounds.x
    ...
    async prefetch(): Promise<void> {
        ...
        try {
            const lease = await this.options.surface.media.acquireImage({
                media,
                visiblePixels: {
                    width: 1,
                    height: 1,
    ...
    private ensureImage(allowPreview = true): void {
        if (
            ...
            || !this.hasImage()
            || this.pending
            || this.viewport.zoom < (this.options.minimumLoadZoom ?? 0)
            || this.bounds.width <= 0
        ...
        try {
            ...
            const visiblePixels = {
                width: this.bounds.width * this.viewport.zoom * resolution,
                height: this.bounds.height * this.viewport.zoom * resolution,
            }
            const media = this.media!
            ...
            if (
                ...
                && (this.current.lease.renditionId === desired.id || (this.current.lease.intrinsicSize.width >= (desired.width ?? visiblePixels.width) && this.current.lease.intrinsicSize.height >= (desired.height ?? visiblePixels.height)))
            )
                return

            const preview = allowPreview
                && this.options.progressive !== false
                && this.current?.identity !== identity
            ...
            if (this.failed.has(
                JSON.stringify([identity, rendition.id]),
            ))
                return
    ...
    private async load(
        ...
        try {
            lease = await this.options.surface.media.acquireImage({
                media,
                visiblePixels,
            ...
            this.options.onImageLoaded?.({
                ...
                intrinsicSize: { ...this.current.lease.intrinsicSize },
            })
            ...
            if (
                ...
                && this.visible
                && this.media
                && mediaIdentity(this.media) === pending.identity
                && this.current?.identity === pending.identity
            ) {
            ...
            if (this.pending === pending) {
                this.pending = null
                this.failed.add(
                    JSON.stringify([pending.identity, pending.renditionId]),
                )
            }
    ...
    private syncGeometry(): void {
        this.options.surface.resources.updateGroup(
            this.group,
            { position: {
                x: this.bounds.x,
                y: this.bounds.y,
    ...
    private syncPaths(): void {
        ...
        this.options.surface.resources.updatePath(
            this.mask,
            [{
                path,
                fill: { color: '#ffffff' },
        ...
        this.options.surface.resources.updatePath(this.placeholder, this.placeholderPaint ? [{
            path,
            fill: this.placeholderPaint,
        }] : [])
    }
    ...
    private syncMesh(): void {
        ...
        if (
            ...
            && height > 0
            && this.options.fit
            && this.options.fit !== 'stretch'
        ) {
            const scale = this.options.fit === 'contain' ? Math.min(width / intrinsic.width, height / intrinsic.height) : Math.max(
