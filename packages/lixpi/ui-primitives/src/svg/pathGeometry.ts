...
function parsePathNumbers(rawArgs: string): number[] {
    return (rawArgs.match(/[-+]?(?:\d*\.\d+|\d+\.?\d*)(?:[eE][-+]?\d+)?/g) ?? []).map(Number)
}
...
export function flattenSvgPath(svgPath: string): PathPoint[] {
    const remainder = svgPath.replace(/[-+]?(?:\d*\.\d+|\d+\.?\d*)(?:[eE][-+]?\d+)?/g, '').replace(/[MmLlHhVvCcQqZz\s,]/g, '')
    if (remainder) throw new Error('Path flattening supports M, L, H, V, C, Q and Z commands')
    ...
    while ((match = commandRegex.exec(svgPath)) !== null) {
        ...
        const count = ({ M: 2, L: 2, H: 1, V: 1, C: 6, Q: 4, Z: 0 } as Record<string, number>)[command.toUpperCase()]
        if (args.some(value => !Number.isFinite(value)) || (count === 0 ? args.length !== 0 : args.length === 0 || args.length % count !== 0)) throw new Error('Invalid SVG path coordinates')
        if (points.length === 0 && command.toUpperCase() !== 'M') throw new Error('SVG paths must start with a move command')
        if (points.length > 0 && command.toUpperCase() === 'M') throw new Error('Path flattening requires a single continuous subpath')
        const isRelative = command === command.toLowerCase()
        const absolutePoint = (x: number, y: number): PathPoint =>
            isRelative
...
export function getPointAtPathLength(points: PathPoint[], targetLength: number): PathPointWithTangent | null {
...
export function isPointNearPath(point: PathPoint, points: PathPoint[], radius: number): boolean {
