...
const parseViewBox = (value: string | null): [number, number, number, number] | null => {
    ...
    const parts = value
        .trim()
        .split(/[\s,]+/).map(part => Number.parseFloat(part))
...
export const extractSvgPathIcon = (svgMarkup: string): SvgPathIcon => {
    const parser = new DOMParser()
    const svgDoc = parser.parseFromString(svgMarkup, 'image/svg+xml')
    const svgEl = svgDoc.querySelector('svg')
    const viewBox = parseViewBox(svgEl?.getAttribute('viewBox') ?? null)
    const width = viewBox?.[2]
        ?? parseDimension(svgEl?.getAttribute('width') ?? null)
        ?? 24
    const height = viewBox?.[3]
        ?? parseDimension(svgEl?.getAttribute('height') ?? null)
        ?? 24

    return {
        pathData: Array.from(
            svgDoc.querySelectorAll('path'),
        ).map(path => path.getAttribute('d') || '').filter(Boolean),
        minX: viewBox?.[0] ?? 0,
        minY: viewBox?.[1] ?? 0,
        width,
        height,
...
export const appendSvgPathIcon = (
    iconGroup: any,
    svgMarkup: string,
    {
        x,
