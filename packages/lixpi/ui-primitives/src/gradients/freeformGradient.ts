export type FreeformGradientColor = {
    r: number
    g: number
    b: number
}

export type FreeformGradientPoint = {
    x: number
    y: number
}

export type FreeformGradientBitmapSize = {
    width: number
    height: number
}
...
export class FreeformGradientRenderer {
    static readonly bitmapSize: FreeformGradientBitmapSize = {
        width: 60,
        height: 80,
    }
    ...
    static paintImageData(
        ...
        for (let y = 0; y < height; y++) {
            for (let x = 0; x < width; x++) {
                const color = FreeformGradientRenderer.sampleColor(
                    x / width,
                    y / height,
                    colors,
                    positions,
                ...
                data[index] = Math.round(
                    FreeformGradientRenderer.clamp(
                        color.r,
                        0,
                        255,
                    ),
