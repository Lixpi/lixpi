...
export function parseHexColor(hex: string, fallback = '#000000'): RgbColor {
    const normalized = normalizeHexColor(hex) ?? normalizeHexColor(fallback) ?? '#000000'
...
export function adjustHexColor(
    ...
    const hsl = rgbToHsl(parseHexColor(hex, fallback))
...
export function mixHexColors(fromHex: string, toHex: string, amount: number, fallback = '#000000'): string {
