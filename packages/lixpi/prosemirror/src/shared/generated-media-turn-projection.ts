...
export const collectResponseTextById = (root: ProseMirrorJsonNode): Record<string, string> => {
    const responseTextById: Record<string, string> = {}

    const visitContainer = (node: ProseMirrorJsonNode): void => {
        ...
        for (const child of children) {
            ...
            if (child.type === 'aiResponseMessage') {
                ...
                if (responseId) {
                    responseTextById[responseId] = [previousUserText, collectProseMirrorText(child).trim()].filter(Boolean)
                        .join('\n')
                }
...
const cloneProseMirrorJsonNode = (node: ProseMirrorJsonNode): ProseMirrorJsonNode => {
    return {
        ...node,
...
const isGeneratedMediaProjectionNode = (node: ProseMirrorJsonNode): boolean => node.type === 'aiGeneratedImage' || node.type === 'aiGeneratedVideo'
...
const generatedMediaNodeMatchesLocator = (
    node: ProseMirrorJsonNode,
    locator: GeneratedMediaTurnLocator,
): boolean => {
    if (!isGeneratedMediaProjectionNode(node))
        return false
    ...
    if (
        locator.mediaType
        && attrs.mediaType
        && attrs.mediaType !== locator.mediaType
    )
        return false
    ...
    if (
        locator.assetId
        && attrs.assetId
        && attrs.assetId !== locator.assetId
    )
        return false

    if (
        locator.mediaRunId
        && attrs.mediaRunId !== locator.mediaRunId
    )
        return false
    ...
    if (
        locatorVariantIndex != null
        && nodeVariantIndex != null
        && nodeVariantIndex !== locatorVariantIndex
    )
        return false
...
const projectionNodeMatchesLocator = (
    node: ProseMirrorJsonNode,
    locator: GeneratedMediaTurnLocator,
): boolean => {
    if (!isGeneratedMediaProjectionNode(node))
...
const containerContainsGeneratedMedia = (
    container: ProseMirrorJsonNode,
    locator: GeneratedMediaTurnLocator,
): boolean => {
    if (generatedMediaNodeMatchesLocator(container, locator))
        return true

    return Boolean(container.content?.some(child => containerContainsGeneratedMedia(child, locator)))
}

const nodeRunAttrsMatchLocator = (
    node: ProseMirrorJsonNode,
    locator: GeneratedMediaTurnLocator,
): boolean => {
    const attrs = node.attrs ?? {}
    ...
    if (
        locator.reasoningRunId
        && attrs.reasoningRunId === locator.reasoningRunId
    )
        return true
    ...
    if (
        locator.reasoningModelId
        && attrs.reasoningModelId === locator.reasoningModelId
    )
        return true
...
const getReasoningContainer = (
    ...
    if (sections.length === 0) {
        ...
        if (nodeRunAttrsMatchLocator(responseNode, locator))
            return responseNode
...
const responseMessageMatchesLocator = (
    responseNode: ProseMirrorJsonNode,
    locator: GeneratedMediaTurnLocator,
): boolean => {
    ...
    if (
        locator.responseMessageId
        && responseId !== locator.responseMessageId
    )
        return false
    ...
    return Boolean(
        getReasoningContainer(responseNode, locator),
    )
}

const createSingleGeneratedMediaFilter = (locator: GeneratedMediaTurnLocator): ProjectionNodeFilter | undefined => {
    if (
        !locator.mediaRunId
        && !locator.assetId
    )
...
const getGenerationTraceMediaRunId = (node: ProseMirrorJsonNode): string => {
...
const isMediaGenerationTraceProjectionNode = (node: ProseMirrorJsonNode): boolean => {
    const attrs = node.attrs ?? {}
...
const containerHasMatchingGenerationTrace = (
    container: ProseMirrorJsonNode,
    locator: GeneratedMediaTurnLocator,
): boolean => {
    if (
...
const collectMatchingGeneratedMediaLineageIds = (
    container: ProseMirrorJsonNode,
    locator: GeneratedMediaTurnLocator,
): LineageIds => {
...
const lineageEventMatchesIds = (
    node: ProseMirrorJsonNode,
    lineageIds: LineageIds,
): boolean => {
    const attrs = node.attrs ?? {}

    if (attrs.kind === 'branch-origin')
        return lineageIds.branchOriginNodeIds.has(
            String(attrs.branchOriginNodeId ?? ''),
        )

    if (attrs.kind === 'branch-line')
        return lineageIds.branchLineNodeIds.has(
            String(attrs.branchLineNodeId ?? ''),
        )

    return lineageIds.branchForkNodeIds.has(
        String(attrs.branchForkNodeId ?? ''),
    )
}

const createLineageEventProjectionNode = (
...
const createProjectionNodeFilter = (
    container: ProseMirrorJsonNode,
    locator: GeneratedMediaTurnLocator,
    limitToLocatorMedia: boolean,
    ...
    return node => {
        if (isGeneratedMediaProjectionNode(node))
            return mediaFilter ? mediaFilter(node) : true

        if (
            shouldFilterGenerationTraces
            && isMediaGenerationTraceProjectionNode(node)
        )
            return getGenerationTraceMediaRunId(node) === locator.mediaRunId

        if (node.type !== 'aiLineageEvent')
            return true

        if (
            shouldFilterLineageEvents
            && lineageIds
            && !lineageEventMatchesIds(node, lineageIds)
        )
            return false
...
const cloneProjectionNodeTree = (
    ...
    if (
        forceGenerationDetailsOpen
        && cloned.type === 'aiCollapsibleBlock'
    ) {
        cloned.attrs = {
            ...(cloned.attrs ?? {}),
            isOpen: true,
    ...
    if (
        reasoningModelId
        && cloned.type === 'aiLineageEvent'
        && !cloned.attrs?.reasoningModelId
    ) {
        cloned.attrs = {
            ...(cloned.attrs ?? {}),
            reasoningModelId,
...
const cloneProjectionNode = (
    node: ProseMirrorJsonNode,
    forceGenerationDetailsOpen: boolean,
    shouldKeepNode?: ProjectionNodeFilter,
    reasoningModelId?: string,
): ProseMirrorJsonNode => {
    return cloneProjectionNodeTree(
        node,
        forceGenerationDetailsOpen,
        shouldKeepNode,
        reasoningModelId,
    ) ?? cloneProseMirrorJsonNode(node)
}

const getProjectedMediaGenerationProgress = (
    node: ProseMirrorJsonNode,
    locator: GeneratedMediaTurnLocator,
    ...
    if (
        !generationProgress
        || typeof generationProgress !== 'object'
...
export const getGeneratedMediaProgressFromThreadContent = (
...
const isGenerationInvocationAnchor = (
    node: ProseMirrorJsonNode,
    locator: GeneratedMediaTurnLocator,
): boolean => {
    if (node.type !== 'aiCollapsibleBlock')
    ...
    if (
        ...
        return traceMediaRunId === locator.mediaRunId
...
const getGenerationInvocationPrompt = (node: ProseMirrorJsonNode | undefined): string => {
    if (
        !node
        || !isMediaGenerationTraceProjectionNode(node)
    )
    ...
    const toolPrompt = trace
        && typeof trace === 'object'
        ? (trace as Record<string, unknown>).toolPrompt
        : undefined

    if (
        typeof toolPrompt === 'string'
        && toolPrompt.trim()
    )
...
const removeTrailingMediaPrompt = (
    ...
    while (promptIndex >= 0) {
        ...
        while (
            preambleIndex >= 0
            && /\s/u.test(normalizedPreamble[preambleIndex]!)
        )
            preambleIndex -= 1

        if (
            preambleIndex < 0
            || normalizedPreamble[preambleIndex] !== normalizedMediaPrompt[promptIndex]
        )
            return normalizedPreamble
...
const replaceUnderstandRequestSummary = (
    item: OperationProgressItem,
    preambleSummary: string,
): OperationProgressItem => {
    return {
        ...item,
        ...(item.id === 'lineage:understand-request'
            && preambleSummary
...
const createProjectedGenerationProgress = (
    generationProgress: MediaGenerationProgressState,
    preambleSummary: string,
    mediaPrompt: string,
): MediaGenerationProgressState => {
    const items = (generationProgress.progress.items ?? []).filter(item => item.id !== MEDIA_GENERATION_PROMPT_PROGRESS_ITEM_ID).map(
        item => replaceUnderstandRequestSummary(item, preambleSummary),
    )
...
const insertGenerationProgressTimelineForProjection = (
    node: ProseMirrorJsonNode,
    locator: GeneratedMediaTurnLocator,
): boolean => {
    const children = node.content ?? []
    const generatedMediaIndex = children.findIndex(
        child =>
            Boolean(
    ...
    for (const child of children) {
        if (insertGenerationProgressTimelineForProjection(child, locator))
            return true
    }
...
const createProjectionDocument = (
    threadId: string,
...
const materializeReasoningSectionLineageEventsForProjection = (
    node: ProseMirrorJsonNode,
    lineageProjectionScope: AiLineageProjectionScope,
): void => {
    ...
    const existingEventIds = new Set(
        (node.content ?? []).filter(child => child.type === 'aiLineageEvent').map(getLineageEventIdentity),
    )
    const reasoningModelId = String(node.attrs?.reasoningModelId ?? '')
    const eventNodes = events.map(event => createLineageEventProjectionNode(event, reasoningModelId)).filter(eventNode => {
    ...
    if (eventNodes.length > 0)
        node.content = [...eventNodes, ...(node.content ?? [])]
...
const isGenerationTraceProjectionNode = (node: ProseMirrorJsonNode): boolean => isMediaGenerationTraceProjectionNode(node)
...
const relocateLineageEventsToResolverAudit = (node: ProseMirrorJsonNode): void => {
    if (!node.content)
        return
...
const cloneResponseForProjection = (
    ...
    if (sections.length === 0) {
        const shouldKeepNode = createProjectionNodeFilter(
            responseNode,
            locator,
            limitToLocatorMedia,
        )
        const cloned = cloneProjectionNode(
            responseNode,
            forceGenerationDetailsOpen,
            shouldKeepNode,
            locator.reasoningModelId,
        ...
        if (includeGenerationProgressTimeline)
            insertGenerationProgressTimelineForProjection(cloned, locator)
    ...
    const clonedSection = cloneProjectionNode(
        selectedSection,
        forceGenerationDetailsOpen,
        shouldKeepNode,
        locator.reasoningModelId,
    ...
    if (includeGenerationProgressTimeline)
        insertGenerationProgressTimelineForProjection(clonedSection, locator)
...
const cloneBranchMarkerResponseForProjection = (
    responseNode: ProseMirrorJsonNode,
    descriptor: BranchMarkerTurnDescriptor,
    forceGenerationDetailsOpen: boolean,
    lineageProjectionScope: AiLineageProjectionScope,
...
export const buildBranchMarkerTurnProjectionFromThreadContent = (
    threadContent: unknown,
    descriptor: BranchMarkerTurnDescriptor,
    ...
    const latestTurn = ownTurn
        || !allowLatestTurnFallback
        ? {
            userMessage: null,
    ...
    return {
        ...
        descriptor,
        content: createProjectionDocument(
            options.threadId,
            threadNode.attrs,
            messages,
        ),
...
export const buildGeneratedMediaTurnProjectionFromThreadContent = (
    threadContent: unknown,
    locator: GeneratedMediaTurnLocator,
    ...
    const visitContainer = (
        ...
        for (const child of node.content ?? []) {
            ...
            if (child.type === 'aiResponseMessage') {
                ...
                if (responseMessageMatchesLocator(child, locator))
                    return match

                continue
            }

            const nestedMatch = visitContainer(
                child,
                scopedThreadAttrs,
    ...
    const matchedThreadId = typeof match.threadAttrs?.threadId === 'string'
...
export const getGeneratedImageTurnInfoFromThreadContent = (
    threadContent: unknown,
    ...
    const locator: GeneratedMediaTurnLocator = typeof responseMessageIdOrLocator === 'object'
        ...
        : {
            responseMessageId: responseMessageIdOrLocator,
            reasoningModelId,
        }

    let latestUserMessage: ProseMirrorJsonNode | null = null

    const visitContainer = (node: ProseMirrorJsonNode): GeneratedImageTurnInfo | null => {
        for (const child of node.content ?? []) {
            if (child.type === 'aiUserMessage') {
