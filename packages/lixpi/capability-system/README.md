...
Character Creator owns its full backend runtime beside its Tool, Skills, shared plan, and resources. Its module definition registers the media strategy that reauthorizes source Assets through an injected port, analyzes evidence, prepares lossless identity crops, attaches a dedicated text-free neutral-mannequin pose image to every face, body, prop, or action request, schedules and assesses a configurable 3-to-10-shot graph with one paid attempt per shot, publishes nested durable progress, composes the deterministic text-free 3840x2560 PNG, emits the trace, and clears transient work. Every shot receives the original identity evidence directly and can run independently; generated shots never become conditioning inputs for later shots. The original source is ordered first for identity fidelity and the required panel pose is ordered immediately second for spatial control. It does not import API code, choose a hidden model, retry a failed shot, render typography into the sheet, or persist an output Asset.
