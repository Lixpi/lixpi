...
export function validateJsonSchemaValue(schema: unknown, value: unknown): JsonSchemaValidationResult {
    const errors: string[] = []
    validateNode(schema, value, '$', errors, 0, schema)
...
function validateNode(
    ...
    if (typeof schema.$ref === 'string') {
        ...
        else validateNode(referenced, value, path, errors, depth + 1, rootSchema)
        return
    }
    ...
    if (Array.isArray(schema.allOf)) {
        for (const child of schema.allOf) validateNode(child, value, path, errors, depth + 1, rootSchema)
    }
    if (Array.isArray(schema.anyOf)) {
        const matches = schema.anyOf.filter(child => validateBranch(child, value, depth + 1, rootSchema))
        if (matches.length === 0) errors.push(`${path}: value does not match anyOf`)
    }
    if (Array.isArray(schema.oneOf)) {
        const matches = schema.oneOf.filter(child => validateBranch(child, value, depth + 1, rootSchema))
        if (matches.length !== 1) errors.push(`${path}: value must match exactly one oneOf branch`)
    }
    if (schema.not !== undefined && validateBranch(schema.not, value, depth + 1, rootSchema)) {
    ...
    if (isRecord(value)) validateObject(schema, value, path, errors, depth, rootSchema)
}
...
function validateArray(
    ...
    if (typeof schema.minItems === 'number' && value.length < schema.minItems) errors.push(`${path}: array is too short`)
    if (typeof schema.maxItems === 'number' && value.length > schema.maxItems) errors.push(`${path}: array is too long`)
    if (schema.uniqueItems === true && new Set(value.map(stableStringify)).size !== value.length) {
        errors.push(`${path}: array items must be unique`)
    }
    if (schema.items !== undefined) {
        value.forEach((item, index) => validateNode(schema.items, item, `${path}[${index}]`, errors, depth + 1, rootSchema))
    }
}
...
function validateObject(
    ...
    for (const [key, child] of Object.entries(value)) {
        ...
        if (Object.hasOwn(properties, key)) {
            validateNode(properties[key], child, `${path}.${key}`, errors, depth + 1, rootSchema)
            ...
            validateNode(schema.additionalProperties, child, `${path}.${key}`, errors, depth + 1, rootSchema)
        }
    }
}

function validateBranch(schema: unknown, value: unknown, depth: number, rootSchema: unknown): boolean {
    const errors: string[] = []
    validateNode(schema, value, '$', errors, depth, rootSchema)
...
function resolveLocalReference(rootSchema: unknown, reference: string): unknown {
    ...
    for (const encodedPart of reference.slice(2).split('/')) {
        ...
        if (!isSafeProperty(part) || !isRecord(cursor) || !Object.hasOwn(cursor, part)) return undefined
        cursor = cursor[part]
    }
    return cursor
}
