...
export class CapabilityMediaDagRunner<Node extends CapabilityMediaDagNode, Result> {
    constructor(
        ...
        if (
            ...
            throw new Error('CAPABILITY_MEDIA_CONCURRENCY_INVALID')

        if (
            !Number.isInteger(transportRetryLimit)
    ...
    async run(args: {
        ...
        onNodeBlocked?: (
            node: Node,
            blocked: CapabilityMediaDagBlockedNode,
        ) => Promise<void> | void
        ...
        for (const nodeId of results.keys()) {
            if (!knownNodeIds.has(nodeId))
                throw new Error(`CAPABILITY_MEDIA_INITIAL_RESULT_NODE_UNKNOWN:${nodeId}`)

            dag.setStatus(nodeId, 'completed')
        }

        const blockedNodes = new Map<string, CapabilityMediaDagBlockedNode>()
        const unsortedEvents: Array<Omit<CapabilityMediaDagEvent, 'sequence'> & {
            nodeOrder: number
            eventOrder: number
        }> = []
        const nodeOrder = new Map(
            this.nodes.map((node, index) => [node.nodeId, index]),
        )
        ...
        try {
            while (dag.hasPending()) {
                ...
                if (ready.length === 0)
                    throw new Error('CAPABILITY_MEDIA_DAG_STALLED')
        ...
        const events = unsortedEvents
            .sort((left, right) => left.nodeOrder - right.nodeOrder || left.eventOrder - right.eventOrder).map(
            (
                {
    ...
    private getBlockedNode(
        ...
        const missingBindings = (node.outputBindings ?? []).filter(binding => binding.required && !context.boundOutputs.has(binding.bindingKey))
        ...
        return {
            ...
            missingOutputNodeIds: [...new Set(
                missingBindings.map(binding => binding.sourceNodeId),
            )],
        }
    }
...
const isRetryableTransportError = (error: unknown): boolean => {
    ...
    return candidate?.retryable === true
        ...
        || ['ETIMEDOUT', 'ECONNRESET', 'EAI_AGAIN'].includes(
