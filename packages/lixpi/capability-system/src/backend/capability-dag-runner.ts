...
const SUCCESS_STATUSES = new Set<CapabilityDagNodeStatus>(['completed', 'skipped'])
...
export class CapabilityDagRunner<Node extends CapabilityDagNode> {
    ...
    constructor(nodes: readonly Node[]) {
        ...
        this.nodesById = new Map(nodes.map(node => [node.nodeId, node]))
        this.validate(nodes)
    ...
    getReadyNodes(): Node[] {
        return this.orderedNodeIds.flatMap(nodeId => {
            ...
            if (!node || this.statuses.get(nodeId) !== 'pending') return []
    ...
    setStatus(nodeId: string, status: CapabilityDagNodeStatus): void {
        if (!this.nodesById.has(nodeId)) throw new Error(`CAPABILITY_DAG_NODE_UNKNOWN:${nodeId}`)
    ...
    cancelPending(): Node[] {
        ...
        for (const nodeId of this.orderedNodeIds) {
            ...
            const node = this.nodesById.get(nodeId)
            if (node) cancelled.push(node)
        }
        return cancelled
    }

    snapshot(): Readonly<Record<string, CapabilityDagNodeStatus>> {
        return Object.freeze(Object.fromEntries(this.orderedNodeIds.map(nodeId => [nodeId, this.statuses.get(nodeId)!])))
    }

    private validate(nodes: readonly Node[]): void {
        if (nodes.length === 0) throw new Error('CAPABILITY_DAG_EMPTY')
        const seen = new Set<string>()
        for (const node of nodes) {
            if (!/^[a-z0-9]+(?:-[a-z0-9]+)*$/u.test(node.nodeId)) {
                throw new Error(`CAPABILITY_DAG_NODE_ID_INVALID:${node.nodeId}`)
            }
            if (seen.has(node.nodeId)) throw new Error(`CAPABILITY_DAG_NODE_ID_DUPLICATE:${node.nodeId}`)
            seen.add(node.nodeId)
        }
        for (const node of nodes) {
            const dependencies = new Set<string>()
            for (const dependency of node.dependsOn) {
                if (dependency === node.nodeId) throw new Error(`CAPABILITY_DAG_SELF_DEPENDENCY:${node.nodeId}`)
                if (!seen.has(dependency)) throw new Error(`CAPABILITY_DAG_DEPENDENCY_UNKNOWN:${node.nodeId}:${dependency}`)
                if (dependencies.has(dependency)) throw new Error(`CAPABILITY_DAG_DEPENDENCY_DUPLICATE:${node.nodeId}:${dependency}`)
    ...
    private assertAcyclic(nodes: readonly Node[]): void {
        ...
        const visit = (nodeId: string): void => {
            ...
            if (visiting.has(nodeId)) throw new Error(`CAPABILITY_DAG_CYCLE:${nodeId}`)
            visiting.add(nodeId)
            const node = this.nodesById.get(nodeId)
