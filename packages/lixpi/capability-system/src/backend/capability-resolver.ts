...
export class SealedResolvedCapabilityPlan {
    ...
    constructor(
        ...
        this.serializable = deepFreeze(structuredClone(serializable))
        this.resourcesByKey = new Map(resources.map(resource => [
    ...
    getResource(capabilityId: string, resourceId: string): LoadedCapabilityResource | undefined {
        const resource = this.resourcesByKey.get(resourceKey(capabilityId, resourceId))
        return resource ? { ...resource, bytes: resource.bytes.slice() } : undefined
    }
...
export async function resolveCapabilities(
    ...
    const queuedIds = new Set(roots.map(reference => reference.capabilityId))
    let frontier: PendingCapability[] = roots.map(reference => ({
    ...
    while (frontier.length > 0) {
        ...
        for (const pending of frontier) {
            ...
            if (!record || record.status !== 'active') throw inaccessibleCapabilityError(pending.capabilityId)
            if (record.kind !== pending.expectedKind) {
                throw new CapabilityError(
                    'CAPABILITY_MANIFEST_INVALID',
                    `Capability ${pending.capabilityId} does not match its declared kind`,
                )
            }
            ...
            verifyHash(manifestBytes, record.manifestBlobHash, 'CAPABILITY_MANIFEST_INTEGRITY_FAILED', pending.capabilityId)
            ...
            if (validation.manifest.capabilityId !== record.capabilityId || validation.manifest.kind !== record.kind) {
                throw new CapabilityError(
                    'CAPABILITY_MANIFEST_INVALID',
            ...
            resolvedById.set(pending.capabilityId, {
            ...
            for (const reference of validation.manifest.references) {
                if (resolvedById.has(reference.capabilityId) || queuedIds.has(reference.capabilityId)) continue
    ...
    if (resourceRequests.length > maxResources) {
        throw new CapabilityError(
            'CAPABILITY_RESOLUTION_LIMIT_EXCEEDED',
            `Capability closure exceeds the ${maxResources} resource limit`,
        )
    }

    const resources = await Promise.all(resourceRequests.map(async request => {
        throwIfAborted(options.signal)
    ...
    return new SealedResolvedCapabilityPlan({
        rootCapabilityIds: roots.map(reference => reference.capabilityId),
        capabilities,
        resolvedManifests: capabilities.map(capability => ({
            capabilityId: capability.capabilityId,
...
function deduplicateReferences(references: readonly CapabilityPromptReference[]): CapabilityPromptReference[] {
    ...
    for (const reference of references) {
        ...
        if (existing && existing.kind !== reference.kind) {
            throw new CapabilityError(
                'CAPABILITY_MANIFEST_INVALID',
                `Capability ${reference.capabilityId} is referenced with conflicting kinds`,
            )
        }
        if (!existing) byId.set(reference.capabilityId, { ...reference })
    }
    return [...byId.values()]
}

function parseManifest(bytes: Uint8Array, capabilityId: string): unknown {
    try {
