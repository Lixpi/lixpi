...
function createActionTimelineCanvasView(host: CapabilityArtifactCanvasHost): CapabilityArtifactCanvasView {
    ...
    const root = html`
        <div className="action-timeline-body">
            ...
            <div className="action-timeline-legend">
                <span className="action-timeline-legend-chip">@ Asset</span><span>References stay attached to the beat where they are used.</span>
            </div>
...
function createActionTimelinePromptReferenceView(host: CapabilityPromptReferenceHost): CapabilityPromptReferenceView {
    ...
    const root = html`<span className="action-timeline-reference"
        >${host.title}${segmentCount > 0 ? ` · ${segmentCount} segments` : ''}</span
    >` as HTMLSpanElement
    host.container.appendChild(root)
