...
export const selectCharacterPanelReferenceEntries = (
    ...
    const hasApprovedEditTargetIdentity = candidates.some(
        entry => (
            entry.role === 'edit-target-identity'
        ),
    )
    const assignedFaceSourceIds = new Set(
        evidence.facts.flatMap(
            fact => (
    ...
    const croppedFaceSourceIds = new Set(
        candidates.flatMap(
            entry => (
    ...
    return candidates.filter(entry => {
        ...
        if (!entry.sourceAssetId)
            return !hasApprovedEditTargetIdentity

        if (
            hasApprovedEditTargetIdentity
            && !assignedFaceSourceIds.has(entry.sourceAssetId)
        )
            return false
...
const deduplicateCompositionComponents = (entries: readonly CharacterReferencePackEntry[]): CharacterReferencePackEntry[] => {
    ...
    for (const entry of entries) {
        ...
        if (
            !existing
            || getReferencePriority(entry) > getReferencePriority(existing)
        )
...
const getReferencePriority = (entry: CharacterReferencePackEntry): number => {
    if (entry.role === 'edit-target-identity')
        return 3

    if (entry.role === 'edit-target')
        return 2
