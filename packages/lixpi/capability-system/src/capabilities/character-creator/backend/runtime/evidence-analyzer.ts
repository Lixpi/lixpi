...
export const analyzeCharacterEvidence = async (args: {
    sources: readonly ResolvedCharacterReference[]
    editTargets?: readonly ResolvedCharacterReference[]
    referenceAliases?: ReadonlyArray<{
        assetId: string
    ...
    if (!args.analyzer)
        throw new Error('CHARACTER_EVIDENCE_ANALYZER_REQUIRED')

    const analysis = await args.analyzer.analyze(args)
    const facts = resolveUnambiguousObservedSources(analysis.facts ?? [], args.sources).map(
        fact => ({
            ...fact,
    ...
    const affectedPanelIds = unique(analysis.affectedPanelIds ?? []).filter(panelId => panelIds.size === 0 || panelIds.has(panelId))
    ...
    return {
        ...
        regenerationScope,
        affectedPanelIds: regenerationScope === 'full-sheet'
            && panelIds.size > 0
            ? [...panelIds]
            : affectedPanelIds,
        facts,
...
export const selectCharacterEvidenceFacts = (args: {
    ...
    return [...byFeature.values()].flatMap(candidates => {
        ...
        (left, right) =>
            Number(
                changed.has(
                    normalize(right.feature),
                ),
            )
            - Number(
                changed.has(
                    normalize(left.feature),
                ),
            ),
    )
}
    ...
    return facts.map(
        fact =>
            fact.visibility === 'observed'
                ...
                ? {
                    ...fact,
                    sourceAssetId: soleSourceAssetId,
                }
                : fact,
    )
}
...
const validateEvidenceCoordinates = (
    ...
    for (const fact of facts) {
        ...
        if (
            ...
            throw new Error('CHARACTER_EVIDENCE_REGION_SOURCE_REQUIRED')

        if (!fact.sourceAssetId)
            continue

        const source = sourcesById.get(fact.sourceAssetId)

        if (!source)
            throw new Error('CHARACTER_EVIDENCE_SOURCE_UNKNOWN')

        if (!fact.sourceRegion)
            continue
...
const targetSpecificity = (
    fact: CharacterEvidenceProfile['facts'][number],
    targetAngle: string,
): number => (fact.targetAngles.some(angle => targetAngle.includes(angle)) ? 1 : 0)
...
const resolveEditTargetPolicy = ({
    ...
    if (
        reportedPolicy === 'identity-only'
        || reportedPolicy === 'discard'
    )
...
const clampConfidence = (value: number): number => Math.max(
    0,
    Math.min(1, value),
)
