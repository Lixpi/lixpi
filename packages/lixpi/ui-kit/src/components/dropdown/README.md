...
- Surface container with configured shadow via `settings.dropdown.styles.popoverBoxShadow` (no border on popover)
...
- Trigger and icon alignment is owned by the component stylesheet and does not depend on application utility classes
...
- `popoverClassName`: Adds a consumer-owned class to the InfoBubble popover surface for portal stacking or other integration-specific styling
...
- Shadow is configured via `settings.dropdown.styles.popoverBoxShadow` and applied as CSS variable `--dropdown-popover-box-shadow`
