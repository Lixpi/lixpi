...
Reusable DOM loading and error placeholders for asynchronous content. The host supplies content state and retry behavior; the component has no editor or canvas dependency.

## What it is

Class-backed loading and error surfaces for asynchronous content, including lazy node loading and deferred editor initialization. Factories return instances with explicit disposal.
...
The component owns its element and retry listener:
...
```typescript
import { createLoadingPlaceholder } from '@lixpi/ui-kit/components/loading-placeholder'
...
```typescript
import { createErrorPlaceholder } from '@lixpi/ui-kit/components/loading-placeholder'
